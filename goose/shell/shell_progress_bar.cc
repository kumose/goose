    ...
    protected:
        ...
        string HandleSetting(ShellState &state, const PromptComponent &component) override {
            ...
            if (component.literal == "progress_bar") {
                return goose::TerminalProgressBarDisplay::FormatProgressBar(status_bar.display_info,
                                                                            status_bar.percentage);
            }
            if (component.literal == "eta") {
                return goose::TerminalProgressBarDisplay::FormatETA(status_bar.estimated_remaining_seconds);
            }
            return Prompt::HandleSetting(state, component);
        }

        goose::Connection &GetConnection(ShellState &state) override {
            if (!status_bar.connection) {
                status_bar.connection = make_uniq<goose::Connection>(*state.db);
            }
