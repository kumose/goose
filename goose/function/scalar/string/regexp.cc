...
#include <goose/common/vector_operations/ternary_executor.h>
...
namespace goose {
    ...
    static bool RegexOptionsEquals(const xre2::RE2::Options &opt_a, const xre2::RE2::Options &opt_b) {
    ...
    RegexpBaseBindData::RegexpBaseBindData(xre2::RE2::Options options, string constant_string_p,
    ...
    // Regexp Matches
    //===--------------------------------------------------------------------===//
    RegexpMatchesBindData::RegexpMatchesBindData(xre2::RE2::Options options, string constant_string_p,
                                                 bool constant_pattern)
    ...
    RegexpMatchesBindData::RegexpMatchesBindData(xre2::RE2::Options options, string constant_string_p,
                                                 ...
                                                 bool range_success)
        : RegexpBaseBindData(options, std::move(constant_string_p), constant_pattern),
          range_min(std::move(range_min_p)),
    ...
    struct RegexPartialMatch {
        static inline bool Operation(const std::string_view &input, xre2::RE2 &re) {
            return xre2::RE2::PartialMatch(input, re);
        }
    };

    struct RegexFullMatch {
        static inline bool Operation(const std::string_view &input, xre2::RE2 &re) {
            return xre2::RE2::FullMatch(input, re);
        }
    };
    ...
    RegexpReplaceBindData::RegexpReplaceBindData(xre2::RE2::Options options, string constant_string_p,
    ...
    static void RegexReplaceFunction(DataChunk &args, ExpressionState &state, Vector &result) {
        ...
        if (info.constant_pattern) {
            ...
            TernaryExecutor::Execute<string_t, string_t, string_t, string_t>(
                ...
                [&](string_t input, string_t pattern, string_t replace) {
                    RE2 re(CreateStringPiece(pattern), info.options);
                    if (!re.ok()) {
                        throw InvalidInputException(re.error());
                    }
    ...
    RegexpExtractBindData::RegexpExtractBindData(xre2::RE2::Options options, string constant_string_p,
    ...
    static void RegexExtractStructFunction(DataChunk &args, ExpressionState &state, Vector &result) {
        ...
        vector<std::string_view> ws(groupSize);
        ...
        if (input.GetVectorType() == VectorType::CONSTANT_VECTOR) {
            ...
            if (ConstantVector::IsNull(input)) {
                ...
                auto match = xre2::RE2::PartialMatchN(str, lstate.constant_pattern, groups.data(),
            ...
            for (idx_t i = 0; i < count; ++i) {
                ...
                if (ivalidity.RowIsValid(idx)) {
                    ...
                    for (size_t col = 0; col < child_entries.size(); ++col) {
                        ...
                        auto &extracted = ws[col];
                        cdata[i] = string_t(extracted.data(),
                                            UnsafeNumericCast<uint32_t>(match ? extracted.size() : 0));
                    }
                } else {
    ...
    ScalarFunctionSet RegexpMatchesFun::GetFunctions() {
        ...
        regexp_partial_match.AddFunction(ScalarFunction(
            ...
            RegexpMatchesFunction<RegexPartialMatch>,
    ...
    ScalarFunctionSet RegexpExtractFun::GetFunctions() {
        ...
        regexp_extract.AddFunction(ScalarFunction({LogicalType::VARCHAR, LogicalType::VARCHAR}, LogicalType::VARCHAR,
                                                  ...
                                                  RegexInitLocalState, LogicalType::INVALID,
                                                  FunctionStability::CONSISTENT,
                                                  FunctionNullHandling::SPECIAL_HANDLING));
