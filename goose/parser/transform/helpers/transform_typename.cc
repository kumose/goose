...
static SizeModifiers GetSizeModifiers(cantor::PGTypeName &type_name, LogicalTypeId base_type) {
	...
	if (type_name.typmods) {
		for (auto node = type_name.typmods->head; node; node = node->next) {
			auto &const_val = *Transformer::PGPointerCast<cantor::PGAConst>(node->data.ptr_value);
			if (const_val.type != cantor::T_PGAConst ||
			    const_val.val.type != cantor::T_PGInteger) {
...
vector<Value> Transformer::TransformTypeModifiers(cantor::PGTypeName &type_name) {
	vector<Value> type_mods;
	if (type_name.typmods) {
		for (auto node = type_name.typmods->head; node; node = node->next) {
			const auto &const_val = *PGPointerCast<cantor::PGAConst>(node->data.ptr_value);
			if (const_val.type != cantor::T_PGAConst) {
		...
		if (type_mods.size() > 9) {
			const auto name = PGPointerCast<cantor::PGValue>(type_name.names->tail->data.ptr_value)->val.str;
...
LogicalType Transformer::TransformTypeNameInternal(cantor::PGTypeName &type_name) {
	if (type_name.names->length > 1) {
		// qualified typename
		vector<string> names;
		for (auto cell = type_name.names->head; cell; cell = cell->next) {
			names.push_back(PGPointerCast<cantor::PGValue>(cell->data.ptr_value)->val.str);
		}
	...
	auto name = PGPointerCast<cantor::PGValue>(type_name.names->tail->data.ptr_value)->val.str;
	...
	if (base_type == LogicalTypeId::ENUM) {
		...
		idx_t pos = 0;
		for (auto node = type_name.typmods->head; node; node = node->next) {
			auto constant_value = PGPointerCast<cantor::PGAConst>(node->data.ptr_value);
			if (constant_value->type != cantor::T_PGAConst ||
			    constant_value->val.type != cantor::T_PGString) {
	...
	if (base_type == LogicalTypeId::GEOMETRY) {
		...
		auto crs_node = PGPointerCast<cantor::PGAConst>(type_name.typmods->head->data.ptr_value);
		if (crs_node->type != cantor::T_PGAConst || crs_node->val.type != cantor::T_PGString) {
			throw ParserException(
	...
	if (base_type == LogicalTypeId::STRUCT) {
		...
		for (auto node = type_name.typmods->head; node; node = node->next) {
			auto &type_val = *PGPointerCast<cantor::PGList>(node->data.ptr_value);
			if (type_val.length != 2) {
				throw ParserException("Struct entry needs an entry name and a type name");
			}

			auto entry_name_node = PGPointerCast<cantor::PGValue>(type_val.head->data.ptr_value);
			D_ASSERT(entry_name_node->type == cantor::T_PGString);
			auto entry_type_node = PGPointerCast<cantor::PGTypeName>(type_val.tail->data.ptr_value);
			D_ASSERT(entry_type_node->type == cantor::T_PGTypeName);
	...
	if (base_type == LogicalTypeId::MAP) {
		...
		auto key_type =
		    TransformTypeName(*PGPointerCast<cantor::PGTypeName>(type_name.typmods->head->data.ptr_value));
		auto value_type =
		    TransformTypeName(*PGPointerCast<cantor::PGTypeName>(type_name.typmods->tail->data.ptr_value));
...
LogicalType Transformer::TransformTypeName(cantor::PGTypeName &type_name) {
	if (type_name.type != cantor::T_PGTypeName) {
	...
	if (type_name.arrayBounds) {
		...
		for (auto cell = type_name.arrayBounds->head; cell != nullptr; cell = cell->next) {
			...
			auto val = PGPointerCast<cantor::PGValue>(cell->data.ptr_value);
