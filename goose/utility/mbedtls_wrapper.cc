...
const mbedtls_cipher_info_t *MbedTlsWrapper::AESStateMBEDTLS::GetCipher(size_t key_len) {
    switch (cipher) {
        ...
        default:
            throw goose::InternalException("Invalid Encryption/Decryption Cipher: %s",
                                           goose::EncryptionTypes::CipherToString(cipher));
    }
}
...
MbedTlsWrapper::AESStateMBEDTLS::AESStateMBEDTLS(goose::EncryptionTypes::CipherType cipher_p,
                                                 goose::idx_t key_len) : EncryptionState(cipher_p, key_len),
                                                                         context(
                                                                             goose::make_uniq<
    ...
    if (cipher == goose::EncryptionTypes::CBC &&
        mbedtls_cipher_set_padding_mode(context.get(), MBEDTLS_PADDING_PKCS7)) {
...
static void ThrowInsecureRNG() {
    throw goose::InvalidConfigurationException(
        "Goose requires a secure random engine to be loaded to enable secure crypto. Normally, this will be handled automatically by Goose by autoloading the `httpfs` Extension, but that seems to have failed. Please ensure the httpfs extension is loaded manually using `LOAD httpfs`.");
}
...
void MbedTlsWrapper::AESStateMBEDTLS::InitializeInternal(goose::const_data_ptr_t iv, goose::idx_t iv_len,
                                                         goose::const_data_ptr_t aad, goose::idx_t aad_len) {
...
void MbedTlsWrapper::AESStateMBEDTLS::InitializeEncryption(goose::const_data_ptr_t iv, goose::idx_t iv_len,
                                                           goose::const_data_ptr_t key, goose::idx_t key_len_p,
                                                           goose::const_data_ptr_t aad, goose::idx_t aad_len) {
    ThrowInsecureRNG();
}

void MbedTlsWrapper::AESStateMBEDTLS::InitializeDecryption(goose::const_data_ptr_t iv, goose::idx_t iv_len,
                                                           goose::const_data_ptr_t key, goose::idx_t key_len_p,
                                                           goose::const_data_ptr_t aad, goose::idx_t aad_len) {
