...
void ParseRegexOptions(const string &options, xre2::RE2::Options &result, bool *global_replace) {
...
void ParseGroupNameList(ClientContext &context, const string &function_name, Expression &group_expr,
	...
	if (require_constant_pattern) {
		std::string_view const_piece(pattern_string.c_str(), pattern_string.size());
