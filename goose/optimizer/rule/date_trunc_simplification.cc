...
namespace goose {
    ...
    unique_ptr<Expression> DateTruncSimplificationRule::Apply(LogicalOperator &op,
                ...
                //
            {
                ...
                if (rhs_comparison_type == ExpressionType::COMPARE_NOT_DISTINCT_FROM && rhs.value.IsNull()) {
                    ...
                    if (rhs_comparison_type == ExpressionType::COMPARE_NOT_DISTINCT_FROM) {
                        auto comp = make_uniq<BoundConjunctionExpression>(
                            ExpressionType::CONJUNCTION_AND, std::move(gteq),
                            std::move(lt));
                ...
                //
            {
                if (rhs_comparison_type == ExpressionType::COMPARE_DISTINCT_FROM && rhs.value.IsNull()) {
                    ...
                    if (rhs_comparison_type == ExpressionType::COMPARE_DISTINCT_FROM) {
                        auto comp = make_uniq<BoundConjunctionExpression>(
                            ExpressionType::CONJUNCTION_OR, std::move(gteq),
                            std::move(lt));

                        auto isnull =
                                make_uniq<BoundOperatorExpression>(ExpressionType::OPERATOR_IS_NULL,
                                                                   LogicalType::BOOLEAN);
