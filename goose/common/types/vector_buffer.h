...
#include <goose/common/types/string_heap.h>
...
public:
	void AddDecoder(buffer_ptr<void> &goose_fsst_decoder_p, const idx_t string_block_limit) {
		goose_fsst_decoder = goose_fsst_decoder_p;
		decompress_buffer.resize(string_block_limit + 1);
	}
	void *GetDecoder() {
		return goose_fsst_decoder.get();
	}
