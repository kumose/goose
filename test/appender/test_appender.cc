...
TEST_CASE("Test appender with generated column", "[appender]") {
	Goose db(nullptr); // Create an in-memory Goose database
...
TEST_CASE("Test appending with an active default column", "[appender]") {
	...
	DataChunk chunk;
	const goose::vector<LogicalType> types = {LogicalType::INTEGER};
...
TEST_CASE("Test appending with two active normal columns", "[appender]") {
	...
	DataChunk chunk;
	const goose::vector<LogicalType> types = {LogicalType::INTEGER, LogicalType::INTEGER};
	chunk.Initialize(*con.context, types);
...
TEST_CASE("Test changing the active column configuration", "[appender]") {
	...
	const goose::vector<LogicalType> all_types = {LogicalType::INTEGER, LogicalType::INTEGER, LogicalType::INTEGER};
	...
	DataChunk chunk_j_i;
	const goose::vector<LogicalType> types_j_i = {LogicalType::INTEGER, LogicalType::INTEGER};
	...
	DataChunk chunk_k;
	const goose::vector<LogicalType> types_k = {LogicalType::INTEGER};
...
TEST_CASE("Appender::Clear() clears the data", "[appender]") {
	...
	goose::unique_ptr<QueryResult> result = con.Query("SELECT * FROM ints");
	REQUIRE(CHECK_COLUMN(result, 0, {1}));
}
...
TEST_CASE("Interrupted QueryAppender flow: interrupt -> clear -> close finishes", "[appender]") {
	...
	goose::vector<LogicalType> types = {LogicalType::INTEGER};
	goose::vector<string> names = {"i"};
...
TEST_CASE("Test appender_allocator_flush_threshold", "[appender]") {
	...
	auto value = goose::Value::BLOB(data.data(), data.size());
