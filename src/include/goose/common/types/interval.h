...
public:
	...
	GOOSE_API static interval_t FromMicro(int64_t micros);
...
namespace std {
template <>
struct hash<goose::interval_t> {
	size_t operator()(const goose::interval_t &val) const {
		int64_t months, days, micros;
		val.Normalize(months, days, micros);
		using std::hash;

		return hash<int32_t> {}(goose::UnsafeNumericCast<int32_t>(days)) ^
		       hash<int32_t> {}(goose::UnsafeNumericCast<int32_t>(months)) ^ hash<int64_t> {}(micros);
	}
};
} // namespace std
