...
namespace goose {
    namespace {
        ...
        unique_ptr<FunctionData> WriteLogBind(ClientContext &context, ScalarFunction &bound_function,
            ...
            for (idx_t i = 1; i < arguments.size(); i++) {
                ...
                if (arg->alias == "disable_logging") {
                    ...
                    result->level =
                            EnumUtil::FromString<LogLevel>(
                                StringValue::Get(ExpressionExecutor::EvaluateScalar(context, *arg)));
        ...
        void WriteLogValues(T &LogSource, LogLevel level, const string_t *data, const SelectionVector *sel, idx_t size,
            ...
            for (idx_t i = 0; i < size; i++) {
                GOOSE_LOG_INTERNAL(LogSource, type.c_str(), level, data[sel->get_index(i)]);
            }
        }
        ...
        void WriteLogFunction(DataChunk &args, ExpressionState &state, Vector &result) {
            ...
            if (!info.disable_logging) {
                if (info.scope.empty() || info.scope == "connection") {
                    ...
                    WriteLogValues<const DatabaseInstance>(*info.context->db, info.level, input_data, idata.sel,
                                                           args.size(),
                                                           info.type);
                } else if (info.scope == "file_opener") {
                    WriteLogValues<const FileOpener>(*info.context->client_data->file_opener, info.level, input_data,
                                                     idata.sel,
                                                     args.size(), info.type);
                } else {
    ...
    ScalarFunctionSet WriteLogFun::GetFunctions() {
        ...
        set.AddFunction(ScalarFunction({LogicalType::VARCHAR}, LogicalType::ANY, WriteLogFunction, WriteLogBind,
                                       nullptr,
