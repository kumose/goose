...
struct GooseLogData : public GlobalTableFunctionState {
	explicit GooseLogData(shared_ptr<LogStorage> log_storage_p) : log_storage(std::move(log_storage_p)) {
		scan_state = log_storage->CreateScanState(LoggingTargetTable::LOG_ENTRIES);
		log_storage->InitializeScan(*scan_state);
	}
	GooseLogData() : log_storage(nullptr) {
	}
...
static unique_ptr<FunctionData> GooseLogBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> GooseLogInit(ClientContext &context, TableFunctionInitInput &input) {
	if (LogManager::Get(context).CanScan(LoggingTargetTable::LOG_ENTRIES)) {
		return make_uniq<GooseLogData>(LogManager::Get(context).GetLogStorage());
	}
	return make_uniq<GooseLogData>();
}

void GooseLogFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<GooseLogData>();
	if (data.log_storage) {
		data.log_storage->Scan(*data.scan_state, output);
	}
}

unique_ptr<TableRef> GooseLogBindReplace(ClientContext &context, TableFunctionBindInput &input) {
	...
	string sub_query_string = "SELECT l.context_id, scope, connection_id, transaction_id, query_id, thread_id, "
	                          ...
	                          " FROM (SELECT row_number() OVER () AS rowid, * FROM goose_logs()) as l JOIN "
	                          "goose_log_contexts() as c ON l.context_id=c.context_id order by timestamp, l.rowid;";
...
void GooseLogFun::RegisterFunction(BuiltinFunctions &set) {
	TableFunction logs_fun("goose_logs", {}, GooseLogFunction, GooseLogBind, GooseLogInit);
	logs_fun.bind_replace = GooseLogBindReplace;
