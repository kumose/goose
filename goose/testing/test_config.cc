...
namespace goose {
    ...
    static const TestConfigOption test_config_options[] = {
        ...
        {"on_new_connection", "SQL statements to execute on connection", LogicalType::VARCHAR, nullptr},
        {
            ...
            LogicalType::LIST(LogicalType::STRUCT({
                {"env_name", LogicalType::VARCHAR}, {"env_value", LogicalType::VARCHAR}
            })),
            nullptr
        },
        {"inherit_skip_tests", "Path of config to inherit 'skip_tests' from", LogicalType::VARCHAR},
        {
            ...
            LogicalType::LIST(
                LogicalType::STRUCT({
                    {"reason", LogicalType::VARCHAR}, {"paths", LogicalType::LIST(LogicalType::VARCHAR)}
                })),
            nullptr
        },
        ...
        {
            "skip_tag", "Skip tests which match named tag (as singleton set; multiple sets are OR'd)",
            LogicalType::VARCHAR,
    ...
    void TestConfiguration::Initialize() {
        // load config file
        auto config_file = std::getenv("GOOSE_TEST_CONFIG");
        if (config_file) {
        ...
        for (idx_t index = 0; test_config_options[index].name != nullptr; index++) {
            ...
            string env_name = "GOOSE_TEST_" + StringUtil::Upper(config.name);
    ...
    void TestConfiguration::UpdateEnvironment() {
        ...
        test_env["BUILD_DIR"] = string(get_goose_build_directory());
        ...
        test_env["TEMP_DIR"] = temp_dir; // default: goose_unittest_tempdir/$PID
