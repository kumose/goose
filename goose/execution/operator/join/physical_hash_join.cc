...
#include <goose/common/radix_partitioning.h>
#include <goose/common/types/value_map.h>
#include <goose/execution/expression_executor.h>
#include <goose/execution/operator/aggregate/ungrouped_aggregate_state.h>
...
#include <goose/parallel/executor_task.h>
...
namespace goose {
    ...
    PhysicalHashJoin::PhysicalHashJoin(PhysicalPlan &physical_plan, LogicalOperator &op, PhysicalOperator &left,
        ...
        : PhysicalHashJoin(physical_plan, op, left, right, std::move(cond), join_type, {}, {}, {},
                           estimated_cardinality,
                           nullptr) {
    }
    ...
    public:
        ...
        ~HashJoinGlobalSinkState() override {
            GOOSE_LOG(context, PhysicalOperatorLogType, op, "PhysicalHashJoin", "GetData",
    ...
    void PhysicalHashJoin::PrepareFinalize(ClientContext &context, GlobalSinkState &global_state) const {
        ...
        gstate.probe_side_requirement =
                GetPartitioningSpaceRequirement(context, children[0].get().GetTypes(), ht.GetRadixBits(),
                                                gstate.num_threads);
        const auto max_partition_ht_size =
        ...
        gstate.temporary_memory_state->SetMaterializationPenalty(
            GetTupleWidth(children[0].get().GetTypes(), all_constant));
        ...
        void Schedule() override {
            ...
            if (FinalizeSingleThreaded(sink, false)) {
                ...
                for (idx_t entry_idx = 0; entry_idx < entry_count; entry_idx += entries_per_task) {
                    ...
                    finalize_tasks.push_back(make_uniq<HashJoinTableInitTask>(
                        shared_from_this(), context, sink, entry_idx,
            ...
            if (FinalizeSingleThreaded(sink, true)) {
                ...
                for (idx_t chunk_idx = 0; chunk_idx < chunk_count; chunk_idx += chunks_per_task) {
                    ...
                    finalize_tasks.push_back(make_uniq<HashJoinFinalizeTask>(
                        shared_from_this(), context, sink, chunk_idx,
    ...
    public:
        void Schedule() override {
            ...
            auto partition_multiplier =
                    RadixPartitioning::NumberOfPartitions(
                        sink.hash_table->GetRadixBits() - JoinHashTable::INITIAL_RADIX_BITS);
            auto thread_memory = 2 * blocks_per_vector * partition_multiplier * block_size;
            auto repartition_threads = MaxValue<
                idx_t>(sink.temporary_memory_state->GetReservation() / thread_memory, 1);
        ...
        void FinishEvent() override {
            ...
            sink.probe_side_requirement =
                    GetPartitioningSpaceRequirement(sink.context, op.types, sink.hash_table->GetRadixBits(),
                                                    sink.num_threads);

            sink.temporary_memory_state->SetMinimumReservation(sink.max_partition_size +
                                                               sink.hash_table->PointerTableSize(
                                                                   sink.max_partition_count) +
                                                               sink.probe_side_requirement);
    ...
    bool JoinFilterPushdownInfo::CanUseInFilter(const ClientContext &context, optional_ptr<JoinHashTable> ht,
        ...
        return ht && ht->Count() > 1 && ht->Count() <= dynamic_or_filter_threshold && cmp ==
    ...
    unique_ptr<DataChunk> JoinFilterPushdownInfo::FinalizeFilters(ClientContext &context,
                ...
                if (Value::NotDistinctFrom(min_val, max_val)) {
                    ...
                    switch (cmp) {
                        ...
                        case ExpressionType::COMPARE_GREATERTHANOREQUALTO: {
                            auto greater_equals =
                                    make_uniq<ConstantFilter>(ExpressionType::COMPARE_GREATERTHANOREQUALTO,
                                                              std::move(min_val));
                    ...
                    switch (cmp) {
                        ...
                        case ExpressionType::COMPARE_LESSTHANOREQUALTO: {
                            auto less_equals =
                                    make_uniq<ConstantFilter>(ExpressionType::COMPARE_LESSTHANOREQUALTO,
                                                              std::move(max_val));
    ...
    SinkFinalizeType PhysicalHashJoin::Finalize(Pipeline &pipeline, Event &event, ClientContext &context,
        ...
        GOOSE_LOG(context, PhysicalOperatorLogType, *this, "PhysicalHashJoin", "Finalize",
        ...
        if (sink.external) {
            ...
            if (!very_very_skewed &&
                ...
                ht.SetRepartitionRadixBits(sink.temporary_memory_state->GetReservation(), sink.max_partition_size,
                                           sink.max_partition_count);
                GOOSE_LOG(context, PhysicalOperatorLogType, *this, "PhysicalHashJoin", "Repartition",
                ...
                auto new_event = make_shared_ptr<HashJoinRepartitionEvent>(
                    pipeline, *this, sink, sink.local_hash_tables);
    ...
    OperatorResultType PhysicalHashJoin::ExecuteInternal(ExecutionContext &context, DataChunk &input, DataChunk &chunk,
        ...
        if (state.scan_structure.is_null) {
            ...
            if (sink.external) {
                ...
                sink.hash_table->Probe(state.scan_structure, state.lhs_join_keys, state.join_key_state,
                                       state.probe_state);
            }
        }
    ...
    HashJoinGlobalSourceState::HashJoinGlobalSourceState(const PhysicalHashJoin &op, const ClientContext &context)
        : op(op), global_stage(HashJoinSourceStage::INIT), build_chunk_count(0), build_chunk_done(0),
          probe_chunk_count(0),
