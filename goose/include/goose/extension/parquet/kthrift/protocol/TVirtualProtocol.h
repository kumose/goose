...
#include <goose/extension/parquet/kthrift/protocol/TProtocol.h>
...
public:
  ...
  uint32_t readBool(goose::vector<bool>::reference value) {
    (void)value;
    throw TProtocolException(TProtocolException::NOT_IMPLEMENTED,
                             "this protocol does not support reading (yet).");
  }
  ...
  uint32_t skip(TType type) { return ::goose_apache::thrift::protocol::skip(*this, type); }
...
public:
  ...
  uint32_t readBool_virt(goose::vector<bool>::reference value) override {
    return static_cast<Protocol_*>(this)->readBool(value);
  }
  ...
  uint32_t skip(TType type) {
    ...
    return ::goose_apache::thrift::protocol::skip(*prot, type);
  }

  /*
   * Provide a default readBool() implementation for use with
   * goose::vector<bool>, that behaves the same as reading into a normal bool.
   *
...
#endif // #define _GOOSE_THRIFT_PROTOCOL_TVIRTUALPROTOCOL_H_ 1
