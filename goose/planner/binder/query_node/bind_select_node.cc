...
#include <goose/planner/expression_binder/group_binder.h>
#include <goose/planner/expression_binder/having_binder.h>
#include <goose/planner/expression_binder/order_binder.h>
#include <goose/planner/expression_binder/qualify_binder.h>
#include <goose/planner/expression_binder/select_bind_state.h>
#include <goose/planner/expression_binder/select_binder.h>
#include <goose/planner/expression_binder/where_binder.h>
...
namespace goose {
    ...
    goose::unique_ptr<BoundResultModifier> Binder::BindLimit(OrderBinder &order_binder, LimitModifier &limit_mod) {
    ...
    Binder::BindLimitPercent(OrderBinder &order_binder, LimitPercentModifier &limit_mod) {
        auto result = make_uniq<BoundLimitModifier>();
        if (limit_mod.limit) {
        ...
        for (auto &mod: statement.modifiers) {
            ...
            switch (mod->type) {
                ...
                case ResultModifierType::ORDER_MODIFIER: {
                    ...
                    auto &order_binders = order_binder.GetBinders();
                    if (order.orders.size() == 1 && order.orders[0].expression->GetExpressionType() ==
                        ExpressionType::STAR) {
                    ...
                    if (can_replace) {
                        ...
                        order.orders.clear();
                        auto create_sort_key = make_uniq<FunctionExpression>(
                            "create_sort_key", std::move(sort_key_parameters));
    ...
    unique_ptr<Expression> CreateOrderExpression(unique_ptr<Expression> expr, const vector<string> &names,
        ...
        auto result =
                make_uniq<BoundColumnRefExpression>(expr->GetAlias(), sql_types[index],
    ...
    unique_ptr<Expression> FinalizeBindOrderExpression(unique_ptr<Expression> expr, idx_t table_index,
        ...
        switch (constant.value.type().id()) {
            case LogicalTypeId::UBIGINT: {
                ...
                return CreateOrderExpression(std::move(expr), names, sql_types, table_index,
                                             bind_state.GetFinalIndex(index));
            }
                ...
                case ResultModifierType::ORDER_MODIFIER: {
                    ...
                    if (order_by_all) {
                        ...
                        for (idx_t i = 0; i < sql_types.size(); i++) {
                            auto expr = make_uniq<
                                BoundColumnRefExpression>(sql_types[i], ColumnBinding(table_index, i));
                            if (i < names.size()) {
