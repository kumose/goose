...
namespace goose {
    ...
    FunctionExpression::FunctionExpression(string catalog, string schema, const string &function_name,
        ...
        : ParsedExpression(ExpressionType::FUNCTION, ExpressionClass::FUNCTION), catalog(std::move(catalog)),
          ...
          children(std::move(children_p)), distinct(distinct), filter(std::move(filter)),
          order_bys(std::move(order_bys_p)),
    ...
    FunctionExpression::FunctionExpression(const string &function_name,
                                           vector<unique_ptr<ParsedExpression> > children_p,
    ...
    string FunctionExpression::ToString() const {
        return ToString<FunctionExpression, ParsedExpression>(*this, catalog, schema, function_name, is_operator,
                                                              distinct,
    ...
    hash_t FunctionExpression::Hash() const {
        ...
        result = CombineHash(result, goose::Hash<const char *>(schema.c_str()));
        result = CombineHash(result, goose::Hash<const char *>(function_name.c_str()));
        result = CombineHash(result, goose::Hash<bool>(distinct));
        result = CombineHash(result, goose::Hash<bool>(export_state));
        return result;
    }
    ...
    unique_ptr<ParsedExpression> FunctionExpression::Copy() const {
        ...
        auto copy =
                make_uniq<FunctionExpression>(catalog, schema, function_name, std::move(copy_children),
