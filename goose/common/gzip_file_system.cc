...
#include <goose/common/gzip_file_system.h>
...
namespace goose {
    ...
    struct MiniZStreamWrapper : public StreamWrapper {
        ...
        unique_ptr<goose_miniz::mz_stream> mz_stream_ptr;
        bool writing = false;
        goose_miniz::mz_ulong crc;
        idx_t total_size;
        ...
        try {
            ...
            if (file && file->child_handle) {
                ...
                try {
                    ...
                    if (logger) {
                        GOOSE_LOG_ERROR(logger, "MiniZStreamWrapper::~MiniZStreamWrapper()\t\t" + data.Message())
                    }
                } catch (...) { // NOLINT
                }
            }
    ...
    void MiniZStreamWrapper::Initialize(QueryContext context, CompressedFile &file, bool write) {
        Close();
        this->file = &file;
        mz_stream_ptr = make_uniq<goose_miniz::mz_stream>();
        memset(mz_stream_ptr.get(), 0, sizeof(goose_miniz::mz_stream));
        this->writing = write;
        ...
        if (write) {
            ...
            auto ret = mz_deflateInit2(mz_stream_ptr.get(), goose_miniz::MZ_DEFAULT_LEVEL, MZ_DEFLATED,
                                       -MZ_DEFAULT_WINDOW_BITS, 1, 0);
            if (ret != goose_miniz::MZ_OK) {
            ...
            auto ret = goose_miniz::mz_inflateInit2(mz_stream_ptr.get(), -MZ_DEFAULT_WINDOW_BITS);
            if (ret != goose_miniz::MZ_OK) {
                throw InternalException("Failed to initialize miniz");
            }
        }
    ...
    bool MiniZStreamWrapper::Read(StreamData &sd) {
        ...
        if (sd.refresh) {
            ...
            goose_miniz::mz_inflateEnd(mz_stream_ptr.get());
            auto sta = goose_miniz::mz_inflateInit2(mz_stream_ptr.get(), -MZ_DEFAULT_WINDOW_BITS);
            if (sta != goose_miniz::MZ_OK) {
                throw InternalException("Failed to initialize miniz");
            }
        }
        ...
        auto ret = goose_miniz::mz_inflate(mz_stream_ptr.get(), goose_miniz::MZ_NO_FLUSH);
        if (ret != goose_miniz::MZ_OK && ret != goose_miniz::MZ_STREAM_END) {
            throw IOException("Failed to decode gzip stream: %s", goose_miniz::mz_error(ret));
        }
        ...
        if (ret == goose_miniz::MZ_STREAM_END) {
    ...
    void MiniZStreamWrapper::Write(CompressedFile &file, StreamData &sd, data_ptr_t uncompressed_data,
        ...
        crc = goose_miniz::mz_crc32(crc, reinterpret_cast<const unsigned char *>(uncompressed_data),
        ...
        while (remaining > 0) {
            ...
            auto res = mz_deflate(mz_stream_ptr.get(), goose_miniz::MZ_NO_FLUSH);
            if (res != goose_miniz::MZ_OK) {
                D_ASSERT(res != goose_miniz::MZ_STREAM_END);
    ...
    void MiniZStreamWrapper::FlushStream() const {
        ...
        while (true) {
            ...
            auto res = mz_deflate(mz_stream_ptr.get(), goose_miniz::MZ_FINISH);
            sd.out_buff_start += (output_remaining - mz_stream_ptr->avail_out);
            ...
            if (res == goose_miniz::MZ_STREAM_END) {
                break;
            }
            if (res != goose_miniz::MZ_OK) {
                throw InternalException("Failed to compress GZIP block");
    ...
    void MiniZStreamWrapper::Close() {
        ...
        if (writing) {
            ...
            goose_miniz::mz_deflateEnd(mz_stream_ptr.get());
        } else {
            goose_miniz::mz_inflateEnd(mz_stream_ptr.get());
        }
    ...
    void GZipFileSystem::VerifyGZIPHeader(uint8_t gzip_hdr[], idx_t read_count,
        ...
        if (gzip_hdr[2] != GZIP_COMPRESSION_DEFLATE) {
            // compression method
    ...
    bool GZipFileSystem::CheckIsZip(const char *data, goose::idx_t size) {
    ...
    string GZipFileSystem::UncompressGZIPString(const char *data, idx_t size) {
        ...
        auto mz_stream_ptr = make_uniq<goose_miniz::mz_stream>();
        memset(mz_stream_ptr.get(), 0, sizeof(goose_miniz::mz_stream));
        ...
        // TODO this is mostly the same as gzip_file_system.cc
        if (size < GZIP_HEADER_MINSIZE) {
        ...
        // stream is now set to beginning of payload data
        auto status = goose_miniz::mz_inflateInit2(mz_stream_ptr.get(), -MZ_DEFAULT_WINDOW_BITS);
        if (status != goose_miniz::MZ_OK) {
            throw InternalException("Failed to initialize miniz");
        }
        ...
        while (status == goose_miniz::MZ_OK) {
            ...
            status = mz_inflate(mz_stream_ptr.get(), goose_miniz::MZ_NO_FLUSH);
            if (status != goose_miniz::MZ_STREAM_END && status != goose_miniz::MZ_OK) {
