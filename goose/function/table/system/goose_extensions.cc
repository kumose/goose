...
namespace goose {
    ...
    struct GooseExtensionsData : public GlobalTableFunctionState {
        GooseExtensionsData() : offset(0) {
        }
    ...
    static unique_ptr<FunctionData> GooseExtensionsBind(ClientContext &context, TableFunctionBindInput &input,
    ...
    unique_ptr<GlobalTableFunctionState> GooseExtensionsInit(ClientContext &context, TableFunctionInitInput &input) {
        auto result = make_uniq<GooseExtensionsData>();

        auto &fs = FileSystem::GetFileSystem(context);
        auto &db = DatabaseInstance::GetDatabase(context);

        // Firstly, we go over all Default Extensions: goose_extensions always prints those, installed/loaded or not
        ...
        for (idx_t i = 0; i < extension_count; i++) {
            ...
            info.install_mode =
                    extension.statically_loaded
                        ? ExtensionInstallMode::STATICALLY_LINKED
                        : ExtensionInstallMode::NOT_INSTALLED;
        ...
        for (auto &ext_name: extensions) {
            ...
            if (auto &ext_install_info = ext_data.install_info) {
                ...
                if (entry == installed_extensions.end() || !entry->second.installed) {
                    ...
                    if (ext_data.install_info->mode == ExtensionInstallMode::STATICALLY_LINKED && info.file_path.
                        empty()) {
    ...
    void GooseExtensionsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
        auto &data = data_p.global_state->Cast<GooseExtensionsData>();
    ...
    void GooseExtensionsFun::RegisterFunction(BuiltinFunctions &set) {
        TableFunctionSet functions("goose_extensions");
        functions.AddFunction(TableFunction({}, GooseExtensionsFunction, GooseExtensionsBind, GooseExtensionsInit));
