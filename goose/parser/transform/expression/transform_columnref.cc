...
namespace goose {
    QualifiedColumnName TransformQualifiedColumnName(cantor::PGList &list) {
    ...
    unique_ptr<ParsedExpression> Transformer::TransformStarExpression(cantor::PGAStar &star) {
        ...
        if (star.except_list) {
            for (auto head = star.except_list->head; head; head = head->next) {
                auto exclude_column_list = PGPointerCast<cantor::PGList>(head->data.ptr_value);
        ...
        if (star.replace_list) {
            for (auto head = star.replace_list->head; head; head = head->next) {
                auto list = PGPointerCast<cantor::PGList>(head->data.ptr_value);
                D_ASSERT(list->length == 2);
                auto replace_expression =
                        TransformExpression(PGPointerCast<cantor::PGNode>(list->head->data.ptr_value));
                auto value = PGPointerCast<cantor::PGValue>(list->tail->data.ptr_value);
                D_ASSERT(value->type == cantor::T_PGString);
        ...
        if (star.rename_list) {
            for (auto head = star.rename_list->head; head; head = head->next) {
                auto list = PGPointerCast<cantor::PGList>(head->data.ptr_value);
                D_ASSERT(list->length == 2);
                auto rename_column_list = PGPointerCast<cantor::PGList>(list->head->data.ptr_value);
    ...
    unique_ptr<ParsedExpression> Transformer::TransformColumnRef(cantor::PGColumnRef &root) {
        ...
        auto head_node = PGPointerCast<cantor::PGNode>(fields->head->data.ptr_value);
        switch (head_node->type) {
            case cantor::T_PGString: {
                ...
                for (auto node = fields->head; node; node = node->next) {
                    column_names.emplace_back(PGPointerCast<cantor::PGValue>(node->data.ptr_value)->val.str);
                }
