...
#include <goose/common/multi_file/multi_file_column_mapper.h>
...
#include <goose/common/multi_file/union_by_name.h>
#include <algorithm>
...
namespace goose {
    ...
    shared_ptr<MultiFileList> MultiFileReader::CreateFileList(ClientContext &context, const vector<string> &paths,
        ...
        if (res->GetExpandResult() == FileExpandResult::NO_FILES && glob_input.behavior !=
    ...
    unique_ptr<MultiFileList> MultiFileReader::DynamicFilterPushdown(
        ClientContext &context, const MultiFileList &files, const MultiFileOptions &options,
    ...
    void MultiFileReader::BindOptions(MultiFileOptions &options, MultiFileList &files,
    ...
    ReaderInitializeType MultiFileReader::CreateMapping(
        ...
        return CreateMapping(context, reader_data, global_columns, global_column_ids, filters, multi_file_list,
                             bind_data,
    ...
    string GetExtendedMultiFileError(const MultiFileBindData &bind_data, const Expression &expr, BaseFileReader &reader,
        ...
        if (!bind_data.table_columns.empty()) {
            ...
            extended_error = StringUtil::Format(
                ...
                reader.GetFileName(), local_col.name, source_type, target_column, target_type, reader_type,
                function_name,
                function_name);
        } else {
        ...
        first_message = StringUtil::Format("failed to cast column \"%s\" from type %s to %s: ", local_col.name,
                                           source_type,
                                           target_type);
        return extended_error;
    }

    void MultiFileReader::FinalizeChunk(ClientContext &context, const MultiFileBindData &bind_data,
    ...
    TablePartitionInfo MultiFileReader::GetPartitionInfo(ClientContext &context,
    ...
    HivePartitioningIndex::HivePartitioningIndex(string value_p, idx_t index) : value(std::move(value_p)),
        index(index) {
    }
    ...
    void MultiFileOptions::VerifyHiveTypesArePartitions(const std::map<string, string> &partitions) const {
        for (auto &hive_type: hive_types_schema) {
            if (partitions.find(hive_type.first) == partitions.end()) {
                throw InvalidInputException("Unknown hive_type: \"%s\" does not appear to be a partition",
                                            hive_type.first);
            }
        }
    }
    ...
    Value MultiFileOptions::GetHivePartitionValue(const string &value, const string &key,
