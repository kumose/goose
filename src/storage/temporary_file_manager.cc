#include <goose/storage/temporary_file_manager.h>

#include <goose/common/types-import.h>
#include <goose/common/enum_util.h>
#include <goose/parallel/task_scheduler.h>
#include <goose/storage/buffer/temporary_file_information.h>
...
unique_ptr<FileBuffer> TemporaryFileHandle::ReadTemporaryBuffer(QueryContext context,
	...
	D_ASSERT(!::ZSTD_isError(compressed_size));
	...
	D_ASSERT(!::ZSTD_isError(decompressed_size));
...
TemporaryFileManager::CompressBuffer(TemporaryFileCompressionAdaptivity &compression_adaptivity, FileBuffer &buffer,
	...
	D_ASSERT(compression_level >= ::ZSTD_minCLevel() && compression_level <= ::ZSTD_maxCLevel());
	const auto zstd_bound = ::ZSTD_compressBound(buffer.AllocSize());
	compressed_buffer = Allocator::Get(db).Allocate(sizeof(idx_t) + zstd_bound);
	const auto zstd_size = ::ZSTD_compress(compressed_buffer.get() + sizeof(idx_t), zstd_bound,
	                                                  buffer.InternalBuffer(), buffer.AllocSize(), compression_level);
	D_ASSERT(!::ZSTD_isError(zstd_size));
...
To get usage information of the temp_directory, use 'CALL goose_temporary_files();'
		)",
		    used, max);
	}
...
string TemporaryFileManager::CreateTemporaryFileName(const TemporaryFileIdentifier &identifier) const {
	return FileSystem::GetFileSystem(db).JoinPath(
	    temp_directory, StringUtil::Format("goose_temp_storage_%s-%llu.tmp", EnumUtil::ToString(identifier.size),
	...
	if (!temp_directory.empty()) {
		...
		if (!created_directory) {
			...
			fs.ListFiles(temp_directory, [&](const string &path, bool isdir) {
				...
				if (!StringUtil::StartsWith(path, "goose_temp_")) {
					deleted_everything = false;
					return;
				}
