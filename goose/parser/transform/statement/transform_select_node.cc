...
void Transformer::TransformModifiers(cantor::PGSelectStmt &stmt, QueryNode &node) {
	...
	if (stmt.limitCount || stmt.limitOffset) {
		if (stmt.limitCount && stmt.limitCount->type == cantor::T_PGLimitPercent) {
			auto limit_percent_modifier = make_uniq<LimitPercentModifier>();
			auto expr_node = PGPointerCast<cantor::PGLimitPercent>(stmt.limitCount)->limit_percent;
...
bool Transformer::SetOperationsMatch(cantor::PGSelectStmt &root, cantor::PGNode &node) {
	if (node.type != cantor::T_PGSelectStmt) {
		// not a select or set-op - set operations cannot match
		return false;
	}
	auto &stmt = PGCast<cantor::PGSelectStmt>(node);
	if (root.op != stmt.op || root.all != stmt.all) {
		// set operation type does not match
		return false;
	}
	if (root.op != cantor::PG_SETOP_UNION && root.op != cantor::PG_SETOP_UNION_BY_NAME) {
...
void Transformer::TransformSetOperationChildren(cantor::PGSelectStmt &stmt, SetOperationNode &result) {
	D_ASSERT(stmt.larg && stmt.rarg);
	vector<reference<cantor::PGNode>> set_operations;
	...
	for (idx_t i = 0; i < set_operations.size(); i++) {
		...
		if (!SetOperationsMatch(stmt, node)) {
			...
			auto &select = PGCast<cantor::PGSelectStmt>(node);
...
unique_ptr<QueryNode> Transformer::TransformSelectInternal(cantor::PGSelectStmt &stmt) {
	D_ASSERT(stmt.type == cantor::T_PGSelectStmt);
	auto stack_checker = StackCheck();

	unique_ptr<QueryNode> node;

	switch (stmt.op) {
	case cantor::PG_SETOP_NONE: {
		node = make_uniq<SelectNode>();
		auto &result = node->Cast<SelectNode>();
		if (stmt.withClause) {
			TransformCTE(*PGPointerCast<cantor::PGWithClause>(stmt.withClause), node->cte_map);
		}
		if (stmt.windowClause) {
			for (auto window_ele = stmt.windowClause->head; window_ele != nullptr; window_ele = window_ele->next) {
				auto window_def = PGPointerCast<cantor::PGWindowDef>(window_ele->data.ptr_value);
				D_ASSERT(window_def);
		...
		if (stmt.distinctClause != nullptr) {
			...
			auto target = PGPointerCast<cantor::PGNode>(stmt.distinctClause->head->data.ptr_value);
			if (target) {
	...
	case cantor::PG_SETOP_UNION:
	case cantor::PG_SETOP_EXCEPT:
	case cantor::PG_SETOP_INTERSECT:
	case cantor::PG_SETOP_UNION_BY_NAME: {
