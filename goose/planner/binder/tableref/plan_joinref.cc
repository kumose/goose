...
#include <goose/planner/expression_binder/lateral_binder.h>
...
namespace goose {
    ...
    unique_ptr<LogicalOperator> LogicalComparisonJoin::CreateJoin(ClientContext &context, JoinType type,
        ...
        vector<JoinCondition> conditions;
        vector<unique_ptr<Expression> > arbitrary_expressions;
        LogicalComparisonJoin::ExtractJoinConditions(context, type, reftype, left_child, right_child,
                                                     std::move(condition),
    ...
    unique_ptr<LogicalOperator> Binder::CreatePlan(BoundJoinRef &ref) {
        ...
        auto result = LogicalComparisonJoin::CreateJoin(context, ref.type, ref.ref_type, std::move(left),
                                                        std::move(right),
                                                        std::move(ref.condition));
