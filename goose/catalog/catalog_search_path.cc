...
namespace goose {
    ...
    CatalogSearchEntry CatalogSearchEntry::Parse(const string &input) {
        ...
        if (pos < input.size()) {
            throw ParserException("Failed to convert entry \"%s\" to CatalogSearchEntry - expected a single entry",
                                  input);
        }
        return result;
    }
    ...
    void CatalogSearchPath::Set(vector<CatalogSearchEntry> new_paths, CatalogSetPathType set_type) {
        ...
        for (auto &path: new_paths) {
            ...
            if (path.catalog.empty()) {
                ...
                if (catalog) {
                    auto schema = catalog->GetSchema(context, catalog->GetDefaultSchema(),
