...
namespace goose {
    ...
    GetKnownColumnValues(const string &filename, const HivePartitioningFilterInfo &filter_info) {
        ...
        if (filter_info.hive_enabled) {
            ...
            for (auto &partition: partitions) {
                ...
                if (lookup_column_id != column_map.end()) {
                    result.insert(
                        make_pair(lookup_column_id->second,
                                  PartitioningColumnValue(partition.first, partition.second)));
                }
            }
        }
    ...
    static void ConvertKnownColRefToConstants(ClientContext &context, unique_ptr<Expression> &expr,
                                              const unordered_map<column_t, PartitioningColumnValue> &
                                              known_column_values,
                                              idx_t table_index) {
    ...
    Value HivePartitioning::GetValue(ClientContext &context, const string &key, const string &str_val,
        ...
        if (!value.TryCastAs(context, type)) {
            throw InvalidInputException("Unable to cast '%s' (from hive partition column '%s') to: '%s'",
                                        value.ToString(),
    ...
    idx_t HivePartitionedColumnData::RegisterNewPartition(HivePartitionKey key,
                                                          PartitionedColumnDataAppendState &state) {
        idx_t partition_id;
        if (global_state) {
