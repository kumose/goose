...
public:
	...
	GOOSE_API static timestamp_t FromString(const string &str, bool use_offset);
	...
	GOOSE_API static TimestampCastResult TryConvertTimestampTZ(const char *str, idx_t len, timestamp_t &result,
	...
	GOOSE_API static TimestampCastResult TryConvertTimestamp(const char *str, idx_t len, timestamp_t &result,
	                                                          const bool use_offset,
	                                                          optional_ptr<int32_t> nanos = nullptr,
	                                                          bool strict = false);
	GOOSE_API static TimestampCastResult TryConvertTimestamp(const char *str, idx_t len, timestamp_ns_t &result);
	GOOSE_API static timestamp_t FromCString(const char *str, idx_t len, bool use_offset = false,
	                                          optional_ptr<int32_t> nanos = nullptr);
	//! Convert a date object to a string in the format "YYYY-MM-DD hh:mm:ss"
	GOOSE_API static string ToString(timestamp_t timestamp);

	GOOSE_API static date_t GetDate(timestamp_t timestamp);

	GOOSE_API static dtime_t GetTime(timestamp_t timestamp);
	GOOSE_API static dtime_ns_t GetTimeNs(timestamp_ns_t timestamp);
	//! Create a Timestamp object from a specified (date, time) combination
	GOOSE_API static timestamp_t FromDatetime(date_t date, dtime_t time);
	GOOSE_API static bool TryFromDatetime(date_t date, dtime_t time, timestamp_t &result);
	GOOSE_API static bool TryFromDatetime(date_t date, dtime_tz_t timetz, timestamp_t &result);
	//! Scale up to ns
	GOOSE_API static bool TryFromTimestampNanos(timestamp_t ts, int32_t nanos, timestamp_ns_t &result);
	...
	GOOSE_API static void Convert(timestamp_t date, date_t &out_date, dtime_t &out_time);
	//! Extract the date and time from a given timestamp object
	GOOSE_API static void Convert(timestamp_ns_t date, date_t &out_date, dtime_t &out_time, int32_t &out_nanos);
	//! Returns current timestamp
	GOOSE_API static timestamp_t GetCurrentTimestamp();

	//! Convert the epoch (in sec) to a timestamp
	GOOSE_API static timestamp_t FromEpochSecondsPossiblyInfinite(int64_t s);
	GOOSE_API static timestamp_t FromEpochSeconds(int64_t s);
	//! Convert the epoch (in ms) to a timestamp
	GOOSE_API static timestamp_t FromEpochMsPossiblyInfinite(int64_t ms);
	GOOSE_API static timestamp_t FromEpochMs(int64_t ms);
	//! Convert the epoch (in microseconds) to a timestamp
	GOOSE_API static timestamp_t FromEpochMicroSeconds(int64_t micros);
	//! Convert the epoch (in nanoseconds) to a timestamp
	GOOSE_API static timestamp_t FromEpochNanoSecondsPossiblyInfinite(int64_t nanos);
	GOOSE_API static timestamp_t FromEpochNanoSeconds(int64_t nanos);

	//! Construct ns timestamps from various epoch units
	GOOSE_API static timestamp_ns_t TimestampNsFromEpochMicros(int64_t micros);
	GOOSE_API static timestamp_ns_t TimestampNsFromEpochMillis(int64_t millis);

	//! Try convert a timestamp to epoch (in nanoseconds)
	GOOSE_API static bool TryGetEpochNanoSeconds(timestamp_t timestamp, int64_t &result);
	//! Convert the epoch (in seconds) to a timestamp
	GOOSE_API static int64_t GetEpochSeconds(timestamp_t timestamp);
	//! Convert the epoch (in ms) to a timestamp
	GOOSE_API static int64_t GetEpochMs(timestamp_t timestamp);
	//! Convert a timestamp to epoch (in microseconds)
	GOOSE_API static int64_t GetEpochMicroSeconds(timestamp_t timestamp);
	//! Convert a timestamp to epoch (in nanoseconds)
	GOOSE_API static int64_t GetEpochNanoSeconds(timestamp_t timestamp);
	GOOSE_API static int64_t GetEpochNanoSeconds(timestamp_ns_t timestamp);
	//! Convert a timestamp to a rounded epoch at a given resolution.
	GOOSE_API static int64_t GetEpochRounded(timestamp_t timestamp, const int64_t power_of_ten);
	//! Convert a timestamp to a Julian Day
	GOOSE_API static double GetJulianDay(timestamp_t timestamp);

	//! Decompose a timestamp into its components
	GOOSE_API static TimestampComponents GetComponents(timestamp_t timestamp);
	GOOSE_API static time_t ToTimeT(timestamp_t);
	GOOSE_API static timestamp_t FromTimeT(time_t);

	GOOSE_API static bool TryParseUTCOffset(const char *str, idx_t &pos, idx_t len, int &hh, int &mm, int &ss);

	GOOSE_API static string FormatError(const string &str);
	GOOSE_API static string FormatError(string_t str);
	GOOSE_API static string UnsupportedTimezoneError(const string &str);
	GOOSE_API static string UnsupportedTimezoneError(string_t str);
	GOOSE_API static string RangeError(const string &str);
	GOOSE_API static string RangeError(string_t str);
};
...
//! Timestamp
template <>
struct hash<goose::timestamp_t> {
	std::size_t operator()(const goose::timestamp_t &k) const {
		using std::hash;
		return hash<int64_t>()((int64_t)k);
	}
};
template <>
struct hash<goose::timestamp_ms_t> {
	std::size_t operator()(const goose::timestamp_ms_t &k) const {
		using std::hash;
		return hash<int64_t>()((int64_t)k);
	}
};
template <>
struct hash<goose::timestamp_ns_t> {
	std::size_t operator()(const goose::timestamp_ns_t &k) const {
		using std::hash;
		return hash<int64_t>()((int64_t)k);
	}
};
template <>
struct hash<goose::timestamp_sec_t> {
	std::size_t operator()(const goose::timestamp_sec_t &k) const {
		using std::hash;
		return hash<int64_t>()((int64_t)k);
	}
};
template <>
struct hash<goose::timestamp_tz_t> {
	std::size_t operator()(const goose::timestamp_tz_t &k) const {
		using std::hash;
		return hash<int64_t>()((int64_t)k);
	}
};
