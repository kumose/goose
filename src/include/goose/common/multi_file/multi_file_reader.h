...
#include <goose/common/multi_file/multi_file_states.h>
...
public:
	...
	GOOSE_API static unique_ptr<MultiFileReader> Create(const TableFunction &table_function);
	//! Create a default MultiFileReader, function_name is used for errors
	GOOSE_API static unique_ptr<MultiFileReader> CreateDefault(const string &function_name = "");
	...
	GOOSE_API shared_ptr<MultiFileList>
	CreateFileList(ClientContext &context, const Value &input,
	               const FileGlobInput &glob_input = FileGlobOptions::DISALLOW_EMPTY);

	//! Parse the named parameters of a multi-file reader
	GOOSE_API virtual bool ParseOption(const string &key, const Value &val, MultiFileOptions &options,
	                                    ClientContext &context);
	//! Perform filter pushdown into the MultiFileList. Returns a new MultiFileList if filters were pushed down
	GOOSE_API virtual unique_ptr<MultiFileList> ComplexFilterPushdown(ClientContext &context, MultiFileList &files,
	                                                                   const MultiFileOptions &options,
	                                                                   MultiFilePushdownInfo &info,
	                                                                   vector<unique_ptr<Expression>> &filters);
	GOOSE_API virtual unique_ptr<MultiFileList>
	...
	//! directly.
	GOOSE_API virtual bool Bind(MultiFileOptions &options, MultiFileList &files, vector<LogicalType> &return_types,
	                             vector<string> &names, MultiFileReaderBindData &bind_data);
	//! Bind the options of the multi-file reader, potentially emitting any extra columns that are required
	GOOSE_API virtual void BindOptions(MultiFileOptions &options, MultiFileList &files,
	                                    vector<LogicalType> &return_types, vector<string> &names,
	                                    MultiFileReaderBindData &bind_data);

	//! Initialize global state used by the MultiFileReader
	GOOSE_API virtual unique_ptr<MultiFileReaderGlobalState>
	...
	GOOSE_API virtual void FinalizeBind(MultiFileReaderData &reader_data, const MultiFileOptions &file_options,
	...
	GOOSE_API virtual ReaderInitializeType
	...
	GOOSE_API virtual void FinalizeChunk(ClientContext &context, const MultiFileBindData &bind_data,
	...
	GOOSE_API virtual void GetPartitionData(ClientContext &context, const MultiFileReaderBindData &bind_data,
	                                         const MultiFileReaderData &reader_data,
	                                         optional_ptr<MultiFileReaderGlobalState> global_state,
	                                         const OperatorPartitionInfo &partition_info,
	                                         OperatorPartitionData &partition_data);

	GOOSE_API static void GetVirtualColumns(ClientContext &context, MultiFileReaderBindData &bind_data,
	...
	GOOSE_API virtual ReaderInitializeType InitializeReader(MultiFileReaderData &reader_data,
	...
	GOOSE_API virtual shared_ptr<BaseFileReader> CreateReader(ClientContext &context, GlobalTableFunctionState &gstate,
	                                                           BaseUnionData &union_data,
	                                                           const MultiFileBindData &bind_data);
	GOOSE_API virtual shared_ptr<BaseFileReader> CreateReader(ClientContext &context, GlobalTableFunctionState &gstate,
	                                                           const OpenFileInfo &file, idx_t file_idx,
	                                                           const MultiFileBindData &bind_data);
	GOOSE_API virtual shared_ptr<BaseFileReader> CreateReader(ClientContext &context, const OpenFileInfo &file,
	                                                           BaseFileReaderOptions &options,
	                                                           const MultiFileOptions &file_options,
	                                                           MultiFileReaderInterface &interface);

	//! Get partition info
	GOOSE_API virtual TablePartitionInfo GetPartitionInfo(ClientContext &context,
	...
	GOOSE_API virtual unique_ptr<Expression> GetConstantVirtualColumn(MultiFileReaderData &reader_data,
	                                                                   idx_t column_id, const LogicalType &type);

	//! Gets an expression to evaluate the given virtual column
	GOOSE_API virtual unique_ptr<Expression>
	...
	GOOSE_API virtual unique_ptr<MultiFileReader> Copy() const;

	GOOSE_API virtual FileGlobInput GetGlobInput(MultiFileReaderInterface &interface);
