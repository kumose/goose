...
namespace goose {
    ...
    Transaction &GooseTransactionManager::StartTransaction(ClientContext &context) {
        ...
        auto transaction = make_uniq<GooseTransaction>(*this, context, start_time, transaction_id,
    ...
    GooseTransactionManager::CheckpointDecision::CheckpointDecision(CheckpointType type) : can_checkpoint(true),
        type(type) {
    }
    ...
    void GooseTransactionManager::Checkpoint(ClientContext &context, bool force) {
        ...
        if (!force) {
            ...
            if (!lock) {
                ...
                throw TransactionException(
                    "Cannot CHECKPOINT: there are other write transactions active. Try using FORCE "
