...
# make this data frame available as a view in goose
...
rel = goose.from_csv_auto(temp_file_name)
print(rel)
...
print(goose.filter(test_df, 'i > 1'))
print(goose.project(test_df, 'i + 1'))
print(goose.order(test_df, 'j'))
print(goose.limit(test_df, 2))

print(goose.aggregate(test_df, "sum(i)"))
print(goose.distinct(test_df))

# when chaining only the first call needs to include the data frame parameter
print(goose.filter(test_df, 'i > 1').project('i + 1, j').order('j').limit(2))
...
# create a table in goose from the relation
...
res = goose.query_df(test_df, 'my_name_for_test_df', 'SELECT * FROM my_name_for_test_df')
print(res.df())
