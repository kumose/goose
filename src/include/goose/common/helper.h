...
#define GOOSE_EXPLICIT_FALLTHROUGH [[clang::fallthrough]]
#elif __has_cpp_attribute(gnu::fallthrough)
#define GOOSE_EXPLICIT_FALLTHROUGH [[gnu::fallthrough]]
#else
#define GOOSE_EXPLICIT_FALLTHROUGH
#endif
...
{
	return shared_ptr<DATA_TYPE>(goose_base_std::make_shared<DATA_TYPE>(std::forward<ARGS>(args)...));
}
...
#ifdef GOOSE_ENABLE_DEPRECATED_API
template <typename S, typename T, typename... Args>
...
#ifdef GOOSE_DEBUG_MOVE
template<class T>
typename std::remove_reference<T>::type&& move(T&& t) noexcept {
	// the nonsensical sizeof check ensures this is never instantiated
	static_assert(sizeof(T) == 0, "Use std::move instead of unqualified move or goose::move");
}
#endif

template <class T, class... ARGS>
static goose::unique_ptr<T> make_unique(ARGS&&... __args) { // NOLINT: mimic std style
...
template <class T, class... ARGS>
static goose::shared_ptr<T> make_shared(ARGS&&... __args) { // NOLINT: mimic std style
#ifndef GOOSE_ENABLE_DEPRECATED_API
