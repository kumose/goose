...
PhysicalOperator &PhysicalPlanGenerator::CreatePlan(LogicalCreateIndex &op) {
	...
	if (!op.table.is_goose_table()) {
		throw BinderException("Indexes can only be created on Goose tables.");
	}
	...
	auto &gs_table = op.table.Cast<GooseTableEntry>();
	IndexBuildBindInput bind_input {context, gs_table, *op.info, op.unbound_expressions};
