...
namespace goose {
    ...
    struct GooseExtensionLoadState {
        explicit GooseExtensionLoadState(DatabaseInstance &db_p, ExtensionInitResult &init_result_p)
            : db(db_p), init_result(init_result_p), database_data(nullptr) {
        }

        //! Create a GooseExtensionLoadState reference from a C API opaque pointer
        static GooseExtensionLoadState &Get(goose_extension_info info) {
            D_ASSERT(info);
            return *reinterpret_cast<goose::GooseExtensionLoadState *>(info);
        }

        //! Convert to an opaque C API pointer
        goose_extension_info ToCStruct() {
            return reinterpret_cast<goose_extension_info>(this);
        }
        ...
        //! This is the goose_database struct that will be passed to the extension during initialization. Note that the
    ...
    struct ExtensionAccess {
        ...
        static goose_extension_access CreateAccessStruct() {
            return {SetError, GetDatabase, GetAPI};
        }

        //! Called by the extension to indicate failure to initialize the extension
        static void SetError(goose_extension_info info, const char *error) {
            auto &load_state = GooseExtensionLoadState::Get(info);
        ...
        static goose_database GetDatabase(goose_extension_info info) {
            auto &load_state = GooseExtensionLoadState::Get(info);

            try {
                // Create the goose_database
                load_state.database_data = make_uniq<DatabaseWrapper>();
                load_state.database_data->database = make_shared_ptr<Goose>(load_state.db);
                return reinterpret_cast<goose_database>(load_state.database_data.get());
                ...
                load_state.error_data =
                        ErrorData(ExceptionType::UNKNOWN_TYPE,
                                  "Unknown error in GetDatabase when trying to load extension!");
                return nullptr;
            }
        }

        //! Called by the extension get a pointer the correctly versioned extension C API struct.
        static const void *GetAPI(goose_extension_info info, const char *version) {
            ...
            if (load_state.init_result.abi_type == ExtensionABIType::C_STRUCT) {
                ...
                // ignored because C_STRUCT_UNSTABLE extensions are tied 1:1 to goose verions meaning they will always
    ...
    //===--------------------------------------------------------------------===//
#ifndef GOOSE_DISABLE_EXTENSION_LOAD
    // The C++ init function
    typedef void (*ext_init_fun_t)(ExtensionLoader &);

    // The C init function
    typedef bool (*ext_init_c_api_fun_t)(goose_extension_info info, goose_extension_access *access);
    ...
    static void ComputeSHA256String(const string &to_hash, string *res) {
        ...
        *res = goose_mbedtls::MbedTlsWrapper::ComputeSha256Hash(to_hash);
    }
    ...
    ParsedExtensionMetaData ExtensionHelper::ParseExtensionMetaData(const char *metadata) noexcept {
        ...
        if (extension_abi_metadata == "C_STRUCT") {
            ...
            result.goose_capi_version = FilterZeroAtEnd(metadata_field[2]);
        } else if (extension_abi_metadata == "C_STRUCT_UNSTABLE") {
            result.abi_type = ExtensionABIType::C_STRUCT_UNSTABLE;
            result.goose_version = FilterZeroAtEnd(metadata_field[2]);
            ...
            result.goose_version = "unknown";
            result.extension_abi_metadata = extension_abi_metadata;
        }

        result.signature = string(
            metadata, ParsedExtensionMetaData::FOOTER_SIZE - ParsedExtensionMetaData::SIGNATURE_SIZE);
        return result;
    }
    ...
    ParsedExtensionMetaData ExtensionHelper::ParseExtensionMetaData(FileHandle &handle) {
        ...
        if (handle.GetFileSize() < ParsedExtensionMetaData::FOOTER_SIZE) {
            throw InvalidInputException(
                "File '%s' is not a Goose extension. Valid Goose extensions must be at least %llu bytes", handle.path,
    ...
    bool ExtensionHelper::CheckExtensionSignature(FileHandle &handle, ParsedExtensionMetaData &parsed_metadata,
        ...
        for (auto &key: ExtensionHelper::GetPublicKeys(allow_community_extensions)) {
            if (goose_mbedtls::MbedTlsWrapper::IsValidSha256Signature(key, parsed_metadata.signature, two_level_hash)) {
                return true;
                break;
            }
        }
    ...
    bool ExtensionHelper::TryInitialLoad(DatabaseInstance &db, FileSystem &fs, const string &extension,
        ...
        if (!ExtensionHelper::IsFullPath(extension)) {
            ...
            auto ComputeLocalExtensionPath = [&fs](const string &base_path, const string &extension_name) -> string {
                ...
                return fs.JoinPath(local_path, extension_name + ".goose_extension");
            };
    ...
    void ExtensionHelper::LoadExternalExtensionInternal(DatabaseInstance &db, FileSystem &fs, const string &extension,
        ...
        if (extension_init_result.abi_type == ExtensionABIType::CPP) {
            auto init_fun_name = extension_init_result.filebase + "_goose_cpp_init";
            ...
            try {
                ...
                ErrorData error(e);
                throw InvalidInputException(
                    "Initialization function \"%s\" from file \"%s\" threw an exception: \"%s\"",
        ...
        if (extension_init_result.abi_type == ExtensionABIType::C_STRUCT ||
            ...
            GooseExtensionLoadState load_state(db, extension_init_result);
    ...
    string ExtensionHelper::ExtractExtensionPrefixFromPath(const string &path) {
        ...
        if (first_colon == string::npos || first_colon < 2) {
            // needs to be at least two characters because windows c: ...
            return "";
        }
