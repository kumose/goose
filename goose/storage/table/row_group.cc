...
#include <goose/storage/statistics/string_stats.h>
...
#include <goose/storage/table/row_id_column_data.h>
...
void RowGroup::ScanCommitted(CollectionScanState &state, DataChunk &result, TableScanType type) {
	auto &transaction_manager = GooseTransactionManager::Get(GetCollection().GetAttached());
...
// GetPartitionStats
//===--------------------------------------------------------------------===//
struct GoosePartitionRowGroup : public PartitionRowGroup {
	explicit GoosePartitionRowGroup(shared_ptr<RowGroup> row_group_p, bool is_exact_p)
...
PartitionStatistics RowGroup::GetPartitionStats(SegmentNode<RowGroup> &row_group) {
	...
	if (row_group_ref.HasUnloadedDeletes() || row_group_ref.GetVersionInfoIfLoaded()) {
		...
		result.partition_row_group = make_shared_ptr<GoosePartitionRowGroup>(row_group.ReferenceNode(), false);
	} else {
		result.count_type = CountType::COUNT_EXACT;
		result.partition_row_group = make_shared_ptr<GoosePartitionRowGroup>(row_group.ReferenceNode(), true);
	}
