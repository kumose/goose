...
unique_ptr<SampleOptions> Transformer::TransformSampleOptions(optional_ptr<cantor::PGNode> options) {
	if (!options) {
		return nullptr;
	}
	auto result = make_uniq<SampleOptions>();
	auto &sample_options = PGCast<cantor::PGSampleOptions>(*options);
	auto &sample_size = *PGPointerCast<cantor::PGSampleSize>(sample_options.sample_size);
