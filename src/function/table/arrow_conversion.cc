...
#include <goose/common/types/arrow_aux_data.h>
#include <goose/common/types/arrow_string_view_type.h>
...
static void GetValidityMask(ValidityMask &mask, ArrowArray &array, idx_t chunk_offset, idx_t size,
	...
	// In certains we don't need to or cannot copy arrow's validity mask to goose.
	//
	// The conditions where we do want to copy arrow's mask to goose are:
	// 1. nulls exist
	...
	if (array.null_count != 0 && array.n_buffers > 0 && array.buffers[0]) {
		...
		mask.EnsureWritable();
#if STANDARD_VECTOR_SIZE > 64 && !GOOSE_IS_BIG_ENDIAN
...
void ArrowToGooseConversion::SetValidityMask(Vector &vector, ArrowArray &array, idx_t chunk_offset, idx_t size,
...
static void ArrowToGooseList(Vector &vector, ArrowArray &array, idx_t chunk_offset, ArrowArrayScanState &array_state,
                              idx_t size, const ArrowType &arrow_type, int64_t nested_offset,
                              const ValidityMask *parent_mask, int64_t parent_offset) {
	auto &list_info = arrow_type.GetTypeInfo<ArrowListInfo>();
	ArrowToGooseConversion::SetValidityMask(vector, array, chunk_offset, size, parent_offset, nested_offset);
	...
	ArrowToGooseConversion::SetValidityMask(child_vector, *array.children[0], chunk_offset, list_size, array.offset,
	...
	if (list_size == 0 && start_offset == 0) {
		...
		ArrowToGooseConversion::ColumnArrowToGoose(child_vector, child_array, chunk_offset, child_state, list_size,
		                                             child_type, -1);
		return;
	}
	...
	case ArrowArrayPhysicalType::DICTIONARY_ENCODED:
		...
		ArrowToGooseConversion::ColumnArrowToGooseDictionary(child_vector, child_array, chunk_offset, child_state,
		                                                       list_size, child_type,
		                                                       NumericCast<int64_t>(start_offset));
		break;
	case ArrowArrayPhysicalType::RUN_END_ENCODED:
		ArrowToGooseConversion::ColumnArrowToGooseRunEndEncoded(child_vector, child_array, chunk_offset, child_state,
		                                                          list_size, child_type,
		                                                          NumericCast<int64_t>(start_offset));
		break;
...
static void ArrowToGooseArray(Vector &vector, ArrowArray &array, idx_t chunk_offset, ArrowArrayScanState &array_state,
                               idx_t size, const ArrowType &arrow_type, int64_t nested_offset,
                               const ValidityMask *parent_mask, int64_t parent_offset) {
	...
	ArrowToGooseConversion::SetValidityMask(child_vector, *array.children[0], chunk_offset, child_count, array.offset,
	...
	if (child_count == 0 && child_offset == 0) {
		D_ASSERT(!child_array.dictionary);
		ArrowToGooseConversion::ColumnArrowToGoose(child_vector, child_array, chunk_offset, child_state, child_count,
		                                             child_type, -1);
	} else {
		if (child_array.dictionary) {
			...
			ArrowToGooseConversion::ColumnArrowToGoose(child_vector, child_array, chunk_offset, child_state,
			                                             child_count, child_type, NumericCast<int64_t>(child_offset));
		}
	}
}

static void ArrowToGooseMapVerify(Vector &vector, idx_t count) {
	auto valid_check = MapVector::CheckMapValidity(vector, count);
	switch (valid_check) {
	case MapInvalidReason::VALID:
		break;
	case MapInvalidReason::DUPLICATE_KEY: {
		throw InvalidInputException("Arrow map contains duplicate key, which isn't supported by Goose map type");
	}
	case MapInvalidReason::NULL_KEY: {
		throw InvalidInputException("Arrow map contains NULL as map key, which isn't supported by Goose map type");
	}
	default: {
...
static void SetVectorString(Vector &vector, idx_t size, char *cdata, T *offsets) {
	...
	for (idx_t row_idx = 0; row_idx < size; row_idx++) {
		...
		if (str_len > NumericLimits<uint32_t>::Maximum()) { // LCOV_EXCL_START
			throw ConversionException("Goose does not support Strings over 4GB");
		} // LCOV_EXCL_STOP
...
void ArrowToGooseConversion::ColumnArrowToGooseRunEndEncoded(Vector &vector, const ArrowArray &array,
	...
	D_ASSERT(vector.GetType() == values_type.GetGooseType());
	...
	if (!run_end_encoding.run_ends) {
		...
		run_end_encoding.run_ends = make_uniq<Vector>(run_ends_type.GetGooseType(), compressed_size);
		run_end_encoding.values = make_uniq<Vector>(values_type.GetGooseType(), compressed_size);

		ArrowToGooseConversion::ColumnArrowToGoose(*run_end_encoding.run_ends, run_ends_array, chunk_offset,
		                                             array_state, compressed_size, run_ends_type);
		auto &values = *run_end_encoding.values;
		ArrowToGooseConversion::SetValidityMask(values, values_array, chunk_offset, compressed_size,
		                                         NumericCast<int64_t>(parent_offset), nested_offset);
		ArrowToGooseConversion::ColumnArrowToGoose(values, values_array, chunk_offset, array_state, compressed_size,
		                                             values_type);
	}

	idx_t scan_offset = GetEffectiveOffset(array, NumericCast<int64_t>(parent_offset), chunk_offset, nested_offset);
	auto physical_type = run_ends_type.GetGooseType().InternalType();
	switch (physical_type) {
	case PhysicalType::INT16:
...
void ArrowToGooseConversion::ColumnArrowToGoose(Vector &vector, ArrowArray &array, idx_t chunk_offset,
	...
	if (!ignore_extensions && arrow_type.HasExtension()) {
		if (arrow_type.extension_data->arrow_to_goose) {
			// Convert the storage and then call the cast function
			Vector input_data(arrow_type.extension_data->GetInternalType());
			ColumnArrowToGoose(input_data, array, chunk_offset, array_state, size, arrow_type, nested_offset,
			                    parent_mask, parent_offset, /*ignore_extensions*/ true);
			arrow_type.extension_data->arrow_to_goose(array_state.context, input_data, vector, size);
			return;
		}
	}
	...
	case LogicalTypeId::LIST: {
		ArrowToGooseList(vector, array, chunk_offset, array_state, size, arrow_type, nested_offset, parent_mask,
		                  NumericCast<int64_t>(parent_offset));
		break;
	}
	case LogicalTypeId::ARRAY: {
		ArrowToGooseArray(vector, array, chunk_offset, array_state, size, arrow_type, nested_offset, parent_mask,
		                   NumericCast<int64_t>(parent_offset));
		break;
	}
	case LogicalTypeId::MAP: {
		ArrowToGooseList(vector, array, chunk_offset, array_state, size, arrow_type, nested_offset, parent_mask,
		                  NumericCast<int64_t>(parent_offset));
		ArrowToGooseMapVerify(vector, size);
		break;
	}
	case LogicalTypeId::STRUCT: {
		...
		for (idx_t child_idx = 0; child_idx < NumericCast<idx_t>(array.n_children); child_idx++) {
			...
			ArrowToGooseConversion::SetValidityMask(child_entry, child_array, chunk_offset, size, array.offset,
			                                         nested_offset);
			...
			switch (array_physical_type) {
			case ArrowArrayPhysicalType::DICTIONARY_ENCODED:
				ArrowToGooseConversion::ColumnArrowToGooseDictionary(
				...
				break;
			case ArrowArrayPhysicalType::RUN_END_ENCODED:
				ColumnArrowToGooseRunEndEncoded(child_entry, child_array, chunk_offset, child_state, size, child_type,
				                                 nested_offset, &struct_validity_mask,
				                                 NumericCast<uint64_t>(array.offset));
				break;
			case ArrowArrayPhysicalType::DEFAULT:
				ColumnArrowToGoose(child_entry, child_array, chunk_offset, child_state, size, child_type,
	...
	case LogicalTypeId::UNION: {
		...
		for (idx_t child_idx = 0; child_idx < NumericCast<idx_t>(array.n_children); child_idx++) {
			...
			ArrowToGooseConversion::SetValidityMask(child, child_array, chunk_offset, size,
			                                         NumericCast<int64_t>(parent_offset), nested_offset);
			auto array_physical_type = child_type.GetPhysicalType();

			switch (array_physical_type) {
			case ArrowArrayPhysicalType::DICTIONARY_ENCODED:
				ArrowToGooseConversion::ColumnArrowToGooseDictionary(child, child_array, chunk_offset, child_state,
				                                                       size, child_type);
				break;
			case ArrowArrayPhysicalType::RUN_END_ENCODED:
				ArrowToGooseConversion::ColumnArrowToGooseRunEndEncoded(child, child_array, chunk_offset, child_state,
				                                                          size, child_type);
				break;
			case ArrowArrayPhysicalType::DEFAULT:
				ArrowToGooseConversion::ColumnArrowToGoose(child, child_array, chunk_offset, child_state, size,
...
static void SetSelectionVectorLoopWithChecks(SelectionVector &sel, data_ptr_t indices_p, idx_t size) {
	...
	for (idx_t row = 0; row < size; row++) {
		if (indices[row] > NumericLimits<uint32_t>::Maximum()) {
			throw ConversionException("Goose only supports indices that fit on an uint32");
		}
...
void ArrowToGooseConversion::ColumnArrowToGooseDictionary(Vector &vector, ArrowArray &array, idx_t chunk_offset,
                                                            ArrowArrayScanState &array_state, idx_t size,
                                                            const ArrowType &arrow_type, int64_t nested_offset,
	...
	if (array_state.CacheOutdated(array.dictionary)) {
		...
		ArrowToGooseConversion::SetValidityMask(*base_vector, *array.dictionary, chunk_offset,
		...
		case ArrowArrayPhysicalType::DICTIONARY_ENCODED:
			ColumnArrowToGooseDictionary(*base_vector, *array.dictionary, chunk_offset, array_state,
			                              NumericCast<idx_t>(array.dictionary->length), dictionary_type);
			break;
		case ArrowArrayPhysicalType::RUN_END_ENCODED:
			ColumnArrowToGooseRunEndEncoded(*base_vector, *array.dictionary, chunk_offset, array_state,
			                                 NumericCast<idx_t>(array.dictionary->length), dictionary_type);
			break;
		case ArrowArrayPhysicalType::DEFAULT:
			ColumnArrowToGoose(*base_vector, *array.dictionary, chunk_offset, array_state,
			                    NumericCast<idx_t>(array.dictionary->length), dictionary_type);
			break;
		default:
			throw NotImplementedException("ArrowArrayPhysicalType not recognized");
	...
	auto offset_type = arrow_type.GetGooseType();
...
void ArrowTableFunction::ArrowToGoose(ArrowScanLocalState &scan_state, const arrow_column_map_t &arrow_convert_data,
	...
	for (idx_t idx = 0; idx < output.ColumnCount(); idx++) {
		...
		if (rowid_column_index != COLUMN_IDENTIFIER_ROW_ID) {
			if (col_idx == COLUMN_IDENTIFIER_ROW_ID) {
				...
				// that Goose believes the Arrow array is in.
				col_idx += 1;
				arrow_array_idx += 1;
			}
		} else {
		...
		switch (array_physical_type) {
		case ArrowArrayPhysicalType::DICTIONARY_ENCODED:
			ArrowToGooseConversion::ColumnArrowToGooseDictionary(output.data[idx], array, scan_state.chunk_offset,
			                                                       array_state, output.size(), arrow_type);
			break;
		case ArrowArrayPhysicalType::RUN_END_ENCODED:
			ArrowToGooseConversion::ColumnArrowToGooseRunEndEncoded(output.data[idx], array, scan_state.chunk_offset,
			                                                          array_state, output.size(), arrow_type);
			break;
		case ArrowArrayPhysicalType::DEFAULT:
			ArrowToGooseConversion::SetValidityMask(output.data[idx], array, scan_state.chunk_offset, output.size(),
			                                         parent_array.offset, -1);
			ArrowToGooseConversion::ColumnArrowToGoose(output.data[idx], array, scan_state.chunk_offset, array_state,
