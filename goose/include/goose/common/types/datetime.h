...
//! Time
template <>
struct hash<goose::dtime_t> {
	std::size_t operator()(const goose::dtime_t &k) const {
		using std::hash;
...
template <>
struct hash<goose::dtime_tz_t> {
	std::size_t operator()(const goose::dtime_tz_t &k) const {
		using std::hash;
