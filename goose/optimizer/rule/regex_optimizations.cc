...
namespace goose {
    ...
    static LikeString GetLikeStringEscaped(xre2::Regexp *regexp, bool contains = false) {
        D_ASSERT(regexp->op() == xre2::kRegexpLiteralString || regexp->op() == xre2::kRegexpLiteral);
        LikeString ret;

        if (regexp->parse_flags() & xre2::Regexp::FoldCase ||
            !(regexp->parse_flags() & xre2::Regexp::OneLine)) {
        ...
        if (regexp->op() == xre2::kRegexpLiteralString) {
    ...
    static LikeString LikeMatchFromRegex(xre2::RE2 &pattern) {
        ...
        while (cur_sub_index < num_subs) {
            switch (subs[cur_sub_index]->op()) {
                ...
                case xre2::kRegexpStar:
                    ...
                    ret.exists = false;
                    return ret;
                case xre2::kRegexpLiteralString:
                case xre2::kRegexpLiteral: {
                ...
                case xre2::kRegexpEndText:
                case xre2::kRegexpEmptyMatch:
                case xre2::kRegexpBeginText: {
                    break;
                }
                default:
    ...
    unique_ptr<Expression> RegexOptimizationRule::Apply(LogicalOperator &op, vector<reference<Expression> > &bindings,
        ...
        xre2::RE2::Options parsed_options = regexp_bind_data.options;
        ...
        xre2::RE2 pattern(patt_str, parsed_options);
        if (!pattern.ok()) {
        ...
        if (pattern.Regexp()->op() == xre2::kRegexpLiteralString ||
            pattern.Regexp()->op() == xre2::kRegexpLiteral) {
            // convert to contains.
        ...
        auto like_expression =
                make_uniq<BoundFunctionExpression>(root.return_type, LikeFun::GetFunction(), std::move(root.children),
                                                   nullptr);
