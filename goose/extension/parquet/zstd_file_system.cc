...
struct ZstdStreamWrapper : public StreamWrapper {
	...
	::ZSTD_DStream *zstd_stream_ptr = nullptr;
	::ZSTD_CStream *zstd_compress_ptr = nullptr;
	bool writing = false;
	...
	try {
		...
		if (file && file->child_handle) {
			...
			try {
				...
				if (logger) {
					GOOSE_LOG_ERROR(logger, "ZstdStreamWrapper::~ZstdStreamWrapper()\t\t" + data.Message());
				}
			} catch (...) { // NOLINT
			}
		}
...
void ZstdStreamWrapper::Initialize(QueryContext context, CompressedFile &file, bool write) {
	...
	this->writing = write;
	if (write) {
		zstd_compress_ptr = ::ZSTD_createCStream();
	} else {
		zstd_stream_ptr = ::ZSTD_createDStream();
	}
}
...
bool ZstdStreamWrapper::Read(StreamData &sd) {
	...
	auto res = ::ZSTD_decompressStream(zstd_stream_ptr, &out_buffer, &in_buffer);
	if (::ZSTD_isError(res)) {
		throw IOException(::ZSTD_getErrorName(res));
	}
...
void ZstdStreamWrapper::Write(CompressedFile &file, StreamData &sd, data_ptr_t uncompressed_data,
	...
	while (remaining > 0) {
		...
		out_buffer.pos = 0;
		auto res =
		    ::ZSTD_compressStream2(zstd_compress_ptr, &out_buffer, &in_buffer, ::ZSTD_e_continue);
		if (::ZSTD_isError(res)) {
			throw IOException(::ZSTD_getErrorName(res));
		}
...
void ZstdStreamWrapper::FlushStream() {
	...
	while (true) {
		...
		auto res =
		    ::ZSTD_compressStream2(zstd_compress_ptr, &out_buffer, &in_buffer, ::ZSTD_e_end);
		if (::ZSTD_isError(res)) {
			throw IOException(::ZSTD_getErrorName(res));
		}
		idx_t written_to_output = out_buffer.pos;
...
void ZstdStreamWrapper::Close() {
	...
	if (zstd_stream_ptr) {
		::ZSTD_freeDStream(zstd_stream_ptr);
	}
	if (zstd_compress_ptr) {
		::ZSTD_freeCStream(zstd_compress_ptr);
	}
