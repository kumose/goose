...
#include <goose/extension/parquet/writer/parquet_write_stats.h>
...
struct BaseParquetOperator {
	...
	static uint64_t XXHash64(const TGT &target_value) {
		return goose::XXH64(&target_value, sizeof(target_value), 0);
	}
...
struct ParquetBaseGeoOperator : public BaseParquetOperator {
	...
	template <class SRC, class TGT>
	static uint64_t XXHash64(const TGT &target_value) {
		return goose::XXH64(target_value.GetData(), target_value.GetSize(), 0);
	}
...
struct ParquetIntervalOperator : public BaseParquetOperator {
	...
	template <class SRC, class TGT>
	static uint64_t XXHash64(const TGT &target_value) {
		return goose::XXH64(target_value.bytes, ParquetIntervalTargetType::PARQUET_INTERVAL_SIZE, 0);
	}
};
...
struct ParquetUUIDOperator : public BaseParquetOperator {
	...
	template <class SRC, class TGT>
	static uint64_t XXHash64(const TGT &target_value) {
		return goose::XXH64(target_value.bytes, ParquetUUIDTargetType::PARQUET_UUID_SIZE, 0);
	}
