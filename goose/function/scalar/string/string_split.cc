...
struct ConstantRegexpStringSplit {
	static idx_t Find(const char *input_data, idx_t input_size, const char *delim_data, idx_t delim_size,
	                  idx_t &match_size, void *data) {
		D_ASSERT(data);
		auto regex = reinterpret_cast<xre2::RE2 *>(data);
		std::string_view match;
		if (!regex->Match(std::string_view(input_data, input_size), 0, input_size, RE2::UNANCHORED, &match, 1)) {
...
struct RegexpStringSplit {
	static idx_t Find(const char *input_data, idx_t input_size, const char *delim_data, idx_t delim_size,
	                  idx_t &match_size, void *data) {
		xre2::RE2 regex(std::string_view(delim_data, delim_size));
		if (!regex.ok()) {
