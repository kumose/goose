...
#include <goose/common/types/conflict_manager.h>
...
#include <goose/planner/constraints/list.h>
...
#include <goose/planner/expression_binder/check_binder.h>
#include <goose/planner/expression_binder/constant_binder.h>
...
#include <goose/storage/table/update_state.h>
...
// Scan
//===--------------------------------------------------------------------===//
void DataTable::InitializeScan(ClientContext &context, GooseTransaction &transaction, TableScanState &state,
...
void DataTable::InitializeScanWithOffset(GooseTransaction &transaction, TableScanState &state,
...
void DataTable::Scan(GooseTransaction &transaction, DataChunk &result, TableScanState &state) {
...
// Fetch
//===--------------------------------------------------------------------===//
void DataTable::Fetch(GooseTransaction &transaction, DataChunk &result, const vector<StorageIndex> &column_ids,
...
bool DataTable::CanFetch(GooseTransaction &transaction, const row_t row_id) {
...
void DataTable::AppendLock(GooseTransaction &transaction, TableAppendState &state) {
...
void DataTable::InitializeAppend(GooseTransaction &transaction, TableAppendState &state) {
...
void DataTable::FinalizeAppend(GooseTransaction &transaction, TableAppendState &state) {
	row_groups->FinalizeAppend(transaction, state);
}

void DataTable::ScanTableSegment(GooseTransaction &transaction, idx_t row_start, idx_t count,
...
void DataTable::WriteToLog(GooseTransaction &transaction, WriteAheadLog &log, idx_t row_start, idx_t count,
                           optional_ptr<StorageCommitState> commit_state) {
...
void DataTable::RevertAppend(GooseTransaction &transaction, idx_t start_row, idx_t count) {
	lock_guard<mutex> lock(append_lock);
...
ErrorData DataTable::AppendToIndexes(optional_ptr<TableIndexList> delete_indexes, DataChunk &table_chunk,
	...
	D_ASSERT(IsMainTable());
	auto active_checkpoint = GetAttached().GetTransactionManager().Cast<GooseTransactionManager>().GetActiveCheckpoint();
...
unique_ptr<TableDeleteState> DataTable::InitializeDelete(TableCatalogEntry &table, ClientContext &context,
                                                         const vector<unique_ptr<BoundConstraint>> &bound_constraints) {
	auto &transaction = GooseTransaction::Get(context, db);
	// Bind all indexes.
	info->BindIndexes(context);
