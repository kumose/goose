...
#include <goose/execution/adaptive_filter.h>
...
struct ParquetReaderScanState {
	...
	goose_base_std::unique_ptr<goose_apache::thrift::protocol::TProtocol> thrift_file_proto;
...
public:
	...
	const goose_parquet::FileMetaData *GetFileMetadata() const;
	string static GetUniqueFileIdentifier(const goose_parquet::EncryptionAlgorithm &encryption_algorithm);

	uint32_t Read(goose_apache::thrift::TBase &object, TProtocol &iprot);
	uint32_t ReadEncrypted(goose_apache::thrift::TBase &object, TProtocol &iprot,
	                       CryptoMetaData &aad_crypto_metadata) const;
	uint32_t ReadData(goose_apache::thrift::protocol::TProtocol &iprot, const data_ptr_t buffer,
	                  const uint32_t buffer_size);
	uint32_t ReadDataEncrypted(goose_apache::thrift::protocol::TProtocol &iprot, const data_ptr_t buffer,
	...
	static void GetPartitionStats(const goose_parquet::FileMetaData &metadata, vector<PartitionStatistics> &result,
...
private:
	...
	const goose_parquet::RowGroup &GetGroup(ParquetReaderScanState &state);
	...
	MultiFileColumnDefinition ParseColumnDefinition(const goose_parquet::FileMetaData &file_meta_data,
