...
// otherwise we have different definitions for mbedtls_pk_context /
...
void MbedTlsWrapper::SHA256State::AddBytes(goose::const_data_ptr_t input_bytes, goose::idx_t len) {
	auto context = reinterpret_cast<mbedtls_sha256_context *>(sha_context);
...
void MbedTlsWrapper::SHA256State::AddBytes(goose::data_ptr_t input_bytes, goose::idx_t len) {
	AddBytes(goose::const_data_ptr_t(input_bytes), len);
}
...
void MbedTlsWrapper::SHA256State::FinalizeDerivedKey(goose::data_ptr_t hash) {
	auto context = reinterpret_cast<mbedtls_sha256_context *>(sha_context);

	if (mbedtls_sha256_finish(context, (goose::data_ptr_t)hash)) {
		throw std::runtime_error("SHA256 Error");
	}
}
...
const mbedtls_cipher_info_t *MbedTlsWrapper::AESStateMBEDTLS::GetCipher(size_t key_len){
	...
	switch(cipher){
		case goose::EncryptionTypes::CipherType::GCM:
		    switch (key_len) {
		    case 16:
		...
		case goose::EncryptionTypes::CipherType::CTR:
		    switch (key_len) {
		    case 16:
		...
		case goose::EncryptionTypes::CipherType::CBC:
			switch (key_len) {
			case 16:
		...
		default:
				throw goose::InternalException("Invalid Encryption/Decryption Cipher: %s", goose::EncryptionTypes::CipherToString(cipher));
	}
}

void MbedTlsWrapper::AESStateMBEDTLS::SecureClearData(goose::data_ptr_t data, goose::idx_t len) {
	mbedtls_platform_zeroize(data, len);
}

MbedTlsWrapper::AESStateMBEDTLS::AESStateMBEDTLS(goose::EncryptionTypes::CipherType cipher_p, goose::idx_t key_len) : EncryptionState(cipher_p, key_len), context(goose::make_uniq<mbedtls_cipher_context_t>()) {
	...
	if (cipher == goose::EncryptionTypes::CBC && mbedtls_cipher_set_padding_mode(context.get(), MBEDTLS_PADDING_PKCS7)) {
...
static void ThrowInsecureRNG() {
	throw goose::InvalidConfigurationException("Goose requires a secure random engine to be loaded to enable secure crypto. Normally, this will be handled automatically by Goose by autoloading the `httpfs` Extension, but that seems to have failed. Please ensure the httpfs extension is loaded manually using `LOAD httpfs`.");
}

void MbedTlsWrapper::AESStateMBEDTLS::GenerateRandomData(goose::data_ptr_t data, goose::idx_t len) {
	ThrowInsecureRNG();
}

void MbedTlsWrapper::AESStateMBEDTLS::InitializeInternal(goose::const_data_ptr_t iv, goose::idx_t iv_len, goose::const_data_ptr_t aad, goose::idx_t aad_len){
...
void MbedTlsWrapper::AESStateMBEDTLS::InitializeEncryption(goose::const_data_ptr_t iv, goose::idx_t iv_len, goose::const_data_ptr_t key, goose::idx_t key_len_p, goose::const_data_ptr_t aad, goose::idx_t aad_len) {
	ThrowInsecureRNG();
}

void MbedTlsWrapper::AESStateMBEDTLS::InitializeDecryption(goose::const_data_ptr_t iv, goose::idx_t iv_len, goose::const_data_ptr_t key, goose::idx_t key_len_p, goose::const_data_ptr_t aad, goose::idx_t aad_len) {
	mode = goose::EncryptionTypes::DECRYPT;

	if (key_len_p != key_len) {
		throw goose::InternalException("Invalid encryption key length, expected %llu, got %llu", key_len, key_len_p);
	}
...
size_t MbedTlsWrapper::AESStateMBEDTLS::Process(goose::const_data_ptr_t in, goose::idx_t in_len, goose::data_ptr_t out,
                                                   goose::idx_t out_len) {

	// GCM works in-place, CTR and CBC don't
	auto use_out_copy = in == out && cipher != goose::EncryptionTypes::CipherType::GCM;

	auto out_ptr = out;
	std::unique_ptr<goose::data_t[]> out_copy;
	if (use_out_copy) {
		out_copy.reset(new goose::data_t[out_len]);
		out_ptr = out_copy.get();
	}

	size_t out_len_res = goose::NumericCast<size_t>(out_len);
...
void MbedTlsWrapper::AESStateMBEDTLS::FinalizeGCM(goose::data_ptr_t tag, goose::idx_t tag_len){

	switch (mode) {

	case goose::EncryptionTypes::ENCRYPT: {
	...
	case goose::EncryptionTypes::DECRYPT: {
		if (mbedtls_cipher_check_tag(context.get(), tag, tag_len)) {
			throw goose::InvalidInputException(
	...
	default:
		throw goose::InternalException("Unhandled encryption mode %d", static_cast<int>(mode));
	}
}

size_t MbedTlsWrapper::AESStateMBEDTLS::Finalize(goose::data_ptr_t out, goose::idx_t out_len, goose::data_ptr_t tag,
													goose::idx_t tag_len) {
	...
	if (cipher == goose::EncryptionTypes::GCM) {
