...
GooseTransaction::GooseTransaction(GooseTransactionManager &manager, ClientContext &context_p, transaction_t start_time,
...
void GooseTransaction::PushDelete(DataTable &table, RowVersionManager &info, idx_t vector_idx, row_t rows[], idx_t count,
                                 idx_t base_row) {
	ModifyTable(table);
