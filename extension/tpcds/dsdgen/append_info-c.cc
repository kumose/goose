...
append_info *append_info_get(void *info_list, int table_id) {
	auto &append_vector = *((goose::vector<goose::unique_ptr<tpcds_append_information>> *)info_list);
...
void append_varchar(append_info info, const char *value) {
	...
	if (append_info->IsNull()) {
		...
		append_info->appender.Append<goose::string_t>(goose::string_t(value));
	}
}
...
void append_date(append_info info, int64_t value) {
	...
	if (append_info->IsNull() || value < 0) {
		...
		auto ddate = goose::Date::FromDate(dTemp.year, dTemp.month, dTemp.day);
		append_info->appender.Append<goose::date_t>(ddate);
	}
}
...
void append_decimal(append_info info, decimal_t *val) {
	...
	D_ASSERT(type.id() == goose::LogicalTypeId::DECIMAL);
	D_ASSERT(goose::DecimalType::GetScale(type) == val->precision);
