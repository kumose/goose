...
protected:
	GOOSE_API BaseAppender(Allocator &allocator, const AppenderType type);
	GOOSE_API BaseAppender(Allocator &allocator, vector<LogicalType> types, const AppenderType type,
	                        const idx_t flush_count = DEFAULT_FLUSH_COUNT);

public:
	GOOSE_API virtual ~BaseAppender();
	...
	GOOSE_API void BeginRow();
	//! Finishes appending the current row.
	GOOSE_API void EndRow();

	// Append functions
	template <class T>
	void Append(T value) = delete;
	GOOSE_API void Append(DataChunk &target, const Value &value, idx_t col, idx_t row);

	GOOSE_API void Append(const char *value, uint32_t length);
	...
	GOOSE_API void AppendDataChunk(DataChunk &value);
...
public:
	GOOSE_API Appender(Connection &con, const string &database_name, const string &schema_name,
	                    const string &table_name, const idx_t flush_memory_threshold = DConstants::INVALID_INDEX);
	GOOSE_API Appender(Connection &con, const string &schema_name, const string &table_name,
	                    const idx_t flush_memory_threshold = DConstants::INVALID_INDEX);
	GOOSE_API Appender(Connection &con, const string &table_name,
	                    const idx_t flush_memory_threshold = DConstants::INVALID_INDEX);
	GOOSE_API ~Appender() override;
...
public:
	GOOSE_API QueryAppender(Connection &con, string query, vector<LogicalType> types,
	                         vector<string> names = vector<string>(), string table_name = string(),
	                         const idx_t flush_memory_threshold = DConstants::INVALID_INDEX);
	GOOSE_API ~QueryAppender() override;
...
public:
	GOOSE_API InternalAppender(ClientContext &context, TableCatalogEntry &table,
	                            const idx_t flush_count = DEFAULT_FLUSH_COUNT,
	                            const idx_t flush_memory_threshold = DConstants::INVALID_INDEX);
	GOOSE_API ~InternalAppender() override;
...
template <>
GOOSE_API void BaseAppender::Append(bool value);
template <>
GOOSE_API void BaseAppender::Append(int8_t value);
template <>
GOOSE_API void BaseAppender::Append(int16_t value);
template <>
GOOSE_API void BaseAppender::Append(int32_t value);
template <>
GOOSE_API void BaseAppender::Append(int64_t value);
template <>
GOOSE_API void BaseAppender::Append(hugeint_t value);
template <>
GOOSE_API void BaseAppender::Append(uhugeint_t value);
template <>
GOOSE_API void BaseAppender::Append(uint8_t value);
template <>
GOOSE_API void BaseAppender::Append(uint16_t value);
template <>
GOOSE_API void BaseAppender::Append(uint32_t value);
template <>
GOOSE_API void BaseAppender::Append(uint64_t value);
template <>
GOOSE_API void BaseAppender::Append(float value);
template <>
GOOSE_API void BaseAppender::Append(double value);
template <>
GOOSE_API void BaseAppender::Append(date_t value);
template <>
GOOSE_API void BaseAppender::Append(dtime_t value);
template <>
GOOSE_API void BaseAppender::Append(timestamp_t value);
template <>
GOOSE_API void BaseAppender::Append(interval_t value);
template <>
GOOSE_API void BaseAppender::Append(const char *value);
template <>
GOOSE_API void BaseAppender::Append(string_t value);
template <>
GOOSE_API void BaseAppender::Append(Value value);
template <>
GOOSE_API void BaseAppender::Append(std::nullptr_t value);
