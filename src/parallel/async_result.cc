...
#include <goose/parallel/sleep_async_task.h>
#endif
...
AsyncResult &AsyncResult::operator=(goose::SourceResultType t) {
	return operator=(AsyncResult(t));
}

AsyncResult &AsyncResult::operator=(goose::AsyncResultType t) {
	return operator=(AsyncResult(t));
}
...
vector<unique_ptr<AsyncTask>> AsyncResult::GenerateTestTasks() {
	...
	case 0:
		...
		tasks.push_back(make_uniq<SleepAsyncTask>(rand() % 32));
		tasks.push_back(make_uniq<SleepAsyncTask>(rand() % 32));
#ifndef AVOID_GOOSE_DEBUG_ASYNC_THROW
	case 1:
