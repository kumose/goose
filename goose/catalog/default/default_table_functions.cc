...
FROM histogram_values(source, col_name, bin_count := bin_count, technique := technique);
	...
	{DEFAULT_SCHEMA, "goose_logs_parsed", {"log_type"}, {}, R"(
SELECT * EXCLUDE (message), UNNEST(parse_goose_log_message(log_type, message))
FROM goose_logs(denormalized_table=1)
WHERE type ILIKE log_type
)"},
	{DEFAULT_SCHEMA, "goose_profiling_settings", {}, {}, R"(
