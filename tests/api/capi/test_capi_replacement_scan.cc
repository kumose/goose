...
void number_scanner(goose_replacement_scan_info info, const char *table_name, void *data) {
	...
	goose_replacement_scan_set_function_name(info, "range");
	auto val = goose_create_int64(number + num_data->number);
	goose_replacement_scan_add_parameter(info, val);
...
TEST_CASE("Test replacement scans in C API", "[capi]") {
	...
	goose_add_replacement_scan(tester.database, number_scanner, (void *)base_number, destroy_base_number);
...
void error_replacement_scan(goose_replacement_scan_info info, const char *table_name, void *data) {
	goose_replacement_scan_set_error(NULL, NULL);
	goose_replacement_scan_set_error(info, NULL);
	goose_replacement_scan_set_error(info, "error in replacement scan");
}
...
TEST_CASE("Test error replacement scan", "[capi]") {
	...
	goose_add_replacement_scan(tester.database, error_replacement_scan, NULL, NULL);
