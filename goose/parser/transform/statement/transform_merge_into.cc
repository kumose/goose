...
namespace goose {
    unique_ptr<MergeIntoAction> Transformer::TransformMergeIntoAction(cantor::PGMatchAction &action) {
        auto result = make_uniq<MergeIntoAction>();
        if (action.andClause) {
            result->condition = TransformExpression(action.andClause);
        }
        switch (action.actionType) {
            case cantor::MERGE_ACTION_TYPE_UPDATE:
                ...
                break;
            case cantor::MERGE_ACTION_TYPE_DELETE:
                result->action_type = MergeActionType::MERGE_DELETE;
                break;
            case cantor::MERGE_ACTION_TYPE_INSERT:
                ...
                break;
            case cantor::MERGE_ACTION_TYPE_DO_NOTHING:
                result->action_type = MergeActionType::MERGE_DO_NOTHING;
                break;
            case cantor::MERGE_ACTION_TYPE_ERROR:
    ...
    unique_ptr<SQLStatement> Transformer::TransformMergeInto(cantor::PGMergeIntoStmt &stmt) {
