...
namespace goose {
    bool Highlighting::IsEnabled() {
        return goose_shell::ShellHighlight::IsEnabled();
    }

    static tokenType convertToken(goose::SimplifiedTokenType token_type) {
        switch (token_type) {
            case goose::SimplifiedTokenType::SIMPLIFIED_TOKEN_IDENTIFIER:
                return tokenType::TOKEN_IDENTIFIER;
            case goose::SimplifiedTokenType::SIMPLIFIED_TOKEN_NUMERIC_CONSTANT:
                return tokenType::TOKEN_NUMERIC_CONSTANT;
            case goose::SimplifiedTokenType::SIMPLIFIED_TOKEN_STRING_CONSTANT:
                return tokenType::TOKEN_STRING_CONSTANT;
            case goose::SimplifiedTokenType::SIMPLIFIED_TOKEN_OPERATOR:
                return tokenType::TOKEN_OPERATOR;
            case goose::SimplifiedTokenType::SIMPLIFIED_TOKEN_KEYWORD:
                return tokenType::TOKEN_KEYWORD;
            case goose::SimplifiedTokenType::SIMPLIFIED_TOKEN_COMMENT:
                return tokenType::TOKEN_COMMENT;
            default:
                throw goose::InternalException("Unrecognized token type");
        }
    }

    static vector<highlightToken> GetParseTokens(char *buf, size_t len) {
        string sql(buf, len);
        auto parseTokens = goose::Parser::Tokenize(sql);
    ...
    string Highlighting::HighlightText(char *buf, size_t len, size_t start_pos, size_t end_pos,
        ...
        for (size_t i = 0; i < tokens.size(); i++) {
            ...
            goose_shell::HighlightElementType element_type;
            switch (token.type) {
                case tokenType::TOKEN_KEYWORD:
                    element_type = goose_shell::HighlightElementType::KEYWORD;
                    break;
                case tokenType::TOKEN_NUMERIC_CONSTANT:
                    element_type = goose_shell::HighlightElementType::NUMERIC_CONSTANT;
                    break;
                case tokenType::TOKEN_STRING_CONSTANT:
                    element_type = goose_shell::HighlightElementType::STRING_CONSTANT;
                    break;
                case tokenType::TOKEN_CONTINUATION:
                    element_type = goose_shell::HighlightElementType::CONTINUATION;
                    break;
                case tokenType::TOKEN_CONTINUATION_SELECTED:
                    element_type = goose_shell::HighlightElementType::CONTINUATION_SELECTED;
                    break;
                case tokenType::TOKEN_BRACKET:
                    element_type = goose_shell::HighlightElementType::BRACKET;
                    break;
                case tokenType::TOKEN_ERROR:
                    element_type = goose_shell::HighlightElementType::ERROR_TOKEN;
                    break;
                case tokenType::TOKEN_COMMENT:
                    element_type = goose_shell::HighlightElementType::COMMENT;
                    break;
                default:
                    element_type = goose_shell::HighlightElementType::NONE;
                    break;
            }
            ...
            if (token.extra_highlighting == ExtraHighlightingType::UNDERLINE) {
                ...
                // add bold
                if (intensity == goose_shell::PrintIntensity::UNDERLINE) {
                    intensity = goose_shell::PrintIntensity::BOLD_UNDERLINE;
                } else if (intensity == goose_shell::PrintIntensity::BOLD) {
                    // might be a weird choice - but "flip" bold fonts if we encounter them
                    intensity = goose_shell::PrintIntensity::STANDARD;
                } else {
                    intensity = goose_shell::PrintIntensity::BOLD;
                }
            }
            string terminal_code = goose_shell::ShellHighlight::TerminalCode(color, intensity);
            if (terminal_code.empty()) {
                ss << text;
            } else {
                ss << terminal_code;
                ss << text;
                ss << goose_shell::ShellHighlight::ResetTerminalCode();
            }
        }
        return ss.str();
    }
