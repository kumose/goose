...
#define O_DIRECT 0
    ...
    public:
        void Close() override {
            if (fd != -1) {
                close(fd);
                fd = -1;
                GOOSE_LOG_FILE_SYSTEM_CLOSE((*this));
            }
        };
    };
    ...
    static string AdditionalProcessInfo(FileSystem &fs, pid_t pid) {
        if (pid == getpid()) {
            return "Lock is already held in current process, likely another Goose instance";
        }
    ...
    static string AdditionalProcessInfo(FileSystem &fs, pid_t pid) {
        ...
        return StringUtil::Format("Conflicting lock is held in %s%s",
                                  !process_name.empty()
                                      ? StringUtil::Format("%s (PID %d)", process_name, pid)
                                      : StringUtil::Format("PID %d", pid),
                                  !process_owner.empty() ? StringUtil::Format(" by user %s", process_owner) : "");
    }
            ...
            if (file_type != FileType::FILE_TYPE_FIFO && file_type != FileType::FILE_TYPE_SOCKET) {
                ...
                if (has_error) {
                    if (retained_errno == ENOTSUP) {
                        ...
                        if (flags.Lock() == FileLockType::READ_LOCK) {
                            ...
                            extended_error =
                                    "File locks are not supported for this file system, cannot open the file in "
                ...
                if (has_error) {
                    if (extended_error.empty()) {
                        ...
                        if (rc == -1) {
                            // fnctl does not want to help us
    ...
    void LocalFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
        ...
        while (nr_bytes > 0) {
            ...
            if (bytes_read == -1) {
                throw IOException({{"errno", std::to_string(errno)}}, "Could not read from file \"%s\": %s",
                                  handle.path,
                                  strerror(errno));
            }
            if (bytes_read == 0) {
        ...
        GOOSE_LOG_FILE_SYSTEM_READ(handle, bytes_to_read, location - UnsafeNumericCast<idx_t>(bytes_to_read));
    }
    ...
    int64_t LocalFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes) {
        ...
        GOOSE_LOG_FILE_SYSTEM_READ(handle, bytes_read, unix_handle.current_pos);
    ...
    void LocalFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
        ...
        GOOSE_LOG_FILE_SYSTEM_WRITE(handle, nr_bytes, location);
    }
    ...
    int64_t LocalFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes) {
        ...
        GOOSE_LOG_FILE_SYSTEM_WRITE(handle, nr_bytes, unix_handle.current_pos);
    ...
    void LocalFileSystem::CreateDirectory(const string &directory, optional_ptr<FileOpener> opener) {
        ...
        if (stat(normalized_dir, &st) != 0) {
            ...
            if (mkdir(normalized_dir, 0755) != 0 && errno != EEXIST) {
                throw IOException({{"errno", std::to_string(errno)}}, "Failed to create directory \"%s\": %s",
                                  directory,
                                  strerror(errno));
            }
    ...
    bool LocalFileSystem::ListFilesExtended(const string &directory,
        ...
        goose::unique_ptr<DIR, std::function<void(DIR *)> > dir_unique_ptr(dir, [](DIR *d) { closedir(d); });
    ...
    static string AdditionalLockInfo(const std::wstring path) {
        ...
        // allocate
        auto process_info_buffer = goose::unique_ptr<RM_PROCESS_INFO[]>(new RM_PROCESS_INFO[process_info_size_needed]);
    ...
    unique_ptr<FileHandle> LocalFileSystem::OpenFile(const string &path_p, FileOpenFlags flags,
        ...
        if (opener) {
            ...
            GOOSE_LOG_FILE_SYSTEM_OPEN((*handle));
        }
    ...
    void LocalFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
        ...
        GOOSE_LOG_FILE_SYSTEM_READ(handle, bytes_read, location);
    }
    ...
    int64_t LocalFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes) {
        ...
        GOOSE_LOG_FILE_SYSTEM_READ(handle, bytes_read, pos - bytes_read);
        return bytes_read;
    }
    ...
    void LocalFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
        ...
        if (bytes_written != nr_bytes) {
            throw IOException("Could not write all bytes from file \"%s\": wanted=%lld wrote=%lld", handle.path,
                              nr_bytes,
                              bytes_written);
        }
        GOOSE_LOG_FILE_SYSTEM_WRITE(handle, bytes_written, location);
    }
    ...
    int64_t LocalFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes) {
        ...
        pos += bytes_written;
        GOOSE_LOG_FILE_SYSTEM_WRITE(handle, bytes_written, pos - bytes_written);
        return bytes_written;
    }
    ...
    vector<OpenFileInfo> LocalFileSystem::FetchFileWithoutGlob(const string &path, FileOpener *opener,
                                                               bool absolute_path) {
