...
#include <goose/common/enums/thread_pin_mode.h>
...
#include <goose/storage/compression/bitpacking.h>
...
struct DBConfigOptions {
	...
	//! The number of external threads that work on Goose tasks. Default: 1.
	...
	//! Serialize the metadata on checkpoint with compatibility for a given Goose version.
	SerializationCompatibility serialization_compatibility = SerializationCompatibility::Default();
	//! Initialize the database with the standard set of Goose functions
	...
	//! Goose API surface
	string goose_api;
	//! Metadata from Goose callers
	string custom_user_agent;
	//! Encrypt the temp files
	bool temp_file_encryption = false;
	//! The default block allocation size for new goose database files (new as-in, they do not yet exist).
	idx_t default_block_alloc_size = DEFAULT_BLOCK_ALLOC_SIZE;
	//! The default block header size for new goose database files.
	idx_t default_block_header_size = GOOSE_BLOCK_HEADER_STORAGE_SIZE;
...
public:
	GOOSE_API DBConfig();
	explicit GOOSE_API DBConfig(bool read_only);
	GOOSE_API DBConfig(const case_insensitive_map_t<Value> &config_dict, bool read_only);
	GOOSE_API ~DBConfig();
	...
	case_insensitive_map_t<goose::unique_ptr<StorageExtension>> storage_extensions;
...
public:
	GOOSE_API static DBConfig &GetConfig(ClientContext &context);
	GOOSE_API static DBConfig &GetConfig(DatabaseInstance &db);
	GOOSE_API static DBConfig &Get(AttachedDatabase &db);
	GOOSE_API static const DBConfig &GetConfig(const ClientContext &context);
	GOOSE_API static const DBConfig &GetConfig(const DatabaseInstance &db);
	GOOSE_API static vector<ConfigurationOption> GetOptions();
	GOOSE_API static vector<ConfigurationAlias> GetAliases();
	GOOSE_API static idx_t GetOptionCount();
	GOOSE_API static idx_t GetAliasCount();
	GOOSE_API static vector<string> GetOptionNames();
	GOOSE_API static bool IsInMemoryDatabase(const char *database_path);

	GOOSE_API void AddExtensionOption(const string &name, string description, LogicalType parameter,
	                                   const Value &default_value = Value(), set_option_callback_t function = nullptr,
	                                   SetScope default_scope = SetScope::SESSION);
	GOOSE_API bool HasExtensionOption(const string &name);
	//! Fetch an option by index. Returns a pointer to the option, or nullptr if out of range
	GOOSE_API static optional_ptr<const ConfigurationOption> GetOptionByIndex(idx_t index);
	//! Fetcha n alias by index, or nullptr if out of range
	GOOSE_API static optional_ptr<const ConfigurationAlias> GetAliasByIndex(idx_t index);
	//! Fetch an option by name. Returns a pointer to the option, or nullptr if none exists.
	GOOSE_API static optional_ptr<const ConfigurationOption> GetOptionByName(const String &name);
	GOOSE_API void SetOption(const ConfigurationOption &option, const Value &value);
	GOOSE_API void SetOption(optional_ptr<DatabaseInstance> db, const ConfigurationOption &option, const Value &value);
	GOOSE_API void SetOptionByName(const string &name, const Value &value);
	GOOSE_API void SetOptionsByName(const case_insensitive_map_t<Value> &values);
	GOOSE_API void ResetOption(optional_ptr<DatabaseInstance> db, const ConfigurationOption &option);
	GOOSE_API void SetOption(const String &name, Value value);
	GOOSE_API void ResetOption(const String &name);
	GOOSE_API void ResetGenericOption(const String &name);
	static LogicalType ParseLogicalType(const string &type);

	GOOSE_API void CheckLock(const String &name);

	GOOSE_API static idx_t ParseMemoryLimit(const string &arg);

	//! Returns the list of possible compression functions for the physical type.
	GOOSE_API vector<reference<CompressionFunction>> GetCompressionFunctions(const PhysicalType physical_type);
	//! Returns the compression function matching the compression and physical type.
	GOOSE_API optional_ptr<CompressionFunction> GetCompressionFunction(CompressionType type,
	                                                                    const PhysicalType physical_type);
	//! Sets the disabled compression methods
	GOOSE_API void SetDisabledCompressionMethods(const vector<CompressionType> &disabled_compression_methods);
	//! Returns a list of disabled compression methods
	GOOSE_API vector<CompressionType> GetDisabledCompressionMethods() const;

	//! Returns the encode function matching the encoding name.
	GOOSE_API optional_ptr<EncodingFunction> GetEncodeFunction(const string &name) const;
	GOOSE_API void RegisterEncodeFunction(const EncodingFunction &function) const;
	//! Returns the encode function names.
	GOOSE_API vector<reference<EncodingFunction>> GetLoadedEncodedFunctions() const;
	//! Returns the encode function matching the encoding name.
	GOOSE_API ArrowTypeExtension GetArrowExtension(ArrowExtensionMetadata info) const;
	GOOSE_API ArrowTypeExtension GetArrowExtension(const LogicalType &type) const;
	GOOSE_API bool HasArrowExtension(const LogicalType &type) const;
	GOOSE_API bool HasArrowExtension(ArrowExtensionMetadata info) const;
	GOOSE_API void RegisterArrowExtension(const ArrowTypeExtension &extension) const;

	bool operator==(const DBConfig &other);
	bool operator!=(const DBConfig &other);

	GOOSE_API CastFunctionSet &GetCastFunctions();
	GOOSE_API CollationBinding &GetCollationBinding();
	GOOSE_API IndexTypeSet &GetIndexTypes();
