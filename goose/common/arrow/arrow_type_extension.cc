...
#include <goose/common/arrow/schema_metadata.h>
...
ArrowTypeExtension::ArrowTypeExtension(ArrowExtensionMetadata &extension_metadata, unique_ptr<ArrowType> type)
	...
	type_extension = make_shared_ptr<ArrowTypeExtensionData>(type->GetGooseType());
}
...
ArrowTypeExtension::ArrowTypeExtension(string vendor_name, string type_name,
                                       ...
                                       shared_ptr<ArrowTypeExtensionData> type, cast_arrow_goose_t arrow_to_goose,
                                       cast_goose_arrow_t goose_to_arrow)
    : populate_arrow_schema(populate_arrow_schema), get_type(get_type),
      extension_metadata(ArrowExtensionMetadata::ARROW_EXTENSION_NON_CANONICAL, std::move(vendor_name),
                         std::move(type_name), {}),
      type_extension(std::move(type)) {
	type_extension->arrow_to_goose = arrow_to_goose;
	type_extension->goose_to_arrow = goose_to_arrow;
}
...
unique_ptr<ArrowType> ArrowTypeExtension::GetType(const ArrowSchema &schema,
	...
	auto goose_type = type_extension->GetGooseType();
	return make_uniq<ArrowType>(goose_type);
}
...
LogicalTypeId ArrowTypeExtension::GetLogicalTypeId() const {
	return type_extension->GetGooseType().id();
}

LogicalType ArrowTypeExtension::GetLogicalType() const {
	return type_extension->GetGooseType();
}
...
void ArrowTypeExtension::PopulateArrowSchema(GooseArrowSchemaHolder &root_holder, ArrowSchema &child,
                                             const LogicalType &goose_type, ClientContext &context,
                                             const ArrowTypeExtension &extension) {
	if (extension.populate_arrow_schema) {
		extension.populate_arrow_schema(root_holder, child, goose_type, context, extension);
		return;
	}
...
struct ArrowJson {
	...
	static void PopulateSchema(GooseArrowSchemaHolder &root_holder, ArrowSchema &schema, const LogicalType &type,
...
struct ArrowBool8 {
	static void ArrowToGoose(ClientContext &context, Vector &source, Vector &result, idx_t count) {
	...
	static void GooseToArrow(ClientContext &context, Vector &source, Vector &result, idx_t count) {
...
struct ArrowGeometry {
	static unique_ptr<ArrowType> GetType(const ArrowSchema &schema, const ArrowSchemaMetadata &schema_metadata) {
		...
		if (!extension_metadata.empty()) {
			unique_ptr<goose_yyjson::yyjson_doc, void (*)(goose_yyjson::yyjson_doc *)> doc(
			    goose_yyjson::yyjson_read(extension_metadata.data(), extension_metadata.size(),
			                               goose_yyjson::YYJSON_READ_NOFLAG),
			    goose_yyjson::yyjson_doc_free);
			if (!doc) {
				throw SerializationException("Invalid JSON in GeoArrow metadata");
			}

			goose_yyjson::yyjson_val *val = yyjson_doc_get_root(doc.get());
			if (!yyjson_is_obj(val)) {
				throw SerializationException("Invalid GeoArrow metadata: not a JSON object");
			}

			goose_yyjson::yyjson_val *edges = yyjson_obj_get(val, "edges");
	...
	static void ArrowToGoose(ClientContext &, Vector &source, Vector &result, idx_t count) {
...
void ArrowTypeExtensionSet::Initialize(const DBConfig &config) {
	...
	config.RegisterArrowExtension(
	    {"arrow.bool8", "c",
	     make_shared_ptr<ArrowTypeExtensionData>(LogicalType::BOOLEAN, LogicalType::TINYINT, ArrowBool8::ArrowToGoose,
	                                             ArrowBool8::GooseToArrow)});

	config.RegisterArrowExtension(
	    {"Goose", "hugeint", "w:16", make_shared_ptr<ArrowTypeExtensionData>(LogicalType::HUGEINT)});
	config.RegisterArrowExtension(
	    {"Goose", "uhugeint", "w:16", make_shared_ptr<ArrowTypeExtensionData>(LogicalType::UHUGEINT)});
	config.RegisterArrowExtension(
	    {"Goose", "time_tz", "w:8", make_shared_ptr<ArrowTypeExtensionData>(LogicalType::TIME_TZ)});

	config.RegisterArrowExtension(
	    {"geoarrow.wkb", ArrowGeometry::PopulateSchema, ArrowGeometry::GetType,
	     make_shared_ptr<ArrowTypeExtensionData>(LogicalType::GEOMETRY(), LogicalType::BLOB, ArrowGeometry::ArrowToGoose,
	                                             ArrowGeometry::GooseToArrow)});
	...
	config.RegisterArrowExtension({"Goose", "bit", &ArrowBit::PopulateSchema, &ArrowBit::GetType,
	                               make_shared_ptr<ArrowTypeExtensionData>(LogicalType::BIT), nullptr, nullptr});

	config.RegisterArrowExtension({"Goose", "bignum", &ArrowBignum::PopulateSchema, &ArrowBignum::GetType,
