...
#include <goose/planner/subquery/flatten_dependent_join.h>
...
#include <goose/function/aggregate/distributive_function_utils.h>
...
#include <goose/planner/subquery/has_correlated_expressions.h>
#include <goose/planner/subquery/rewrite_correlated_expressions.h>
...
namespace goose {
    FlattenDependentJoins::FlattenDependentJoins(Binder &binder, const CorrelatedColumns &correlated,
                                                 bool perform_delim,
        ...
        switch (plan->type) {
            case LogicalOperatorType::LOGICAL_DEPENDENT_JOIN: {
                ...
                if (parent) {
                    ...
                    if (entry->second) {
                        op.children[0] =
                                PushDownDependentJoin(std::move(op.children[0]), parent_propagate_null_values,
                                                      lateral_depth);
                    } else {
                    ...
                    RewriteCorrelatedExpressions recursive_rewriter(base_binding, correlated_map, next_lateral_depth,
                                                                    true);
                ...
                if (!op.perform_delim) {
                    ...
                    auto row_number = make_uniq<BoundWindowExpression>(ExpressionType::WINDOW_ROW_NUMBER,
                                                                       LogicalType::BIGINT,
                                                                       nullptr, nullptr);
                ...
                if (delim_join->children[1]->type == LogicalOperatorType::LOGICAL_MATERIALIZED_CTE) {
                    ...
                    if (entry != flatten.has_correlated_expressions.end()) {
                        if (!entry->second) {
                            ...
                            cte->children[1] = Decorrelate(std::move(delim_join), parent_propagate_null_values,
                                                           lateral_depth);
                            return cte;
                        }
                    }
                ...
                delim_join->children[1] =
                        flatten.PushDownDependentJoin(std::move(delim_join->children[1]), propagate_null_values,
                                                      lateral_depth);
                ...
                for (idx_t i = 0; i < op.correlated_columns.size(); i++) {
                    ...
                    op.duplicate_eliminated_columns.push_back(
                        make_uniq<BoundColumnRefExpression>(col.type, col.binding));
                ...
                if (op.is_lateral_join && op.subquery_type == SubqueryType::INVALID) {
                    ...
                    CreateDelimJoinConditions(op, op.correlated_columns, plan_columns, flatten.delim_offset,
                                              op.perform_delim);
                    ...
                    if (!op.arbitrary_expressions.empty()) {
                        ...
                        if (op.join_type != JoinType::INNER) {
                            throw BinderException(
                                "Join condition for non-inner LATERAL JOIN must be a comparison between the "
                                "left and right side");
                        }
    ...
    unique_ptr<LogicalOperator> FlattenDependentJoins::PushDownDependentJoinInternal(unique_ptr<LogicalOperator> plan,
        ...
        switch (plan->type) {
            ...
            case LogicalOperatorType::LOGICAL_PROJECTION: {
                ...
                if (exit_projection) {
                    auto cross_product =
                            LogicalCrossProduct::Create(Decorrelate(std::move(plan->children[0])),
                                                        std::move(delim_scan));
            ...
            case LogicalOperatorType::LOGICAL_AGGREGATE_AND_GROUP_BY: {
                ...
                if (!perform_delim) {
                    ...
                    for (idx_t i = 0; i < correlated_columns.size(); i++) {
                        ...
                        auto first_fun =
                                make_uniq<BoundAggregateExpression>(std::move(first_aggregate),
                                                                    std::move(aggr_children), nullptr,
                ...
                for (idx_t i = 0; i < correlated_columns.size(); i++) {
                    ...
                    if (join.type == LogicalOperatorType::LOGICAL_COMPARISON_JOIN ||
                        ...
                        auto conjunction = make_uniq<BoundConjunctionExpression>(
                            ExpressionType::CONJUNCTION_AND, std::move(comparison),
            ...
            case LogicalOperatorType::LOGICAL_LIMIT: {
                ...
                if (plan->children[0]->type == LogicalOperatorType::LOGICAL_ORDER_BY) {
                    ...
                    child = PushDownDependentJoinInternal(std::move(order_by->children[0]),
                ...
                auto row_num_ref =
                        make_uniq<BoundColumnRefExpression>(rownum_alias, LogicalType::BIGINT,
                                                            ColumnBinding(window_index, 0));
            ...
            case LogicalOperatorType::LOGICAL_UNION: {
                    ...
                    if (plan->children[i]->type == LogicalOperatorType::LOGICAL_CROSS_PRODUCT) {
                        ...
                        for (idx_t col_idx = 0; col_idx < bindings.size(); col_idx++) {
                            expressions.push_back(
                                make_uniq<BoundColumnRefExpression>(types[col_idx], bindings[col_idx]));
                        }
                ...
                for (idx_t i = 1; i < plan->children.size(); i++) {
                    D_ASSERT(
                        plan->children[0]->GetColumnBindings().size() == plan->children[i]->GetColumnBindings().size());
                }
                for (auto &child: plan->children) {
                    child->ResolveOperatorTypes();
                }
            ...
            case LogicalOperatorType::LOGICAL_RECURSIVE_CTE: {
                ...
                RewriteCorrelatedExpressions recursive_rewriter(this->base_binding, correlated_map, lateral_depth + 1,
                                                                true);
            ...
            case LogicalOperatorType::LOGICAL_DELIM_JOIN: {
                throw BinderException(
                    "Nested lateral joins or lateral joins in correlated subqueries are not (yet) supported");
            }
