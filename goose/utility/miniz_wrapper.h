...
public:
	...
	~MiniZStream() {
		...
		case MiniZStreamType::MINIZ_TYPE_INFLATE:
			goose_miniz::mz_inflateEnd(&stream);
			break;
		case MiniZStreamType::MINIZ_TYPE_DEFLATE:
			goose_miniz::mz_deflateEnd(&stream);
			break;
		default:
			break;
		}
	...
	void FormatException(const char *error_msg, int mz_ret) {
		auto err = goose_miniz::mz_error(mz_ret);
	...
	void Decompress(const char *compressed_data, size_t compressed_size, char *out_data, size_t out_size) {
		...
		while (compressed_size > 0) {
			...
			mz_ret = mz_inflate(&stream, goose_miniz::MZ_FINISH);
			if (mz_ret != goose_miniz::MZ_OK && mz_ret != goose_miniz::MZ_STREAM_END) {
	...
	static size_t MaxCompressedLength(size_t input_size) {
		return goose_miniz::mz_compressBound(input_size) + GZIP_HEADER_MINSIZE + GZIP_FOOTER_SIZE;
	}
	...
	static void InitializeGZIPFooter(unsigned char *gzip_footer, goose_miniz::mz_ulong crc, idx_t uncompressed_size) {
	...
	void Compress(const char *uncompressed_data, size_t uncompressed_size, char *out_data, size_t *out_size) {
		auto mz_ret =
		    mz_deflateInit2(&stream, goose_miniz::MZ_DEFAULT_LEVEL, MZ_DEFLATED, -MZ_DEFAULT_WINDOW_BITS, 1, 0);
		if (mz_ret != goose_miniz::MZ_OK) {
			FormatException("Failed to initialize miniz", mz_ret);
		}
		...
		mz_ret = mz_deflate(&stream, goose_miniz::MZ_FINISH);
		if (mz_ret != goose_miniz::MZ_OK && mz_ret != goose_miniz::MZ_STREAM_END) {
			FormatException("Failed to compress GZIP block", mz_ret);
		}
		auto gzip_footer = gzip_body + stream.total_out;
		auto crc = goose_miniz::mz_crc32(MZ_CRC32_INIT, reinterpret_cast<const unsigned char *>(uncompressed_data),
		                                  uncompressed_size);
...
private:
	void ResetStreamInternal() {
		memset(&stream, 0, sizeof(goose_miniz::mz_stream));
	}

private:
	goose_miniz::mz_stream stream;
