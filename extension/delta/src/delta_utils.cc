...
ffi::EngineError *GooseEngineError::AllocateError(ffi::KernelError etype, ffi::KernelStringSlice msg) {
	auto error = new GooseEngineError;
	error->etype = etype;
	error->error_message = string(msg.ptr, msg.len);
	return error;
}

string GooseEngineError::KernelErrorEnumToString(ffi::KernelError err) {
...
void GooseEngineError::Throw(string from_where) {
...
uintptr_t PredicateVisitor::VisitConstantFilter(const string &col_name, const ConstantFilter &filter,
	...
	auto maybe_left =
	    ffi::visit_expression_column(state, KernelUtils::ToDeltaString(col_name), GooseEngineError::AllocateError);
	...
	case LogicalType::VARCHAR: {
		...
		auto maybe_right = ffi::visit_expression_literal_string(state, KernelUtils::ToDeltaString(col_name),
		                                                        GooseEngineError::AllocateError);
