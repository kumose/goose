...
#include <goose/execution/index/art/art_builder.h>
#include <goose/execution/index/art/art_key.h>
#include <goose/execution/index/art/art_merger.h>
#include <goose/execution/index/art/art_operator.h>
#include <goose/execution/index/art/art_scanner.h>
#include <goose/execution/index/art/base_leaf.h>
#include <goose/execution/index/art/base_node.h>
#include <goose/execution/index/art/iterator.h>
#include <goose/execution/index/art/leaf.h>
#include <goose/execution/index/art/node256.h>
#include <goose/execution/index/art/node256_leaf.h>
...
namespace goose {
    ...
    ART::ART(const string &name, const IndexConstraintType index_constraint_type, const vector<column_t> &column_ids,
        ...
        : BoundIndex(name, ART::TYPE_NAME, index_constraint_type, column_ids, table_io_manager, unbound_expressions,
                     db),
    ...
    unique_ptr<IndexScanState> ART::TryInitializeScan(const Expression &expr, const Expression &filter_expr) {
        ...
        if (filter_match) {
            ...
            low_comparison_type = between.lower_inclusive
    ...
    void ART::GenerateKeyVectors(ArenaAllocator &allocator, DataChunk &input, Vector &row_ids,
                                 unsafe_vector<ARTKey> &keys,
    ...
    ErrorData ART::Insert(IndexLock &l, DataChunk &chunk, Vector &row_ids, IndexAppendInfo &info) {
        ...
        if (conflict_type == ARTConflictType::CONSTRAINT) {
            auto msg = AppendRowError(chunk, conflict_idx.GetIndex());
            return ErrorData(ConstraintException("PRIMARY KEY or UNIQUE constraint violation: duplicate key \"%s\"",
                                                 msg));
        }
    ...
    idx_t ART::TryDelete(IndexLock &state, DataChunk &entries, Vector &row_ids,
    ...
    bool ART::SearchCloseRange(ARTKey &lower_bound, ARTKey &upper_bound, bool left_equal, bool right_equal,
                               idx_t max_count,
                               set<row_t> &row_ids) {
    ...
    void ART::VerifyLeaf(const Node &leaf, const ARTKey &key, DeleteIndexInfo delete_index_info,
    ...
    string ART::ToStringInternal(bool display_ascii) {
        if (tree.HasMetadata()) {
            return "\nART: \n" + tree.ToString(
                       *this, ToStringOptions(0, false, display_ascii, nullptr, 0, 0, true, false));
        }
        return "[empty]";
    }
