...
namespace goose {
    static void AssertVariant(const BaseStatistics &stats) {
        if (TURBO_UNLIKELY(stats.GetStatsType() != StatisticsType::VARIANT_STATS)) {
    ...
    static string ToStringInternal(const BaseStatistics &stats) {
        string result;
        result = StringUtil::Format("fully_shredded: %s",
                                    VariantShreddedStats::IsFullyShredded(stats) ? "true" : "false");
    ...
    string VariantStats::ToString(const BaseStatistics &stats) {
        ...
        if (is_shredded) {
            result += ", shredding: {";
            result += StringUtil::Format("typed_value_type: %s, ",
                                         ToStructuredType(stats.child_stats[1].type).ToString());
