...
#include <goose/common/cgroups.h>
...
#define GOOSE_SETTING_CALLBACK(_PARAM)                                                                                \
	{                                                                                                                  \
		_PARAM::Name, _PARAM::Description, _PARAM::InputType, nullptr, nullptr, nullptr, nullptr, nullptr,             \
		    _PARAM::DefaultScope, _PARAM::DefaultValue, _PARAM::OnSet                                                  \
	}
#define GOOSE_GLOBAL(_PARAM)                                                                                          \
	{                                                                                                                  \
...
#define GOOSE_GLOBAL_LOCAL(_PARAM)                                                                                    \
	{                                                                                                                  \
...
#define GOOSE_SETTING_ALIAS(_ALIAS, _SETTING_INDEX)                                                                   \
...
#define FINAL_ALIAS                                                                                                    \
    ...
    static const ConfigurationOption internal_options[] = {
        ...
        GOOSE_GLOBAL(AccessModeSetting),
        GOOSE_GLOBAL(AllocatorBackgroundThreadsSetting),
        GOOSE_GLOBAL(AllocatorBulkDeallocationFlushThresholdSetting),
        GOOSE_GLOBAL(AllocatorFlushThresholdSetting),
        GOOSE_GLOBAL(AllowCommunityExtensionsSetting),
        GOOSE_SETTING(AllowExtensionsMetadataMismatchSetting),
        GOOSE_GLOBAL(AllowParserOverrideExtensionSetting),
        GOOSE_GLOBAL(AllowPersistentSecretsSetting),
        GOOSE_GLOBAL(AllowUnredactedSecretsSetting),
        GOOSE_GLOBAL(AllowUnsignedExtensionsSetting),
        GOOSE_GLOBAL(AllowedDirectoriesSetting),
        GOOSE_GLOBAL(AllowedPathsSetting),
        GOOSE_SETTING(ArrowLargeBufferSizeSetting),
        GOOSE_SETTING(ArrowLosslessConversionSetting),
        GOOSE_SETTING(ArrowOutputListViewSetting),
        GOOSE_SETTING_CALLBACK(ArrowOutputVersionSetting),
        GOOSE_SETTING(AsofLoopJoinThresholdSetting),
        GOOSE_GLOBAL(AutoinstallExtensionRepositorySetting),
        GOOSE_GLOBAL(AutoinstallKnownExtensionsSetting),
        GOOSE_GLOBAL(AutoloadKnownExtensionsSetting),
        GOOSE_GLOBAL(BlockAllocatorMemorySetting),
        GOOSE_SETTING(CatalogErrorMaxSchemasSetting),
        GOOSE_GLOBAL(CheckpointThresholdSetting),
        GOOSE_GLOBAL(CustomExtensionRepositorySetting),
        GOOSE_LOCAL(CustomProfilingSettingsSetting),
        GOOSE_GLOBAL(CustomUserAgentSetting),
        GOOSE_SETTING(DebugAsofIejoinSetting),
        GOOSE_SETTING_CALLBACK(DebugCheckpointAbortSetting),
        GOOSE_SETTING(DebugCheckpointSleepMsSetting),
        GOOSE_LOCAL(DebugForceExternalSetting),
        GOOSE_SETTING(DebugForceNoCrossProductSetting),
        GOOSE_SETTING_CALLBACK(DebugPhysicalTableScanExecutionStrategySetting),
        GOOSE_SETTING(DebugSkipCheckpointOnCommitSetting),
        GOOSE_SETTING(DebugVerifyBlocksSetting),
        GOOSE_SETTING_CALLBACK(DebugVerifyVectorSetting),
        GOOSE_SETTING_CALLBACK(DebugWindowModeSetting),
        GOOSE_GLOBAL(DefaultBlockSizeSetting),
        GOOSE_SETTING_CALLBACK(DefaultCollationSetting),
        GOOSE_SETTING_CALLBACK(DefaultNullOrderSetting),
        GOOSE_SETTING_CALLBACK(DefaultOrderSetting),
        GOOSE_GLOBAL(DefaultSecretStorageSetting),
        GOOSE_GLOBAL(DisableDatabaseInvalidationSetting),
        GOOSE_SETTING(DisableTimestamptzCastsSetting),
        GOOSE_GLOBAL(DisabledCompressionMethodsSetting),
        GOOSE_GLOBAL(DisabledFilesystemsSetting),
        GOOSE_GLOBAL(DisabledLogTypes),
        GOOSE_GLOBAL(DisabledOptimizersSetting),
        GOOSE_GLOBAL(GooseAPISetting),
        GOOSE_SETTING(DynamicOrFilterThresholdSetting),
        GOOSE_GLOBAL(EnableExternalAccessSetting),
        GOOSE_GLOBAL(EnableExternalFileCacheSetting),
        GOOSE_SETTING(EnableFSSTVectorsSetting),
        GOOSE_LOCAL(EnableHTTPLoggingSetting),
        GOOSE_GLOBAL(EnableHTTPMetadataCacheSetting),
        GOOSE_GLOBAL(EnableLogging),
        GOOSE_SETTING(EnableMacroDependenciesSetting),
        GOOSE_SETTING(EnableObjectCacheSetting),
        GOOSE_LOCAL(EnableProfilingSetting),
        GOOSE_LOCAL(EnableProgressBarSetting),
        GOOSE_LOCAL(EnableProgressBarPrintSetting),
        GOOSE_SETTING(EnableViewDependenciesSetting),
        GOOSE_GLOBAL(EnabledLogTypes),
        GOOSE_LOCAL(ErrorsAsJSONSetting),
        GOOSE_SETTING(ExperimentalMetadataReuseSetting),
        GOOSE_LOCAL(ExplainOutputSetting),
        GOOSE_GLOBAL(ExtensionDirectoriesSetting),
        GOOSE_GLOBAL(ExtensionDirectorySetting),
        GOOSE_GLOBAL(ExternalThreadsSetting),
        GOOSE_LOCAL(FileSearchPathSetting),
        GOOSE_GLOBAL(ForceBitpackingModeSetting),
        GOOSE_GLOBAL(ForceCompressionSetting),
        GOOSE_GLOBAL(ForceVariantShredding),
        GOOSE_LOCAL(HomeDirectorySetting),
        GOOSE_LOCAL(HTTPLoggingOutputSetting),
        GOOSE_GLOBAL(HTTPProxySetting),
        GOOSE_GLOBAL(HTTPProxyPasswordSetting),
        GOOSE_GLOBAL(HTTPProxyUsernameSetting),
        GOOSE_SETTING(IeeeFloatingPointOpsSetting),
        GOOSE_SETTING(ImmediateTransactionModeSetting),
        GOOSE_SETTING(IndexScanMaxCountSetting),
        GOOSE_SETTING_CALLBACK(IndexScanPercentageSetting),
        GOOSE_SETTING(IntegerDivisionSetting),
        GOOSE_LOCAL(LambdaSyntaxSetting),
        GOOSE_SETTING(LateMaterializationMaxRowsSetting),
        GOOSE_GLOBAL(LockConfigurationSetting),
        GOOSE_LOCAL(LogQueryPathSetting),
        GOOSE_GLOBAL(LoggingLevel),
        GOOSE_GLOBAL(LoggingMode),
        GOOSE_GLOBAL(LoggingStorage),
        GOOSE_LOCAL(MaxExpressionDepthSetting),
        GOOSE_GLOBAL(MaxMemorySetting),
        GOOSE_GLOBAL(MaxTempDirectorySizeSetting),
        GOOSE_SETTING(MaxVacuumTasksSetting),
        GOOSE_SETTING(MergeJoinThresholdSetting),
        GOOSE_SETTING(NestedLoopJoinThresholdSetting),
        GOOSE_SETTING(OldImplicitCastingSetting),
        GOOSE_SETTING(OrderByNonIntegerLiteralSetting),
        GOOSE_SETTING_CALLBACK(OrderedAggregateThresholdSetting),
        GOOSE_SETTING(PartitionedWriteFlushThresholdSetting),
        GOOSE_SETTING(PartitionedWriteMaxOpenFilesSetting),
        GOOSE_GLOBAL(PasswordSetting),
        GOOSE_SETTING_CALLBACK(PerfectHtThresholdSetting),
        GOOSE_GLOBAL(PinThreadsSetting),
        GOOSE_SETTING(PivotFilterThresholdSetting),
        GOOSE_SETTING(PivotLimitSetting),
        GOOSE_SETTING(PreferRangeJoinsSetting),
        GOOSE_SETTING(PreserveIdentifierCaseSetting),
        GOOSE_SETTING(PreserveInsertionOrderSetting),
        GOOSE_SETTING(ProduceArrowStringViewSetting),
        GOOSE_LOCAL(ProfileOutputSetting),
        GOOSE_LOCAL(ProfilingCoverageSetting),
        GOOSE_LOCAL(ProfilingModeSetting),
        GOOSE_LOCAL(ProgressBarTimeSetting),
        GOOSE_SETTING(ScalarSubqueryErrorOnMultipleRowsSetting),
        GOOSE_GLOBAL(SchedulerProcessPartialSetting),
        GOOSE_LOCAL(SchemaSetting),
        GOOSE_LOCAL(SearchPathSetting),
        GOOSE_GLOBAL(SecretDirectorySetting),
        GOOSE_SETTING_CALLBACK(StorageBlockPrefetchSetting),
        GOOSE_GLOBAL(StorageCompatibilityVersionSetting),
        GOOSE_LOCAL(StreamingBufferSizeSetting),
        GOOSE_GLOBAL(TempDirectorySetting),
        GOOSE_GLOBAL(TempFileEncryptionSetting),
        GOOSE_GLOBAL(ThreadsSetting),
        GOOSE_GLOBAL(UsernameSetting),
        GOOSE_SETTING_CALLBACK(ValidateExternalFileCacheSetting),
        GOOSE_GLOBAL(VariantMinimumShreddingSize),
        GOOSE_SETTING(WriteBufferRowGroupCountSetting),
        GOOSE_GLOBAL(ZstdMinStringLengthSetting),
        FINAL_SETTING
    };

    static const ConfigurationAlias setting_aliases[] = {
        GOOSE_SETTING_ALIAS("memory_limit", 90),
        GOOSE_SETTING_ALIAS("null_order", 38),
        GOOSE_SETTING_ALIAS("profiling_output", 109),
        GOOSE_SETTING_ALIAS("user", 124),
        GOOSE_SETTING_ALIAS("wal_autocheckpoint", 22),
        GOOSE_SETTING_ALIAS("worker_threads", 123),
        FINAL_ALIAS
    };
    ...
    LogicalType DBConfig::ParseLogicalType(const string &type) {
        ...
        if (StringUtil::StartsWith(type, "UNION(") && StringUtil::EndsWith(type, ")")) {
            ...
            for (idx_t member_idx = 0; member_idx < union_members_vect.size(); member_idx++) {
                ...
                vector<string> union_member_parts = StringUtil::SplitWithParentheses(
                    union_members_vect[member_idx], ' ');
        ...
        if (StringUtil::StartsWith(type, "STRUCT(") && StringUtil::EndsWith(type, ")")) {
            ...
            for (idx_t member_idx = 0; member_idx < struct_members_vect.size(); member_idx++) {
                ...
                vector<string> struct_member_parts = StringUtil::SplitWithParentheses(
                    struct_members_vect[member_idx], ' ');
        ...
        LogicalType type_id = StringUtil::CIEquals(type, "ANY")
                                  ? LogicalType::ANY
                                  : TransformStringToLogicalTypeId(type);
    ...
    void DBConfig::AddExtensionOption(const string &name, string description, LogicalType parameter,
        ...
        extension_parameters.insert(make_pair(
            name, ExtensionOption(std::move(description), std::move(parameter), function, default_value,
                                  default_scope)));
    ...
    const string DBConfig::UserAgent() const {
        ...
        if (!options.goose_api.empty()) {
            user_agent += " " + options.goose_api;
        }
    ...
    SerializationCompatibility SerializationCompatibility::FromString(const string &input) {
        ...
        if (!serialization_version.IsValid()) {
            ...
            throw InvalidInputException("The version string '%s' is not a known Goose version, valid options are: %s",
                                        input, StringUtil::Join(candidates, ", "));
        }
        SerializationCompatibility result;
        result.goose_version = input;
