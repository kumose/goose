#include "core_functions/scalar/map_functions.h"
    ...
    namespace {
        ...
        unique_ptr<Expression> SwitchBindExpression(FunctionBindExpressionInput &input) {
            ...
            for (idx_t i = 0; i < keys_unpacked.size(); i++) {
                if (base_expr) {
                    auto max_type =
                            LogicalType::MaxLogicalType(input.context, base_expr->return_type,
                                                        keys_unpacked[i]->return_type);
                    ...
                    case_check.when_expr =
                            BoundCastExpression::AddCastToType(input.context, std::move(keys_unpacked[i]),
                                                               LogicalType::BOOLEAN);
                }
