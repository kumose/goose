...
TEST_CASE("Test streaming results in C API", "[capi]") {
	...
	while (true) {
		...
		if (state == GOOSE_PENDING_RESULT_READY) {
			break;
		}
	}
	...
	idx_t value = goose::DConstants::INVALID_INDEX;
	idx_t result_count = 0;
	while (chunk) {
		auto old_value = value;

		auto vector = chunk->GetVector(0);
		uint32_t *data = (uint32_t *)goose_vector_get_data(vector);
		value = data[0];
		if (old_value != goose::DConstants::INVALID_INDEX) {
...
TEST_CASE("Test streaming arrow results in C API", "[capi][arrow]") {
	...
	goose_result_arrow_array(result->InternalResult(), chunk->GetChunk(), nullptr);

	int nb_row = 0;
	while (chunk) {
		ArrowArray *arrow_array = new ArrowArray();
		goose_result_arrow_array(result->InternalResult(), chunk->GetChunk(), (goose_arrow_array *)&arrow_array);
...
TEST_CASE("Test query progress and interrupt in C API", "[capi]") {
	...
	// test null handling
	REQUIRE(goose_query_progress(nullptr).percentage == -1.0);
	goose_interrupt(nullptr);
	...
	REQUIRE(goose_query_progress(tester.connection).percentage == -1.0);
	// test zero progress with query
	REQUIRE(prepared.Prepare(tester, "select count(*) from tbl where a = (select min(a) from tbl_2)"));
	REQUIRE(pending.PendingStreaming(prepared));
	REQUIRE(goose_query_progress(tester.connection).percentage == 0.0);

	// test progress
	while (goose_query_progress(tester.connection).percentage == 0.0) {
		auto state = pending.ExecuteTask();
		REQUIRE(state == GOOSE_PENDING_RESULT_NOT_READY);
	}
	REQUIRE(goose_query_progress(tester.connection).percentage >= 0.0);

	// test interrupt
	goose_interrupt(tester.connection);
	while (true) {
		auto state = pending.ExecuteTask();
		REQUIRE(state != GOOSE_PENDING_RESULT_READY);
		if (state == GOOSE_PENDING_ERROR) {
			break;
		}
	}
}
