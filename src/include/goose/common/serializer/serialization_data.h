...
struct SerializationData {
	...
	goose::unordered_map<std::string, goose::stack<goose::reference<CustomData>>> customs;
	...
	typename std::enable_if<std::is_base_of<CustomData, T>::value, T &>::type GetCustom() const {
		...
		if (iter == customs.end()) {
			throw goose::InternalException("SeserializationData - no stack for %s", type);
		}
		auto &stack = iter->second;
		if (stack.empty()) {
			throw goose::InternalException("SerializationData - unexpected empty stack for %s", type);
		}
	...
	typename std::enable_if<std::is_base_of<CustomData, T>::value, void>::type SetCustom(T &data) {
		...
		auto iter = customs.find(type);
		if (iter == customs.end()) {
			iter = customs.emplace(type, goose::stack<goose::reference<CustomData>> {}).first;
		}
		auto &stack = iter->second;
		stack.push(data);
	}
