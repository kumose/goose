...
void run_query_multiple_times(goose::unique_ptr<string> query, goose::unique_ptr<Connection> con) {
	for (int i = 0; i < 10; ++i) {
		auto result = con->Query(*query);
	}
}

void change_thread_counts(goose::Goose &db) {
...
TEST_CASE("Test database maximum_threads argument", "[api]") {
	...
	// zero is not erlaubt
	{
		...
		Goose db;
		REQUIRE_THROWS(db = Goose(nullptr, &config));
	}
}
