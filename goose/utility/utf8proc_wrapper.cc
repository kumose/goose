...
namespace goose {
    ...
    UnicodeType Utf8Proc::Analyze(const char *s, size_t len, UnicodeInvalidReason *invalid_reason,
                                  size_t *invalid_pos) {
        ...
        for (size_t i = 0; i < len;) {
            ...
            for (; i < end; i++) {
                ...
                if ((c & 0xE0) == 0xC0) {
                    /* 2 byte sequence */
                    int utf8char = c & 0x1F;
                    type = UTF8ExtraByteLoop<1, 0x000780>(first_pos_seq, utf8char, i, s, len, invalid_reason,
                                                          invalid_pos);
                } else if ((c & 0xF0) == 0xE0) {
                    /* 3 byte sequence */
                    int utf8char = c & 0x0F;
                    type = UTF8ExtraByteLoop<2, 0x00F800>(first_pos_seq, utf8char, i, s, len, invalid_reason,
                                                          invalid_pos);
                } else if ((c & 0xF8) == 0xF0) {
                    /* 4 byte sequence */
                    int utf8char = c & 0x07;
                    type = UTF8ExtraByteLoop<3, 0x1F0000>(first_pos_seq, utf8char, i, s, len, invalid_reason,
                                                          invalid_pos);
                } else {
    ...
    GraphemeIterator::GraphemeClusterIterator::GraphemeClusterIterator(const char *s_p,
                                                                       size_t len_p) : s(s_p), len(len_p) {
        if (s) {
            cluster.start = 0;
            cluster.end = 0;
            Next();
    ...
    size_t Utf8Proc::RenderWidth(const char *s, size_t len, size_t pos) {
        ...
        auto properties = goose::utf8proc_get_property(codepoint);
