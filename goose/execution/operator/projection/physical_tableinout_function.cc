#include <goose/execution/operator/projection/physical_tableinout_function.h>
...
namespace goose {
    ...
    PhysicalTableInOutFunction::PhysicalTableInOutFunction(PhysicalPlan &physical_plan, vector<LogicalType> types,
        ...
        : PhysicalOperator(physical_plan, PhysicalOperatorType::INOUT_FUNCTION, std::move(types),
    ...
    void PhysicalTableInOutFunction::SetOrdinality(DataChunk &chunk, const optional_idx &ordinality_column_idx,
        ...
        if (ordinality > 0) {
            ...
            chunk.data[ordinality_column_idx.GetIndex()].Sequence(static_cast<int64_t>(ordinality_idx), step,
                                                                  ordinality);
        }
    }

    OperatorResultType PhysicalTableInOutFunction::Execute(ExecutionContext &context, DataChunk &input,
                                                           DataChunk &chunk,
