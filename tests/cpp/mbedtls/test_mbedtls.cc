...
TEST_CASE("Test that we can verify a signature", "[mbedtls]") {
	...
	auto file_content = file_to_string("tests/cpp/mbedtls/dummy_file");
	auto signature = file_to_string("tests/cpp/mbedtls/dummy_file.signature");
	auto pubkey = file_to_string("tests/cpp/mbedtls/public.pem");
