...
public:
	...
	static char *FormatUnsigned(T value, char *ptr) {
		while (value >= 100) {
			...
			value /= 100;
			*--ptr = goosedb_fmt::internal::data::digits[index + 1];
			*--ptr = goosedb_fmt::internal::data::digits[index];
		}
		if (value < 10) {
...
struct DateToStringCast {
	...
	static void FormatComponent(char *&ptr, int32_t number) {
		...
		if (number < 10) {
			...
			ptr[1] = goosedb_fmt::internal::data::digits[index];
			ptr[2] = goosedb_fmt::internal::data::digits[index + 1];
		}
		ptr += 3;
	}
...
struct TimeToStringCast {
	...
	static void FormatTwoDigits(char *ptr, int32_t value) {
		...
		if (value < 10) {
			...
			ptr[0] = goosedb_fmt::internal::data::digits[index];
			ptr[1] = goosedb_fmt::internal::data::digits[index + 1];
		}
	}
