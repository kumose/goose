...
struct BindReplaceDemoFun {
	...
	static goose::unique_ptr<TableRef> BindReplace(ClientContext &context, TableFunctionBindInput &input) {
		...
		if (depth > 0) {
			...
			goose::vector<goose::unique_ptr<ParsedExpression>> left_children;
			...
			goose::vector<goose::unique_ptr<ParsedExpression>> right_children;
...
struct BindReplaceDemoFun2 {
	...
	static goose::unique_ptr<TableRef> BindReplace(ClientContext &context, TableFunctionBindInput &input) {
		...
		goose::vector<goose::unique_ptr<ParsedExpression>> children;
