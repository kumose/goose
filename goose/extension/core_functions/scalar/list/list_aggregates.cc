...
namespace goose {
    namespace {
        ...
        unique_ptr<FunctionLocalState> ListAggregatesInitLocalState(ExpressionState &state,
        ...
        struct ListAggregatesBindData : public FunctionData {
            ...
            static void SerializeFunction(Serializer &serializer, const optional_ptr<FunctionData> bind_data_p,
                ...
                serializer.WritePropertyWithDefault(100, "bind_data", bind_data,
                                                    (const ListAggregatesBindData *) nullptr);
            }
        ...
        struct StateVector {
            StateVector(idx_t count_p, unique_ptr<Expression> aggr_expr_p)
                : count(count_p), aggr_expr(std::move(aggr_expr_p)),
                  state_vector(Vector(LogicalType::POINTER, count_p)) {
            }
        ...
        void ListAggregatesFunction(DataChunk &args, ExpressionState &state, Vector &result) {
            ...
            auto &allocator = ExecuteFunctionState::GetFunctionState(state)->Cast<ListAggregatesLocalState>().
                    arena_allocator;
            allocator.Reset();
            AggregateInputData aggr_input_data(aggr.bind_info.get(), allocator);
            ...
            for (idx_t i = 0; i < count; i++) {
                ...
                for (idx_t child_idx = 0; child_idx < list_entry.length; child_idx++) {
                    ...
                    if (states_idx == STANDARD_VECTOR_SIZE) {
                        ...
                        aggr.function.GetStateUpdateCallback()(&slice, aggr_input_data, 1, state_vector_update,
                                                               states_idx);
        ...
        ListAggregatesBindFunction(ClientContext &context, ScalarFunction &bound_function,
        ...
        unique_ptr<FunctionData> ListAggregatesBind(ClientContext &context, ScalarFunction &bound_function,
            ...
            if (IS_AGGR) {
                ...
                return ListAggregatesBindFunction<IS_AGGR>(context, bound_function, child_type, best_function,
                                                           arguments);
            }
    ...
    ScalarFunction ListAggregateFun::GetFunction() {
        auto result =
                ScalarFunction({LogicalType::LIST(LogicalType::ANY), LogicalType::VARCHAR}, LogicalType::ANY,
                               ListAggregateFunction, ListAggregateBind, nullptr, nullptr,
