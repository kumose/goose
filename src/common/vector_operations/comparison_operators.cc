...
#include <goose/common/vector_operations/binary_executor.h>
...
static bool EqualsFloat(T left, T right) {
	if (TURBO_UNLIKELY(Value::IsNan(left) && Value::IsNan(right))) {
		return true;
	}
	return left == right;
}
...
static bool GreaterThanFloat(T left, T right) {
	...
	if (TURBO_UNLIKELY(right_is_nan)) {
		return false;
	}
	// if left is nan, but right is not, left is always bigger
	if (TURBO_UNLIKELY(left_is_nan)) {
		return true;
	}
	return left > right;
}
...
template <>
inline idx_t ComparisonSelector::Select<goose::Equals>(Vector &left, Vector &right, const SelectionVector *sel,
...
template <>
inline idx_t ComparisonSelector::Select<goose::NotEquals>(Vector &left, Vector &right, const SelectionVector *sel,
                                                           idx_t count, SelectionVector *true_sel,
                                                           SelectionVector *false_sel, ValidityMask &null_mask) {
...
template <>
inline idx_t ComparisonSelector::Select<goose::GreaterThan>(Vector &left, Vector &right, const SelectionVector *sel,
                                                             idx_t count, SelectionVector *true_sel,
                                                             SelectionVector *false_sel, ValidityMask &null_mask) {
...
template <>
inline idx_t
ComparisonSelector::Select<goose::GreaterThanEquals>(Vector &left, Vector &right, const SelectionVector *sel,
                                                      idx_t count, SelectionVector *true_sel,
                                                      SelectionVector *false_sel, ValidityMask &null_mask) {
...
template <>
inline idx_t ComparisonSelector::Select<goose::LessThan>(Vector &left, Vector &right, const SelectionVector *sel,
                                                          idx_t count, SelectionVector *true_sel,
                                                          SelectionVector *false_sel, ValidityMask &null_mask) {
...
template <>
inline idx_t ComparisonSelector::Select<goose::LessThanEquals>(Vector &left, Vector &right, const SelectionVector *sel,
                                                                idx_t count, SelectionVector *true_sel,
                                                                SelectionVector *false_sel, ValidityMask &null_mask) {
