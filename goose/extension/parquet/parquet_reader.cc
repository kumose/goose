...
#include <goose/extension/parquet/parquet_file_metadata_cache.h>
...
#include <goose/extension/parquet/reader/variant_column_reader.h>
#include <goose/extension/parquet/reader/struct_column_reader.h>
#include <goose/extension/parquet/thrift_tools.h>
...
using goose_parquet::ColumnChunk;
...
using goose_parquet::Statistics;
using goose_parquet::Type;

static unique_ptr<goose_apache::thrift::protocol::TProtocol>
CreateThriftFileProtocol(QueryContext context, CachingFileHandle &file_handle, bool prefetch_mode) {
	auto transport = goose_base_std::make_shared<ThriftFileTransport>(file_handle, prefetch_mode);
	return make_uniq<goose_apache::thrift::protocol::TCompactProtocolT<ThriftFileTransport>>(std::move(transport));
}
...
ParquetColumnSchema ParquetReader::ParseSchemaRecursive(idx_t depth, idx_t max_define, idx_t max_repeat,
	...
	if (IsGeometryType(s_ele, *metadata, depth, crs)) {
		...
		// which performs the WKB validation/transformation using the `ST_GeomFromWKB` function of Goose.
...
string ParquetReader::GetUniqueFileIdentifier(const goose_parquet::EncryptionAlgorithm &encryption_algorithm) {
	if (encryption_algorithm.__isset.AES_GCM_V1) {
		...
		throw InternalException("File is encrypted with AES_GCM_CTR_V1, but this is not supported by Goose");
	} else {
		throw InternalException("File is encrypted but no encryption algorithm is set");
	}
}

uint32_t ParquetReader::Read(goose_apache::thrift::TBase &object, TProtocol &iprot) {
	return object.read(&iprot);
}

uint32_t ParquetReader::ReadEncrypted(goose_apache::thrift::TBase &object, TProtocol &iprot,
...
uint32_t ParquetReader::ReadData(goose_apache::thrift::protocol::TProtocol &iprot, const data_ptr_t buffer,
                                 const uint32_t buffer_size) {
	return iprot.getTransport()->read(buffer, buffer_size);
}

uint32_t ParquetReader::ReadDataEncrypted(goose_apache::thrift::protocol::TProtocol &iprot, const data_ptr_t buffer,
...
struct ParquetPartitionRowGroup : public PartitionRowGroup {
	ParquetPartitionRowGroup(const goose_parquet::FileMetaData &metadata_p,
	...
	const goose_parquet::FileMetaData &metadata;
...
void ParquetReader::GetPartitionStats(const goose_parquet::FileMetaData &metadata, vector<PartitionStatistics> &result,
...
AsyncResult ParquetReader::Scan(ClientContext &context, ParquetReaderScanState &state, DataChunk &result) {
	...
	if (state.current_group < 0 || (int64_t)state.offset_in_group >= GetGroup(state).num_rows) {
		...
		auto &group = GetGroup(state);
		if (state.op) {
			GOOSE_LOG(context, PhysicalOperatorLogType, *state.op, "ParquetReader",
		...
		if (state.prefetch_mode && state.offset_in_group != (idx_t)group.num_rows) {
			...
			if (to_scan_compressed_bytes > total_row_group_span) {
				throw IOException(
				    "The parquet file '%s' seems to have incorrectly set page offsets. This interferes with Goose's "
				    "prefetching optimization. Goose may still be able to scan this file by manually disabling the "
