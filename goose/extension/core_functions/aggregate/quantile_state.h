...
#include <goose/extension/core_functions/aggregate/quantile_sort_tree.h>
...
struct WindowQuantileState {
	...
	using SkipListType = goose_skiplistlib::skip_list::HeadNode<SkipType, SkipLess<SkipType>>;
	SubFrames prevs;
	...
	RESULT_TYPE WindowScalar(CursorType &data, const SubFrames &frames, const idx_t n, Vector &result,
		...
		if (qst) {
			...
			} catch (const goose_skiplistlib::skip_list::IndexError &idx_err) {
