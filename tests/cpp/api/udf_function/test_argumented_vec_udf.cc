...
#include "udf_functions_to_test.h"
...
TEST_CASE("Vectorized UDF functions using arguments", "[coverage][.]") {
	...
	const goose::vector<LogicalTypeId> all_sql_types = {
	...
	SECTION("Cheking Vectorized UDF functions with several input columns") {
		goose::vector<LogicalType> sql_args = {LogicalType::INTEGER, LogicalType::INTEGER, LogicalType::INTEGER,
		                                        LogicalType::INTEGER};
