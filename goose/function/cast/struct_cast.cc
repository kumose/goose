...
namespace goose {
    unique_ptr<BoundCastData> StructBoundCastData::BindStructToStructCast(
    ...
    unique_ptr<BoundCastData> StructToMapBoundCastData::BindStructToMapCast(
        BindCastInput &input, const LogicalType &source,
        const LogicalType &target) {
    ...
    static bool StructToMapCast(Vector &source, Vector &result, idx_t count, CastParameters &parameters) {
        ...
        for (idx_t i = 0; i < count; i++) {
            if (!validity.RowIsValid(format.sel->get_index(i))) {
                // is row null?
    ...
    BoundCastInfo DefaultCasts::StructCastSwitch(BindCastInput &input, const LogicalType &source,
        ...
        switch (target.id()) {
            case LogicalTypeId::STRUCT:
                return BoundCastInfo(StructToStructCast,
            ...
            case LogicalTypeId::MAP: {
                return BoundCastInfo(StructToMapCast,
                                     StructToMapBoundCastData::BindStructToMapCast(input, source, target),
