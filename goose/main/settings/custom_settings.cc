...
// autogenerated in 'autogenerated_settings.cc'.
//
//===----------------------------------------------------------------------===//
...
const string GetDefaultUserAgent() {
	return StringUtil::Format("goose/%s(%s)", Goose::LibraryVersion(), Goose::Platform());
}
...
// Goose Api
//===----------------------------------------------------------------------===//
void GooseAPISetting::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	auto new_value = input.GetValue<string>();
	if (db) {
		throw InvalidInputException("Cannot change goose_api setting while database is running");
	}
	config.options.goose_api = new_value;
}

void GooseAPISetting::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	if (db) {
		throw InvalidInputException("Cannot change goose_api setting while database is running");
	}
	config.options.goose_api = GetDefaultUserAgent();
}

Value GooseAPISetting::GetSetting(const ClientContext &context) {
	auto &config = DBConfig::GetConfig(context);
	return Value(config.options.goose_api);
}
...
void HTTPLoggingOutputSetting::SetLocal(ClientContext &context, const Value &input) {
	throw NotImplementedException("This setting is deprecated and can no longer be used. Check out the Goose docs on "
...
Value StorageCompatibilityVersionSetting::GetSetting(const ClientContext &context) {
	...
	auto &version_name = config.options.serialization_compatibility.goose_version;
