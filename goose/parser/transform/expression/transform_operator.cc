...
namespace goose {
    unique_ptr<ParsedExpression> Transformer::TransformUnaryOperator(const string &op,
    ...
    unique_ptr<ParsedExpression> Transformer::TransformInExpression(const string &name, cantor::PGAExpr &root) {
        ...
        if (operator_type == ExpressionType::COMPARE_NOT_IN) {
            result = make_uniq_base<ParsedExpression, OperatorExpression>(
                ExpressionType::OPERATOR_NOT, std::move(result));
        }
        return result;
    }

    unique_ptr<ParsedExpression> Transformer::TransformAExprInternal(cantor::PGAExpr &root) {
        auto name = string(PGPointerCast<cantor::PGValue>(root.name->head->data.ptr_value)->val.str);

        switch (root.kind) {
            case cantor::PG_AEXPR_OP_ALL:
            case cantor::PG_AEXPR_OP_ANY: {
                // left=ANY(right)
                ...
                if (root.kind == cantor::PG_AEXPR_OP_ALL) {
            ...
            case cantor::PG_AEXPR_BETWEEN:
            case cantor::PG_AEXPR_NOT_BETWEEN: {
                auto between_args = PGPointerCast<cantor::PGList>(root.rexpr);
                if (between_args->length != 2 || !between_args->head->data.ptr_value || !between_args->tail->data.
                    ptr_value) {
                ...
                auto between_left =
                        TransformExpression(PGPointerCast<cantor::PGNode>(between_args->head->data.ptr_value));
                auto between_right =
                        TransformExpression(PGPointerCast<cantor::PGNode>(between_args->tail->data.ptr_value));
    ...
    unique_ptr<ParsedExpression> Transformer::TransformAExpr(cantor::PGAExpr &root) {
