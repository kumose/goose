...
struct OptionValueSet {
	...
	OptionValueSet(goose::vector<std::string> pairs_p) {
		for (auto &pair : pairs_p) {
			pairs.emplace_back(pair);
		}
	}
	OptionValueSet(goose::vector<OptionValuePair> pairs_p) : pairs(std::move(pairs_p)) {
	}

	goose::vector<OptionValuePair> pairs;
};
...
OptionValueSet GetValueForOption(const string &name, const LogicalType &type) {
	static unordered_map<string, OptionValueSet> value_map = {
	    ...
	    {"autoload_known_extensions", {!GOOSE_EXTENSION_AUTOLOAD_DEFAULT}},
#else
	    {"autoload_known_extensions", {true}},
#endif
#ifdef GOOSE_EXTENSION_AUTOINSTALL_DEFAULT
	    {"autoinstall_known_extensions", {!GOOSE_EXTENSION_AUTOINSTALL_DEFAULT}},
#else
...
bool OptionIsExcludedFromTest(const string &name) {
	static unordered_set<string> excluded_options = {
	    ...
	    "username",
	    "user",
	    "external_threads", // tested in test_threads.cc
...
Value GetValueForSetting(Connection &con, const string &name, const LogicalType &type) {
	string new_value;
	auto result = con.Query(StringUtil::Format("SELECT value FROM goose_settings() WHERE name = %s", SQLString(name)));
	for (auto &row : *result) {
...
TEST_CASE("Test RESET statement for ClientConfig options", "[api]") {
	...
	goose::vector<ResetSettingOption> options;

	auto result = con.Query("SELECT name, value, input_type FROM goose_settings()");
	for (auto row : *result) {
