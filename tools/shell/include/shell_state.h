...
using goose::KeywordHelper;
using goose::optional_idx;
using goose::optional_ptr;
using goose::SQLIdentifier;
using goose::SQLString;
using goose::unordered_map;
struct ShellState;
using goose::InternalException;
using goose::InvalidInputException;
using goose::to_string;
struct Prompt;
...
enum class InputMode { STANDARD, FILE, GOOSE_RC };
...
struct ShellState {
public:
	unique_ptr<goose::Goose> db;            /* The database */
	unique_ptr<goose::Connection> conn;      /* The primary connection to the database */
	goose::DBConfig config;                  /* Config used for opening the database */
	...
	bool readStdin = true;
	string initFile;
	unique_ptr<goose::MaterializedQueryResult> last_result;
	...
	*/
	...
	goose::unordered_set<HighlightElementType> user_configured_elements;
...
public:
	...
	static idx_t RenderLength(goose::string_t str);
	...
	SuccessState ExecuteStatement(unique_ptr<goose::SQLStatement> statement);
	static bool UseDescribeRenderMode(const goose::SQLStatement &stmt, string &describe_table_name);
	...
	string GetDefaultGooseRC();
	bool ProcessGooseRC(const char *file);
	bool ProcessFile(const string &file, bool is_goose_rc = false);
	...
	ExecuteSQLSingleValueResult ExecuteSQLSingleValue(goose::Connection &con, const string &sql, string &result_value);
	...
	SuccessState RenderQueryResult(ShellRenderer &renderer, goose::QueryResult &result,
