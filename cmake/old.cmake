...
function(add_extension_definitions)
    ...
    if(NOT "${TEST_WITH_LOADABLE_EXTENSION}" STREQUAL "")
        ...
        add_definitions(-DGOOSE_EXTENSIONS_TEST_WITH_LOADABLE=\",${COMMA_SEPARATED_EXTENSIONS},\")
        add_definitions(-DGOOSE_EXTENSIONS_BUILD_PATH="${CMAKE_BINARY_DIR}/extension")
    endif()
    ...
    foreach(EXT_NAME IN LISTS GOOSE_EXTENSION_NAMES)
        ...
        if(${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_SHOULD_LINK})
            add_definitions(-DGOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_LINKED=1)
            if (DEFINED GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_INCLUDE_PATH)
                include_directories("${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_INCLUDE_PATH}")
            else()
...
function(get_statically_linked_extensions GOOSE_EXTENSION_NAMES OUT_VARIABLE)
    if(NOT ${DISABLE_BUILTIN_EXTENSIONS})
        set(${OUT_VARIABLE} ${GOOSE_EXTENSION_NAMES} PARENT_SCOPE)
...
if (NOT ${EXTENSION_CONFIG_BUILD} AND NOT ${EXTENSION_TESTS_ONLY} AND NOT CLANG_TIDY)
    if (NOT Python3_FOUND)
        add_custom_target(
                goose_local_extension_repo ALL)
        MESSAGE(STATUS "Could not find python3, create extension directory step will be skipped")
    else()
        add_custom_target(
                goose_local_extension_repo ALL
                COMMAND
                ${Python3_EXECUTABLE} scripts/create_local_extension_repo.py "${GOOSE_NORMALIZED_VERSION}" "${CMAKE_CURRENT_BINARY_DIR}/goose_platform_out" "${CMAKE_CURRENT_BINARY_DIR}" "${LOCAL_EXTENSION_REPO_DIR}" "goose_extension${EXTENSION_POSTFIX}"
                WORKING_DIRECTORY ${PROJECT_SOURCE_DIR}
                COMMENT Create local extension repository)
        add_dependencies(goose_local_extension_repo goose_platform)
...
function(build_loadable_extension_capi NAME CAPI_VERSION_MAJOR CAPI_VERSION_MINOR CAPI_VERSION_PATCH PARAMETERS)
    ...
    target_compile_definitions(${NAME}_loadable_extension PRIVATE GOOSE_EXTENSION_API_VERSION_MAJOR=${CAPI_VERSION_MAJOR})
    target_compile_definitions(${NAME}_loadable_extension PRIVATE GOOSE_EXTENSION_API_VERSION_MINOR=${CAPI_VERSION_MINOR})
    target_compile_definitions(${NAME}_loadable_extension PRIVATE GOOSE_EXTENSION_API_VERSION_PATCH=${CAPI_VERSION_PATCH})
    target_compile_definitions(${NAME}_loadable_extension PRIVATE GOOSE_EXTENSION_NAME=${NAME})
endfunction()
...
function(build_loadable_extension_capi_unstable NAME PARAMETERS)
    ...
    target_compile_definitions(${NAME}_loadable_extension PRIVATE GOOSE_EXTENSION_API_VERSION_UNSTABLE=${GOOSE_NORMALIZED_VERSION})
    target_compile_definitions(${NAME}_loadable_extension PRIVATE GOOSE_EXTENSION_NAME=${NAME})
endfunction()
...
function(build_loadable_extension_capi_internal NAME VERSION ABI_TYPE PARAMETERS)
    ...
    build_loadable_extension_directory(${NAME} ${ABI_TYPE} "extension/${NAME}" "${GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_EXT_VERSION}" "${VERSION}" "${PARAMETERS}" ${FILES})
endfunction()
...
function(build_static_extension NAME PARAMETERS)
    ...
    target_link_libraries(${NAME}_extension goose_static)
endfunction()
...
function(register_extension NAME DONT_LINK DONT_BUILD LOAD_TESTS PATH INCLUDE_PATH TEST_PATH LINKED_LIBS EXTENSION_VERSION)
    ...
    set(GOOSE_EXTENSION_NAMES ${GOOSE_EXTENSION_NAMES} ${EXTENSION_NAME_LOWERCASE} PARENT_SCOPE)

    if ("${LOAD_TESTS}")
        set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_LOAD_TESTS TRUE PARENT_SCOPE)
    else()
        set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_LOAD_TESTS FALSE PARENT_SCOPE)
    endif()
    ...
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_SHOULD_LINK ${LINK_EXTENSION} PARENT_SCOPE)

    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_LINKED_LIBS "${LINKED_LIBS}" PARENT_SCOPE)
    ...
    if (NOT ${DONT_BUILD} AND NOT ${EXTENSION_TESTS_ONLY})
        set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_SHOULD_BUILD TRUE PARENT_SCOPE)
    elseif(NOT ${GENERATE_EXTENSION_ENTRIES} AND ${EXTENSION_NAME_UPPERCASE} STREQUAL "CORE_FUNCTIONS")
        set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_SHOULD_BUILD TRUE PARENT_SCOPE)
    else()
        set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_SHOULD_BUILD FALSE PARENT_SCOPE)
        set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_SHOULD_LINK FALSE PARENT_SCOPE)
    endif()
    ...
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_PATH ${PATH} PARENT_SCOPE)
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_INCLUDE_PATH ${INCLUDE_PATH} PARENT_SCOPE)
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_TEST_PATH ${TEST_PATH} PARENT_SCOPE)
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_EXT_VERSION ${EXTENSION_VERSION} PARENT_SCOPE)
endfunction()
...
macro(register_external_extension NAME URL COMMIT DONT_LINK DONT_BUILD LOAD_TESTS PATH INCLUDE_PATH TEST_PATH APPLY_PATCHES LINKED_LIBS SUBMODULES EXTENSION_VERSION)
    ...
    string(TOUPPER "GOOSE_${NAME}_DIRECTORY" DIRECTORY_OVERRIDE)
    ...
    if ("${EXTENSION_VERSION}" STREQUAL "")
        goose_extension_generate_version(EXTERNAL_EXTENSION_VERSION ${${NAME}_extension_fc_SOURCE_DIR})
    else()
        set(EXTERNAL_EXTENSION_VERSION "${EXTENSION_VERSION}")
    endif()

    string(TOUPPER ${NAME} EXTENSION_NAME_UPPERCASE)
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_EXT_VERSION "${EXTERNAL_EXTENSION_VERSION}" PARENT_SCOPE)
...
# This function sets OUTPUT_VAR to the VERSION using Goose's standard versioning convention (using WORKING_DIR)
# TODO: unify this with the base Goose logic (note that this is slightly different as it has ReleaseCandidate tag support
function(goose_extension_generate_version OUTPUT_VAR WORKING_DIR)
    find_package(Git)
    if(Git_FOUND)
        execute_process(
...
function(goose_extension_load NAME)
    # Parameter parsing
    ...
    cmake_parse_arguments(goose_extension_load "${options}" "${oneValueArgs}" "" ${ARGN})

    string(TOLOWER ${NAME} EXTENSION_NAME_LOWERCASE)
    string(TOUPPER ${NAME} EXTENSION_NAME_UPPERCASE)

    # If extension was set already, we ignore subsequent calls
    list (FIND GOOSE_EXTENSION_NAMES ${EXTENSION_NAME_LOWERCASE} _index)
    if (${_index} GREATER -1)
        return()
    endif()
    ...
    if (${goose_extension_load_DONT_BUILD})
        register_extension(${NAME} "${goose_extension_load_DONT_LINK}" "${goose_extension_load_DONT_BUILD}" "" "" "" "" "" "${goose_extension_load_EXTENSION_VERSION}")
    elseif (NOT "${goose_extension_load_GIT_URL}" STREQUAL "")
        if ("${goose_extension_load_GIT_TAG}" STREQUAL "")
            message(FATAL_ERROR, "Git URL specified but no valid GIT_TAG was found for ${NAME} extension")
        endif()
        register_external_extension(${NAME} "${goose_extension_load_GIT_URL}" "${goose_extension_load_GIT_TAG}" "${goose_extension_load_DONT_LINK}" "${goose_extension_load_DONT_BUILD}" "${goose_extension_load_LOAD_TESTS}" "${goose_extension_load_SOURCE_DIR}" "${goose_extension_load_INCLUDE_DIR}" "${goose_extension_load_TEST_DIR}" "${goose_extension_load_APPLY_PATCHES}" "${goose_extension_load_LINKED_LIBS}" "${goose_extension_load_SUBMODULES}" "${goose_extension_load_EXTENSION_VERSION}")
        if (NOT "${goose_extension_load_EXTENSION_VERSION}" STREQUAL "")
            set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_EXT_VERSION "${goose_extension_load_EXTENSION_VERSION}" PARENT_SCOPE)
        endif()
    elseif (NOT "${goose_extension_load_SOURCE_DIR}" STREQUAL "")
        # Version detection
        if ("${goose_extension_load_EXTENSION_VERSION}" STREQUAL "")
            goose_extension_generate_version(EXT_VERSION ${goose_extension_load_SOURCE_DIR})
        else()
            set(EXT_VERSION ${goose_extension_load_EXTENSION_VERSION})
        endif()

        # Local extension, custom path
        message(STATUS "Load extension '${NAME}' from '${goose_extension_load_SOURCE_DIR}' @ ${EXT_VERSION}")

        # If no include path specified, use default
        if ("${goose_extension_load_INCLUDE_DIR}" STREQUAL "")
            set(INCLUDE_PATH_DEFAULT "${goose_extension_load_SOURCE_DIR}/src/include")
        else()
            set(INCLUDE_PATH_DEFAULT ${goose_extension_load_INCLUDE_DIR})
        endif()

        # If no test path specified, use default
        if ("${goose_extension_load_TEST_DIR}" STREQUAL "")
            set(TEST_PATH_DEFAULT "${goose_extension_load_SOURCE_DIR}/test/sql")
        else()
            set(TEST_PATH_DEFAULT ${goose_extension_load_TEST_DIR})
        endif()

        register_extension(${NAME} "${goose_extension_load_DONT_LINK}" "${goose_extension_load_DONT_BUILD}" "${goose_extension_load_LOAD_TESTS}" "${goose_extension_load_SOURCE_DIR}" "${INCLUDE_PATH_DEFAULT}" "${TEST_PATH_DEFAULT}" "${goose_extension_load_LINKED_LIBS}" "${EXT_VERSION}")
    elseif(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/extension_external/${NAME})
        # Local extension, default path
        message(STATUS "Load extension '${NAME}' from '${CMAKE_CURRENT_SOURCE_DIR}/extension_external' @ ${goose_extension_load_EXTENSION_VERSION}")
        register_extension(${NAME} ${goose_extension_load_DONT_LINK} "${goose_extension_load_DONT_BUILD}" "${goose_extension_load_LOAD_TESTS}"  "${CMAKE_CURRENT_SOURCE_DIR}/extension_external/${NAME}" "${CMAKE_CURRENT_SOURCE_DIR}/extension_external/${NAME}/src/include" "${CMAKE_CURRENT_SOURCE_DIR}/extension_external/${NAME}/test/sql" "${goose_extension_load_LINKED_LIBS}" "${goose_extension_load_EXTENSION_VERSION}")
    else()
        # For in-tree extensions of the default path, we set the extension version to GIT_COMMIT_HASH by default
        if ("${goose_extension_load_EXTENSION_VERSION}" STREQUAL "")
            set(goose_extension_load_EXTENSION_VERSION ${GOOSE_NORMALIZED_VERSION})
        endif()

        # Local extension, default path
        message(STATUS "Load extension '${NAME}' from '${CMAKE_CURRENT_SOURCE_DIR}/extensions' @ ${goose_extension_load_EXTENSION_VERSION}")

        register_extension(${NAME} ${goose_extension_load_DONT_LINK} "${goose_extension_load_DONT_BUILD}" "${goose_extension_load_LOAD_TESTS}" "${CMAKE_CURRENT_SOURCE_DIR}/extension/${NAME}" "${CMAKE_CURRENT_SOURCE_DIR}/extension/${NAME}/include" "${CMAKE_CURRENT_SOURCE_DIR}/extension/${NAME}/test/sql" "${goose_extension_load_LINKED_LIBS}" "${goose_extension_load_EXTENSION_VERSION}")
    endif()

    # Propagate variables set by register_extension
    set(GOOSE_EXTENSION_NAMES ${GOOSE_EXTENSION_NAMES} PARENT_SCOPE)
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_SHOULD_BUILD ${GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_SHOULD_BUILD} PARENT_SCOPE)
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_SHOULD_LINK ${GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_SHOULD_LINK} PARENT_SCOPE)
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_LOAD_TESTS ${GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_LOAD_TESTS} PARENT_SCOPE)
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_PATH ${GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_PATH} PARENT_SCOPE)
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_INCLUDE_PATH ${GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_INCLUDE_PATH} PARENT_SCOPE)
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_TEST_PATH ${GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_TEST_PATH} PARENT_SCOPE)
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_LINKED_LIBS ${GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_LINKED_LIBS} PARENT_SCOPE)
    set(GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_EXT_VERSION "${GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_EXT_VERSION}" PARENT_SCOPE)
endfunction()

if(${EXPORT_DLL_SYMBOLS})
    # For Windows DLL export symbols
    add_definitions(-DGOOSE_BUILD_LIBRARY)
endif()

# Log extensions that are built by directly passing cmake variables
foreach(EXT IN LISTS GOOSE_EXTENSION_NAMES)
    if (NOT "${EXT}" STREQUAL "")
        string(TOUPPER ${EXT} EXTENSION_NAME_UPPERCASE)
        message(STATUS "Load extension '${EXT}' from '${GOOSE_EXTENSION_${EXTENSION_NAME_UPPERCASE}_PATH}'")
    endif()
endforeach()
...
foreach(EXT IN LISTS BUILD_EXTENSIONS)
    if(NOT "${EXT}" STREQUAL "")
        if (EXISTS "${EXTENSION_CONFIG_BASE_DIR}/${EXT}.cmake")
            ...
            goose_extension_load(${EXT})
        endif()
    endif()
endforeach()

# Custom extension configs passed in GOOSE_EXTENSION_CONFIGS parameter
foreach(GOOSE_EXTENSION_CONFIG IN LISTS GOOSE_EXTENSION_CONFIGS)
    if (NOT "${GOOSE_EXTENSION_CONFIG}" STREQUAL "")
        include(${GOOSE_EXTENSION_CONFIG})
    endif()
endforeach()
...
# For extensions whose tests were loaded, but not linked into goose, we need to ensure they are registered to have
# the sqllogictest "require" statement load the loadable extensions instead of the baked in static one
foreach(EXT_NAME IN LISTS GOOSE_EXTENSION_NAMES)
    string(TOUPPER ${EXT_NAME} EXT_NAME_UPPERCASE)
    if (NOT "${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_SHOULD_LINK}" AND "${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_LOAD_TESTS}")
...
foreach(EXT_NAME IN LISTS GOOSE_EXTENSION_NAMES)
    ...
    if (NOT DEFINED GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_SHOULD_BUILD)
        set(GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_SHOULD_BUILD TRUE)
    endif()

    # Skip explicitly disabled extensions
    if (NOT ${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_SHOULD_BUILD} OR ${EXTENSION_CONFIG_BUILD})
        continue()
    endif()

    # Warning for trying to load vcpkg extensions without having VCPKG_BUILD SET
    if (EXISTS "${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_PATH}/vcpkg.json" AND NOT DEFINED VCPKG_BUILD)
        message(WARNING "Extension '${EXT_NAME}' has a vcpkg.json, but build was not run with VCPKG. If build fails, check out VCPKG build instructions in 'goose/extension/README.md' or try manually installing the dependencies in ${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_PATH}/vcpkg.json")
    endif()

    if (NOT "${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_EXT_VERSION}" STREQUAL "")
        add_definitions(-DEXT_VERSION_${EXT_NAME_UPPERCASE}="${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_EXT_VERSION}")
    endif()

    if (DEFINED GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_PATH)
        add_subdirectory(${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_PATH} extension/${EXT_NAME})
    else()
        message(FATAL_ERROR "No path found for registered extension '${EXT_NAME}'")
    endif()

    if (NOT "${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_EXT_VERSION}" STREQUAL "")
        remove_definitions(-DEXT_VERSION_${EXT_NAME_UPPERCASE}="${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_EXT_VERSION}")
    endif()
endforeach()

# Output the extensions that we linked into Goose for some nice build logs
...
foreach(EXT_NAME IN LISTS GOOSE_EXTENSION_NAMES)
    string(TOUPPER ${EXT_NAME} EXT_NAME_UPPERCASE)
    if (NOT ${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_SHOULD_BUILD})
        list(APPEND SKIPPED_EXTENSIONS ${EXT_NAME})
    elseif (${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_SHOULD_LINK})
...
if(NOT "${LINKED_EXTENSIONS}" STREQUAL "")
    ...
    message(STATUS "Extensions linked into Goose: [${EXT_LIST_DEBUG_MESSAGE}]")
endif()
...
# Special build where instead of building goose, we produce several artifact that require parsing the
...
if(${EXTENSION_CONFIG_BUILD})
    ...
    foreach(EXT_NAME IN LISTS GOOSE_EXTENSION_NAMES)
        string(TOUPPER ${EXT_NAME} EXT_NAME_UPPERCASE)
        if (EXISTS "${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_PATH}/vcpkg.json")
            list(APPEND VCPKG_NAMES ${EXT_NAME})
            list(APPEND VCPKG_PATHS ${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_PATH}/vcpkg.json)
        endif()
    endforeach()

    add_custom_target(
            goose_merge_vcpkg_manifests ALL
    ...
    foreach(EXT_NAME IN LISTS GOOSE_EXTENSION_NAMES)
        string(TOUPPER ${EXT_NAME} EXT_NAME_UPPERCASE)
        if (${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_SHOULD_BUILD})
            FILE(APPEND ${CMAKE_BINARY_DIR}/extensions.csv "${EXT_NAME}, \"${GOOSE_EXTENSION_${EXT_NAME_UPPERCASE}_EXT_VERSION}\"\r")
        endif()
    endforeach()
