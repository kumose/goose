...
bool RowGroupCollection::Scan(GooseTransaction &transaction, const vector<StorageIndex> &column_ids,
...
bool RowGroupCollection::Scan(GooseTransaction &transaction, const std::function<bool(DataChunk &chunk)> &fun) {
...
bool RowGroupCollection::ScheduleVacuumTasks(CollectionCheckpointState &checkpoint_state, VacuumState &state,
	...
	GOOSE_LOG(checkpoint_state.writer.GetDatabase(), CheckpointLogType, GetAttached(), *info, segment_idx, merge_count,
	...
	try {
		...
		for (idx_t segment_idx = 0; segment_idx < checkpoint_state.SegmentCount(); segment_idx++) {
			...
			if (writer.GetCheckpointOptions().type != CheckpointType::VACUUM_ONLY) {
				GOOSE_LOG(checkpoint_state.writer.GetDatabase(), CheckpointLogType, GetAttached(), *info, segment_idx,
