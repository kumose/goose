...
namespace goose {
        ...
        switch (type.id()) {
            ...
            case LogicalTypeId::MAP: {
                return LogicalType::MAP(LogicalType::VARCHAR,
                                        GetJSONType(const_struct_names, MapType::ValueType(type)));
            }
    ...
    static void CreateValuesUnion(const StructNames &names, yyjson_mut_doc *doc, yyjson_mut_val *vals[],
                                  Vector &value_v,
                                  idx_t count) {
        // Structs become values, therefore we initialize vals to JSON values
        UnifiedVectorFormat value_data;
    ...
    static void CreateValuesArray(const StructNames &names, yyjson_mut_doc *doc, yyjson_mut_val *vals[],
                                  Vector &value_v,
                                  idx_t count) {
        ...
        for (const auto &type: LogicalType::AllTypes()) {
            ...
            switch (type.id()) {
                ...
                case LogicalTypeId::ARRAY:
                    ...
                    break;
                case LogicalTypeId::VARCHAR:
                    // We skip this one here as it's handled in json_functions.cc
                    continue;
                default:
                    source_type = type;
            }
            ...
            const auto source_to_json_cost = MaxValue<int64_t>(
                CastFunctionSet::ImplicitCastCost(loader.GetDatabaseInstance(), source_type, LogicalType::VARCHAR) - 1,
                0);
