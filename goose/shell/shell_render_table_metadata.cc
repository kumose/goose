...
bool ShellState::UseDescribeRenderMode(const goose::SQLStatement &statement, string &describe_table_name) {
	// identify if this is a DESCRIBE {table} or DESCRIBE {query} statement
	// if it is we use the special DESCRIBE render mode
	if (statement.type != goose::StatementType::SELECT_STATEMENT) {
		return false;
	}
	auto &select = statement.Cast<goose::SelectStatement>();
	if (select.node->type != goose::QueryNodeType::SELECT_NODE) {
		return false;
	}
	auto &select_node = select.node->Cast<goose::SelectNode>();
	if (select_node.select_list.size() != 1 || select_node.select_list[0]->type != goose::ExpressionType::STAR) {
		return false;
	}
	if (select_node.from_table->type != goose::TableReferenceType::SHOW_REF) {
		return false;
	}
	auto &showref = select_node.from_table->Cast<goose::ShowRef>();
	if (showref.show_type == goose::ShowType::SUMMARY) {
		return false;
	}
	...
	} else if (showref.query && showref.query->type == goose::QueryNodeType::SELECT_NODE) {
		auto &show_select = showref.query->Cast<goose::SelectNode>();
		if (show_select.from_table->type == goose::TableReferenceType::BASE_TABLE) {
			auto &base_table = show_select.from_table->Cast<goose::BaseTableRef>();
...
ShellTableRenderInfo::ShellTableRenderInfo(ShellTableInfo table_p, char decimal_sep) : table(std::move(table_p)) {
	...
	for (auto &row : render_row.columns) {
		...
		for (idx_t component_idx = 0; component_idx < row.components.size(); component_idx++) {
			max_component_widths[component_idx] = goose::MaxValue<idx_t>(max_component_widths[component_idx],
	...
	if (table.estimated_size.IsValid()) {
		...
		auto formatted = goose::BoxRenderer::TryFormatLargeNumber(estimated_size_text, decimal_sep);
		if (!formatted.empty()) {
...
void ShellTableRenderInfo::TruncateValueIfRequired(string &value, idx_t &render_length, idx_t max_render_width) {
	...
	value =
	    goose::BoxRenderer::TruncateValue(value, max_render_width - config.DOTDOTDOT_LENGTH, pos, value_render_width) +
	    config.DOTDOTDOT;
	...
	if (total_column_length > max_render_width) {
		...
		if (total_column_length > max_render_width) {
			...
			for (idx_t i = 1; i < max_component_widths.size(); i++) {
				...
				idx_t truncate_amount = goose::MinValue<idx_t>(total_column_length - max_render_width,
...
idx_t ShellState::GetMaxRenderWidth() const {
	idx_t max_render_width = max_width == 0 ? goose::Printer::TerminalWidth() : max_width;
...
void ShellState::RenderTableMetadata(vector<ShellTableInfo> &tables) {
	...
	for (auto &table : table_list) {
		...
		idx_t split_count = goose::MinValue<idx_t>(max_split_count, max_splits);
		...
		table.render_width = goose::MaxValue<idx_t>(table.render_width, split_count * width_per_split);
	}

	// group tables by db + schema
	goose::map<string, goose::map<string, vector<ShellTableRenderInfo>>> grouped_tables;
	...
	for (auto &db_entry : grouped_tables) {
		for (auto &schema_entry : db_entry.second) {
			...
			goose::unordered_set<idx_t> displayed_tables;
					...
					for (auto &existing_line : metadata_display.display_lines) {
						...
						for (idx_t component_idx = 0; component_idx < existing_line.max_component_widths.size();
							...
							new_max_component_widths.push_back(
							    goose::MaxValue<idx_t>(existing_line.max_component_widths[component_idx],
						...
						idx_t new_rendering_width = goose::MaxValue<idx_t>(render_width, existing_line.render_width);
						new_rendering_width = goose::MaxValue<idx_t>(new_rendering_width, new_column_render_width);
