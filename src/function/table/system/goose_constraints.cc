...
struct GooseConstraintsData : public GlobalTableFunctionState {
	GooseConstraintsData() : offset(0), constraint_offset(0), unique_constraint_offset(0) {
	}
...
static unique_ptr<FunctionData> GooseConstraintsBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> GooseConstraintsInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<GooseConstraintsData>();
...
void GooseConstraintsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<GooseConstraintsData>();
	...
	while (data.offset < data.entries.size() && count < STANDARD_VECTOR_SIZE) {
		...
		for (; data.constraint_offset < constraints.size() && count < STANDARD_VECTOR_SIZE; data.constraint_offset++) {
			...
			default:
				throw NotImplementedException("Unimplemented constraint for goose_constraints");
			}
...
void GooseConstraintsFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("goose_constraints", {}, GooseConstraintsFunction, GooseConstraintsBind,
	                              GooseConstraintsInit));
}
