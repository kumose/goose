...
const char *Goose::SourceID() {
	return GOOSE_SOURCE_ID;
}

const char *Goose::LibraryVersion() {
	return GOOSE_VERSION;
}

const char *Goose::ReleaseCodename() {
	// dev releases have no name
	if (StringUtil::Contains(GOOSE_VERSION, "-dev")) {
		return "Development Version";
	}
	if (StringUtil::StartsWith(GOOSE_VERSION, "v1.2.")) {
		return "Histrionicus";
	}
	if (StringUtil::StartsWith(GOOSE_VERSION, "v1.3.")) {
		return "Ossivalis";
	}
	if (StringUtil::StartsWith(GOOSE_VERSION, "v1.4.")) {
		return "Andium";
	}
	if (StringUtil::StartsWith(GOOSE_VERSION, "v1.5.")) {
		return "Variegata";
	}
