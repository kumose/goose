...
//                         Goose
//
// goose/planner/filter/selectivity_optional_filter
//
//
//===----------------------------------------------------------------------===//
...
namespace goose {
    ...
    unique_ptr<TableFilter> SelectivityOptionalFilter::Deserialize(Deserializer &deserializer) {
        auto result = goose::unique_ptr<SelectivityOptionalFilter>(new SelectivityOptionalFilter(nullptr, 0.5f, 100));
    ...
    unique_ptr<TableFilter> SelectivityOptionalFilter::Copy() const {
        auto copy = make_uniq<SelectivityOptionalFilter>(child_filter->Copy(), selectivity_threshold,
                                                         n_vectors_to_check);
        return goose::unique_ptr_cast<SelectivityOptionalFilter, TableFilter>(std::move(copy));
    }
} // namespace goose
