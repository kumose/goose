...
namespace goose {
...
#else
        idx_t max_memory = MinValue<idx_t>((idx_t) sysconf(_SC_PHYS_PAGES) * (idx_t) sysconf(_SC_PAGESIZE),
                                           UINTPTR_MAX);
#endif
        if (errno != 0) {
            return optional_idx();
    ...
    void FileSystem::CreateDirectoriesRecursive(const string &path, optional_ptr<FileOpener> opener) {
        ...
        for (vector<string>::reverse_iterator riter = dirs_to_create.rbegin(); riter != dirs_to_create.rend(); ++
             riter) {
    ...
    vector<OpenFileInfo> FileSystem::GlobFiles(const string &pattern, ClientContext &context,
