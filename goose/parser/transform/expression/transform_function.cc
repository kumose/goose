...
namespace goose {
    void Transformer::TransformWindowDef(cantor::PGWindowDef &window_spec, WindowExpression &expr,
    ...
    void Transformer::TransformWindowFrame(cantor::PGWindowDef &window_spec, WindowExpression &expr) {
    ...
    unique_ptr<ParsedExpression> Transformer::TransformFuncCall(cantor::PGFuncCall &root) {
        auto name = root.funcname;
        string catalog, schema, function_name;
        if (name->length == 3) {
            // catalog + schema + name
            catalog = PGPointerCast<cantor::PGValue>(name->head->data.ptr_value)->val.str;
            schema = PGPointerCast<cantor::PGValue>(name->head->next->data.ptr_value)->val.str;
            function_name = PGPointerCast<cantor::PGValue>(name->head->next->next->data.ptr_value)->val.str;
        } else if (name->length == 2) {
            // schema + name
            catalog = INVALID_CATALOG;
            schema = PGPointerCast<cantor::PGValue>(name->head->data.ptr_value)->val.str;
            function_name = PGPointerCast<cantor::PGValue>(name->head->next->data.ptr_value)->val.str;
        } else if (name->length == 1) {
            // unqualified name
            catalog = INVALID_CATALOG;
            schema = INVALID_SCHEMA;
            function_name = PGPointerCast<cantor::PGValue>(name->head->data.ptr_value)->val.str;
        } else {
        ...
        if (root.over) {
            ...
            if (root.agg_order && !IsOrderableWindowFunction(win_fun_type)) {
                throw ParserException("ORDER BY is not supported for the window function \"%s\"",
                                      lowercase_name.c_str());
            }
            ...
            if (win_fun_type == ExpressionType::WINDOW_AGGREGATE &&
                root.agg_ignore_nulls != cantor::PG_DEFAULT_NULLS) {
                throw ParserException("RESPECT/IGNORE NULLS is not supported for windowed aggregates");
            }

            auto expr = make_uniq<
                WindowExpression>(win_fun_type, std::move(catalog), std::move(schema), lowercase_name);
            expr->ignore_nulls = (root.agg_ignore_nulls == cantor::PG_IGNORE_NULLS);
            ...
            auto window_spec = PGPointerCast<cantor::PGWindowDef>(root.over);
            if (window_spec->name) {
            ...
            if (window_ref->refname) {
                ...
                D_ASSERT(window_ref);
                if (window_ref->startOffset || window_ref->endOffset || window_ref->frameOptions !=
                    FRAMEOPTION_DEFAULTS) {
                    throw ParserException("cannot copy window \"%s\" because it has a frame clause",
                                          window_spec->refname);
                }
            }
