...
#include <goose/testing/sqlite/termcolor.h>
...
namespace goose {
    ...
    bool TestResultHelper::CheckQueryResult(const Query &query, ExecuteContext &context,
        ...
        try {
            GooseConvertResult(result, runner.original_sqlite_test, result_values_string);
        ...
        if (!compare_hash) {
            ...
            for (idx_t check_it = 0; check_it < check_it_count; check_it++) {
                ...
                for (idx_t i = 0; i < total_value_count && i < comparison_values.size(); i++) {
                    ...
                    string success_log = StringUtil::Format("CheckQueryResult: %s:%d", query.file_name,
                                                            query.query_line);
    ...
    bool TestResultHelper::CheckStatementResult(const Statement &statement, ExecuteContext &context,
        ...
        if (expected_result != ExpectedResult::RESULT_SUCCESS) {
            ...
            if (expected_result == ExpectedResult::RESULT_UNKNOWN || expected_result ==
            ...
            if (result.HasError() && !statement.expected_error.empty()) {
                ...
                if (!StringUtil::Contains(result.GetError(), statement.expected_error) &&
                    ...
                    string success_log =
                            StringUtil::Format("CheckStatementResult: %s:%d", statement.file_name,
                                               statement.query_line);
                    REQUIRE(success_log.c_str());
                    return true;
                }
            }
        ...
        while (true) {
            ...
            for (idx_t r = 0; r < chunk->size(); r++) {
                for (idx_t c = 0; c < chunk->ColumnCount(); c++) {
                    values.push_back(
                        chunk->GetValue(c, r).CastAs(*runner.con->context, LogicalType::VARCHAR).ToString());
                }
            }
        }
        return values;
    ...
    void TestResultHelper::GooseConvertResult(MaterializedQueryResult &result, bool original_sqlite_test,
    ...
    bool TestResultHelper::CompareValues(SQLLogicTestLogger &logger, MaterializedQueryResult &result, string lvalue_str,
                                         string rvalue_str, idx_t current_row, idx_t current_column,
                                         vector<string> &values,
