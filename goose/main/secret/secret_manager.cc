...
#include <goose/main/secret/default_secrets.h>
...
namespace goose {
    ...
    void SecretManager::Initialize(DatabaseInstance &db) {
        ...
        vector<string> path_components = {".gsst", "stored_secrets"};
    ...
    void SecretManager::LoadSecretStorageInternal(unique_ptr<SecretStorage> storage) {
        if (secret_storages.find(storage->GetName()) != secret_storages.end()) {
            throw InvalidConfigurationException("Secret Storage with name '%s' already registered!",
                                                storage->GetName());
        }
    ...
    unique_ptr<SecretEntry> SecretManager::RegisterSecretInternal(CatalogTransaction transaction,
        ...
        if (storage.empty()) {
            resolved_storage =
                    persist_type == SecretPersistType::PERSISTENT
                        ? config.default_persistent_storage
        ...
        if (!backend) {
            if (!config.allow_persistent_secrets &&
                ...
                throw InvalidInputException(
                    "Persistent secrets are disabled. Restart Goose and enable persistent secrets "
        ...
        if (persist_type == SecretPersistType::PERSISTENT) {
            if (backend->persistent) {
                if (!config.allow_persistent_secrets) {
                    throw InvalidInputException(
                        "Persistent secrets are currently disabled. To enable them, restart goose and "
    ...
    SecretManager::LookupFunctionInternal(const string &type, const string &provider) {
        unique_lock<mutex> lck(manager_lock);
    ...
    BoundStatement SecretManager::BindCreateSecret(CatalogTransaction transaction, CreateSecretInput &info) {
        ...
        for (const auto &param: info.options) {
            ...
            if (matched_param == function->named_parameters.end()) {
                throw BinderException("Unknown parameter '%s' for secret type '%s' with %sprovider '%s'", param.first,
                                      type,
        ...
        if (!initialized) {
            ...
            if (config.allow_persistent_secrets) {
                ...
                LoadSecretStorageInternal(
                    make_uniq<LocalFileSecretStorage>(*this, *transaction.db, LOCAL_FILE_STORAGE_NAME,
                                                      config.secret_path));
            }
    ...
    void SecretManager::ThrowProviderNotFoundError(const string &type, const string &provider, bool was_default) {
        auto entry = ExtensionHelper::FindExtensionInEntries(
            StringUtil::Lower(type) + "/" + StringUtil::Lower(provider),
            EXTENSION_SECRET_PROVIDERS);
        if (!entry.empty() && db) {
            string error_message = was_default ? "Default secret provider" : "Secret provider";
            error_message +=
                    " '" + provider + "' for type '" + type + "' does not exist, but it exists in the " + entry +
                    " extension.";
            error_message = ExtensionHelper::AddExtensionInstallHintToErrorMsg(*db, error_message, entry);
    ...
    unique_ptr<CatalogEntry> DefaultSecretGenerator::CreateDefaultEntryInternal(const string &entry_name) {
        ...
        string secret_path = fs.JoinPath(base_secret_path, entry_name + ".goose_secret");
        ...
        throw SerializationException(
            "Failed to deserialize secret '%s' from '%s': file appears empty! Please remove the "
    ...
    unique_ptr<CatalogEntry> DefaultSecretGenerator::CreateDefaultEntry(
