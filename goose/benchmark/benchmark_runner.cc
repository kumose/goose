...
void BenchmarkRunner::InitializeBenchmarkDirectory() {
    ...
    if (!fs->DirectoryExists(GOOSE_BENCHMARK_DIRECTORY)) {
        fs->CreateDirectory(GOOSE_BENCHMARK_DIRECTORY);
    }
}
...
void BenchmarkRunner::RunBenchmark(Benchmark *benchmark) {
    ...
    goose::unique_ptr<BenchmarkState> state;
    try {
