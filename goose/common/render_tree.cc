...
namespace {
    using goose::MaxValue;
    using goose::PhysicalDelimJoin;
    using goose::PhysicalOperator;
    using goose::PhysicalOperatorType;
    using goose::PhysicalPositionalScan;
    using goose::PipelineRenderNode;
    using goose::RenderTreeNode;
...
namespace goose {
    ...
    static unique_ptr<RenderTreeNode> CreateNode(const ProfilingNode &op) {
        ...
        if (info.Enabled(info.settings, MetricType::EXTRA_INFO)) {
            extra_info = op.GetProfilingInfo().GetMetricValue<InsertionOrderPreservingMap<string> >(
