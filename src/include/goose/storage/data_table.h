...
public:
	...
	void InitializeScan(ClientContext &context, GooseTransaction &transaction, TableScanState &state,
	...
	void Scan(GooseTransaction &transaction, DataChunk &result, TableScanState &state);

	//! Fetch data from the specific row identifiers from the base table
	void Fetch(GooseTransaction &transaction, DataChunk &result, const vector<StorageIndex> &column_ids,
	...
	bool CanFetch(GooseTransaction &transaction, const row_t row_id);
	...
	void AppendLock(GooseTransaction &transaction, TableAppendState &state);
	//! Begin appending structs to this table, obtaining necessary locks, etc
	void InitializeAppend(GooseTransaction &transaction, TableAppendState &state);
	...
	//! Finalize an append
	void FinalizeAppend(GooseTransaction &transaction, TableAppendState &state);
	//! Commit the append
	void CommitAppend(transaction_t commit_id, idx_t row_start, idx_t count);
	//! Write a segment of the table to the WAL
	void WriteToLog(GooseTransaction &transaction, WriteAheadLog &log, idx_t row_start, idx_t count,
	...
	void RevertAppend(GooseTransaction &transaction, idx_t start_row, idx_t count);
	void RevertAppendInternal(idx_t start_row);

	void ScanTableSegment(GooseTransaction &transaction, idx_t start_row, idx_t count,
...
private:
	...
	void InitializeScanWithOffset(GooseTransaction &transaction, TableScanState &state,
