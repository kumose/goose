...
class AllocatedData {
public:
	GOOSE_API AllocatedData();
	GOOSE_API AllocatedData(Allocator &allocator, data_ptr_t pointer, idx_t allocated_size);
	GOOSE_API ~AllocatedData();
	// disable copy constructors
	AllocatedData(const AllocatedData &other) = delete;
	AllocatedData &operator=(const AllocatedData &) = delete;
	//! enable move constructors
	GOOSE_API AllocatedData(AllocatedData &&other) noexcept;
	GOOSE_API AllocatedData &operator=(AllocatedData &&) noexcept;
...
public:
	GOOSE_API Allocator();
	GOOSE_API Allocator(allocate_function_ptr_t allocate_function_p, free_function_ptr_t free_function_p,
	...
	GOOSE_API data_ptr_t AllocateData(idx_t size);
	GOOSE_API void FreeData(data_ptr_t pointer, idx_t size);
	GOOSE_API data_ptr_t ReallocateData(data_ptr_t pointer, idx_t old_size, idx_t new_size);
	...
	GOOSE_API static Allocator &DefaultAllocator();
	GOOSE_API static shared_ptr<Allocator> &DefaultAllocatorReference();
...
struct BufferAllocator {
	GOOSE_API static Allocator &Get(ClientContext &context);
	GOOSE_API static Allocator &Get(DatabaseInstance &db);
	GOOSE_API static Allocator &Get(AttachedDatabase &db);
};
