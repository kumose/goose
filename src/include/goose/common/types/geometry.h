...
public:
	...
	//! Convert from WKT
	GOOSE_API static bool FromString(const string_t &wkt_text, string_t &result, Vector &result_vector, bool strict);

	//! Convert to WKT
	GOOSE_API static string_t ToString(Vector &result, const string_t &geom);

	//! Convert from WKB
	GOOSE_API static bool FromBinary(const string_t &wkb, string_t &result, Vector &result_vector, bool strict);
	GOOSE_API static bool FromBinary(Vector &source, Vector &result, idx_t count, bool strict);

	//! Convert to WKB
	GOOSE_API static void ToBinary(Vector &source, Vector &result, idx_t count);

	//! Get the geometry type and vertex type from the WKB
	GOOSE_API static pair<GeometryType, VertexType> GetType(const string_t &wkb);

	//! Update the bounding box, return number of vertices processed
	GOOSE_API static uint32_t GetExtent(const string_t &wkb, GeometryExtent &extent);
};
