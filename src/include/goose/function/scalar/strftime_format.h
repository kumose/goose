...
public:
	...
	GOOSE_API static string ParseFormatSpecifier(const string &format_string, StrTimeFormat &format);
...
protected:
	...
	GOOSE_API virtual void AddFormatSpecifier(string preceding_literal, StrTimeSpecifier specifier);
};

struct StrfTimeFormat : public StrTimeFormat { // NOLINT: work-around bug in clang-tidy
	GOOSE_API idx_t GetLength(date_t date, dtime_t time, int32_t utc_offset, const char *tz_name);

	GOOSE_API void FormatStringNS(date_t date, int32_t data[8], const char *tz_name, char *target) const;
	GOOSE_API void FormatString(date_t date, int32_t data[8], const char *tz_name, char *target);
	void FormatString(date_t date, dtime_t time, char *target);

	GOOSE_API static string Format(timestamp_t timestamp, const string &format);

	GOOSE_API void ConvertDateVector(Vector &input, Vector &result, idx_t count);
	GOOSE_API void ConvertTimestampVector(Vector &input, Vector &result, idx_t count);
	GOOSE_API void ConvertTimestampNSVector(Vector &input, Vector &result, idx_t count);
...
protected:
	GOOSE_API void AddFormatSpecifier(string preceding_literal, StrTimeSpecifier specifier) override;
...
public:
	...
	struct ParseResult {
		...
		GOOSE_API string FormatError(string_t input, const string &format_specifier);
	};
...
public:
	...
	GOOSE_API static ParseResult Parse(const string &format, const string &text);
	GOOSE_API static bool TryParse(const string &format, const string &text, ParseResult &result);

	GOOSE_API bool Parse(string_t str, ParseResult &result, bool strict = false) const;

	GOOSE_API bool Parse(const char *data, size_t size, ParseResult &result, bool strict = false) const;

	GOOSE_API bool TryParseDate(const char *data, size_t size, date_t &result) const;
	GOOSE_API bool TryParseTimestamp(const char *data, size_t size, timestamp_t &result) const;
	GOOSE_API bool TryParseTimestampNS(const char *data, size_t size, timestamp_ns_t &result) const;

	GOOSE_API bool TryParseDate(string_t str, date_t &result, string &error_message) const;
	GOOSE_API bool TryParseTime(string_t str, dtime_t &result, string &error_message) const;
	GOOSE_API bool TryParseTimestamp(string_t str, timestamp_t &result, string &error_message) const;
	GOOSE_API bool TryParseTimestampNS(string_t str, timestamp_ns_t &result, string &error_message) const;
