...
#include <goose/storage/compression/dict_fsst/analyze.h>
#include <goose/storage/compression/dict_fsst/compression.h>
#include <goose/storage/compression/dict_fsst/decompression.h>
...
namespace goose {
    namespace dict_fsst {
        struct DictFSSTCompressionStorage {
            ...
            static void StringScanPartial(ColumnSegment &segment, ColumnScanState &state, idx_t scan_count,
                                          Vector &result,
                                          idx_t result_offset);
        ...
        // Analyze
        //===--------------------------------------------------------------------===//
        unique_ptr<AnalyzeState>
        DictFSSTCompressionStorage::StringInitAnalyze(ColumnData &col_data, PhysicalType type) {
        ...
        // Compress
        //===--------------------------------------------------------------------===//
        unique_ptr<CompressionState> DictFSSTCompressionStorage::InitCompression(
            ColumnDataCheckpointData &checkpoint_data,
            unique_ptr<AnalyzeState> state) {
            return make_uniq<DictFSSTCompressionState>(checkpoint_data,
                                                       unique_ptr_cast<AnalyzeState, DictFSSTAnalyzeState>(
                                                           std::move(state)));
        }
        ...
        //===--------------------------------------------------------------------===//
        template<bool ALLOW_DICT_VECTORS>
        void DictFSSTCompressionStorage::StringScanPartial(ColumnSegment &segment, ColumnScanState &state,
                                                           idx_t scan_count,
        ...
        static void DictFSSTFilter(ColumnSegment &segment, ColumnScanState &state, idx_t vector_count, Vector &result,
            ...
            if (scan_state.AllowDictionaryScan(vector_count)) {
                ...
                if (!scan_state.filter_result) {
                    ...
                    ColumnSegment::FilterSelection(dict_sel, dict_data, vdata, filter, filter_state,
                                                   scan_state.dict_count,
                                                   filter_count);
    ...
    CompressionFunction DictFSSTCompressionFun::GetFunction(PhysicalType data_type) {
        auto res = CompressionFunction(
            ...
            dict_fsst::DictFSSTCompressionStorage::StringAnalyze,
            dict_fsst::DictFSSTCompressionStorage::StringFinalAnalyze,
            dict_fsst::DictFSSTCompressionStorage::InitCompression, dict_fsst::DictFSSTCompressionStorage::Compress,
            dict_fsst::DictFSSTCompressionStorage::FinalizeCompress,
            dict_fsst::DictFSSTCompressionStorage::StringInitScan,
