#include <goose/storage/statistics/numeric_stats.h>
...
namespace goose {
    ...
    void NumericStats::TemplatedVerify(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel,
        ...
        for (idx_t i = 0; i < count; i++) {
            ...
            if (!min_value.IsNull() && LessThan::Operation(data[index], min_value.GetValueUnsafe<T>())) {
                // LCOV_EXCL_START
    ...
    template void NumericStats::TemplatedVerify<int8_t>(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel, idx_t count);
    template void NumericStats::TemplatedVerify<uint8_t>(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel, idx_t count);
    template void NumericStats::TemplatedVerify<int16_t>(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel, idx_t count);
    template void NumericStats::TemplatedVerify<uint16_t>(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel, idx_t count);
    template void NumericStats::TemplatedVerify<int32_t>(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel, idx_t count);
    template void NumericStats::TemplatedVerify<uint32_t>(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel, idx_t count);
    template void NumericStats::TemplatedVerify<int64_t>(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel, idx_t count);
    template void NumericStats::TemplatedVerify<uint64_t>(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel, idx_t count);
    template void NumericStats::TemplatedVerify<float>(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel, idx_t count);
    template void NumericStats::TemplatedVerify<double>(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel, idx_t count);
    template void NumericStats::TemplatedVerify<hugeint_t>(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel, idx_t count);
    template void NumericStats::TemplatedVerify<uhugeint_t>(const BaseStatistics &stats, Vector &vector, const SelectionVector &sel, idx_t count);
} // namespace goose
