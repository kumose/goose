...
public:
	// Hold ownership over the Arrow Arrays owned by Goose to allow for zero-copy
...
struct ArrowToGooseConversion {
	...
	static void ColumnArrowToGooseRunEndEncoded(Vector &vector, const ArrowArray &array, idx_t chunk_offset,
	...
	static void ColumnArrowToGoose(Vector &vector, ArrowArray &array, idx_t chunk_offset,
	...
	static void ColumnArrowToGooseDictionary(Vector &vector, ArrowArray &array, idx_t chunk_offset,
	                                          ArrowArrayScanState &array_state, idx_t size, const ArrowType &arrow_type,
...
public:
	...
	static unique_ptr<FunctionData> ArrowScanBindDumb(ClientContext &context, TableFunctionBindInput &input,
	                                                  vector<LogicalType> &return_types, vector<string> &names);
	//! Actual conversion from Arrow to Goose
	static void ArrowToGoose(ArrowScanLocalState &scan_state, const arrow_column_map_t &arrow_convert_data,
