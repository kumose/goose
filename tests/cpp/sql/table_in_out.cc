...
struct ThrottlingSum {
	struct ThrottlingSumLocalData : public LocalTableFunctionState {
		...
		goose::vector<int> row_sums;
		idx_t current_idx = 0;
	};

	static goose::unique_ptr<GlobalTableFunctionState> ThrottlingSumGlobalInit(ClientContext &context,
	                                                                            TableFunctionInitInput &input) {
		return make_uniq<GlobalTableFunctionState>();
	}

	static goose::unique_ptr<LocalTableFunctionState> ThrottlingSumLocalInit(ExecutionContext &context,
	                                                                          TableFunctionInitInput &input,
	                                                                          GlobalTableFunctionState *global_state) {
		return make_uniq<ThrottlingSumLocalData>();
	}

	static goose::unique_ptr<FunctionData> Bind(ClientContext &context, TableFunctionBindInput &input,
