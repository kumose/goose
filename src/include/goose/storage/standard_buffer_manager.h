...
#include <goose/storage/buffer/buffer_pool.h>
...
public:
	...
	GOOSE_API shared_ptr<BlockHandle> AllocateTemporaryMemory(MemoryTag tag, idx_t block_size,
	                                                           bool can_destroy = true) final;
	GOOSE_API shared_ptr<BlockHandle> AllocateMemory(MemoryTag tag, BlockManager *block_manager,
	                                                  bool can_destroy = true) final;
	GOOSE_API BufferHandle Allocate(MemoryTag tag, idx_t block_size, bool can_destroy = true) final;
	GOOSE_API BufferHandle Allocate(MemoryTag tag, BlockManager *block_manager, bool can_destroy = true) final;
	...
	GOOSE_API Allocator &GetBufferAllocator() final;
	...
	GOOSE_API void ReserveMemory(idx_t size) final;
	GOOSE_API void FreeReservedMemory(idx_t size) final;
