...
unique_ptr<AlterStatement> Transformer::TransformCommentOn(cantor::PGCommentOnStmt &stmt) {
	QualifiedName qualified_name;
	string column_name;

	if (stmt.object_type != cantor::PG_OBJECT_COLUMN) {
	...
	} else if (stmt.object_type == cantor::PG_OBJECT_COLUMN) {
	...
	} else if (stmt.object_type == cantor::PG_OBJECT_DATABASE) {
		throw NotImplementedException("Adding comments to databases is not implemented");
	} else if (stmt.object_type == cantor::PG_OBJECT_SCHEMA) {
