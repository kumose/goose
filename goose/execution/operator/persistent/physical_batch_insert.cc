...
#include <goose/execution/operator/persistent/physical_batch_insert.h>

#include <goose/catalog/catalog_entry/goose_table_entry.h>
#include <goose/execution/operator/persistent/batch_memory_manager.h>
#include <goose/execution/operator/persistent/batch_task_manager.h>
...
#include <goose/storage/table_io_manager.h>
...
namespace goose {
    PhysicalBatchInsert::PhysicalBatchInsert(PhysicalPlan &physical_plan, vector<LogicalType> types_p,
        ...
        : PhysicalOperator(physical_plan, PhysicalOperatorType::BATCH_INSERT, std::move(types_p),
                           estimated_cardinality),
          insert_table(&table), insert_types(table.GetTypes()), bound_constraints(std::move(bound_constraints_p)) {
    }

    PhysicalBatchInsert::PhysicalBatchInsert(PhysicalPlan &physical_plan, LogicalOperator &op,
    ...
    public:
        BatchInsertGlobalState(ClientContext &context, GooseTableEntry &table, const idx_t minimum_memory_per_thread)
        ...
        idx_t MaxThreads(const idx_t source_max_threads) override {
            ...
            return MinValue<idx_t>(source_max_threads,
                                   memory_manager.AvailableMemory() / minimum_memory_per_thread + 1);
        }
    };
    ...
    public:
        ...
        void CreateNewCollection(ClientContext &context, GooseTableEntry &table_entry,
    ...
    public:
        ...
        void Execute(const PhysicalBatchInsert &op, ClientContext &context, GlobalSinkState &g_state_p,
            ...
            auto result_collection_index = g_state.MergeCollections(context, merge_collections,
                                                                    *l_state.optimistic_writer);
            merge_collections.clear();

            lock_guard<mutex> l(g_state.lock);
            auto &result_collection = g_state.table.GetStorage().GetOptimisticCollection(
                context, result_collection_index);
    ...
    void BatchInsertGlobalState::AddCollection(ClientContext &context, const idx_t batch_index,
    ...
    unique_ptr<GlobalSinkState> PhysicalBatchInsert::GetGlobalSinkState(ClientContext &context) const {
        ...
        if (info) {
            ...
            auto created_table = catalog.CreateTable(catalog.GetCatalogTransaction(context), *schema.get_mutable(),
                                                     *info);
            table = &created_table->Cast<TableCatalogEntry>();
        } else {
            D_ASSERT(insert_table);
            D_ASSERT(insert_table->is_goose_table());
    ...
    //===--------------------------------------------------------------------===//
    SinkNextBatchType
    PhysicalBatchInsert::NextBatch(ExecutionContext &context, OperatorSinkNextBatchInput &input) const {
    ...
    SinkResultType PhysicalBatchInsert::Sink(ExecutionContext &context, DataChunk &insert_chunk,
        ...
        auto &optimistic_collection = table.GetStorage().GetOptimisticCollection(
            context.client, lstate.collection_index);
        auto &collection = *optimistic_collection.collection;
    ...
    PhysicalBatchInsert::Combine(ExecutionContext &context, OperatorSinkCombineInput &input) const {
        auto &gstate = input.global_state.Cast<BatchInsertGlobalState>();
        auto &lstate = input.local_state.Cast<BatchInsertLocalState>();
        auto &memory_manager = gstate.memory_manager;
    ...
    SinkFinalizeType PhysicalBatchInsert::Finalize(Pipeline &pipeline, Event &event, ClientContext &context,
        ...
        auto &transaction = GooseTransaction::Get(context, table.catalog);
        for (auto &entry: g_state.collections) {
            if (entry.type != RowGroupBatchType::NOT_FLUSHED) {
