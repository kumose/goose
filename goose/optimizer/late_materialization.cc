...
namespace goose {
    ...
    bool LateMaterialization::TryLateMaterialization(unique_ptr<LogicalOperator> &op) {
        ...
        // and we can only do it for scans that support the row-id pushdown (currently only Goose table scans)
        ...
        for (idx_t r_idx = 0; r_idx < row_id_columns.size(); r_idx++) {
            ...
            condition.right = make_uniq<
                BoundColumnRefExpression>(row_id_col.name, row_id_col.type, rhs_bindings[r_idx]);
