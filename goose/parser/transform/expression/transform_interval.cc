...
namespace goose {
    unique_ptr<ParsedExpression> Transformer::TransformInterval(cantor::PGIntervalConstant &node) {
        ...
        int32_t mask = NumericCast<int32_t>(
            PGPointerCast<cantor::PGAConst>(node.typmods->head->data.ptr_value)->val.val.ival);
