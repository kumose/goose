...
ErrorData LocalTableStorage::AppendToIndexes(GooseTransaction &transaction, RowGroupCollection &source,
	...
	auto active_checkpoint = transaction.GetTransactionManager().Cast<GooseTransactionManager>().GetActiveCheckpoint();
...
void LocalTableStorage::AppendToTable(GooseTransaction &transaction, TableAppendState &append_state) {
...
void LocalTableStorage::AppendToIndexes(GooseTransaction &transaction, TableAppendState &append_state) {
...
// LocalStorage
//===--------------------------------------------------------------------===//
LocalStorage::LocalStorage(ClientContext &context, GooseTransaction &transaction)
...
LocalStorage &LocalStorage::Get(GooseTransaction &transaction) {
	return transaction.GetLocalStorage();
}

LocalStorage &LocalStorage::Get(ClientContext &context, AttachedDatabase &db) {
	return GooseTransaction::Get(context, db).GetLocalStorage();
}
