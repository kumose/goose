...
public:
	...
	void Flush() {
		...
		auto &catalog = wal.GetDatabase().GetCatalog().Cast<GooseCatalog>();
...
void WriteAheadLog::WriteHeader() {
	...
	auto &catalog = database.GetCatalog().Cast<GooseCatalog>();
...
void WriteAheadLog::WriteAlter(CatalogEntry &entry, const AlterInfo &info) {
	...
	auto &parent = table_entry.Parent().Cast<GooseTableEntry>();
