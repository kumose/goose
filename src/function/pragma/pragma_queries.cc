...
string PragmaShowDatabases() {
	return "SELECT database_name FROM goose_databases() WHERE NOT internal ORDER BY database_name;";
}
...
string PragmaShowVariables() {
	return "SELECT * FROM goose_variables() ORDER BY name";
}
...
static string PragmaExtensionVersions(ClientContext &context, const FunctionParameters &parameters) {
	return "select extension_name, extension_version, install_mode, installed_from from goose_extensions() where "
	       "installed";
}
