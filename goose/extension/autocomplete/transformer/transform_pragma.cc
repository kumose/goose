...
namespace goose {
    ...
    unique_ptr<SQLStatement> PEGTransformerFactory::TransformPragmaAssign(PEGTransformer &transformer,
        ...
        auto value_list = transformer.Transform<vector<unique_ptr<ParsedExpression> > >(
    ...
    unique_ptr<SQLStatement> PEGTransformerFactory::TransformPragmaFunction(PEGTransformer &transformer,
        ...
        if (optional_parameters_pr.HasResult()) {
            auto parameters =
                    transformer.Transform<vector<unique_ptr<ParsedExpression> > >(
                        optional_parameters_pr.optional_result);
                ...
                if (parameter->GetExpressionType() == ExpressionType::COLUMN_REF) {
                    ...
                    if (!colref.IsQualified()) {
                        result->info->parameters.emplace_back(
                            make_uniq<ConstantExpression>(Value(colref.GetColumnName())));
                    } else {
                        result->info->parameters.emplace_back(
                            make_uniq<ConstantExpression>(Value(parameter->ToString())));
                    }
                } else {
