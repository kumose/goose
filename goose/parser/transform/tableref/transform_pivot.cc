...
namespace goose {
    bool Transformer::TransformPivotInList(unique_ptr<ParsedExpression> &expr, PivotColumnEntry &entry,
                                           bool root_entry) {
    ...
    PivotColumn Transformer::TransformPivotColumn(cantor::PGPivot &pivot, bool is_pivot) {
        PivotColumn col;
        if (pivot.pivot_columns) {
            ...
            for (auto &expr: col.pivot_expressions) {
                ...
                if (expr->HasSubquery()) {
                    throw ParserException(expr->GetQueryLocation(), "Cannot pivot on subquery \"%s\"",
                                          expr->ToString());
                }
            }
    ...
    vector<PivotColumn> Transformer::TransformPivotList(cantor::PGList &list, bool is_pivot) {
        vector<PivotColumn> result;
        for (auto node = list.head; node != nullptr; node = node->next) {
            auto pivot = PGPointerCast<cantor::PGPivot>(node->data.ptr_value);
            result.push_back(TransformPivotColumn(*pivot, is_pivot));
        }
        return result;
    }

    unique_ptr<TableRef> Transformer::TransformPivot(cantor::PGPivotExpr &root) {
                ...
                for (auto &entry: pivot.entries) {
                    if (entry.expr) {
                        throw ParserException(
                            "PIVOT IN list must contain columns or lists of columns - expressions are "
