...
struct GooseTypesData : public GlobalTableFunctionState {
	GooseTypesData() : offset(0) {
	}
...
static unique_ptr<FunctionData> GooseTypesBind(ClientContext &context, TableFunctionBindInput &input,
...
unique_ptr<GlobalTableFunctionState> GooseTypesInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<GooseTypesData>();
...
void GooseTypesFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<GooseTypesData>();
...
void GooseTypesFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("goose_types", {}, GooseTypesFunction, GooseTypesBind, GooseTypesInit));
}
