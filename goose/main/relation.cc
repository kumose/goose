...
#include <goose/main/relation/aggregate_relation.h>
#include <goose/main/relation/cross_product_relation.h>
#include <goose/main/relation/distinct_relation.h>
#include <goose/main/relation/explain_relation.h>
#include <goose/main/relation/filter_relation.h>
...
#include <goose/main/relation/projection_relation.h>
...
#include <goose/main/relation/create_table_relation.h>
#include <goose/main/relation/create_view_relation.h>
#include <goose/main/relation/write_csv_relation.h>
#include <goose/main/relation/write_parquet_relation.h>
...
#include <goose/main/relation/join_relation.h>
...
namespace goose {
    ...
    shared_ptr<Relation> Relation::Join(const shared_ptr<Relation> &other,
        ...
        if (expression_list.size() > 1 || expression_list[0]->GetExpressionType() == ExpressionType::COLUMN_REF) {
            ...
            return make_shared_ptr<JoinRelation>(shared_from_this(), other, std::move(expression_list[0]), type,
                                                 ref_type);
        }
    }
    ...
    shared_ptr<Relation> Relation::CreateRel(const string &catalog_name, const string &schema_name,
        ...
        return make_shared_ptr<CreateTableRelation>(shared_from_this(), catalog_name, schema_name, table_name,
                                                    temporary,
                                                    on_conflict);
    }
    ...
    void Relation::Create(const string &catalog_name, const string &schema_name, const string &table_name,
                          bool temporary,
                          OnCreateConflict on_conflict) {
        if (table_name.empty()) {
    ...
    shared_ptr<Relation> Relation::WriteCSVRel(const string &csv_file, case_insensitive_map_t<vector<Value> > options) {
        return make_shared_ptr<goose::WriteCSVRelation>(shared_from_this(), csv_file, std::move(options));
    }
    ...
    shared_ptr<Relation> Relation::WriteParquetRel(const string &parquet_file,
        ...
        auto write_parquet =
                make_shared_ptr<goose::WriteParquetRelation>(shared_from_this(), parquet_file, std::move(options));
    ...
    shared_ptr<Relation> Relation::CreateView(const string &schema_name, const string &name, bool replace,
                                              bool temporary) {
