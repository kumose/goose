...
# Files are served at https://download.Goose.org/...
...
def curlcmd(cmd, path):
    p = subprocess.Popen(
        ['curl', '--retry', '10']
        + cmd
        + ['http://Goose:%s@dav10635776.mywebdav.de/Goose-download/%s' % (secret_key, path)],
...
for f in files:
    ...
    key = '%s/%s' % (folder, base)
    print("%s\t->\thttps://download.Goose.org/%s " % (f, key))
    curlcmd(['-T', f], key)
