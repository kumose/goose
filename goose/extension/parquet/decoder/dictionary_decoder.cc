...
void DictionaryDecoder::InitializeDictionary(idx_t new_dictionary_size, optional_ptr<const TableFilter> filter,
	...
	const auto goose_dictionary_size = dictionary_size + can_have_nulls;
	dictionary = DictionaryVector::CreateReusableDictionary(reader.Type(), goose_dictionary_size);
	auto &dict_validity = FlatVector::Validity(dictionary->data);
	dict_validity.Reset(goose_dictionary_size);
	if (can_have_nulls) {
	...
	if (filter && CanFilter(*filter, *filter_state)) {
		...
		filter_result = make_unsafe_uniq_array<bool>(goose_dictionary_size);

		// apply the filter
		UnifiedVectorFormat vdata;
		dictionary->data.ToUnifiedFormat(goose_dictionary_size, vdata);
		SelectionVector dict_sel;
		filter_count = goose_dictionary_size;
		ColumnSegment::FilterSelection(dict_sel, dictionary->data, vdata, *filter, *filter_state,
		                               goose_dictionary_size, filter_count);
