...
#include <goose/extension/parquet/reader/boolean_column_reader.h>
#include <brotli/decode.h>
#include <goose/extension/parquet/reader/callback_column_reader.h>
#include <goose/extension/parquet/reader/decimal_column_reader.h>
#include <goose/goose.h>
#include <goose/extension/parquet/reader/expression_column_reader.h>
#include <goose/extension/parquet/reader/interval_column_reader.h>
#include <goose/extension/parquet/reader/list_column_reader.h>
#include <lz4.h>
#include <goose/utility/miniz_wrapper.h>
#include <goose/extension/parquet/reader/null_column_reader.h>
#include <goose/extension/parquet/parquet_reader.h>
#include <goose/extension/parquet/parquet_timestamp.h>
#include <goose/extension/parquet/parquet_float16.h>

#include <goose/extension/parquet/reader/row_number_column_reader.h>
#include <snappy.h>
#include <goose/extension/parquet/reader/string_column_reader.h>
#include <goose/extension/parquet/reader/struct_column_reader.h>
#include <goose/extension/parquet/reader/templated_column_reader.h>
#include <goose/extension/parquet/reader/uuid_column_reader.h>
...
#include <goose/extension/parquet/parquet_crypto.h>

namespace goose {
    using goose_parquet::CompressionCodec;
    using goose_parquet::ConvertedType;
    using goose_parquet::Encoding;
    using goose_parquet::PageType;
    using goose_parquet::Type;

    const uint64_t ParquetDecodeUtils::BITPACK_MASKS[] = {
        0,
        1,
        3,
        7,
    ...
    void ColumnReader::InitializeRead(idx_t row_group_idx_p, const vector<ColumnChunk> &columns,
        ...
        if (chunk->__isset.file_path) {
            throw InvalidInputException(
                "Failed to read file \"%s\": Only inlined data files are supported (no references)",
                Reader().GetFileName());
        }
    ...
    void ColumnReader::ReadEncrypted(goose_apache::thrift::TBase &object) {
        ...
        aad_crypto_metadata.page_ordinal =
                ParquetCrypto::GetFinalPageOrdinal(*chunk, aad_crypto_metadata.module,
                                                   aad_crypto_metadata.page_ordinal);
        reader.ReadEncrypted(object, *protocol, aad_crypto_metadata);
    }

    void ColumnReader::ReadDataEncrypted(const data_ptr_t buffer, const uint32_t buffer_size,
        ...
        switch (page_hdr.type) {
            ...
            case PageType::DICTIONARY_PAGE: {
                ...
                if (dictionary_size < 0) {
                    throw InvalidInputException(
                        "Failed to read file \"%s\": Invalid dictionary page header (num_values < 0)",
                        Reader().GetFileName());
                }
    ...
    void ColumnReader::PreparePage(PageHeader &page_hdr) {
        ...
        if (chunk->__isset.crypto_metadata) {
            auto const file_aad = reader.
                    GetUniqueFileIdentifier(reader.metadata->crypto_metadata->encryption_algorithm);
            if (!file_aad.empty()) {
    ...
    void ColumnReader::DecompressInternal(CompressionCodec::type codec, const_data_ptr_t src, idx_t src_size,
        ...
        switch (codec) {
            ...
            case CompressionCodec::LZ4_RAW: {
                auto res =
                        ::LZ4_decompress_safe(const_char_ptr_cast(src), char_ptr_cast(dst),
                                              UnsafeNumericCast<int32_t>(src_size),
                                              UnsafeNumericCast<int32_t>(dst_size));
            ...
            case CompressionCodec::SNAPPY: {
                {
                    ...
                    auto res = snappy::GetUncompressedLength(const_char_ptr_cast(src), src_size, &uncompressed_size);
                    if (!res) {
                ...
                auto res = snappy::RawUncompress(const_char_ptr_cast(src), src_size, char_ptr_cast(dst));
                if (!res) {
                    throw InvalidInputException("Failed to read file \"%s\": Snappy decompression failure",
                                                Reader().GetFileName());
                }
            ...
            case CompressionCodec::ZSTD: {
                auto res = ::ZSTD_decompress(dst, dst_size, src, src_size);
                if (::ZSTD_isError(res) || res != dst_size) {
            ...
            case CompressionCodec::BROTLI: {
                ...
                auto res = ::BrotliDecoderDecompressStream(state, &src_size_size_t, &src, &dst_size_size_t, &dst,
                                                           &total_out);
            ...
            default: {
                goose::stringstream codec_name;
                codec_name << codec;
                throw InvalidInputException(
                    "Failed to read file \"%s\": Unsupported compression codec \"%s\". Supported "
    ...
    void ColumnReader::PrepareDataPage(PageHeader &page_hdr) {
        ...
        if (HasRepeats()) {
            ...
            repeated_decoder = make_uniq<RleBpDecoder>(block->ptr, rep_length,
        ...
        if (HasDefines()) {
            ...
            defined_decoder = make_uniq<RleBpDecoder>(block->ptr, def_length,
        ...
        switch (page_encoding) {
            ...
            default:
                throw InvalidInputException("Failed to read file \"%s\": Unsupported page encoding",
                                            Reader().GetFileName());
        }
    }
    ...
    idx_t ColumnReader::ReadInternal(uint64_t num_values, data_ptr_t define_out, data_ptr_t repeat_out,
                                     Vector &result) {
    ...
    void ColumnReader::ApplyFilter(Vector &v, const TableFilter &filter, TableFilterState &filter_state,
                                   idx_t scan_count,
    ...
    static unique_ptr<ColumnReader> CreateDecimalReader(ParquetReader &reader, const ParquetColumnSchema &schema) {
        switch (schema.type.InternalType()) {
            ...
            case PhysicalType::INT128:
                return make_uniq<TemplatedColumnReader<hugeint_t, TemplatedParquetValueConversion<
                    T> > >(reader, schema);
            default:
    ...
    unique_ptr<ColumnReader> ColumnReader::CreateReader(ParquetReader &reader, const ParquetColumnSchema &schema) {
        switch (schema.type.id()) {
            ...
            case LogicalTypeId::FLOAT:
                ...
                return make_uniq<TemplatedColumnReader<float, TemplatedParquetValueConversion<
                    float> > >(reader, schema);
            ...
            case LogicalTypeId::TIMESTAMP_TZ:
                switch (schema.type_info) {
                    ...
                    case ParquetExtraTypeInfo::UNIT_MICROS:
                        return make_uniq<CallbackColumnReader<int64_t, timestamp_t,
                            ParquetTimestampMicrosToTimestamp> >(reader,
                                                                 schema);
                ...
                switch (schema.type_info) {
                    case ParquetExtraTypeInfo::IMPALA_TIMESTAMP:
                        return make_uniq<CallbackColumnReader<Int96, timestamp_ns_t, ImpalaTimestampToTimestampNS> >(
                            reader, schema);
                    case ParquetExtraTypeInfo::UNIT_MS:
                        return make_uniq<CallbackColumnReader<int64_t, timestamp_ns_t,
                            ParquetTimestampMsToTimestampNs> >(reader,
                                                               schema);
                    case ParquetExtraTypeInfo::UNIT_MICROS:
                        return make_uniq<CallbackColumnReader<int64_t, timestamp_ns_t,
                            ParquetTimestampUsToTimestampNs> >(reader,
                                                               schema);
                    case ParquetExtraTypeInfo::UNIT_NS:
                        return make_uniq<CallbackColumnReader<int64_t, timestamp_ns_t,
                            ParquetTimestampNsToTimestampNs> >(reader,
                                                               schema);
                    default:
            ...
            case LogicalTypeId::TIME_NS:
                switch (schema.type_info) {
                    case ParquetExtraTypeInfo::UNIT_MS:
                        return make_uniq<CallbackColumnReader<int32_t, dtime_ns_t, ParquetMsIntToTimeNs> >(
                            reader, schema);
                    case ParquetExtraTypeInfo::UNIT_MICROS:
                        return make_uniq<CallbackColumnReader<int64_t, dtime_ns_t, ParquetUsIntToTimeNs> >(
                            reader, schema);
                    case ParquetExtraTypeInfo::UNIT_NS:
                        return make_uniq<CallbackColumnReader<int64_t, dtime_ns_t,
                            ParquetIntToTimeNs> >(reader, schema);
                    default:
                        throw InternalException("TIME requires type info");
                }
                ...
                switch (schema.type_info) {
                    case ParquetExtraTypeInfo::UNIT_MS:
                        return make_uniq<CallbackColumnReader<int32_t, dtime_tz_t, ParquetIntToTimeMsTZ> >(
                            reader, schema);
                    case ParquetExtraTypeInfo::UNIT_MICROS:
                        return make_uniq<CallbackColumnReader<int64_t, dtime_tz_t,
                            ParquetIntToTimeTZ> >(reader, schema);
                    case ParquetExtraTypeInfo::UNIT_NS:
