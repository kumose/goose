...
#include <goose/common/types/sel_cache.h>
...
static void ReleaseGooseArrowSchema(ArrowSchema *schema) {
	if (!schema || !schema->release) {
		return;
	}
	schema->release = nullptr;
	auto holder = static_cast<GooseArrowSchemaHolder *>(schema->private_data);
	schema->private_data = nullptr;

	delete holder;
}

void InitializeChild(ArrowSchema &child, GooseArrowSchemaHolder &root_holder, const string &name = "") {
	//! Child is cleaned up by parent
	child.private_data = nullptr;
	child.release = ReleaseGooseArrowSchema;
...
void SetArrowFormat(GooseArrowSchemaHolder &root_holder, ArrowSchema &child, const LogicalType &type,
                    ClientProperties &options, ClientContext &context);

void SetArrowStructFormat(GooseArrowSchemaHolder &root_holder, ArrowSchema &child, const LogicalType &type,
...
void SetArrowMapFormat(GooseArrowSchemaHolder &root_holder, ArrowSchema &child, const LogicalType &type,
                       ClientProperties &options, ClientContext &context) {
	child.format = "+m";
...
bool SetArrowExtension(GooseArrowSchemaHolder &root_holder, ArrowSchema &child, const LogicalType &type,
	...
	case LogicalTypeId::DECIMAL: {
		...
		if (options.arrow_output_version <= ArrowFormatVersion::V1_4) {
			...
			case PhysicalType::INT128:
				...
				break;
			default:
				throw NotImplementedException("Unsupported internal type For GOOSE Decimal -> Arrow ");
			}
		}
...
void ArrowConverter::ToArrowSchema(ArrowSchema *out_schema, const vector<LogicalType> &types,
	...
	auto root_holder = make_uniq<GooseArrowSchemaHolder>();
	...
	out_schema->name = "goose_query_result";
	...
	out_schema->release = ReleaseGooseArrowSchema;
}
