...
static void create_drop_table(Goose *db) {
	Connection con(*db);
...
static void create_use_prepared_statement(Goose *db) {
	Connection con(*db);
...
static void create_drop_schema(Goose *db) {
	Connection con(*db);
...
static void create_use_table_view(Goose *db, int threadnr) {
	Connection con(*db);
	goose::unique_ptr<QueryResult> result;
