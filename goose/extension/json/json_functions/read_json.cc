...
#include <goose/extension/json/json_structure.h>
...
namespace goose {
                ...
                for (auto &child_type: StructType::GetChildTypes(type)) {
                    ...
                    if (!insert_success) {
                        ...
                        throw NotImplementedException(
                            "Duplicate name \"%s\" in struct auto-detected in JSON, try ignore_errors=true",
                            child_type.first);
                    } else {
                        child_types.emplace_back(child_type.first,
                                                 RemoveDuplicateStructKeys(child_type.second, ignore_errors));
                    }
                }
    ...
    struct AutoDetectState {
        AutoDetectState(ClientContext &context_p, MultiFileBindData &bind_data_p, const vector<OpenFileInfo> &files,
            ...
            : context(context_p), bind_data(bind_data_p), files(files), date_format_map(date_format_map),
              files_scanned(0),
        ...
        static idx_t ExecuteInternal(AutoDetectState &auto_detect_state, JSONStructureNode &node, const idx_t file_idx,
            ...
            while (remaining != 0) {
                ...
                if (!node.ContainsVarchar()) {
                    // Can't refine non-VARCHAR types
                    continue;
                }
    ...
    void JSONScan::AutoDetect(ClientContext &context, MultiFileBindData &bind_data, vector<LogicalType> &return_types,
        ...
        // Same idea as in union_by_name.h
        ...
        for (idx_t task_idx = 0; task_idx < num_tasks; task_idx++) {
            ...
            auto task =
                    make_uniq<JSONSchemaTask>(executor, auto_detect_state, task_nodes[task_idx], file_idx_start,
                                              file_idx_end);
