...
namespace goose {
    ...
    public:
        ZSTDAnalyzeState(CompressionInfo &info, DBConfig &config) : AnalyzeState(info), config(config),
                                                                    context(nullptr) {
    ...
    public:
        ...
        idx_t NewSegment() {
            if (current_buffer == &segment_handle) {
                ...
                throw InternalException(
                    "We are asking for a new segment, but somehow we're still writing vector data onto "
        ...
        void InitializeVector() {
            ...
            ::ZSTD_CCtx_reset(analyze_state->context, ::ZSTD_reset_session_only);
            ::ZSTD_CCtx_refCDict(analyze_state->context, nullptr);
            ::ZSTD_CCtx_setParameter(analyze_state->context, ::ZSTD_c_compressionLevel,
                                     GetCompressionLevel());
            in_vector = true;
        }
        ...
        void CompressString(const string_t &string, bool end_of_vector) {
            ...
            const auto end_mode = end_of_vector ? ::ZSTD_e_end : ::ZSTD_e_continue;
            ...
            while (true) {
                ...
                compress_result =
                        ::ZSTD_compressStream2(analyze_state->context, &out_buffer, &in_buffer, end_mode);
    ...
    unique_ptr<CompressionState> ZSTDStorage::InitCompression(ColumnDataCheckpointData &checkpoint_data,
        ...
        return make_uniq<ZSTDCompressionState>(checkpoint_data,
                                               unique_ptr_cast<AnalyzeState, ZSTDAnalyzeState>(
                                                   std::move(analyze_state_p)));
    }
    ...
    public:
        ...
        ZSTDVectorScanState &LoadVector(idx_t vector_idx, idx_t internal_offset) {
            ...
            if (scan_state.metadata.block_offset + string_lengths_size + scan_state.metadata.compressed_size >
                ...
                scan_state.in_buffer.size =
                        MinValue(metadata.compressed_size,
                                 block_manager.GetBlockSize() - sizeof(block_id_t) - current_offset);
            } else {
                scan_state.in_buffer.size =
            ...
            ::ZSTD_DCtx_refDDict(decompression_context, nullptr);
        ...
        void DecompressString(ZSTDVectorScanState &scan_state, data_ptr_t destination, idx_t uncompressed_length) {
            ...
            while (true) {
                ...
                size_t res = ::ZSTD_decompressStream(
                    /* zds = */ decompression_context,
                                /* output =*/&out_buffer,
                                /* input =*/&in_buffer);
                scan_state.compressed_scan_count += in_buffer.pos - old_pos;
                if (::ZSTD_isError(res)) {
                    throw InvalidInputException("ZSTD Decompression failed: %s", ::ZSTD_getErrorName(res));
                }
        ...
        void Skip(ZSTDVectorScanState &scan_state, idx_t count) {
            if (!skip_buffer) {
                skip_buffer = Allocator::DefaultAllocator().Allocate(::ZSTD_DStreamOutSize());
            }
    ...
    public:
        ...
        ::ZSTD_DCtx *decompression_context = nullptr;
    ...
    // Scan base data
    //===--------------------------------------------------------------------===//
    void ZSTDStorage::StringScanPartial(ColumnSegment &segment, ColumnScanState &state, idx_t scan_count,
                                        Vector &result,
                                        idx_t result_offset) {
