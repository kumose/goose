...
#define TEST_ALL_LOG_MACROS(LOG_LEVEL, SOURCE, LOG_TYPE)                                                               \
	GOOSE_LOG_INTERNAL(SOURCE, "default", LOG_LEVEL, "log-a-lot: 'simple'")                                           \
	GOOSE_LOG_INTERNAL(SOURCE, "default", LOG_LEVEL, "log-a-lot: '%s'", "format")                                     \
	GOOSE_LOG_INTERNAL(SOURCE, "default", LOG_LEVEL, string("log-a-lot: 'string type'"))                              \
	GOOSE_LOG_INTERNAL(SOURCE, "custom_type", LOG_LEVEL, "log-a-lot: 'simple with type'")                             \
	GOOSE_LOG_INTERNAL(SOURCE, "custom_type", LOG_LEVEL, "log-a-lot: '%s'", "format with type")                       \
	GOOSE_LOG_INTERNAL(SOURCE, "custom_type", LOG_LEVEL, string("log-a-lot: 'string type with type'"))
...
void test_logging(const string &minimum_level, const string &enabled_log_types, const string &disabled_log_types) {
	...
	goose::vector<Value> default_types = {"default", "default"};
	goose::vector<string> log_levels = {"TRACE", "DEBUG", "INFO", "WARNING", "ERROR", "FATAL"};
	...
	goose::vector<Value> expected_types;
	goose::vector<Value> expected_messages;
	goose::vector<Value> expected_log_levels;
	idx_t num_runs = 2;
	...
	auto res = con.Query("SELECT type, log_level, message, from goose_logs where starts_with(message, 'log-a-lot:')");
...
static goose::unique_ptr<LocalTableFunctionState>
...
static goose::unique_ptr<FunctionData> TestLoggingBind(ClientContext &context, TableFunctionBindInput &input,
...
static void TestLoggingFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	...
	GOOSE_LOG_INTERNAL(local_state.context, "duckdb.", LogLevel::LOG_INFO, "thread_logger");
...
TEST_CASE("Test thread context logger", "[logging][.]") {
	...
	goose::TableFunction tf("test_thread_logger", {}, TestLoggingFunction, TestLoggingBind, nullptr,
	...
	auto res = con.Query("SELECT scope, message, from goose_logs where starts_with(message, 'thread_logger')");
...
TEST_CASE("Test pluggable log storage", "[logging][.]") {
	...
	goose::shared_ptr<LogStorage> base_ptr = my_log_storage;
