...
#include <goose/optimizer/late_materialization_helper.h>
...
namespace goose {
    ...
    TopNWindowElimination::TopNWindowElimination(ClientContext &context_p, Optimizer &optimizer,
                                                 optional_ptr<column_binding_map_t<unique_ptr<BaseStatistics> > >
                                                 stats_p)
    ...
    TopNWindowElimination::CreateRowNumberGenerator(unique_ptr<Expression> aggregate_column_ref) const {
        ...
        // array_length
        auto &array_length_entry = catalog.GetEntry<
            ScalarFunctionCatalogEntry>(context, DEFAULT_SCHEMA, "array_length");
        ...
        auto bound_array_length_fun = function_binder.BindScalarFunction(
            array_length_fun, std::move(array_length_exprs));
    ...
    vector<unique_ptr<Expression> > TopNWindowElimination::GenerateAggregatePayload(
        ...
        for (idx_t i = 0; i < bindings.size(); i++) {
            ...
            if (window.children[0]->type == LogicalOperatorType::LOGICAL_PROJECTION) {
                ...
                const auto child_column_idx =
                        static_cast<idx_t>(std::find(window_child_bindings.begin(), window_child_bindings.end(),
                                                     binding) -
    ...
    vector<ColumnBinding> TopNWindowElimination::TraverseProjectionBindings(
        const std::vector<ColumnBinding> &old_bindings,
    ...
    TopNWindowElimination::ExtractOptimizerParameters(const LogicalWindow &window, const LogicalFilter &filter,
        ...
        params.payload_type = aggregate_payload.size() > 1
                                  ? TopNPayloadType::STRUCT_PACK
                                  : TopNPayloadType::SINGLE_COLUMN;
        auto &window_expr = window.expressions[0]->Cast<BoundWindowExpression>();
    ...
    bool TopNWindowElimination::CanUseLateMaterialization(const LogicalWindow &window,
        ...
        if (!projections.empty()) {
            ...
            for (auto projection_id: projections) {
                projs.push_back(make_uniq<BoundColumnRefExpression>(lhs_get->types[projection_id],
                                                                    ColumnBinding{
                                                                        lhs_get->table_index, projection_id
                                                                    }));
            }
    ...
    unique_ptr<LogicalOperator> TopNWindowElimination::ConstructJoin(unique_ptr<LogicalOperator> lhs,
        ...
        const idx_t rowid_column_count =
                params.include_row_number
                    ? rhs->types.size() - (aggregate_offset + 1)
