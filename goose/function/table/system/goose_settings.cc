...
namespace goose {
    struct GooseSettingValue {
        ...
        inline bool operator<(const GooseSettingValue &rhs) const {
            return name < rhs.name;
        };
    };

    struct GooseSettingsData : public GlobalTableFunctionState {
        GooseSettingsData() : offset(0) {
        }

        vector<GooseSettingValue> settings;
        idx_t offset;
    };

    static unique_ptr<FunctionData> GooseSettingsBind(ClientContext &context, TableFunctionBindInput &input,
    ...
    unique_ptr<GlobalTableFunctionState> GooseSettingsInit(ClientContext &context, TableFunctionInitInput &input) {
        auto result = make_uniq<GooseSettingsData>();
        ...
        for (idx_t i = 0; i < options_count; i++) {
            ...
            for (auto &alias: value.aliases) {
                GooseSettingValue alias_value = value;
    ...
    void GooseSettingsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
        auto &data = data_p.global_state->Cast<GooseSettingsData>();
    ...
    void GooseSettingsFun::RegisterFunction(BuiltinFunctions &set) {
        set.AddFunction(
            TableFunction("goose_settings", {}, GooseSettingsFunction, GooseSettingsBind, GooseSettingsInit));
    }
} // namespace goose
