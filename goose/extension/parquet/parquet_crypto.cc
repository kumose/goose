...
using goose_apache::thrift::protocol::TCompactProtocolFactoryT;
using goose_apache::thrift::transport::TTransport;
...
uint32_t ParquetCrypto::Read(TBase &object, TProtocol &iprot, const string &key,
	...
	auto dprot = tproto_factory.getProtocol(
	    goose_base_std::make_shared<DecryptionTransport>(iprot, key, encryption_util_p, crypto_meta_data));
	...
	auto simple_prot =
	    tsimple_proto_factory.getProtocol(goose_base_std::make_shared<SimpleReadTransport>(all.get(), all.GetSize()));
...
uint32_t ParquetCrypto::Write(const TBase &object, TProtocol &oprot, const string &key,
	...
	auto eprot =
	    tproto_factory.getProtocol(goose_base_std::make_shared<EncryptionTransport>(oprot, key, encryption_util_p));
...
static string Base64Decode(const string &key) {
	...
	auto output = goose::unique_ptr<unsigned char[]>(new unsigned char[result_size]);
