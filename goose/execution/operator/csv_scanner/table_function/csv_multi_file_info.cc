#include <goose/execution/operator/csv_scanner/csv_multi_file_info.h>
#include <goose/execution/operator/csv_scanner/csv_file_scanner.h>
#include <goose/execution/operator/csv_scanner/global_csv_state.h>
...
#include <goose/execution/operator/persistent/csv_rejects_table.h>
#include <goose/common/bind_helpers.h>
...
namespace goose {
    ...
    CSVSchema CSVSchemaDiscovery::SchemaDiscovery(ClientContext &context, shared_ptr<CSVBufferManager> &buffer_manager,
...

        {
            ...
            idx_t rows_read = sniffer.LinesSniffed() -
                              (options.dialect_options.skip_rows.GetValue() + options.dialect_options.header.
                               GetValue());
        ...
        while (total_number_of_rows < required_number_of_lines && current_file < files_to_sniff) {
            ...
            if (file_buffer_manager->GetBuffer(0)->actual_size == 0) {
                ...
                schemas.emplace_back(sniffer_result.names, sniffer_result.return_types, option_copy.file_path,
                                     rows_read);
            }
            total_number_of_rows += sniffer.LinesSniffed();
            if (sniffer.EmptyOrOnlyHeader()) {
    ...
    void CSVMultiFileInfo::BindReader(ClientContext &context, vector<LogicalType> &return_types, vector<string> &names,
        ...
        if (!bind_data.file_options.union_by_name) {
            if (options.auto_detect) {
                csv_data.csv_schema =
                        CSVSchemaDiscovery::SchemaDiscovery(context, csv_data.buffer_manager, options,
    ...
    void CSVMultiFileInfo::FinalizeBindData(MultiFileBindData &multi_file_data) {
        ...
        if (!options.force_not_null_names.empty()) {
            ...
            goose::unordered_set<string> column_names;
            for (auto &name: names) {
    ...
    optional_idx CSVMultiFileInfo::MaxThreads(const MultiFileBindData &bind_data,
    ...
    unique_ptr<GlobalTableFunctionState> CSVMultiFileInfo::InitializeGlobalState(ClientContext &context,
        ...
        return make_uniq<CSVGlobalState>(context, csv_data.options, bind_data.file_list->GetTotalFileCount(),
                                         bind_data);
    }
    ...
    shared_ptr<BaseFileReader> CSVMultiFileInfo::CreateReader(ClientContext &context,
        ...
        return make_shared_ptr<CSVFileScan>(context, union_data.GetFileName(), std::move(options),
                                            bind_data.file_options,
                                            csv_names, csv_types, csv_data.csv_schema, gstate.SingleThreadedRead(),
                                            nullptr,
                                            false);
    }
