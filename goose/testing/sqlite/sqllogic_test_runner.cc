...
#include <goose/testing/sqlite/sqllogic_parser.h>
...
namespace goose {
    ...
    SQLLogicTestRunner::SQLLogicTestRunner(string dbpath) : dbpath(std::move(dbpath)), finished_processing_file(false) {
        ...
        if (env_var) {
            ...
            local_extension_repo = string() + "/repository";
        }
    ...
    void SQLLogicTestRunner::ExecuteCommand(goose::unique_ptr<Command> command) {
        if (InLoop()) {
    ...
    ExtensionLoadResult SQLLogicTestRunner::LoadExtension(Goose &db, const std::string &extension) {
        auto &test_config = TestConfiguration::Get();
    ...
    void SQLLogicTestRunner::LoadDatabase(string dbpath, bool load_extensions) {
        ...
        try {
            db = make_uniq<Goose>(dbpath, config.get());
    ...
    string SQLLogicTestRunner::LoopReplacement(string text, const vector<LoopDefinition> &loops) {
        for (auto &active_loop: loops) {
            if (active_loop.tokens.empty()) {
                ...
                // foreach loop
                text = ReplaceLoopIterator(text, active_loop.loop_iterator_name,
                                           active_loop.tokens[active_loop.loop_idx]);
            }
        }
        return text;
    }
    ...
    string SQLLogicTestRunner::ReplaceKeywords(string input) {
        ...
        input = StringUtil::Replace(input, "__BUILD_DIRECTORY__", get_goose_build_directory());
    ...
    RequireResult SQLLogicTestRunner::CheckRequire(SQLLogicParser &parser, const vector<string> &params) {
        ...
        if (param == "no_latest_storage") {
            ...
            return RequireResult::MISSING;
#elif defined(GOOSE_ALTERNATIVE_VERIFY)
    ...
    void SQLLogicTestRunner::ExecuteFile(string script) {
        ...
        while (parser.NextStatement()) {
            ...
            while (token.type == SQLLogicTokenType::SQLLOGIC_SKIP_IF || token.type ==
                ...
                if (is_system_comparison) {
                    bool our_system = system_name == "goose";
                    if (original_sqlite_test) {
            ...
            if (token.type == SQLLogicTokenType::SQLLOGIC_STATEMENT) {
                ...
                if (token.parameters[0] == "ignore_error_messages" || token.parameters[0] ==
                ...
                string default_local_repo = string(get_goose_build_directory()) + "/repository";
                if (env_actual == nullptr && env_var == "LOCAL_EXTENSION_REPO" &&
                    ...
                    // More proper solution is wrapping std::getenv in a goose::test_getenv, and having a way to inject env
                    // variables
                ...
                environment_variables[env_var] = env_actual;
                add_env_tag(file_tags, token.parameters[0],
                            token.parameters.size() == 2 ? &token.parameters[1] : nullptr);
                ...
                if (!initial_db.empty() && !is_read_only && (
                        !db || db->instance->config.options.database_path.empty())) {
