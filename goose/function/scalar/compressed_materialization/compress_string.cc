    ...
    namespace {
        ...
        inline RESULT_TYPE StringCompressInternal(const string_t &input) {
            ...
            if (sizeof(RESULT_TYPE) <= string_t::INLINE_LENGTH) {
                ...
                TemplatedReverseMemCpy<string_t::INLINE_LENGTH>(result_ptr + REMAINDER,
        ...
        void StringDecompressFunction(DataChunk &args, ExpressionState &state, Vector &result) {
            auto &allocator = ExecuteFunctionState::GetFunctionState(state)->Cast<StringDecompressLocalState>().
                    allocator;
            allocator.Reset();
