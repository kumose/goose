...
TEST_CASE("Test DB config configuration", "[api]") {
	...
	map<string, goose::vector<string>> test_options;
...
TEST_CASE("Test user_agent", "[api]") {
	{
		// Default goose_api is cpp
		Goose db(nullptr);
		Connection con(db);
		auto res = con.Query("PRAGMA user_agent");
		REQUIRE_THAT(res->GetValue(0, 0).ToString(), Catch::Matchers::Matches("goose/.*(.*) cpp"));
	}
	{
		// The latest provided goose_api is used
		DBConfig config;
		config.SetOptionByName("goose_api", "capi");
		config.SetOptionByName("goose_api", "go");
		Goose db("", &config);
		Connection con(db);
		auto res = con.Query("PRAGMA user_agent");
		REQUIRE_THAT(res->GetValue(0, 0).ToString(), Catch::Matchers::Matches("goose/.*(.*) go"));
	}
}
...
TEST_CASE("Test secret creation with a custom secret_directory configuration", "[api]") {
	...
	string my_secret_file = fs.JoinPath(my_secret_dir, "my_secret.goose_secret");
...
TEST_CASE("Test secret creation with a custom secret_directory configuration update", "[api]") {
	...
	string my_other_secret_file = fs.JoinPath(new_secret_dir, "my_other_secret.goose_secret");
