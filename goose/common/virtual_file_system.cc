...
namespace goose {
    ...
    FileSystem &VirtualFileSystem::FindFileSystem(const string &path, optional_ptr<DatabaseInstance> db_instance) {
        ...
        if (!fs && db_instance) {
            ...
            if (!required_extension.empty() && db_instance && !db_instance->ExtensionIsLoaded(required_extension)) {
                ...
                if (!ExtensionHelper::CanAutoloadExtension(required_extension) ||
                    ...
                    auto error_message = "File " + path + " requires the extension " + required_extension +
                                         " to be loaded";
                    error_message =
                            ExtensionHelper::AddExtensionInstallHintToErrorMsg(
                                *db_instance, error_message, required_extension);
        ...
        if (!disabled_file_systems.empty() && disabled_file_systems.find(fs->GetName()) != disabled_file_systems.
            end()) {
