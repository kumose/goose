...
#include <goose/verification/copied_statement_verifier.h>
#include <goose/verification/deserialized_statement_verifier.h>
#include <goose/verification/external_statement_verifier.h>
#include <goose/verification/parsed_statement_verifier.h>
#include <goose/verification/prepared_statement_verifier.h>
#include <goose/verification/unoptimized_statement_verifier.h>
#include <goose/verification/no_operator_caching_verifier.h>
#include <goose/verification/fetch_row_verifier.h>
...
namespace goose {
    ...
    StatementVerifier::StatementVerifier(VerificationType type, string name, unique_ptr<SQLStatement> statement_p,
        ...
        : type(type), name(std::move(name)), statement(std::move(statement_p)),
          select_statement(statement->type == StatementType::SELECT_STATEMENT
                               ? &statement->Cast<SelectStatement>()
                               : nullptr),
    ...
    string StatementVerifier::CompareResults(const StatementVerifier &other) {
        D_ASSERT(type == VerificationType::ORIGINAL);
        string error;
        if (materialized_result->HasError() != other.materialized_result->HasError()) {
            // LCOV_EXCL_START
        ...
        if (!ColumnDataCollection::ResultEquals(materialized_result->Collection(),
                                                other.materialized_result->Collection(),
                                                error)) {
            // LCOV_EXCL_START
            string result = other.name + " statement differs from original result!\n";
            result += "Original Result:\n" + materialized_result->ToString();
