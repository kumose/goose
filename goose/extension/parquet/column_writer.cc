...
#include <goose/extension/parquet/parquet_bss_encoder.h>
#include <goose/extension/parquet/parquet_statistics.h>
#include <goose/extension/parquet/parquet_writer.h>
#include <goose/extension/parquet/writer/array_column_writer.h>
#include <goose/extension/parquet/writer/boolean_column_writer.h>
#include <goose/extension/parquet/writer/decimal_column_writer.h>
#include <goose/extension/parquet/writer/enum_column_writer.h>
...
#include <goose/extension/parquet/writer/struct_column_writer.h>
#include <goose/extension/parquet/writer/variant_column_writer.h>
#include <goose/extension/parquet/writer/templated_column_writer.h>
...
using namespace goose_miniz;   // NOLINT
...
void ColumnWriterStatistics::WriteGeoStats(goose_parquet::GeospatialStatistics &stats) {
...
ColumnWriter::ColumnWriter(ParquetWriter &writer, ParquetColumnSchema &&column_schema_p, vector<string> schema_path_p)
	...
	can_have_nulls = column_schema.repetition_type == goose_parquet::FieldRepetitionType::OPTIONAL;
}
...
void ColumnWriter::CompressPage(MemoryStream &temp_writer, size_t &compressed_size, data_ptr_t &compressed_data,
	...
	case CompressionCodec::SNAPPY: {
		compressed_size = snappy::MaxCompressedLength(temp_writer.GetPosition());
		compressed_buf = BufferAllocator::Get(writer.GetContext()).Allocate(compressed_size);
		snappy::RawCompress(const_char_ptr_cast(temp_writer.GetData()), temp_writer.GetPosition(),
		                           char_ptr_cast(compressed_buf.get()), &compressed_size);
		compressed_data = compressed_buf.get();
		D_ASSERT(compressed_size <= snappy::MaxCompressedLength(temp_writer.GetPosition()));
		break;
	}
	case CompressionCodec::LZ4_RAW: {
		compressed_size = ::LZ4_compressBound(UnsafeNumericCast<int32_t>(temp_writer.GetPosition()));
		compressed_buf = BufferAllocator::Get(writer.GetContext()).Allocate(compressed_size);
	...
	case CompressionCodec::ZSTD: {
		compressed_size = ::ZSTD_compressBound(temp_writer.GetPosition());
		compressed_buf = BufferAllocator::Get(writer.GetContext()).Allocate(compressed_size);
