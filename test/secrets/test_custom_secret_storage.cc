...
"[secret][.]"
)
 {
	...
	auto &secret_manager = goose::SecretManager::Get(*db.instance);

	auto storage_ptr = goose::make_uniq<TestSecretStorage>("test_storage", *db.instance, log, 30);
	...
	// Inspect current goose_secrets output
	auto result = con.Query("SELECT name, storage from goose_secrets() ORDER BY type, name, storage");
...
"[secret][.]"
)
 {
	...
	auto storage_ptr = goose::make_uniq<TestSecretStorage>("test_storage_after", *db.instance, log1, 30);
	secret_manager.LoadSecretStorage(std::move(storage_ptr));

	// Correct tie-break offset: 0 places it before temporary and persistent
	auto storage_ptr2 = goose::make_uniq<TestSecretStorage>("test_storage_before", *db.instance, log2, 0);
	...
	// Inspect current goose_secrets output
	auto result = con.Query("SELECT name, storage from goose_secrets() ORDER BY name, storage");
...
"[secret][.]"
)
 {
	...
	TestSecretLog log;
	auto storage_ptr = goose::make_uniq<TestSecretStorage>("failing_storage", *db.instance, log, 10);
...
"[secret][.]"
)
 {
	...
	REQUIRE_NO_FAIL(con.Query("FROM goose_secrets();"));
...
"[secret][.]"
)
 {
	...
	TestSecretLog log;
	auto storage_ptr = goose::make_uniq<TestSecretStorage>("memory", *db.instance, log, 50);
