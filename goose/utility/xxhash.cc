...
#include <goose/utility/xxhash_static.h>
...
// Goose: prefixed with XXHASH_ to avoid name conflicts
...
#define XXH_STATIC_ASSERT(c)   { enum { XXH_static_assert = 1/(int)(!!(c)) }; }    /* use only *after* variable declarations */
    ...
    *****************************/
    ...
    XXHASH_FORCE_INLINE_TEMPLATE U32 XXH32_endian_align(const void *input, size_t len, U32 seed, XXH_endianess endian,
                                                        XXH_alignment align) {
...
#define XXH_get32bits(p) XXH_readLE32_align(p, endian, align)
    ...
    XXHASH_FORCE_INLINE_TEMPLATE U64 XXH64_endian_align(const void *input, size_t len, U64 seed, XXH_endianess endian,
                                                        XXH_alignment align) {
        const BYTE *p = (const BYTE *) input;
...
#define XXH_get64bits(p) XXH_readLE64_align(p, endian, align)
    ...
    ****************************************************/
    ...
    XXHASH_FORCE_INLINE_TEMPLATE XXH_errorcode XXH32_update_endian(XXH32_state_t *state, const void *input, size_t len,
    ...
    XXHASH_FORCE_INLINE_TEMPLATE U32 XXH32_digest_endian(const XXH32_state_t *state, XXH_endianess endian) {
        ...
        if (state->large_len) {
            h32 = XXH_rotl32(state->v1, 1) + XXH_rotl32(state->v2, 7) + XXH_rotl32(state->v3, 12) + XXH_rotl32(
                      state->v4, 18);
        } else {
    ...
    XXHASH_FORCE_INLINE_TEMPLATE XXH_errorcode XXH64_update_endian(XXH64_state_t *state, const void *input, size_t len,
                                                                   XXH_endianess endian) {
        const BYTE *p = (const BYTE *) input;
        const BYTE *const bEnd = p + len;
