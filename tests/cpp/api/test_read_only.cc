...
TEST_CASE("Test connection using a read only database", "[readonly]") {
	...
	REQUIRE_THROWS(db = make_uniq<Goose>(nullptr, &readonly_config));
	// cannot create a read-only database in a new directory
	REQUIRE_THROWS(db = make_uniq<Goose>(dbdir, &readonly_config));

	// create the database file and initialize it with data
	db = make_uniq<Goose>(dbdir);
	...
	REQUIRE_NOTHROW(db = make_uniq<Goose>(dbdir, &readonly_config));
	con = make_uniq<Connection>(*db);
	...
	// REQUIRE_NOTHROW(db = make_uniq<Goose>(dbdir, true));
	// REQUIRE_NOTHROW(db2 = make_uniq<Goose>(dbdir, true));
	// db.reset();
	// db2.reset();

	// // however, if there is read-only database, we can't connect a read-write database
	// REQUIRE_NOTHROW(db = make_uniq<Goose>(dbdir, true));
	// REQUIRE_THROWS(db2 = make_uniq<Goose>(dbdir));
	// db.reset();
	// db2.reset();
	...
	// REQUIRE_THROWS(db2 = make_uniq<Goose>(dbdir, true));
	// db.reset();
	// db2.reset();
	DeleteDatabase(dbdir);
}
...
TEST_CASE("Test view creation using a read only database", "[readonly]") {
	...
	{ auto db_rw = Goose(dbdir); }
	db = make_uniq<Goose>(dbdir, &readonly_config);
	// create the database file and initialize it with data
	con = make_uniq<Connection>(*db);

	REQUIRE_NOTHROW(con->TableFunction("goose_tables")->CreateView("boo", true, true));
	con.reset();
	db.reset();
