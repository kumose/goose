...
public:
	...
	//! i.e., 2^(8*3-1) - 1.
	GOOSE_API static constexpr uint32_t MAX_DATA_SIZE = 8388607;
	//! Header size of a Bignum is always 3 bytes.
	GOOSE_API static constexpr uint8_t BIGNUM_HEADER_SIZE = 3;
	//! Max(e such that 10**e fits in a digit_t)
	GOOSE_API static constexpr uint8_t DECIMAL_SHIFT = 9;
	//! 10 ** DECIMAL_SHIFT
	GOOSE_API static constexpr digit_t DECIMAL_BASE = 1000000000;
	//! Bytes of a digit_t
	GOOSE_API static constexpr uint8_t DIGIT_BYTES = sizeof(digit_t);
	//! Bits of a digit_t
	GOOSE_API static constexpr uint8_t DIGIT_BITS = DIGIT_BYTES * 8;
	...
	GOOSE_API static void Verify(const bignum_t &input);

	//! Sets the header of a bignum (i.e., char* blob), depending on the number of bytes that bignum needs and if it's a
	//! negative number
	GOOSE_API static void SetHeader(char *blob, uint64_t number_of_bytes, bool is_negative);
	//! Initializes and returns a blob with value 0, allocated in Vector& result
	GOOSE_API static bignum_t InitializeBignumZero(Vector &result);
	GOOSE_API static string InitializeBignumZero();

	//! Switch Case of To Bignum Convertion
	GOOSE_API static BoundCastInfo NumericToBignumCastSwitch(const LogicalType &source);
	...
	GOOSE_API static bool VarcharFormatting(const string_t &value, idx_t &start_pos, idx_t &end_pos, bool &is_negative,
	                                         bool &is_zero);

	//! Converts a char to a Digit
	GOOSE_API static int CharToDigit(char c);
	//! Converts a Digit to a char
	GOOSE_API static char DigitToChar(int digit);
	//! Function to convert a string_t into a vector of bytes
	GOOSE_API static void GetByteArray(vector<uint8_t> &byte_array, bool &is_negative, const string_t &blob);
	//! Function to create a BIGNUM blob from a byte array containing the absolute value, plus an is_negative bool
	GOOSE_API static string FromByteArray(uint8_t *data, idx_t size, bool is_negative);
	//! Function to convert BIGNUM blob to a VARCHAR
	GOOSE_API static string BignumToVarchar(const bignum_t &blob);
	//! Function to convert Varchar to BIGNUM blob
	GOOSE_API static string VarcharToBignum(const string_t &value);
	//! ----------------------------------- Double Cast ----------------------------------- //
	GOOSE_API static bool BignumToDouble(const bignum_t &blob, double &result, bool &strict);
	template <class T>
...
template <>
GOOSE_API bool TryCastToBignum::Operation(double double_value, bignum_t &result_value, Vector &result,
                                           CastParameters &parameters);

template <>
GOOSE_API bool TryCastToBignum::Operation(float float_value, bignum_t &result_value, Vector &result,
                                           CastParameters &parameters);

template <>
GOOSE_API bool TryCastToBignum::Operation(string_t input_value, bignum_t &result_value, Vector &result,
                                           CastParameters &parameters);

struct BignumCastToVarchar {
	template <class SRC>
	GOOSE_API static inline string_t Operation(SRC input, Vector &result) {
...
struct BignumToDoubleCast {
	template <class SRC, class DST>
	GOOSE_API static inline bool Operation(SRC input, DST &result, bool strict = false) {
