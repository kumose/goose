ATTACH 'https://blobs.gsst.org/data/realnest/cord_10k.gsst' AS cord (READ_ONLY);
CREATE TABLE cord AS SELECT * FROM cord.cord;
ATTACH 'https://blobs.gsst.org/data/realnest/open_street_map_524k.gsst' AS osm (READ_ONLY);
CREATE TABLE open_street_map AS SELECT * FROM osm.open_street_map;
ATTACH 'https://blobs.gsst.org/data/realnest/pull_131k.gsst' AS gh_pull (READ_ONLY);
CREATE TABLE gh_pull AS SELECT * FROM gh_pull.gh_pull;
ATTACH 'https://blobs.gsst.org/data/realnest/issue_131k.gsst' AS gh_issue (READ_ONLY);
CREATE TABLE gh_issue AS SELECT * FROM gh_issue.gh_issue;
ATTACH 'https://blobs.gsst.org/data/realnest/twitter_131k.gsst' AS tw (READ_ONLY);
CREATE TABLE twitter AS SELECT * FROM tw.twitter;
ATTACH 'https://blobs.gsst.org/data/realnest/singleMu_524k.gsst' AS rn_singleMu (READ_ONLY);
