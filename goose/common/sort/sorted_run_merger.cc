#include <goose/common/sorting/sorted_run_merger.h>

#include <goose/common/sorting/sort.h>
#include <goose/common/sorting/sorted_run.h>
#include <goose/common/sorting/sort_key.h>
#include <goose/common/types/row/block_iterator.h>
#include <goose/common/types/row/tuple_data_collection.h>
#include <goose/parallel/parallel_destroy_task.h>

#include <goose/common/vergesort/vergesort.h>
#include <goose/common/pdqsort/pdqsort.h>
...
namespace goose {
    ...
    void SortedRunMergerLocalState::ComputePartitionBoundariesSwitch(SortedRunMergerGlobalState &gstate,
        ...
        switch (sort_key_type) {
            case SortKeyType::NO_PAYLOAD_FIXED_8:
                return TemplatedComputePartitionBoundaries<STATE, SortKeyType::NO_PAYLOAD_FIXED_8>(
                    gstate, p_idx, states);
            case SortKeyType::NO_PAYLOAD_FIXED_16:
                return TemplatedComputePartitionBoundaries<STATE, SortKeyType::NO_PAYLOAD_FIXED_16>(
                    gstate, p_idx, states);
            case SortKeyType::NO_PAYLOAD_FIXED_24:
                return TemplatedComputePartitionBoundaries<STATE, SortKeyType::NO_PAYLOAD_FIXED_24>(
                    gstate, p_idx, states);
            case SortKeyType::NO_PAYLOAD_FIXED_32:
                return TemplatedComputePartitionBoundaries<STATE, SortKeyType::NO_PAYLOAD_FIXED_32>(
                    gstate, p_idx, states);
            case SortKeyType::NO_PAYLOAD_VARIABLE_32:
                return TemplatedComputePartitionBoundaries<STATE, SortKeyType::NO_PAYLOAD_VARIABLE_32>(
                    gstate, p_idx, states);
            ...
            case SortKeyType::PAYLOAD_VARIABLE_32:
                return TemplatedComputePartitionBoundaries<STATE, SortKeyType::PAYLOAD_VARIABLE_32>(
                    gstate, p_idx, states);
            default:
    ...
    template<class STATE>
    void SortedRunMergerLocalState::MergePartitionSwitch(SortedRunMergerGlobalState &gstate,
                                                         unsafe_vector<STATE> &states) {
        switch (sort_key_type) {
            case SortKeyType::NO_PAYLOAD_FIXED_8:
    ...
    void SortedRunMergerLocalState::TemplatedMergePartition(SortedRunMergerGlobalState &gstate,
        ...
        for (idx_t run_idx = 0; run_idx < gstate.num_runs; run_idx++) {
            ...
            for (auto it = BLOCK_ITERATOR(state, run_boundary.begin); it != BLOCK_ITERATOR(state, run_boundary.end); ++
                 it) {
        ...
        static const auto fallback = [](SORT_KEY *begin, SORT_KEY *end) {
            goose_pdqsort::pdqsort_branchless(begin, end);
        };
        goose_vergesort::vergesort(merged_partition_keys, merged_partition_keys + merged_partition_count,
    ...
    void SortedRunMergerLocalState::TemplatedScanPartition(SortedRunMergerGlobalState &gstate, DataChunk &chunk) {
        ...
        const auto merged_partition_keys = reinterpret_cast<SORT_KEY *>(merged_partition.get()) +
                                           merged_partition_index;
        ...
        while (merged_partition_index < merged_partition_count) {
            ...
            if (SORT_KEY::HAS_PAYLOAD) {
                ...
                const auto new_payload_locations =
                        FlatVector::GetData<const data_ptr_t>(
                            sorted_run->payload_append_state.chunk_state.row_locations);
                for (idx_t i = 0; i < count; i++) {
                    auto &key = merged_partition_keys[merged_partition_index + i];
            ...
            sorted_run->key_data->Build(sorted_run->key_append_state.pin_state,
                                        sorted_run->key_append_state.chunk_state, 0,
                                        count);
    ...
    SortedRunMerger::SortedRunMerger(const Sort &sort_p, vector<unique_ptr<SortedRun> > &&sorted_runs_p,
        ...
        : scheduler(TaskScheduler::GetScheduler(*sort_p.context.db)), sort(sort_p),
          sorted_runs(std::move(sorted_runs_p)),
