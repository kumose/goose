...
void ArrowTableSchema::AddColumn(idx_t index, shared_ptr<ArrowType> type, const string &name) {
	...
	types[index] = type->GetGooseType(true);
...
void ArrowType::SetRunEndEncoded() {
	...
	auto actual_type = struct_info.GetChild(1).GetGooseType();
	// Override the goose type to the actual type
	type = actual_type;
...
unique_ptr<ArrowType> ArrowType::GetTypeFromFormat(DBConfig &config, ArrowSchema &schema, string &format) {
	...
	if (format == "+l") {
		...
		auto array_type = LogicalType::ARRAY(child_type->GetGooseType(), fixed_size);
		...
		if (schema.n_children == 0) {
			throw InvalidInputException(
			    "Attempted to convert a STRUCT with no fields to Goose which is not supported");
		}
		for (idx_t type_idx = 0; type_idx < static_cast<idx_t>(schema.n_children); type_idx++) {
			...
			child_types.emplace_back(schema.children[type_idx]->name, children.back()->GetGooseType());
		}
		...
		vector<shared_ptr<ArrowType>> children;
		if (schema.n_children == 0) {
			throw InvalidInputException("Attempted to convert a UNION with no fields to Goose which is not supported");
		}
		for (idx_t type_idx = 0; type_idx < static_cast<idx_t>(schema.n_children); type_idx++) {
			auto type = schema.children[type_idx];

			children.emplace_back(GetArrowLogicalType(config, *type));
			members.emplace_back(type->name, children.back()->GetGooseType());
		}
		...
		key_value.emplace_back(std::make_pair("key", key_type->GetGooseType()));
		key_value.emplace_back(std::make_pair("value", value_type->GetGooseType()));

		auto map_type = LogicalType::MAP(key_type->GetGooseType(), value_type->GetGooseType());
		vector<shared_ptr<ArrowType>> children;
		children.reserve(2);
...
unique_ptr<ArrowType> ArrowType::CreateListType(DBConfig &config, ArrowSchema &child, ArrowVariableSizeType size_type,
	...
	unique_ptr<ArrowTypeInfo> type_info;
	auto type = LogicalType::LIST(child_type->GetGooseType());
	if (view) {
...
LogicalType ArrowType::GetGooseType(bool use_dictionary) const {
	if (use_dictionary && dictionary_type) {
		return dictionary_type->GetGooseType();
	}
	if (!use_dictionary) {
		if (extension_data) {
			return extension_data->GetGooseType();
		}
		return type;
	}
	...
	case LogicalTypeId::STRUCT: {
		...
		for (idx_t i = 0; i < struct_info.ChildCount(); i++) {
			...
			new_children.emplace_back(std::make_pair(child_name, child.GetGooseType(true)));
		}
	...
	case LogicalTypeId::LIST: {
		...
		return LogicalType::LIST(child.GetGooseType(true));
	}
	case LogicalTypeId::MAP: {
		auto &list_info = type_info->Cast<ArrowListInfo>();
		auto &struct_child = list_info.GetChild();
		auto struct_type = struct_child.GetGooseType(true);
...
ArrowTypeExtensionData::GetExtensionTypes(ClientContext &context, const vector<LogicalType> &goose_types) {
	unordered_map<idx_t, const shared_ptr<ArrowTypeExtensionData>> extension_types;
	const auto &db_config = DBConfig::GetConfig(context);
	for (idx_t i = 0; i < goose_types.size(); i++) {
		if (db_config.HasArrowExtension(goose_types[i])) {
			extension_types.insert({i, db_config.GetArrowExtension(goose_types[i]).GetTypeExtension()});
		}
	}
	return extension_types;
}

LogicalType ArrowTypeExtensionData::GetGooseType() const {
	return goose_type;
}
