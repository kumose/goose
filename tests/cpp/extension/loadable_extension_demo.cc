...
#include <goose/common/extension_type_info.h>
...
public:
    ...
    static goose::unique_ptr<FunctionData> QuackBind(ClientContext &context, TableFunctionBindInput &input,
    ...
    static goose::unique_ptr<GlobalTableFunctionState> QuackInit(ClientContext &context,
...
struct QuackExtensionData : public ParserExtensionParseData {
    ...
    goose::unique_ptr<ParserExtensionParseData> Copy() const override {
...
public:
    ...
    static ParserExtensionPlanResult QuackPlanFunction(ParserExtensionInfo *info, ClientContext &context,
                                                       goose::unique_ptr<ParserExtensionParseData> parse_data) {
        ...
        for (const auto &query_input: queries) {
            if (StringUtil::CIEquals(query_input, "override")) {
                ...
                select_node->select_list.push_back(
                    make_uniq<ConstantExpression>(Value("The Goose parser has been overridden")));
        ...
        if (statements.empty()) {
            auto not_implemented_exception =
                    NotImplementedException(
                        "QuackParser has not yet implemented the statements to transform this query");
...
//===--------------------------------------------------------------------===//
extern "C" {
GOOSE_CPP_EXTENSION_ENTRY(loadable_extension_demo, loader) {
    ...
    type_entry->tags["ext:author"] = "Goose Labs";
    ...
    add_point_entry->tags["ext:author"] = "Goose Labs";
    ...
    sub_point_entry->tags["ext:author"] = "Goose Labs";
