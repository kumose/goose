...
#include <goose/utility/fmt/ostream.h>
...
FMT_BEGIN_NAMESPACE
    namespace internal {
        ...
        public:
            ...
            int operator()(T value) {
                if (!int_checker<std::numeric_limits<T>::is_signed || std::is_same<T, int128_t>::value>::fits_in_int(
                    value))
                    FMT_THROW(goose::InvalidInputException("number is too big"));
                return (std::max)(static_cast<int>(value), 0);
            }

            template<typename T, FMT_ENABLE_IF(!is_integral<T>::value)>
            int operator()(T) {
                FMT_THROW(goose::InvalidInputException("precision is not integer"));
                return 0;
            }
        };
        ...
        template<>
        struct make_unsigned_or_bool<int128_t> {
            using type = uint128_t;
        };

        template<>
        struct make_unsigned_or_bool<uint128_t> {
            using type = uint128_t;
        };
        ...
        private:
            ...
            To Cast(From x) {
                return goose::Cast::Operation<From, To>(x);
            }
        ...
        public:
            ...
            template<typename T, FMT_ENABLE_IF(!is_integral<T>::value)>
            unsigned operator()(T) {
                FMT_THROW(goose::InvalidInputException("width is not integer"));
                return 0;
            }
        };
    ...
    public:
        ...
        template<typename T, FMT_ENABLE_IF(goosedb_fmt::internal::is_integral<T>::value)>
    ...
    OutputIt basic_printf_context<OutputIt, Char>::format() {
        ...
        while (it != end) {
            ...
            // Parse type.
            if (it == end)
                FMT_THROW(goose::InvalidInputException("invalid format string"));
