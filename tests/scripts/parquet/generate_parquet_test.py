...
def generate_header(f):
    f.write(
        '''# name: tests/scripts/parquet/test_parquet_reader.test
...
def get_goose_answer(file_path):
    answer = []
    try:
        answer = goose.query("SELECT * FROM parquet_scan('" + file_path + "') limit 50").fetchall()
...
def get_arrow_answer(file_path):
    ...
    try:
        ...
        goose_rel = goose.from_arrow(arrow).limit(50)
        answer = goose_rel.fetchall()
        return answer
    except:
        return 'fail'


def check_result(goose_result, arrow_result):
    if arrow_result == 'fail':
        return 'skip'
    if goose_result == 'fail':
        return 'fail'
    if goose_result != arrow_result:
        return 'fail'
    return 'pass'
...
def generate_test(file_path):
    goose_result = get_goose_answer(file_path)
    arrow_result = get_arrow_answer(file_path)
    result = check_result(goose_result, arrow_result)
    test_body = ""
    if result == 'skip':
        return
    if result == 'fail':
        ...
        test_body += generate_parquet_test_body(result, goose_result, file_path)
    return test_body
