...
class FaultInjectionFileSystem : public goose::LocalFileSystem {
public:
	...
	void Write(goose::FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) override {
		...
		return goose::LocalFileSystem::Write(handle, buffer, nr_bytes, location);
	}

	void FileSync(goose::FileHandle &handle) override {
		PRINT_VERBOSE("FS fsync " << handle.GetPath() << " file_size=" << handle.GetFileSize());
		if (is_db_file(handle)) {
			ThrowInjectedFaultIfSet(FaultInjectionSite::FSYNC);
		}
		return goose::LocalFileSystem::FileSync(handle);
	}

	void RemoveFile(const goose::string &filename,
	                goose::optional_ptr<goose::FileOpener> opener = nullptr) override {
		PRINT_VERBOSE("FS remove " << filename);
		return goose::LocalFileSystem::RemoveFile(filename, opener);
	}

	void Truncate(goose::FileHandle &handle, int64_t new_size) override {
		PRINT_VERBOSE("FS truncate " << handle.GetPath() << " from " << handle.GetFileSize() << " to " << new_size);
		return goose::LocalFileSystem::Truncate(handle, new_size);
	}
	...
	bool Trim(goose::FileHandle &handle, idx_t offset_bytes, idx_t length_bytes) override {
		PRINT_VERBOSE("FS trim " << handle.GetPath() << " offset=" << offset_bytes << " bytes=" << length_bytes);

		std::string nulls(length_bytes, '\0');
		goose::LocalFileSystem::Write(handle, (void *)nulls.data(), length_bytes, offset_bytes);
		return true;
	}
...
protected:
	void ThrowInjectedFaultIfSet(FaultInjectionSite site) {
		...
		if (it != faults.end()) {
			...
			throw goose::IOException("Injected fault");
		}
	}

	bool is_wal_file(const goose::FileHandle &handle) {
		return ends_with(handle.GetPath(), ".db.wal");
	}

	bool is_db_file(const goose::FileHandle &handle) {
		return ends_with(handle.GetPath(), ".db");
	}

	bool is_wal_or_db_file(const goose::FileHandle &handle) {
...
// This implementation of goose::FileSystem will cache writes to the database file in memory until fsync is called.
...
public:
	...
	int64_t Write(goose::FileHandle &handle, void *buffer, int64_t nr_bytes) override {
		// Check appends only occur on the WAL
		REQUIRE(is_wal_file(handle));

		return goose::LocalFileSystem::Write(handle, buffer, nr_bytes);
	}

	void Read(goose::FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) override {
		REQUIRE(is_db_file(handle));
		...
		return goose::LocalFileSystem::Read(handle, buffer, nr_bytes, location);
	}

	int64_t Read(goose::FileHandle &handle, void *buffer, int64_t nr_bytes) override {
		...
		return goose::LocalFileSystem::Read(handle, buffer, nr_bytes);
	}
	...
	void FileSync(goose::FileHandle &handle) override {
		...
		for (const auto &location_and_data : unflushed_chunks) {
			...
			goose::LocalFileSystem::Write(handle, (void *)data.data(), data.size(), location);
		}
		unflushed_chunks.clear();

		goose::LocalFileSystem::FileSync(handle);
	}
...
TEST_CASE("simple fault injection storage test", "[storage][.]") {
	...
	goose::DBConfig config;

	LazyFlushFileSystem *raw_fs = new LazyFlushFileSystem();
	config.file_system = goose::make_uniq<goose::VirtualFileSystem>(goose::unique_ptr<LazyFlushFileSystem>(raw_fs));
	...
	{
		goose::Goose db(file_path, &config);
...
TEST_CASE("fuzzed storage test", "[storage][.]") {
	...
	goose::DBConfig config;
	config.options.set_variables["debug_skip_checkpoint_on_commit"] = goose::Value(true);
	...
	goose::unique_ptr<QueryResult> previous_result;
	for (const auto &action : actions) {
		// Note: the injected file system has to be reset each time. Goose construction seems to be std::move'ing them

		/*
		LazyFlushFileSystem *raw_fs = new LazyFlushFileSystem();
		config.file_system =
		    goose::make_uniq<goose::VirtualFileSystem>(goose::unique_ptr<LazyFlushFileSystem>(raw_fs));
		 */

		FaultInjectionFileSystem *raw_fs = new FaultInjectionFileSystem();
		config.file_system =
		    goose::make_uniq<goose::VirtualFileSystem>(goose::unique_ptr<FaultInjectionFileSystem>(raw_fs));
		...
		case ActionType::TOGGLE_SKIP_CHECKPOINTS_ON_COMMIT:
			...
			config.options.set_variables["debug_skip_checkpoint_on_commit"] = goose::Value(skip_checkpoint_on_commit);
			break;
