...
#include <goose/function/scalar/sequence_utils.h>
...
struct CurrentSequenceValueOperator {
	static int64_t Operation(GooseTransaction &, SequenceCatalogEntry &seq) {
		return seq.CurrentValue();
	}
};

struct NextSequenceValueOperator {
	static int64_t Operation(GooseTransaction &transaction, SequenceCatalogEntry &seq) {
...
struct NextValLocalState : public FunctionLocalState {
	explicit NextValLocalState(GooseTransaction &transaction, SequenceCatalogEntry &sequence)
...
unique_ptr<FunctionLocalState> NextValLocalFunction(ExpressionState &state, const BoundFunctionExpression &expr,
	...
	auto &transaction = GooseTransaction::Get(context, sequence.catalog);
