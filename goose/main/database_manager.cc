...
shared_ptr<AttachedDatabase> DatabaseManager::AttachDatabase(ClientContext &context, AttachInfo &info,
	...
	if (options.db_type.empty() || StringUtil::CIEquals(options.db_type, "goose")) {
	...
	if (!options.db_type.empty()) {
		// we only need to prevent duplicate opening of Goose files
		// if this is not a Goose file but e.g. a CSV or Parquet file, we don't need to do this duplicate protection
...
optional_ptr<AttachedDatabase> DatabaseManager::FinalizeAttach(ClientContext &context, AttachInfo &info,
	...
	auto &transaction = GooseTransaction::Get(context, *system);
	auto &transaction_manager = GooseTransactionManager::Get(*system);
...
void DatabaseManager::GetDatabaseType(ClientContext &context, AttachInfo &info, const DBConfig &config,
                                      AttachOptions &options) {
	// Test if the database is a Goose database file.
	if (StringUtil::CIEquals(options.db_type, "goose")) {
		options.db_type = "";
		return;
	}
