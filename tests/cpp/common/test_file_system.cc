...
TEST_CASE("Test file operations", "[file_system]") {
	goose::unique_ptr<FileSystem> fs = FileSystem::CreateLocal();
	goose::unique_ptr<FileHandle> handle, handle2;
...
TEST_CASE("extract subsystem", "[file_system]") {
	...
	const ::goose::string target_fs = extracted_filesystem->GetName();
	const ::goose::vector<string> disabled_subfilesystems {target_fs};
