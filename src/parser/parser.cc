...
#include <cantor/postgres_parser.h>
...
// This function strips unicode space characters from the query and replaces
// them with regular spaces It returns true if any unicode space characters were
// found and stripped See here for a list of unicode space characters -
  ...
  for (; pos + 2 < qsize; pos++) {
    ...
    if (query[pos] == 0xE2) {
      ...
      goto in_quotes;
               ...
               (query[pos + 1] == '$' ||
                IsValidDollarQuotedStringTagFirstChar(query[pos + 1]))) {
      ...
      for (; pos + 2 < qsize; pos++) {
        if (query[pos] == '$') {
          // end of tag
          dollar_quote_tag =
              string_t(const_char_ptr_cast(start),
                       NumericCast<uint32_t, int64_t>(&query[pos] - start));
...
in_dollar_quotes:
  for (; pos + 2 < qsize; pos++) {
    if (query[pos] == '$' &&
        qsize - (pos + 1) >=
            dollar_quote_tag.GetSize() + 1 && // found '$' and enough space left
        query[pos + dollar_quote_tag.GetSize() + 1] ==
            '$' && // ending '$' at the right spot
        memcmp(&query[pos + 1], dollar_quote_tag.GetData(),
               dollar_quote_tag.GetSize()) == 0) { // tags match
...
void Parser::ThrowParserOverrideError(ParserOverrideResult &result) {
  if (result.type == ParserExtensionResultType::DISPLAY_ORIGINAL_ERROR) {
    throw ParserException(
        ...
        "restarting the "
        "database and "
        "using the setting \"set allow_parser_override_extension=fallback\" to "
        "fallback to the "
        "default parser.",
  ...
  if (result.type == ParserExtensionResultType::DISPLAY_EXTENSION_ERROR) {
    if (result.error.Type() == ExceptionType::NOT_IMPLEMENTED) {
      throw NotImplementedException(
          "Parser override has not yet implemented this "
          "transformer rule.\nOriginal error: %s",
          result.error.RawMessage());
    }
    if (result.error.Type() == ExceptionType::PARSER) {
      throw ParserException(
          "Parser override could not parse this query.\nOriginal error: %s",
          result.error.RawMessage());
    }
    result.error.Throw();
  }
...
void Parser::ParseQuery(const string &query) {
  ...
  string parser_override_option =
      StringUtil::Lower(options.parser_override_setting);
  {
  ...
  }
  {
    if (options.extensions) {
      for (auto &ext : *options.extensions) {
        ...
        if (StringUtil::CIEquals(parser_override_option,
    ...
    cantor::PostgresParser::SetPreserveIdentifierCase(
        options.preserve_identifier_case);
    bool parsing_succeed = false;
    // Creating a new scope to prevent multiple PostgresParser destructors being
    // called which led to some memory issues
    {
      cantor::PostgresParser parser;
      parser.Parse(query);
      if (parser.success) {
    ...
    // If Goose fails to parse the entire sql string, break the string down
    // into individual statements using ';' as the delimiter so that parser
    // extensions can parse the statement
    if (parsing_succeed) {
      // no-op
      // return here would require refactoring into another function. o.w. will
      // just no-op in order to run wrap up code at the end of this function
      ...
      for (auto const &query_statement : queries) {
        ...
        // Creating a new scope to allow extensions to use PostgresParser, which
        // is not reentrant
        {
          cantor::PostgresParser another_parser;
          another_parser.Parse(query_statement);
          // LCOV_EXCL_START
          // first see if Goose can parse this individual query statement
          if (another_parser.success) {
            ...
            transformer.TransformParseTree(another_parser.parse_tree,
                                           statements);
            // important to set in the case of a mixture of DDB and parser ext
            // statements
        ...
        // LCOV_EXCL_START
        // let extensions parse the statement which Goose failed to parse
        ...
        for (auto &ext : *options.extensions) {
          ...
          if (result.type == ParserExtensionResultType::PARSE_SUCCESSFUL) {
            auto statement = make_uniq<ExtensionStatement>(
                ext, std::move(result.parse_data));
            ...
            throw ParserException::SyntaxError(query, result.error,
  ...
  if (!statements.empty()) {
    ...
    for (auto &statement : statements) {
      statement->query =
          query.substr(statement->stmt_location, statement->stmt_length);
...
vector<SimplifiedToken> Parser::Tokenize(const string &query) {
  auto pg_tokens = cantor::PostgresParser::Tokenize(query);
  ...
  for (auto &pg_token : pg_tokens) {
    ...
    case cantor::PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_IDENTIFIER:
      token.type = SimplifiedTokenType::SIMPLIFIED_TOKEN_IDENTIFIER;
      break;
    case cantor::PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_NUMERIC_CONSTANT:
      token.type = SimplifiedTokenType::SIMPLIFIED_TOKEN_NUMERIC_CONSTANT;
      break;
    case cantor::PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_STRING_CONSTANT:
      token.type = SimplifiedTokenType::SIMPLIFIED_TOKEN_STRING_CONSTANT;
      break;
    case cantor::PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_OPERATOR:
      token.type = SimplifiedTokenType::SIMPLIFIED_TOKEN_OPERATOR;
      break;
    case cantor::PGSimplifiedTokenType::PG_SIMPLIFIED_TOKEN_KEYWORD:
      token.type = SimplifiedTokenType::SIMPLIFIED_TOKEN_KEYWORD;
      break;
    // comments are not supported by our tokenizer right now
    case cantor::PGSimplifiedTokenType::
        PG_SIMPLIFIED_TOKEN_COMMENT: // LCOV_EXCL_START
...
vector<SimplifiedToken> Parser::TokenizeError(const string &error_msg) {
  ...
  if (line_pos.IsValid()) {
    ...
    idx_t query_start;
    for (query_start = line_pos.GetIndex() + 6; query_start < error_msg.size();
         query_start++) {
      if (error_msg[query_start] != ':' &&
          !StringUtil::CharacterIsDigit(error_msg[query_start])) {
        break;
      }
    }
      ...
      if (caret_start < error_msg.size()) {
        for (idx_t i = caret_start; i < error_msg.size(); i++) {
          if (error_msg[i] == '^') {
            ...
            caret_position =
                i - caret_start - ((query_start - line_pos.GetIndex()) - 1);
            place_caret = true;
            break;
          }
        }
      ...
      for (auto &query_token : query_tokens) {
        if (place_caret) {
          ...
          if (query_token.start >= caret_position) {
            ...
            query_token.type =
                SimplifiedTokenType::SIMPLIFIED_TOKEN_ERROR_EMPHASIS;
...
KeywordCategory ToKeywordCategory(cantor::PGKeywordCategory type) {
  switch (type) {
  case cantor::PGKeywordCategory::PG_KEYWORD_RESERVED:
    return KeywordCategory::KEYWORD_RESERVED;
  case cantor::PGKeywordCategory::PG_KEYWORD_UNRESERVED:
    return KeywordCategory::KEYWORD_UNRESERVED;
  case cantor::PGKeywordCategory::PG_KEYWORD_TYPE_FUNC:
    return KeywordCategory::KEYWORD_TYPE_FUNC;
  case cantor::PGKeywordCategory::PG_KEYWORD_COL_NAME:
    return KeywordCategory::KEYWORD_COL_NAME;
  case cantor::PGKeywordCategory::PG_KEYWORD_NONE:
...
KeywordCategory Parser::IsKeyword(const string &text) {
  return ToKeywordCategory(cantor::PostgresParser::IsKeyword(text));
}

vector<ParserKeyword> Parser::KeywordList() {
  auto keywords = cantor::PostgresParser::KeywordList();
...
Parser::ParseExpressionList(const string &select_list, ParserOptions options) {
...
void Parser::ParseUpdateList(const string &update_list,
  ...
  // check the statements
  if (parser.statements.size() != 1 ||
      parser.statements[0]->type != StatementType::UPDATE_STATEMENT) {
...
vector<vector<unique_ptr<ParsedExpression>>>
Parser::ParseValuesList(const string &value_list, ParserOptions options) {
  // construct a mock query
  ...
  if (!select_node.from_table ||
      select_node.from_table->type != TableReferenceType::EXPRESSION_LIST) {
