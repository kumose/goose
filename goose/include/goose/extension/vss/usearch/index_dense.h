...
#include <goose/extension/vss/usearch/index.h>
#include <goose/extension/vss/usearch/index_plugins.h>
...
namespace unum {
        ...
        private:
            ...
            struct lookup_key_hash_t {
                ...
                std::size_t operator()(key_and_slot_t const &k) const noexcept {
                    return std::hash<vector_key_t>{}(k.key);
                }
    ...
    aggregated_distances_t distance_between(vector_key_t key, f64_t const* vector, std::size_t thread = any_thread()) const { return distance_between_(key, vector, thread, casts_.to_f64); }
            ...
            std::size_t serialized_length(serialization_config_t config = {}) const noexcept {
                ...
                if (!config.exclude_vectors) {
                    dimensions_length = config.use_64_bit_dimensions
                                            ? sizeof(std::uint64_t) * 2
                                            : sizeof(std::uint32_t) * 2;
                    matrix_length = typed_->size() * metric_.bytes_per_vector();
                }
                return dimensions_length + matrix_length + sizeof(index_dense_head_buffer_t) + typed_->
                       serialized_length();
            }
            ...
            copy_result_t copy(index_dense_copy_config_t config = {}) const {
                ...
                if (!config.force_vector_copy && copy.config_.exclude_vectors)
                    ...
                    for (std::size_t slot = 0; slot != vectors_lookup_.size(); ++slot)
                        copy.vectors_lookup_[slot] = copy.vectors_tape_allocator_.allocate(
                            copy.metric_.bytes_per_vector());
                ...
                if (merge_cycles) {
                    ...
                    executor.dynamic(queries_count, [&](std::size_t thread_idx, std::size_t query_idx) {
                        ...
                        while (true) {
                            ...
                            updated_cluster = *std::lower_bound(clusters.data(), clusters_end, updated_cluster,
                                                                centroid_id);
                ...
                for (std::size_t i = 0; i != typed_->size(); ++i) {
                    ...
                    if (member.key == free_key_)
                        ...
                        slot_lookup_.try_emplace(key_and_slot_t{
                            vector_key_t(member.key), static_cast<compressed_slot_t>(i)
                        });
                }
            }
