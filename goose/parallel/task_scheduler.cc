...
#include <goose/common/concurrentqueue/lightweightsemaphore.h>
...
#ifndef GOOSE_NO_THREADS
typedef goose_moodycamel::ConcurrentQueue<shared_ptr<Task>> concurrent_queue_t;
typedef goose_moodycamel::LightweightSemaphore lightweight_semaphore_t;
...
struct QueueProducerToken {
	...
	goose_moodycamel::ProducerToken queue_token;
};
...
#else
	throw NotImplementedException("Goose was compiled without threads! Background thread loop is not allowed.");
#endif
}
...
void TaskScheduler::SetThreads(idx_t total_threads, idx_t external_threads) {
	...
	if (total_threads != external_threads) {
		throw NotImplementedException(
		    "Goose was compiled without threads! Setting total_threads != external_threads is not allowed.");
	}
#endif
