...
struct CStringConverter {
	template <class SRC, class DST>
	static DST Convert(SRC input) {
		auto result = char_ptr_cast(goose_malloc(input.GetSize() + 1));
		assert(result);
...
struct CBlobConverter {
	...
	static DST Convert(SRC input) {
		goose_blob result;
		result.data = char_ptr_cast(goose_malloc(input.GetSize()));
...
void WriteData(goose_column *column, ColumnDataCollection &source, const vector<column_t> &column_ids) {
	idx_t row = 0;
...
goose_state deprecated_goose_translate_column(MaterializedQueryResult &result, goose_column *column, idx_t col) {
	D_ASSERT(!result.HasError());
	auto &collection = result.Collection();
	idx_t row_count = collection.Count();
	column->deprecated_nullmask = (bool *)goose_malloc(sizeof(bool) * collection.Count());
	...
	if (type_size == 0) {
		...
		// Unsupported type, e.g., a LIST. By returning GooseSuccess here,
		// we allow filling other columns, and return NULL for all unsupported types.
		return GooseSuccess;
	}

	column->deprecated_data = goose_malloc(type_size * row_count);
	...
	case LogicalTypeId::BLOB: {
		WriteData<string_t, goose_blob, CBlobConverter>(column, collection, column_ids);
		break;
	}
	...
	case LogicalTypeId::HUGEINT: {
		WriteData<hugeint_t, goose_hugeint, CHugeintConverter>(column, collection, column_ids);
		break;
	}
	case LogicalTypeId::UHUGEINT: {
		WriteData<uhugeint_t, goose_uhugeint, CUhugeintConverter>(column, collection, column_ids);
		break;
	}
	case LogicalTypeId::INTERVAL: {
		WriteData<interval_t, goose_interval, CIntervalConverter>(column, collection, column_ids);
		break;
	}
	case LogicalTypeId::DECIMAL: {
		...
		case PhysicalType::INT16: {
			WriteData<int16_t, goose_hugeint, CDecimalConverter<int16_t>>(column, collection, column_ids);
			break;
		}
		case PhysicalType::INT32: {
			WriteData<int32_t, goose_hugeint, CDecimalConverter<int32_t>>(column, collection, column_ids);
			break;
		}
		case PhysicalType::INT64: {
			WriteData<int64_t, goose_hugeint, CDecimalConverter<int64_t>>(column, collection, column_ids);
			break;
		}
...
goose_state GooseTranslateResult(unique_ptr<QueryResult> result_p, goose_result *out) {
	auto &result = *result_p;
	D_ASSERT(result_p);
	if (!out) {
		// no result to write to, only return the status
		return !result.HasError() ? GooseSuccess : GooseError;
	}

	memset(out, 0, sizeof(goose_result));

	// initialize the result_data object
	auto result_data = new GooseResultData();
...
bool DeprecatedMaterializeResult(goose_result *result) {
	...
	result->deprecated_columns = (goose_column *)goose_malloc(sizeof(goose_column) * column_count);
	...
	memset(result->deprecated_columns, 0, sizeof(goose_column) * column_count);
	...
	for (idx_t col = 0; col < column_count; col++) {
		auto state = deprecated_goose_translate_column(materialized, &result->deprecated_columns[col], col);
...
static void GooseDestroyColumn(goose_column column, idx_t count) {
	if (column.deprecated_data) {
		if (column.deprecated_type == GOOSE_TYPE_VARCHAR) {
		...
		} else if (column.deprecated_type == GOOSE_TYPE_BLOB) {
			// blob, delete individual blobs
			auto data = reinterpret_cast<goose_blob *>(column.deprecated_data);
			for (idx_t i = 0; i < count; i++) {
				if (data[i].data) {
					goose_free((void *)data[i].data);
				}
			}
		}
		goose_free(column.deprecated_data);
	}
	if (column.deprecated_nullmask) {
		goose_free(column.deprecated_nullmask);
	}
}
...
void goose_destroy_result(goose_result *result) {
	if (result->deprecated_columns) {
		for (idx_t i = 0; i < result->deprecated_column_count; i++) {
			GooseDestroyColumn(result->deprecated_columns[i], result->deprecated_row_count);
		}
		goose_free(result->deprecated_columns);
	}
	if (result->internal_data) {
		auto result_data = reinterpret_cast<goose::GooseResultData *>(result->internal_data);
		delete result_data;
	}
	memset(result, 0, sizeof(goose_result));
}

const char *goose_column_name(goose_result *result, idx_t col) {
	if (!result || col >= goose_column_count(result)) {
		return nullptr;
	}
	auto &result_data = *(reinterpret_cast<goose::GooseResultData *>(result->internal_data));
...
goose_type goose_column_type(goose_result *result, idx_t col) {
	...
	auto &result_data = *(reinterpret_cast<goose::GooseResultData *>(result->internal_data));
	return goose::LogicalTypeIdToC(result_data.result->types[col].id());
}
...
goose_logical_type goose_column_logical_type(goose_result *result, idx_t col) {
	...
	auto &result_data = *(reinterpret_cast<goose::GooseResultData *>(result->internal_data));
	return reinterpret_cast<goose_logical_type>(new goose::LogicalType(result_data.result->types[col]));
}
...
goose_arrow_options goose_result_get_arrow_options(goose_result *result) {
	...
	auto arrow_options_wrapper = new goose::CClientArrowOptionsWrapper(result_data.result->client_properties);
	return reinterpret_cast<goose_arrow_options>(arrow_options_wrapper);
}
...
idx_t goose_row_count(goose_result *result) {
	...
	auto &materialized = reinterpret_cast<goose::MaterializedQueryResult &>(*result_data.result);
...
idx_t goose_rows_changed(goose_result *result) {
	...
	auto &materialized = reinterpret_cast<goose::MaterializedQueryResult &>(*result_data.result);
	if (materialized.properties.return_type != goose::StatementReturnType::CHANGED_ROWS) {
...
goose_error_type goose_result_error_type(goose_result *result) {
	...
	return goose::ErrorTypeToC(result_data.result->GetErrorType());
}
...
idx_t goose_result_chunk_count(goose_result result) {
	...
	if (result_data.result->type != goose::QueryResultType::MATERIALIZED_RESULT) {
...
goose_data_chunk goose_result_get_chunk(goose_result result, idx_t chunk_idx) {
	if (!result.internal_data) {
		return nullptr;
	}
	auto &result_data = *(reinterpret_cast<goose::GooseResultData *>(result.internal_data));
	...
	result_data.result_set_type = goose::CAPIResultSetType::CAPI_RESULT_TYPE_MATERIALIZED;
	auto &materialized = reinterpret_cast<goose::MaterializedQueryResult &>(*result_data.result);
	auto &collection = materialized.Collection();
	if (chunk_idx >= collection.ChunkCount()) {
		return nullptr;
	}
	auto chunk = goose::make_uniq<goose::DataChunk>();
	chunk->Initialize(goose::Allocator::DefaultAllocator(), collection.Types());
...
bool goose_result_is_streaming(goose_result result) {
	...
	if (goose_result_error(&result) != nullptr) {
		return false;
	}
	auto &result_data = *(reinterpret_cast<goose::GooseResultData *>(result.internal_data));
	return result_data.result->type == goose::QueryResultType::STREAM_RESULT;
}

goose_result_type goose_result_return_type(goose_result result) {
	if (!result.internal_data || goose_result_error(&result) != nullptr) {
		return GOOSE_RESULT_TYPE_INVALID;
	}
	auto &result_data = *(reinterpret_cast<goose::GooseResultData *>(result.internal_data));
	switch (result_data.result->properties.return_type) {
	case goose::StatementReturnType::CHANGED_ROWS:
		return GOOSE_RESULT_TYPE_CHANGED_ROWS;
	case goose::StatementReturnType::NOTHING:
		return GOOSE_RESULT_TYPE_NOTHING;
	case goose::StatementReturnType::QUERY_RESULT:
		return GOOSE_RESULT_TYPE_QUERY_RESULT;
	default:
		return GOOSE_RESULT_TYPE_INVALID;
	}
}
...
goose_statement_type goose_result_statement_type(goose_result result) {
	...
	auto &pres = *(reinterpret_cast<goose::GooseResultData *>(result.internal_data));
