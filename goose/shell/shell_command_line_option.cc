...
#define SEP_Record "\x1E"
    ...
    MetadataResult SetReadOnlyMode(ShellState &state, const vector<string> &args) {
        state.config.options.access_mode = goose::AccessMode::READ_ONLY;
        return MetadataResult::SUCCESS;
    }
    ...
    MetadataResult ShowVersionAndExit(ShellState &state, const vector<string> &args) {
        printf("%s (%s) %s\n", goose::Goose::LibraryVersion(), goose::Goose::ReleaseCodename(),
               goose::Goose::SourceID());
    ...
    MetadataResult SetStorageVersion(ShellState &state, const vector<string> &args) {
        ...
        try {
            state.config.options.serialization_compatibility =
                    goose::SerializationCompatibility::FromString(storage_version);
    ...
    static const CommandLineOption command_line_options[] = {
        ...
        {"version", 0, "", nullptr, ShowVersionAndExit, "show Goose version"},
    ...
    optional_ptr<const CommandLineOption> ShellState::FindCommandLineOption(
        const string &option, string &error_msg) const {
    ...
    void ShellState::PrintUsage() {
        ...
        PrintF(" is the name of a Goose database. A new database is created\n"
        ...
        for (auto &option: print_options) {
            ...
            if (!option.arguments.empty()) {
                highlighter.PrintText(" " + option.arguments, PrintOutput::STDOUT,
                                      HighlightElementType::STRING_CONSTANT);
            }
