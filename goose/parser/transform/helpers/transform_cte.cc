...
#include <goose/parser/query_node/recursive_cte_node.h>
...
void Transformer::TransformCTE(cantor::PGWithClause &de_with_clause, CommonTableExpressionMap &cte_map) {
	...
	for (auto cte_ele = de_with_clause.ctes->head; cte_ele != nullptr; cte_ele = cte_ele->next) {
		...
		auto &cte = *PGPointerCast<cantor::PGCommonTableExpr>(cte_ele->data.ptr_value);
		if (cte.recursive_keys) {
			auto key_target = PGPointerCast<cantor::PGNode>(cte.recursive_keys->head->data.ptr_value);
			if (key_target) {
		...
		// we need a query
		if (!cte.ctequery || cte.ctequery->type != cantor::T_PGSelectStmt) {
		...
		if (cte.ctematerialized == cantor::PGCTEMaterializeDefault) {
...
#endif
		} else if (cte.ctematerialized == cantor::PGCTEMaterializeAlways) {
			info->materialized = CTEMaterialize::CTE_MATERIALIZE_ALWAYS;
		} else if (cte.ctematerialized == cantor::PGCTEMaterializeNever) {
...
unique_ptr<SelectStatement> Transformer::TransformRecursiveCTE(cantor::PGCommonTableExpr &cte,
                                                               CommonTableExpressionInfo &info) {
	auto &stmt = *PGPointerCast<cantor::PGSelectStmt>(cte.ctequery);
