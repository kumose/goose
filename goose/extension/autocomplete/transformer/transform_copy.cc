...
namespace goose {
    ...
    unique_ptr<SQLStatement> PEGTransformerFactory::TransformCopyTable(PEGTransformer &transformer,
        ...
        if (copy_options_pr.HasResult()) {
            info->options = transformer.Transform<case_insensitive_map_t<vector<Value> > >(
                copy_options_pr.optional_result);
    ...
    PEGTransformerFactory::TransformGenericCopyOptionListParens(PEGTransformer &transformer,
        ...
        auto generic_options_transformed = transformer.Transform<unordered_map<string, vector<Value> > >(
            generic_options);
