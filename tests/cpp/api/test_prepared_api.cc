...
TEST_CASE("Test destructors of prepared statements", "[api]") {
	...
	goose::unique_ptr<PreparedStatement> prepare;
	goose::unique_ptr<QueryResult> result;
...
TEST_CASE("Test BLOB with PreparedStatement", "[api]") {
	...
	goose::unique_ptr<PreparedStatement> ps = con.Prepare("INSERT INTO blobs VALUES (?::BYTEA)");
	ps->Execute(blob_val);
	REQUIRE(!ps->HasError());
	ps.reset();
...
static goose::unique_ptr<QueryResult> TestExecutePrepared(Connection &con, string query) {
	auto prepared = con.Prepare(query);
	REQUIRE(!prepared->HasError());
