...
The C API provides a stable interface based on Goose's internal C++ code.
Thus, applications built against the C API avoid breaking due to changes in Goose's fast-moving codebase.

## Headers

### Header generation

Goose's C API headers are code generated from JSON definition files.
There are multiple reasons for this:
- easy to structure the C API headers
- code generation in Goose clients based on the C API without parsing a C header
- versioned function-pointer-struct API for Goose extensions based on the C API

The C API currently consists of 2 main parts:
- the C API as defined in `goose.h` (main header)
- the function-pointer-struct in `goose_extension.h` (extension header)

### The main header `goose.h`

The main header contains macros, typedefs and all functions of the Goose C API.
Programs linking to Goose use this API, e.g., the C API-based clients.
To link against Goose via the C API, the `goose.h` file of the desired Goose version
must be included, after which the program can link to a Goose (bundled) library of that specific version.

### The extension API header `goose_extension.h`
...
Goose versions without requiring a rebuild.
Forcing the function-pointer-struct to never change between Goose versions achieves this stability.
...
The only way to add functions to the C api is to add them to the `JSON` definition files in `src/include/goose/main/capi/header_generation/functions`.
Please note that the C API is intended as a minimal wrapper around Goose's internal C++ functions.
...
1. Create an entry in one of the `src/include/goose/main/capi/header_generation/functions/*.json` files.
2. Add the function to the exclusion list in `src/include/goose/main/capi/header_generation/apis/v0/exclusion_list.json` to mark it as excluded. 
...
1. Create an entry in one of the `src/include/goose/main/capi/header_generation/functions/*.json` files.
2. Add the function to `src/include/goose/main/capi/header_generation/apis/v1/*/*.json`. 
3. Run the generation script `scripts/generate_c_api.py` or the Makefile target `make generate-files`.
...
adding a new `JSON` file to the `src/include/goose/main/capi/header_generation/apis/v0` directory.
...
As mentioned before, Goose's C API headers are code generated from JSON definition files.
