...
namespace goose {
    namespace {
        ...
        struct MedianAbsoluteDeviationOperation : QuantileOperation {
            ...
            static void Finalize(STATE &state, T &target, AggregateFinalizeData &finalize_data) {
                ...
                const auto med = interp.template Operation<INPUT_TYPE, MEDIAN_TYPE>(
                    state.v.data(), finalize_data.result);
            ...
            static void Window(AggregateInputData &aggr_input_data, const WindowPartitionInput &partition,
                ...
                if (gstate && gstate->HasTree()) {
                    med = gstate->GetWindowState().template WindowScalar<MEDIAN_TYPE, false>(
                ...
                case LogicalTypeId::DECIMAL:
                    switch (type.InternalType()) {
                        case PhysicalType::INT16:
                            return GetTypedMedianAbsoluteDeviationAggregateFunction<int16_t, int16_t, int16_t>(
                                type, type);
                        case PhysicalType::INT32:
                            return GetTypedMedianAbsoluteDeviationAggregateFunction<int32_t, int32_t, int32_t>(
                                type, type);
                        case PhysicalType::INT64:
                            return GetTypedMedianAbsoluteDeviationAggregateFunction<int64_t, int64_t, int64_t>(
                                type, type);
                        case PhysicalType::INT128:
                            return GetTypedMedianAbsoluteDeviationAggregateFunction<hugeint_t, hugeint_t, hugeint_t>(
                                type, type);
                        default:
    ...
    AggregateFunctionSet MadFun::GetFunctions() {
        ...
        const vector<LogicalType> MAD_TYPES = {
            LogicalType::FLOAT, LogicalType::DOUBLE, LogicalType::DATE,
