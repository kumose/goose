...
public:
	...
	GOOSE_API PreparedStatement(shared_ptr<ClientContext> context, shared_ptr<PreparedStatementData> data,
	                             string query, case_insensitive_map_t<idx_t> named_param_map);
	//! Create a prepared statement that was not successfully prepared
	GOOSE_API explicit PreparedStatement(ErrorData error);

	GOOSE_API ~PreparedStatement();
...
public:
	...
	GOOSE_API const string &GetError();
	...
	GOOSE_API StatementType GetStatementType();
	//! Returns the underlying statement properties
	GOOSE_API StatementProperties GetStatementProperties();
	//! Returns the result SQL types of the prepared statement
	GOOSE_API const vector<LogicalType> &GetTypes();
	//! Returns the result names of the prepared statement
	GOOSE_API const vector<string> &GetNames();
	//! Returns the map of parameter index to the expected type of parameter
	GOOSE_API case_insensitive_map_t<LogicalType> GetExpectedParameterTypes() const;
	...
	//! Create a pending query result of the prepared statement with the given set of arguments
	GOOSE_API unique_ptr<PendingQueryResult> PendingQuery(vector<Value> &values, bool allow_stream_result = true);

	//! Create a pending query result of the prepared statement with the given set named arguments
	GOOSE_API unique_ptr<PendingQueryResult> PendingQuery(case_insensitive_map_t<BoundParameterData> &named_values,
	                                                       bool allow_stream_result = true);

	//! Execute the prepared statement with the given set of values
	GOOSE_API unique_ptr<QueryResult> Execute(vector<Value> &values, bool allow_stream_result = true);

	//! Execute the prepared statement with the given set of named+unnamed values
	GOOSE_API unique_ptr<QueryResult> Execute(case_insensitive_map_t<BoundParameterData> &named_values,
	                                           bool allow_stream_result = true);
