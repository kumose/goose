...
#include <goose/common/types/value_map.h>
...
namespace goose {
    ...
    public:
        TableScanGlobalState(ClientContext &context, const FunctionData *bind_data_p) {
            ...
            auto &gs_table = bind_data.table.Cast<GooseTableEntry>();
            max_threads = gs_table.GetStorage().MaxThreads(context);
        }
    ...
    class GooseIndexScanState : public TableScanGlobalState {
    public:
        GooseIndexScanState(ClientContext &context, const FunctionData *bind_data_p)
    ...
    public:
        unique_ptr<LocalTableFunctionState> InitLocalState(ExecutionContext &context,
            ...
            auto &gs_table = bind_data.table.Cast<GooseTableEntry>();
            auto &storage = gs_table.GetStorage();
            auto &local_storage = LocalStorage::Get(context.client, gs_table.catalog);
        ...
        void TableScanFunc(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) override {
            ...
            auto &gs_table = bind_data.table.Cast<GooseTableEntry>();
            auto &tx = GooseTransaction::Get(context, gs_table.catalog);
            auto &storage = gs_table.GetStorage();
                ...
                switch (phase_to_be_performed) {
                    ...
                    case ExecutionPhase::STORAGE: {
                        ...
                        if (CanRemoveFilterColumns()) {
                            ...
                            storage.Fetch(tx, l_state.all_columns, column_ids, local_vector, scan_count,
                                          l_state.fetch_state);
    ...
    class GooseTableScanState : public TableScanGlobalState {
    public:
        GooseTableScanState(ClientContext &context, const FunctionData *bind_data_p)
            : TableScanGlobalState(context, bind_data_p), bind_data(bind_data_p->Cast<TableScanBindData>()),
              gs_table(bind_data.table.Cast<GooseTableEntry>()),
              tx(GooseTransaction::Get(context, gs_table.catalog)),
              storage(gs_table.GetStorage()), total_rows(storage.GetTotalRows()) {
        }
    ...
    static unique_ptr<LocalTableFunctionState> TableScanInitLocal(ExecutionContext &context,
    ...
    unique_ptr<GlobalTableFunctionState> GooseTableScanInitGlobal(ClientContext &context, TableFunctionInitInput &input,
                                                                  DataTable &storage,
                                                                  const TableScanBindData &bind_data) {
        auto g_state = make_uniq<GooseTableScanState>(context, input.bind_data.get());
        if (bind_data.order_options) {
        ...
        auto &gs_table = bind_data.table.Cast<GooseTableEntry>();
        const auto &columns = gs_table.GetColumns();
        for (const auto &col_idx: input.column_indexes) {
    ...
    unique_ptr<GlobalTableFunctionState> GooseIndexScanInitGlobal(ClientContext &context, TableFunctionInitInput &input,
                                                                 const TableScanBindData &bind_data,
                                                                 set<row_t> &row_ids) {
        auto g_state = make_uniq<GooseIndexScanState>(context, input.bind_data.get());
    ...
    value_set_t GetUniqueValues(vector<reference<ConstantFilter> > &comparisons,
                                vector<reference<InFilter> > &in_filters) {
    ...
    vector<unique_ptr<Expression> > ExtractFilterExpressions(const ColumnDefinition &col,
    ...
    unique_ptr<GlobalTableFunctionState> TableScanInitGlobal(ClientContext &context, TableFunctionInitInput &input) {
        ...
        if (!input.filters) {
            return GooseTableScanInitGlobal(context, input, storage, bind_data);
        }
        ...
        auto &column_list = gs_table.GetColumns();
        ...
        return GooseIndexScanInitGlobal(context, input, bind_data, row_ids);
    }

    static unique_ptr<BaseStatistics> TableScanStatistics(ClientContext &context,
                                                          TableFunctionGetStatisticsInput &input) {
        auto &column_id = input.column_index;
        auto &bind_data = input.bind_data->Cast<TableScanBindData>();
        auto &gs_table = bind_data.table.Cast<GooseTableEntry>();
        auto &local_storage = LocalStorage::Get(context, gs_table.catalog);

        // Don't emit statistics for tables with outstanding transaction-local data.
        if (local_storage.Find(gs_table.GetStorage())) {
            return nullptr;
        }
        ...
        auto &column = gs_table.GetColumn(LogicalIndex(column_id.GetPrimaryIndex()));
        if (column.Generated()) {
            return nullptr;
        }

        auto storage_index = gs_table.GetStorageIndex(column_id);
        return gs_table.GetStatistics(context, storage_index);
    }
    ...
    unique_ptr<NodeStatistics> TableScanCardinality(ClientContext &context, const FunctionData *bind_data_p) {
        ...
        idx_t estimated_cardinality = table_rows + local_storage.AddedRows(gs_table.GetStorage());
    ...
    static unique_ptr<FunctionData> TableScanDeserialize(Deserializer &deserializer, TableFunction &function) {
        ...
        auto result = make_uniq<TableScanBindData>(catalog_entry.Cast<GooseTableEntry>());
