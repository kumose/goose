...
struct GooseEngineError : ffi::EngineError {
	// Allocate a GooseEngineError, function ptr passed to kernel for error allocation
...
struct KernelUtils {
	...
	static T UnpackResult(ffi::ExternResult<T> result, const string &from_where) {
		if (result.tag == ffi::ExternResult<T>::Tag::Err) {
			if (result.err._0) {
				auto error_cast = static_cast<GooseEngineError *>(result.err._0);
