...
#include <goose/benchmark/interpreted_benchmark.h>
...
namespace goose {
    ...
    struct InterpretedBenchmarkState : public BenchmarkState {
        goose::unique_ptr<DBConfig> benchmark_config;
        Goose db;
        Connection con;
        ...
        goose::unique_ptr<DBConfig> GetBenchmarkConfig(const string &version = "") {
    ...
    InterpretedBenchmark::InterpretedBenchmark(string full_path)
        ...
        replacement_mapping["BENCHMARK_DIR"] = BenchmarkRunner::GOOSE_BENCHMARK_DIRECTORY;
    }
    ...
    unique_ptr<QueryResult> InterpretedBenchmark::RunLoadQuery(InterpretedBenchmarkState &state,
                                                               const string &load_query) {
    ...
    unique_ptr<BenchmarkState> InterpretedBenchmark::Initialize(BenchmarkConfiguration &config) {
        ...
        LoadBenchmark();
        goose::unique_ptr<InterpretedBenchmarkState> state;
        ...
        if (!cache_file.empty()) {
            ...
            if (!fs->FileExists(fs->JoinPath(BenchmarkRunner::GOOSE_BENCHMARK_DIRECTORY, cache_file))) {
    ...
    void InterpretedBenchmark::Run(BenchmarkState *state_p) {
        ...
        if (temp_result->type == QueryResultType::STREAM_RESULT) {
            ...
            state.result = unique_ptr_cast<goose::QueryResult, goose::MaterializedQueryResult>(std::move(temp_result));
        }
    }
    ...
    string InterpretedBenchmark::GetDatabasePath() {
        auto fs = FileSystem::CreateLocal();
        if (!cache_db.empty()) {
            return fs->JoinPath(BenchmarkRunner::GOOSE_BENCHMARK_DIRECTORY, cache_db);
        }
        if (in_memory) {
            return "";
        }
        auto db_path = fs->JoinPath(BenchmarkRunner::GOOSE_BENCHMARK_DIRECTORY, DEFAULT_DB_PATH);
