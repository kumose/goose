...
#include <goose/utility/hyperloglog.h>
...
public:
	HLLV1() {
		hll = goose_hll::hll_create();
		goose_hll::hllSparseToDense(hll);
	}

	~HLLV1() {
		goose_hll::hll_destroy(hll);
	}

public:
	static idx_t GetSize() {
		return goose_hll::get_size();
	}
	...
	void ToNew(HyperLogLog &new_hll) const {
		const idx_t mult = goose_hll::num_registers() / HyperLogLog::M;
		...
		for (idx_t i = 0; i < HyperLogLog::M; i++) {
			...
			for (idx_t j = 0; j < mult; j++) {
				D_ASSERT(i * mult + j < goose_hll::num_registers());
				max_old = MaxValue<uint8_t>(max_old, goose_hll::get_register(hll, i * mult + j));
			}
	...
	void FromNew(const HyperLogLog &new_hll) {
		...
		double avg = 0;
		for (idx_t i = 0; i < HyperLogLog::M; i++) {
			const auto max_new = MinValue(new_hll.GetRegister(i), goose_hll::maximum_zeros());
			goose_hll::set_register(hll, i * mult, max_new);
		...
		for (idx_t opt_idx = 0; opt_idx < 5; opt_idx++) {
			...
			for (idx_t i = 0; i < HyperLogLog::M; i++) {
				...
				for (idx_t j = 1; j < mult; j++) {
					D_ASSERT(i * mult + j < goose_hll::num_registers());
					goose_hll::set_register(hll, i * mult + j, register_value);
				}
			}
		}
	}
...
private:
	idx_t Count() const {
		size_t result;
		if (goose_hll::hll_count(hll, &result) != HLL_C_OK) {
