#include <goose/shell/linenoise/linenoise-goose.h>
#include <goose/shell/linenoise/highlighting.h>
#include <goose/shell/linenoise/history.h>
...
namespace goose {
    ...
    static void renderText(size_t &render_pos, char *&buf, size_t &len, size_t pos, size_t cols, size_t plen,
        ...
        if (goose::Utf8Proc::IsValid(buf, len)) {
            ...
            while (cpos < len) {
                size_t char_render_width = goose::Utf8Proc::RenderWidth(buf, len, cpos);
                prev_pos = cpos;
                cpos = goose::Utf8Proc::NextGraphemeCluster(buf, len, cpos);
                ...
                if (total_render_width >= remaining_render_width) {
                    ...
                    if (prev_pos >= pos) {
                        ...
                        while (total_render_width >= remaining_render_width) {
                            size_t start_char_width = goose::Utf8Proc::RenderWidth(buf, len, start_pos);
                            size_t new_start = goose::Utf8Proc::NextGraphemeCluster(buf, len, start_pos);
    ...
    void Linenoise::RefreshSingleLine() const {
        ...
        renderText(render_pos, render_buf, render_len, pos, ws.ws_col, plen, highlight_buffer,
    ...
    void Linenoise::RefreshSearch() {
        ...
        auto old_highlighting = goose_shell::ShellHighlight::IsEnabled();
        ...
        if (no_matches || search_buf.empty()) {
            ...
            goose_shell::ShellHighlight::SetHighlighting(false);
        } else {
        ...
        if (old_highlighting) {
            goose_shell::ShellHighlight::SetHighlighting(true);
        }
        buf = clone.buf;
        len = clone.len;
        pos = clone.pos;
    ...
    string Linenoise::AddContinuationMarkers(const char *buf, size_t len, int plen, int cursor_row,
        ...
        bool truncate_top = truncation == RenderTruncation::TRUNCATE_TOP || truncation ==
                            RenderTruncation::TRUNCATE_BOTH;
    ...
    void Linenoise::RefreshMultiLine() {
        ...
        if (rows > 1) {
            ...
            highlight_buffer =
                    AddContinuationMarkers(render_buf, render_len, plen,
                                           y_scroll > 0 ? new_cursor_row + 1 : new_cursor_row,
                                           y_scroll > 0 ? rows : rows - 1, tokens, truncation);
            render_buf = (char *) highlight_buffer.c_str();
            render_len = highlight_buffer.size();
        }
        if (goose::Utf8Proc::IsValid(render_buf, render_len)) {
            if (Highlighting::IsEnabled()) {
        ...
        if (render_completion_suggestion && !completion_list.completions.empty()) {
            ...
            for (idx_t i = 0; i < completion_list.completions.size(); i++) {
                ...
                auto element_type = goose_shell::HighlightElementType::NONE;
                switch (completion.completion_type) {
                    case CompletionType::KEYWORD:
                    case CompletionType::TYPE_NAME:
                        element_type = goose_shell::HighlightElementType::KEYWORD;
                        break;
                    case CompletionType::CATALOG_NAME:
                        element_type = goose_shell::HighlightElementType::SUGGESTION_CATALOG_NAME;
                        break;
                    case CompletionType::SCHEMA_NAME:
                        element_type = goose_shell::HighlightElementType::SUGGESTION_SCHEMA_NAME;
                        break;
                    case CompletionType::TABLE_NAME:
                        element_type = goose_shell::HighlightElementType::SUGGESTION_TABLE_NAME;
                        break;
                    case CompletionType::COLUMN_NAME:
                        element_type = goose_shell::HighlightElementType::SUGGESTION_COLUMN_NAME;
                        break;
                    case CompletionType::FILE_NAME:
                        element_type = goose_shell::HighlightElementType::SUGGESTION_FILE_NAME;
                        break;
                    case CompletionType::DIRECTORY_NAME:
                        element_type = goose_shell::HighlightElementType::SUGGESTION_DIRECTORY_NAME;
                        break;
                    case CompletionType::SCALAR_FUNCTION:
                    case CompletionType::TABLE_FUNCTION:
                    case CompletionType::PRAGMA_FUNCTION:
                        element_type = goose_shell::HighlightElementType::SUGGESTION_FUNCTION_NAME;
                        break;
                    case CompletionType::SETTING_NAME:
                        element_type = goose_shell::HighlightElementType::SUGGESTION_SETTING_NAME;
                        break;
                    default:
                        break;
                }
                auto &element = goose_shell::ShellHighlight::GetHighlightElement(element_type);
                ...
                if (completion_idx.IsValid() && i == completion_idx.GetIndex()) {
                    ...
                    if (intensity == goose_shell::PrintIntensity::BOLD) {
                        intensity = goose_shell::PrintIntensity::BOLD_UNDERLINE;
                    } else {
                        intensity = goose_shell::PrintIntensity::UNDERLINE;
                    }
                }
                ...
                auto terminal_text = goose_shell::ShellHighlight::TerminalCode(color, intensity);
                completion_text += terminal_text;
                completion_text += rendered_text;
                if (!terminal_text.empty()) {
                    completion_text += goose_shell::ShellHighlight::ResetTerminalCode();
                }
