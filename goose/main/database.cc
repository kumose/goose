...
#include <goose/main/database_file_opener.h>
...
#include <goose/planner/collation_binding.h>
#include <goose/planner/extension_callback.h>
#include <goose/storage/object_cache.h>
#include <goose/storage/standard_buffer_manager.h>
#include <goose/storage/storage_extension.h>
#include <goose/storage/block_allocator.h>
...
#include <goose/main/database_file_path_manager.h>
...
namespace goose {
    ...
    DBConfig::DBConfig(const case_insensitive_map_t<Value> &config_dict,
                       bool read_only) : DBConfig::DBConfig(read_only) {
    ...
    shared_ptr<AttachedDatabase> DatabaseInstance::CreateAttachedDatabase(ClientContext &context, AttachInfo &info,
        ...
        if (!options.db_type.empty()) {
            ...
            if (entry->second->attach != nullptr && entry->second->create_transaction_manager != nullptr) {
                ...
                attached_database =
                        make_shared_ptr<AttachedDatabase>(*this, catalog, *entry->second, context, info.name, info,
                                                          options);
        ...
        // An empty db_type defaults to a goose database file.
        attached_database = make_shared_ptr<AttachedDatabase>(*this, catalog, info.name, info.path, options);
        return attached_database;
    }
        ...
        if (config.options.autoload_known_extensions) {
            ...
            for (auto &option: unrecognized_options_copy) {
                ...
                if (it == config.extension_parameters.end()) {
                    throw InternalException("Extension %s did not provide the '%s' config setting", extension_name,
                                            name);
                }
    ...
    static goose_ext_api_v1 CreateAPIv1Wrapper() {
        return CreateAPIv1();
    }
    ...
    void DatabaseInstance::Initialize(const char *database_path, DBConfig *user_config) {
        ...
        if (!config.options.database_type.empty() && !StringUtil::CIEquals(config.options.database_type, "goose")) {
    ...
    Goose::Goose(const char *path, DBConfig *new_config) : instance(make_shared_ptr<DatabaseInstance>()) {
    ...
    Goose::Goose(const string &path, DBConfig *config) : Goose(path.c_str(), config) {
    }

    Goose::Goose(DatabaseInstance &instance_p) : instance(instance_p.shared_from_this()) {
    }
    ...
    FileSystem &Goose::GetFileSystem() {
    ...
    void DatabaseInstance::Configure(DBConfig &new_config, const char *database_path) {
        ...
        if (config.options.goose_api.empty()) {
            config.SetOptionByName("goose_api", "cpp");
        }
        ...
        config.block_allocator = make_uniq<BlockAllocator>(*config.allocator, config.options.default_block_alloc_size,
                                                           DBConfig::GetSystemAvailableMemory(*config.file_system) * 8 /
                                                           10,
    ...
    bool Goose::ExtensionIsLoaded(const std::string &name) {
    ...
    shared_ptr<EncryptionUtil> DatabaseInstance::GetEncryptionUtil() {
        ...
        auto result = make_shared_ptr<goose_mbedtls::MbedTlsWrapper::AESStateMBEDTLSFactory>();
    ...
    const goose_ext_api_v1 DatabaseInstance::GetExtensionAPIV1() {
