...
#include <goose/storage/checkpoint/write_overflow_strings_to_disk.h>
...
namespace goose {
    ...
    struct FSSTStorage {
        ...
        static bool ParseFSSTSegmentHeader(data_ptr_t base_ptr, xfsst_decoder_t *decoder_out,
    ...
    struct FSSTAnalyzeState : public AnalyzeState {
        ...
        ~FSSTAnalyzeState() override {
            if (fsst_encoder) {
                xfsst_destroy(fsst_encoder);
            }
        }

        xfsst_encoder_t *fsst_encoder = nullptr;
        idx_t count;
    ...
    idx_t FSSTStorage::StringFinalAnalyze(AnalyzeState &state_p) {
        ...
        state.fsst_encoder = xfsst_create(string_count, &fsst_string_sizes[0], &fsst_string_ptrs[0], 0);
        ...
        auto res =
                xfsst_compress(state.fsst_encoder, string_count, &fsst_string_sizes[0], &fsst_string_ptrs[0],
        ...
        auto num_blocks = estimated_base_size / double(state.info.GetBlockSize() - sizeof(xfsst_decoder_t));
        auto symtable_size = num_blocks * sizeof(xfsst_decoder_t);
    ...
    public:
        ...
        void CreateEmptySegment() {
            ...
            auto compressed_segment =
                    ColumnSegment::CreateTransientSegment(db, function, type, info.GetBlockSize(),
        ...
        idx_t Finalize() {
            ...
            if (current_dictionary.end != info.GetBlockSize()) {
                throw InternalException(
                    "dictionary end does not match the block size in FSSTCompressionState::Finalize");
            }
            ...
            if (fsst_encoder != nullptr) {
                memcpy(base_ptr + symbol_table_offset, &fsst_serialized_symbol_table[0],
                       fsst_serialized_symbol_table_size);
            } else {
        ...
        xfsst_encoder_t *fsst_encoder = nullptr;
        unsigned char fsst_serialized_symbol_table[sizeof(xfsst_decoder_t)];
        size_t fsst_serialized_symbol_table_size = sizeof(xfsst_decoder_t);
    };
    ...
    void FSSTStorage::Compress(CompressionState &state_p, Vector &scan_vector, idx_t count) {
        ...
        auto res = xfsst_compress(
            state.fsst_encoder, /* IN: encoder obtained from xfsst_create(). */
    ...
    struct FSSTScanState : public StringScanState {
        ...
        buffer_ptr<void> goose_fsst_decoder;
        void *goose_fsst_decoder_ptr = nullptr;
        ...
        inline string_t DecompressString(StringDictionaryContainer dict, data_ptr_t baseptr,
            ...
            if (all_values_inlined) {
                return FSSTPrimitives::DecompressInlinedValue(goose_fsst_decoder_ptr, str_ptr, str_len);
            } else {
                return FSSTPrimitives::DecompressValue(goose_fsst_decoder_ptr, str_buffer, str_ptr, str_len);
            }
        }
    };
    ...
    unique_ptr<SegmentScanState> FSSTStorage::StringInitScan(const QueryContext &context, ColumnSegment &segment) {
        ...
        state->goose_fsst_decoder = make_buffer<xfsst_decoder_t>();
        auto decoder = reinterpret_cast<xfsst_decoder_t *>(state->goose_fsst_decoder.get());
        auto retval = ParseFSSTSegmentHeader(base_ptr, decoder, &state->current_width, block_size);
        if (!retval) {
            state->goose_fsst_decoder = nullptr;
        }
        state->goose_fsst_decoder_ptr = state->goose_fsst_decoder.get();
    ...
    void FSSTStorage::StringScanPartial(ColumnSegment &segment, ColumnScanState &state, idx_t scan_count,
                                        Vector &result,
                                        idx_t result_offset) {
        ...
        if (enable_fsst_vectors) {
            ...
            if (scan_state.goose_fsst_decoder) {
                D_ASSERT(result_offset == 0 || result.GetVectorType() == VectorType::FSST_VECTOR);
                result.SetVectorType(VectorType::FSST_VECTOR);
                auto string_block_limit = StringUncompressed::GetStringBlockLimit(
                    segment.GetBlockManager().GetBlockSize());
                FSSTVector::RegisterDecoder(result, scan_state.goose_fsst_decoder, string_block_limit);
    ...
    void FSSTStorage::StringFetchRow(ColumnSegment &segment, ColumnFetchState &state, row_t row_id, Vector &result,
        ...
        auto &str_buffer = StringVector::GetStringBuffer(result);
        result_data[result_idx] = FSSTPrimitives::DecompressValue((void *) &decoder, str_buffer,
    ...
    bool FSSTStorage::ParseFSSTSegmentHeader(data_ptr_t base_ptr, xfsst_decoder_t *decoder_out,
                                             bitpacking_width_t *width_out, const idx_t block_size) {
        auto header_ptr = reinterpret_cast<fsst_compression_header_t *>(base_ptr);
        ...
        *width_out = (bitpacking_width_t) (Load<uint32_t>(data_ptr_cast(&header_ptr->bitpacking_width)));
        return xfsst_import(decoder_out, base_ptr + fsst_symbol_table_offset);
    }
