...
namespace goose {
    static goose::unique_ptr<FunctionData> ICUBindIntervalMonths(ClientContext &context, ScalarFunction &bound_function,
    ...
    timestamp_t ICUCalendarAdd::Operation(timestamp_t timestamp, interval_t interval, TZCalendar &calendar_p) {
        ...
        date_t d;
        dtime_t t;
        auto us = MultiplyOperatorOverflowCheck::Operation<int64_t, int64_t,
            int64_t>(millis, Interval::MICROS_PER_MSEC);
    ...
    interval_t ICUCalendarSub::Operation(timestamp_t end_date, timestamp_t start_date, TZCalendar &calendar_p) {
        ...
        auto micros_diff = UnsafeNumericCast<
            int32_t>(ms_diff * Interval::MICROS_PER_MSEC + (end_micros - start_micros));
    ...
    struct ICUDateAdd : public ICUDateFunc {
        ...
        static void ExecuteBinary(DataChunk &args, ExpressionState &state, Vector &result) {
            ...
            BinaryExecutor::Execute<TA, TB, TR>(args.data[0], args.data[1], result, args.size(),
                                                [&](TA left, TB right) {
        ...
        template<typename TA, typename TB, typename TR, typename OP>
        inline static ScalarFunction GetBinaryDateFunction(const LogicalTypeId &left_type,
                                                           const LogicalTypeId &right_type,
                                                           const LogicalTypeId &result_type) {
