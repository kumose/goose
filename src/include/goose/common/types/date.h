...
public:
	...
	GOOSE_API static date_t FromString(const string &str, bool strict = false);
	//! Convert a string in the format "YYYY-MM-DD" to a date object
	GOOSE_API static date_t FromCString(const char *str, idx_t len, bool strict = false);
	//! Convert a date object to a string in the format "YYYY-MM-DD"
	GOOSE_API static string ToString(date_t date);
	//! Try to convert the string as a give "special" date (e.g, PINF, ...)
	//! Returns true if it was successful and updates the scan pos.
	GOOSE_API static bool TryConvertDateSpecial(const char *buf, idx_t len, idx_t &pos, const DateSpecial &special);
	...
	GOOSE_API static DateCastResult TryConvertDate(const char *buf, idx_t len, idx_t &pos, date_t &result,
	                                                bool &special, bool strict = false);

	//! Create a string "YYYY-MM-DD" from a specified (year, month, day)
	//! combination
	GOOSE_API static string Format(int32_t year, int32_t month, int32_t day);

	//! Extract the year, month and day from a given date object
	GOOSE_API static void Convert(date_t date, int32_t &out_year, int32_t &out_month, int32_t &out_day);
	//! Create a Date object from a specified (year, month, day) combination
	GOOSE_API static date_t FromDate(int32_t year, int32_t month, int32_t day);
	GOOSE_API static bool TryFromDate(int32_t year, int32_t month, int32_t day, date_t &result);

	//! Returns true if (year) is a leap year, and false otherwise
	GOOSE_API static bool IsLeapYear(int32_t year);

	//! Returns true if the specified (year, month, day) combination is a valid
	//! date
	GOOSE_API static bool IsValid(int32_t year, int32_t month, int32_t day);
	...
	GOOSE_API static int32_t MonthDays(int32_t year, int32_t month);

	//! Extract the epoch from the date (seconds since 1970-01-01)
	GOOSE_API static int64_t Epoch(date_t date);
	//! Extract the epoch from the date (nanoseconds since 1970-01-01)
	GOOSE_API static int64_t EpochNanoseconds(date_t date);
	//! Extract the epoch from the date (microseconds since 1970-01-01)
	GOOSE_API static int64_t EpochMicroseconds(date_t date);
	//! Extract the epoch from the date (milliseconds since 1970-01-01)
	GOOSE_API static int64_t EpochMilliseconds(date_t date);
	//! Convert the epoch (seconds since 1970-01-01) to a date_t
	GOOSE_API static date_t EpochToDate(int64_t epoch);

	//! Extract the number of days since epoch (days since 1970-01-01)
	GOOSE_API static int32_t EpochDays(date_t date);
	//! Convert the epoch number of days to a date_t
	GOOSE_API static date_t EpochDaysToDate(int32_t epoch);

	//! Extract year of a date entry
	GOOSE_API static int32_t ExtractYear(date_t date);
	//! Extract month of a date entry
	GOOSE_API static int32_t ExtractMonth(date_t date);
	//! Extract day of a date entry
	GOOSE_API static int32_t ExtractDay(date_t date);
	//! Extract the day of the week (1-7)
	GOOSE_API static int32_t ExtractISODayOfTheWeek(date_t date);
	//! Extract the day of the year
	GOOSE_API static int32_t ExtractDayOfTheYear(date_t date);
	//! Extract the day of the year
	GOOSE_API static int64_t ExtractJulianDay(date_t date);
	...
	GOOSE_API static void ExtractISOYearWeek(date_t date, int32_t &year, int32_t &week);
	GOOSE_API static int32_t ExtractISOWeekNumber(date_t date);
	GOOSE_API static int32_t ExtractISOYearNumber(date_t date);
	...
	GOOSE_API static int32_t ExtractWeekNumberRegular(date_t date, bool monday_first = true);
	//! Returns the date of the monday of the current week.
	GOOSE_API static date_t GetMondayOfCurrentWeek(date_t date);

	//! Helper function to parse two digits from a string (e.g. "30" -> 30, "03" -> 3, "3" -> 3)
	GOOSE_API static bool ParseDoubleDigit(const char *buf, idx_t len, idx_t &pos, int32_t &result);
...
//! Date
template <>
struct hash<goose::date_t> {
	std::size_t operator()(const goose::date_t &k) const {
		using std::hash;
