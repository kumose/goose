...
unique_ptr<CreateStatement> Transformer::TransformCreateView(cantor::PGViewStmt &stmt) {
	D_ASSERT(stmt.type == cantor::T_PGViewStmt);
	D_ASSERT(stmt.view);
	...
	if (stmt.aliases && stmt.aliases->length > 0) {
		for (auto c = stmt.aliases->head; c != nullptr; c = lnext(c)) {
			auto val = PGPointerCast<cantor::PGValue>(c->data.ptr_value);
			switch (val->type) {
	...
	if (stmt.withCheckOption != cantor::PGViewCheckOption::PG_NO_CHECK_OPTION) {
