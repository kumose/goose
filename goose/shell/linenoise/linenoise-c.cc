...
using goose::Highlighting;
using goose::History;
using goose::idx_t;
using goose::Linenoise;
using goose::Terminal;
...
void linenoiseAddCompletion(linenoiseCompletions *lc, const char *zLine, const char *completion, size_t nCompletion,
    ...
    auto &completions = *reinterpret_cast<goose::TabCompletion *>(lc);
    goose::Completion c;
    c.original_completion = goose::string(completion, nCompletion);
    c.completion.reserve(completion_start + nCompletion + 1);
    c.completion += goose::string(zLine, completion_start);
...
int linenoiseGetTerminalColorMode() {
    ...
    goose::TerminalColor background_color;
    if (!goose::Terminal::TryGetBackgroundColor(background_color)) {
