...
#include <goose/function/function_serialization.h>

namespace goose {
    BoundAggregateExpression::BoundAggregateExpression(AggregateFunction function,
    ...
    hash_t BoundAggregateExpression::Hash() const {
        ...
        result = CombineHash(result, goose::Hash(IsDistinct()));
        return result;
    }
    ...
    bool BoundAggregateExpression::PropagatesNullValues() const {
        return function.GetNullHandling() == FunctionNullHandling::SPECIAL_HANDLING
                   ? false
    ...
    unique_ptr<Expression> BoundAggregateExpression::Deserialize(Deserializer &deserializer) {
        ...
        auto filter =
                deserializer.ReadPropertyWithExplicitDefault<unique_ptr<Expression> >(
                    204, "filter", unique_ptr<Expression>());
        auto result = make_uniq<BoundAggregateExpression>(std::move(entry.first), std::move(children),
                                                          std::move(filter),
