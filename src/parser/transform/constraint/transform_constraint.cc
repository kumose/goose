...
static void ParseSchemaTableNameFK(cantor::PGRangeVar &input, ForeignKeyInfo &fk_info) {
...
TransformForeignKeyConstraint(cantor::PGConstraint &constraint,
	...
	if (override_fk_column) {
		...
		for (auto kc = constraint.fk_attrs->head; kc; kc = kc->next) {
			auto value = Transformer::PGPointerCast<cantor::PGValue>(kc->data.ptr_value);
...
unique_ptr<Constraint> Transformer::TransformConstraint(cantor::PGConstraint &constraint) {
	switch (constraint.contype) {
	case cantor::PG_CONSTR_UNIQUE:
	case cantor::PG_CONSTR_PRIMARY: {
		bool is_primary_key = constraint.contype == cantor::PG_CONSTR_PRIMARY;
		if (!constraint.keys) {
			throw ParserException("UNIQUE USING INDEX is not supported");
		}
		vector<string> columns;
		for (auto kc = constraint.keys->head; kc; kc = kc->next) {
			auto value = PGPointerCast<cantor::PGValue>(kc->data.ptr_value);
			columns.emplace_back(value->val.str);
		}
		return make_uniq<UniqueConstraint>(columns, is_primary_key);
	}
	case cantor::PG_CONSTR_CHECK: {
...
unique_ptr<Constraint> Transformer::TransformConstraint(cantor::PGConstraint &constraint,
	...
	case cantor::PG_CONSTR_NULL:
		return nullptr;
	case cantor::PG_CONSTR_GENERATED_VIRTUAL: {
