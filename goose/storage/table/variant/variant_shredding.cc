...
#include <goose/common/types/variant_visitor.h>
#include <goose/function/variant/variant_shredding.h>
#include <goose/function/variant/variant_normalize.h>
#include <goose/common/serializer/varint.h>
#ifdef DEBUG
...
struct GooseVariantShreddingState : public VariantShreddingState {
public:
	GooseVariantShreddingState(const LogicalType &type, idx_t total_count)
	    : VariantShreddingState(type, total_count), variant_types(GetVariantType(type)) {
	}
	~GooseVariantShreddingState() override {
	}
...
struct GooseVariantShredding : public VariantShredding {
public:
	explicit GooseVariantShredding(idx_t count) : VariantShredding(), unshredded_values(count) {
	}
	~GooseVariantShredding() override = default;
...
public:
	...
	void AnalyzeVariantValues(UnifiedVariantVectorData &variant, Vector &value, optional_ptr<const SelectionVector> sel,
	                          ...
	                          GooseVariantShreddingState &shredding_state, idx_t count);
...
static vector<uint32_t> UnshreddedObjectChildren(UnifiedVariantVectorData &variant, uint32_t row, uint32_t value_index,
                                                 GooseVariantShreddingState &shredding_state) {
...
void GooseVariantShredding::AnalyzeVariantValues(UnifiedVariantVectorData &variant, Vector &value,
...
void GooseVariantShredding::WriteVariantValues(UnifiedVariantVectorData &variant, Vector &result,
                                                optional_ptr<const SelectionVector> sel,
                                                optional_ptr<const SelectionVector> value_index_sel,
	...
	GooseVariantShreddingState shredding_state(typed_value.GetType(), count);
...
void VariantColumnData::ShredVariantData(Vector &input, Vector &output, idx_t count) {
	...
	GooseVariantShredding shredding(count);
