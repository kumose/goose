...
#define CPPHTTPLIB_NAMESPACE goose_httplib_openssl
#else
#define CPPHTTPLIB_NAMESPACE goose_httplib
#endif
...
typename std::enable_if<!std::is_array<T>::value, goose::unique_ptr<T>>::type
make_unique(Args &&...args) {
  return goose::unique_ptr<T>(new T(std::forward<Args>(args)...));
}
...
make_unique(std::size_t n) {
  ...
  return goose::unique_ptr<T>(new RT[n]);
}
...
using Match = goose::Match;
using Regex = goose::Regex;
...
private:
  ...
  static goose::unique_ptr<detail::MatcherBase>
...
public:
  Result() = default;
  Result(goose::unique_ptr<Response> &&res, Error err,
...
private:
  goose::unique_ptr<Response> res_;
...
private:
  ...
  bool handle_request(Stream &strm, Request &req, Response &res,
                      bool close_connection, Error &error);
  goose::unique_ptr<Response> send_with_content_provider(
...
private:
  goose::unique_ptr<ClientImpl> cli_;
...
inline std::string file_extension(const std::string &path) {
  ...
  if (goose::RegexSearch(path, m, re)) { return m[1].str(); }
  return std::string();
}
...
bool prepare_content_receiver(T &x, int &status,
  ...
  if (decompress) {
    ...
    goose::unique_ptr<decompressor> decompressor;
        ...
        while (pos < buf_size()) {
          ...
          if (start_with_case_ignore(header, header_content_type)) {
            ...
            const Regex re_content_disposition(
                ...
                goose::RegexOptions::CASE_INSENSITIVE);
            ...
            if (RegexMatch(header, m, re_content_disposition)) {
              ...
              if (it != params.end()) {
                ...
                const Regex re_rfc5987_encoding(
                    R"~(^UTF-8''(.+?)$)~", goose::RegexOptions::CASE_INSENSITIVE);
...
inline std::string message_digest(const std::string &s, const EVP_MD *algo) {
  auto context = goose::unique_ptr<EVP_MD_CTX, decltype(&EVP_MD_CTX_free)>(
...
template <typename T>
using CFObjectPtr =
    goose::unique_ptr<typename std::remove_pointer<T>::type, void (*)(CFTypeRef)>;
  ...
  if (res.has_header(auth_key)) {
    ...
    if (pos != std::string::npos) {
      ...
      if (type == "Basic") {
        ...
        s = s.substr(pos + 1);
        auto matches = goose::RegexFindAll(s, re);
        for (auto &m : matches) {
...
inline goose::unique_ptr<detail::MatcherBase>
...
Server::write_content_with_provider(Stream &strm, const Request &req,
  ...
  if (res.content_length_ > 0) {
    ...
    if (res.is_chunked_content_provider_) {
      ...
      goose::unique_ptr<detail::compressor> compressor;
    ...
    if (!req.path.compare(0, entry.mount_point.size(), entry.mount_point)) {
      ...
      if (detail::is_valid_path(sub_path)) {
        ...
        if (detail::is_file(path)) {
          ...
          auto mm = goose::make_shared_ptr<detail::mmap>(path.c_str());
...
inline bool Server::listen_internal() {
  ...
  {
    goose::unique_ptr<TaskQueue> task_queue(new_task_queue());
...
inline bool Server::dispatch_request(Request &req, Response &res,
  ...
  for (const auto &x : handlers) {
    ...
    if (goose::RegexMatch(req.path, req.matches, pattern)) {
      handler(req, res);
      return true;
    }
  }
...
inline goose::unique_ptr<Response> ClientImpl::send_with_content_provider(
    Request &req, const char *body, size_t content_length,
    ContentProvider content_provider,
    ContentProviderWithoutLength content_provider_without_length,
    const std::string &content_type, Error &error) {
