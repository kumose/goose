...
uint64_t LogicalDependencyHashFunction::operator()(const LogicalDependency &a) const {
	...
	hash_t hash = goose::Hash(name.c_str());
	hash = CombineHash(hash, goose::Hash(schema.c_str()));
	hash = CombineHash(hash, goose::Hash(catalog.c_str()));
	hash = CombineHash(hash, goose::Hash<uint8_t>(static_cast<uint8_t>(type)));
	return hash;
}
