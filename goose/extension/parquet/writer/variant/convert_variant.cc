...
#include <goose/extension/parquet/reader/variant/variant_binary_decoder.h>
...
static void CreateMetadata(UnifiedVariantVectorData &variant, Vector &metadata, idx_t count) {
	...
	//! Whereas we can have NumericLimits<uint32_t>::Maximum() *per* string in Goose
...
void VariantColumnWriter::FinalizeSchema(vector<goose_parquet::SchemaElement> &schemas) {
	...
	// variant group
	goose_parquet::SchemaElement top_element;
