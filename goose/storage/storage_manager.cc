...
#include <goose/storage/in_memory_block_manager.h>
...
#include <goose/storage/table/in_memory_checkpoint.h>
...
using SHA256State = goose_mbedtls::MbedTlsWrapper::SHA256State;
...
bool StorageManager::WALStartCheckpoint(MetaBlockPointer meta_block, CheckpointOptions &options) {
	...
	auto &transaction_manager = db.GetTransactionManager().Cast<GooseTransactionManager>();
	options.transaction_id = transaction_manager.GetNewCheckpointId();

	GOOSE_LOG(db.GetDatabase(), TransactionLogType, db, "Start Checkpoint", options.transaction_id);
	if (!wal) {
		return false;
	}
...
void StorageManager::WALFinishCheckpoint(lock_guard<mutex> &) {
	...
	GOOSE_LOG(db.GetDatabase(), TransactionLogType, db, "Finish Checkpoint");
}
...
inline void ClearUserKey(shared_ptr<string> const &encryption_key) {
	if (encryption_key && !encryption_key->empty()) {
		goose_mbedtls::MbedTlsWrapper::AESStateMBEDTLS::SecureClearData(data_ptr_cast(&(*encryption_key)[0]),
		                                                                 encryption_key->size());
...
SingleFileStorageCommitState::~SingleFileStorageCommitState() {
	...
	try {
		...
		ErrorData data(ex);
		try {
			GOOSE_LOG_ERROR(wal.GetDatabase().GetDatabase(),
...
void SingleFileStorageManager::CreateCheckpoint(QueryContext context, CheckpointOptions options) {
	...
	if (options.type != CheckpointType::CONCURRENT_CHECKPOINT) {
		auto &transaction_manager = GetAttached().GetTransactionManager().Cast<GooseTransactionManager>();
...
void SingleFileStorageManager::Destroy() {
	...
	auto &catalog = Catalog::GetCatalog(db).Cast<GooseCatalog>();
	catalog.ScanSchemas([&](SchemaCatalogEntry &entry) { schemas.push_back(entry); });

	vector<reference<GooseTableEntry>> tables;
	for (auto &schema : schemas) {
		schema.get().Scan(CatalogType::TABLE_ENTRY, [&](CatalogEntry &entry) {
			...
			if (entry.type == CatalogType::TABLE_ENTRY) {
				tables.push_back(entry.Cast<GooseTableEntry>());
			}
		});
	}
