...
vector<unique_ptr<ParsedExpression>> Transformer::TransformIndexParameters(cantor::PGList &list,
...
unique_ptr<CreateStatement> Transformer::TransformCreateIndex(cantor::PGIndexStmt &stmt) {
	...
	if (stmt.options) {
		...
		for_each_cell(cell, stmt.options->head) {
			...
			Value val;
			if (def_elem->arg) {
				val = TransformValue(*PGPointerCast<cantor::PGValue>(def_elem->arg))->value;
			} else {
