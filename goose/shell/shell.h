...
#include <goose/shell/shell_progress_bar.h>
...
#include <goose/shell/linenoise/linenoise.h>
#endif
...
void ShellState::Print(PrintOutput output, goose::string_t str) {
...
void ShellState::Print(goose::string_t str) {
...
ShellState::ShellState() : seenInterrupt(0), program_name("ksql") {
    config.error_manager->AddCustomError(
        goose::ErrorType::UNSIGNED_EXTENSION,
...
idx_t ShellState::RenderLength(goose::string_t str) {
...
SuccessState ShellState::ExecuteStatement(unique_ptr<goose::SQLStatement> statement) {
    ...
    unique_ptr<goose::QueryResult> result;
    if (renderer->RequireMaterializedResult()) {
        // we need to materialize the result prior to rendering
        goose::QueryParameters parameters;
        parameters.output_type = goose::QueryResultOutputType::FORCE_MATERIALIZED;
        parameters.memory_type = goose::QueryResultMemoryType::BUFFER_MANAGED;
    ...
    if (properties.return_type == goose::StatementReturnType::CHANGED_ROWS) {
        auto result_chunk = res.Fetch();
        if (result_chunk && result_chunk->size() == 1) {
            // update total changes
            auto row_changes = result_chunk->GetValue(0, 0);
            if (!row_changes.IsNull() && row_changes.DefaultTryCastAs(goose::LogicalType::BIGINT)) {
                last_changes = row_changes.GetValue<int64_t>();
                total_changes += last_changes;
            }
        }
    }
    if (properties.return_type != goose::StatementReturnType::QUERY_RESULT) {
        // only SELECT statements return results that need to be rendered
        return SuccessState::SUCCESS;
    }
    if (res.type == goose::QueryResultType::MATERIALIZED_RESULT) {
        last_result = goose::unique_ptr_cast<goose::QueryResult, MaterializedQueryResult>(std::move(result));
    }
...
SuccessState ShellState::ExecuteSQL(const string &zSql) {
    ...
    try {
        ...
        for (auto &statement: statements) {
            ...
            cMode = mode;
            if (statement->type == goose::StatementType::EXPLAIN_STATEMENT) {
...
*/
static string getTableSchema(goose::Connection &con, const char *zTable) {
    string zSchema;
...
unique_ptr<goose::ProgressBarDisplay> CreateProgressBar() {
...
void ShellState::OpenDB(ShellOpenFlags flags) {
    if (!db) {
        try {
            db = make_uniq<goose::Goose>(zDbFilename.c_str(), &config);
            conn = make_uniq<goose::Connection>(*db);
        } catch (std::exception &ex) {
            goose::ErrorData error(ex);
            PrintDatabaseError(error.Message());
            if (flags == ShellOpenFlags::KEEP_ALIVE_ON_FAILURE) {
                db = make_uniq<goose::Goose>(":memory:", &config);
                conn = make_uniq<goose::Connection>(*db);
            } else {
                exit(1);
            }
        }
        auto &client_config = goose::ClientConfig::GetConfig(*conn->context);
        client_config.display_create_func = CreateProgressBar;
#ifdef SHELL_INLINE_AUTOCOMPLETE
        db->LoadStaticExtension<goose::AutocompleteExtension>();
#endif
        db->LoadStaticExtension<goose::ShellExtension>();
        if (safe_mode) {
    ...
    auto std_out_log_storage = goose::make_shared_ptr<ShellLogStorage>(*this);
    goose::shared_ptr<goose::LogStorage> storage_ptr = std_out_log_storage;
    ...
    log_manager.SetLogLevel(goose::LogLevel::LOG_WARNING);
}
...
FILE *ShellState::OpenOutputFile(const char *zFile, int bTextMode) {
    ...
    if (strcmp(zFile, "stdout") == 0) {
        ...
        const string expanded_path = goose::FileSystem::ExpandPath(zFile, /*opener=*/nullptr);
...
string ShellState::GetSystemPager() {
    const char *goose_pager = getenv("GOOSE_PAGER");

    // Try GOOSE_PAGER first (highest priority for env vars)
    if (goose_pager && strlen(goose_pager) > 0) {
        return goose_pager;
    }
...
bool ShellState::ShouldUsePager() {
    ...
    if (pager_command.empty()) {
        ...
        if (pager_command.empty()) {
            Print(PrintOutput::STDERR, "Warning: No pager configured. Set GOOSE_PAGER or PAGER environment variable\n"
...
bool ShellState::SetOutputMode(const string &mode_name, const char *tbl_name) {
    ...
    if (c2 == 'l' && n2 > 2 && strncmp(mode_str, "lines", n2) == 0) {
        ...
        mode = RenderMode::BOX;
    } else if (c2 == 'd' && strncmp(mode_str, "goosebox", n2) == 0) {
        mode = RenderMode::GOOSEBOX;
        ...
        PrintF(PrintOutput::STDERR, "Error: mode should be one of: "
               "ascii box column csv goosebox html insert json jsonlines latex line "
...
ExecuteSQLSingleValueResult ShellState::ExecuteSQLSingleValue(goose::Connection &con, const string &sql,
                                                              string &result_value) {
    ...
    auto is_query = result->properties.return_type == goose::StatementReturnType::QUERY_RESULT;
    if (!is_query) {
...
bool ShellState::OpenDatabase(const vector<string> &args) {
    ...
    config.options.access_mode = goose::AccessMode::READ_WRITE;
    for (iName = 1; iName < args.size() && args[iName][0] == '-'; iName++) {
        const char *z = args[iName].c_str();
        if (optionMatch(z, "new")) {
            newFlag = true;
        } else if (optionMatch(z, "readonly")) {
            config.options.access_mode = goose::AccessMode::READ_ONLY;
...
MetadataResult ShellState::DisplayTables(const vector<string> &args) {
    ...
    // Parse the filter pattern to check for schema qualification
    try {
        auto components = goose::QualifiedName::ParseComponents(filter_pattern);
        if (components.size() >= 2) {
            // e.g : "schema.table" or "schema.%"
            schema_filter = "%" + components[0] + "%";
            table_filter = "%" + components[1] + "%";
        }
    } catch (const goose::ParserException &) {
...
FROM goose_columns() columns
LEFT JOIN goose_tables() t USING (table_oid)
LEFT JOIN (
...
GROUP BY ALL;
    ...
    for (auto &row: *query_result) {
        ...
        for (auto &column_entry: goose::ListValue::GetChildren(column_val)) {
            ShellColumnInfo column;
            auto &struct_children = goose::StructValue::GetChildren(column_entry);
...
MetadataResult ShellState::DisplayEntries(const vector<string> &args, char type) {
    ...
    // Use Goose's system tables instead of SQLite's sqlite_schema
    if (type == 't') {
        string schema_filter_str;
...
WITH all_objects AS (
  SELECT schema_name, table_name as name FROM goose_tables%s
  UNION ALL
  SELECT schema_name, view_name as name FROM goose_views%s
),
name_counts AS (
...
WHERE type='index' AND tbl_name LIKE ?1)";
    ...
    goose::vector<goose::Value> bind_values;

    if (type == 't') {
        // Bind parameters for the new Goose query
        if (!schema_filter.empty()) {
...
int ShellState::ProcessInput(InputMode mode) {
    ...
    while (errCnt == 0 || !bail_on_error || (!in && stdin_is_interactive)) {
        ...
        if (mode == InputMode::GOOSE_RC && !StringUtil::StartsWith(zLine, ".startup_text")) {
...
static string GetHomeDirectory() {
    goose::LocalFileSystem lfs;
    return lfs.GetHomeDirectory();
}

string ShellState::GetDefaultGooseRC() {
    return GetHomeDirectory() + "/.gooserc";
}

/*
** Read input from the file given by sqliterc_override.  Or if that
** parameter is NULL, take input from ~/.gooserc
**
** Returns true if successful, false otherwise.
*/

bool ShellState::ProcessFile(const string &file, bool is_goose_rc) {
    FILE *inSaved = in;
    int savedLineno = lineno;
    int rc = 0;
    ...
    if (in) {
        ...
        if (stdin_is_interactive && is_goose_rc) {
            input_mode = InputMode::GOOSE_RC;
            goose_rc_path = file;
        }
...
bool ShellState::ProcessGooseRC(const char *file) {
    string path;
    if (!file) {
        // use default .gooserc path
        path = ShellState::GetDefaultGooseRC();
        if (path.empty()) {
...
static void linenoise_completion(const char *zLine, linenoiseCompletions *lc) {
    ...
    try {
        ...
        unique_ptr<goose::Goose> localDB;
        unique_ptr<goose::Connection> localCon;
...
void ShellState::Initialize() {
    normalMode = cMode = mode = RenderMode::GOOSEBOX;
    max_rows = 40;
    colSeparator = SEP_Column;
    rowSeparator = SEP_Row;
    showHeader = true;
    ...
    default_prompt =
        "{max_length:40}{highlight_element:prompt}{setting:current_database_and_schema}{color:reset} " + goose::UserConfig::instance().alter_string + " ";
...
int main(int argc, const char **argv) {
    ...
    ** try to process it.
    */
    if (!data.ProcessGooseRC(data.initFile.empty() ? nullptr : data.initFile.c_str()) && data.bail_on_error) {
        return 1;
    }
    ...
    if (!data.readStdin) {
        ...
        */
        ...
        if (data.stdin_is_interactive) {
            ...
            auto startup_version = StringUtil::Format("Goose %s (%s", goose::Goose::LibraryVersion(),
                                                      goose::Goose::ReleaseCodename());
            if (StringUtil::Contains(goose::Goose::ReleaseCodename(), "Development")) {
                startup_version += ", ";
                startup_version += goose::Goose::SourceID();
            }
            startup_version += ")\n";
            ...
            zHistory = getenv("GOOSE_HISTORY");
            if (!zHistory) {
                zHome = GetHomeDirectory() + "/.goose_history";
