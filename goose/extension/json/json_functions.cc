...
namespace goose {
    ...
    unique_ptr<FunctionLocalState> JSONFunctionLocalState::Init(ExpressionState &state,
    ...
    unique_ptr<FunctionLocalState> JSONFunctionLocalState::InitCastLocalState(CastLocalStateParameters &parameters) {
        return parameters.context
                   ? make_uniq<JSONFunctionLocalState>(*parameters.context)
                    ...
                    if (success) {
                        if (!doc) {
                            HandleCastError::AssignError(
                                JSONCommon::FormatParseError(input.GetDataWriteable(), input.GetSize(), error),
                                parameters);
    ...
    void JSONFunctions::RegisterSimpleCastFunctions(ExtensionLoader &loader) {
        ...
        const auto json_list_to_varchar_cost =
                CastFunctionSet::ImplicitCastCost(db, LogicalType::LIST(LogicalType::JSON()), LogicalTypeId::VARCHAR) -
                1;
        loader.RegisterCastFunction(LogicalType::LIST(LogicalType::JSON()), LogicalTypeId::VARCHAR,
        ...
        BoundCastInfo varchar_to_json_list_info(CastVarcharToJSONList, nullptr,
