...
#include <goose/storage/compression/roaring/roaring.h>
...
namespace goose {
    namespace roaring {
        ...
        void SetInvalidRange(ValidityMask &result, idx_t start, idx_t end) {
            ...
            for (idx_t i = 0;
                 i < AlignValue<idx_t, ValidityMask::BITS_PER_VALUE>(result.Capacity()) / ValidityMask::BITS_PER_VALUE;
                 i++) {
        ...
        void ExtractValidityMaskToData(Vector &src, Vector &dst, idx_t offset, idx_t scan_count) {
            ...
            if (validity.AllValid()) {
                ...
                BitpackingPrimitives::UnPackBuffer<uint8_t>(tmp_buffer.GetData(), data_ptr_cast(validity.GetData()),
                                                            scan_count,
                                                            1);
        ...
        // Fetch
        //===--------------------------------------------------------------------===//
        void RoaringFetchRow(ColumnSegment &segment, ColumnFetchState &state, row_t row_id, Vector &result,
                             idx_t result_idx) {
    ...
    CompressionFunction GetCompressionFunction(PhysicalType data_type) {
        ...
        switch (data_type) {
            ...
            default:
                throw InternalException("Roaring GetCompressionFunction, type %s not handled",
                                        EnumUtil::ToString(data_type));
        }
