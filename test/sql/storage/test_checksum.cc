...
TEST_CASE("Test functioning of checksum", "[storage]") {
	...
	goose::unique_ptr<Goose> database;
	...
	REQUIRE_NOTHROW(database = make_uniq<Goose>(storage_database, config.get()));
	database.reset();
	...
	REQUIRE_THROWS(database = make_uniq<Goose>(storage_database, config.get()));
