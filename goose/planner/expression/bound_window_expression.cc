...
namespace goose {
    ...
    bool BoundWindowExpression::Equals(const BaseExpression &other_p) const {
        ...
        if (!Expression::Equals(start_expr, other.start_expr) || !Expression::Equals(end_expr, other.end_expr) ||
            !Expression::Equals(offset_expr, other.offset_expr) || !
    ...
    idx_t BoundWindowExpression::GetSharedOrders(const vector<BoundOrderByNode> &lhs,
                                                 const vector<BoundOrderByNode> &rhs) {
    ...
    unique_ptr<Expression> BoundWindowExpression::Deserialize(Deserializer &deserializer) {
        ...
        auto result =
                make_uniq<BoundWindowExpression>(expression_type, return_type, std::move(aggregate),
                                                 std::move(bind_info));
        ...
        deserializer.ReadPropertyWithExplicitDefault(211, "default_expr", result->default_expr,
