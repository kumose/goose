...
unique_ptr<ExplainStatement> Transformer::TransformExplain(cantor::PGExplainStmt &stmt) {
	...
	if (stmt.options) {
		for (auto n = stmt.options->head; n; n = n->next) {
			auto def_elem = PGPointerCast<cantor::PGDefElem>(n->data.ptr_value);
			...
			if (elem == "analyze") {
				...
				if (def_elem->arg) {
					...
					auto val = TransformValue(*PGPointerCast<cantor::PGValue>(def_elem->arg))->value;
					format_is_set = true;
