...
void Transformer::TransformGroupByExpression(cantor::PGNode &n, GroupingExpressionMap &map,
...
void Transformer::TransformGroupByNode(cantor::PGNode &n, GroupingExpressionMap &map, SelectNode &result,
                                       vector<GroupingSet> &result_sets) {
	if (n.type == cantor::T_PGGroupingSet) {
		auto &grouping_set = PGCast<cantor::PGGroupingSet>(n);
		switch (grouping_set.kind) {
		case cantor::GROUPING_SET_EMPTY:
			result_sets.emplace_back();
			break;
		case cantor::GROUPING_SET_ALL: {
			result.aggregate_handling = AggregateHandling::FORCE_AGGREGATES;
			break;
		}
		case cantor::GROUPING_SET_SETS: {
			for (auto node = grouping_set.content->head; node; node = node->next) {
				auto pg_node = PGPointerCast<cantor::PGNode>(node->data.ptr_value);
				TransformGroupByNode(*pg_node, map, result, result_sets);
			}
			break;
		}
		case cantor::GROUPING_SET_ROLLUP: {
...
bool Transformer::TransformGroupBy(optional_ptr<cantor::PGList> group, SelectNode &select_node) {
	if (!group) {
		return false;
	}
