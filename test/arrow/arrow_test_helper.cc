...
goose::unique_ptr<goose::ArrowArrayStreamWrapper>
ArrowStreamTestFactory::CreateStream(uintptr_t this_ptr, goose::ArrowStreamParameters &parameters) {
	auto stream_wrapper = goose::make_uniq<goose::ArrowArrayStreamWrapper>();
...
goose::unique_ptr<goose::ArrowArrayStreamWrapper> ArrowTestFactory::CreateStream(uintptr_t this_ptr,
	...
	if (materialized_arrow.types != duck->types) {
		...
		for (idx_t i = 0; i < materialized_arrow.types.size(); i++) {
			if (materialized_arrow.types[i] != duck->types[i] && duck->types[i].id() != LogicalTypeId::ENUM) {
				mismatch_error = true;
				error_msg << "Column " << i << " mismatch. Goose: '" << duck->types[i].ToString() << "'. Arrow '"
