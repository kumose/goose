...
#include <goose/common/jaro_winkler/jaro_winkler.h>
...
namespace goose {
    ...
    namespace string_util_internal {
        ...
        inline string TakePossiblyQuotedItem(const string &str, idx_t &index, char delimiter, char quote) {
            ...
            if (str[index] == quote) {
                ...
                TakeWhile(
                    str, index,
                    [delimiter, quote](char c) {
                        return c != delimiter && c != quote && !StringUtil::CharacterIsSpace(c);
                    },
                    entry);
            }
    ...
    string StringUtil::BytesToHumanReadableString(idx_t bytes, idx_t multiplier) {
        ...
        idx_t array[6] = {};
        const char *unit[2][6] = {
            {"bytes", "KiB", "MiB", "GiB", "TiB", "PiB"}, {"bytes", "kB", "MB", "GB", "TB", "PB"}
        };
    ...
    double StringUtil::SimilarityRating(const string &s1, const string &s2) {
        return goose_jaro_winkler::jaro_winkler_similarity(s1.data(), s1.data() + s1.size(), s2.data(),
                                                           s2.data() + s2.size());
    }
    ...
    static unique_ptr<ComplexJSON>
    ParseJSON(const string &json, yyjson_doc *doc, yyjson_val *root, bool ignore_errors) {
    ...
    const char *StringUtil::EnumToString(const EnumStringLiteral enum_list[], idx_t enum_count, const char *enum_name,
        ...
        throw NotImplementedException("Enum value: unrecognized enum value \"%d\" for enum \"%s\"", enum_value,
                                      enum_name);
    }
