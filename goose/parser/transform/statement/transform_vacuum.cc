...
namespace goose {
    VacuumOptions ParseOptions(const int32_t options) {
        ...
        if (options & cantor::PGVacuumOption::PG_VACOPT_VACUUM) {
            result.vacuum = true;
        }
        if (options & cantor::PGVacuumOption::PG_VACOPT_ANALYZE) {
            result.analyze = true;
        }
        if (options & cantor::PGVacuumOption::PG_VACOPT_VERBOSE) {
            throw NotImplementedException("Verbose vacuum option");
        }
        if (options & cantor::PGVacuumOption::PG_VACOPT_FREEZE) {
            throw NotImplementedException("Freeze vacuum option");
        }
        if (options & cantor::PGVacuumOption::PG_VACOPT_FULL) {
            throw NotImplementedException("Full vacuum option");
        }
        if (options & cantor::PGVacuumOption::PG_VACOPT_NOWAIT) {
            throw NotImplementedException("No Wait vacuum option");
        }
        if (options & cantor::PGVacuumOption::PG_VACOPT_SKIPTOAST) {
            throw NotImplementedException("Skip Toast vacuum option");
        }
        if (options & cantor::PGVacuumOption::PG_VACOPT_DISABLE_PAGE_SKIPPING) {
            throw NotImplementedException("Disable Page Skipping vacuum option");
        }
        return result;
    }

    unique_ptr<SQLStatement> Transformer::TransformVacuum(cantor::PGVacuumStmt &stmt) {
        ...
        if (stmt.va_cols) {
            ...
            for (auto col_node = stmt.va_cols->head; col_node != nullptr; col_node = col_node->next) {
                auto value = PGPointerCast<cantor::PGValue>(col_node->data.ptr_value);
