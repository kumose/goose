...
#include <goose/function/table/read_goose.h>
...
namespace goose {
    struct GooseMultiFileInfo : MultiFileReaderInterface {
        ...
        InitializeLocalState(ExecutionContext &, GlobalTableFunctionState &) override;
    ...
    class GooseFileReaderOptions : public BaseFileReaderOptions {
    public:
        string schema_name;
        string table_name;
    ...
    struct GooseReadBindData : TableFunctionData {
        unique_ptr<GooseFileReaderOptions> options;
    ...
    class GooseReader : public BaseFileReader {
    public:
        GooseReader(ClientContext &context, OpenFileInfo file, const GooseFileReaderOptions &options);

        ~GooseReader() override;
    ...
    struct GooseReadGlobalState : GlobalTableFunctionState {
    };

    struct GooseReadLocalState : LocalTableFunctionState {
        unique_ptr<LocalTableFunctionState> local_state;
        shared_ptr<AttachedDatabaseWrapper> attached_database;
    };

    string GooseFileReaderOptions::GetCandidates(const vector<reference<TableCatalogEntry> > &tables) const {
        if (tables.empty()) {
            return string();
        }
    ...
    bool GooseFileReaderOptions::HasSelection() const {
        if (!table_name.empty()) {
            return true;
        }
    ...
    string GooseFileReaderOptions::PrintOptions() const {
        string options;
        if (!schema_name.empty()) {
    ...
    bool GooseFileReaderOptions::Matches(TableCatalogEntry &table) const {
    ...
    GooseReader::GooseReader(ClientContext &context_p, OpenFileInfo file_p, const GooseFileReaderOptions &options)
    ...
    GooseReader::~GooseReader() {
    }

    AttachedDatabase &GooseReader::GetAttachedDatabase() {
        if (!db_wrapper) {
            auto &db_manager = DatabaseManager::Get(context);
            AttachInfo info;
            info.path = file.path;
            // use invalid UTF-8 so that a conflicting database name cannot be attached by a user
            info.name = "\x80__goose_reader_" + info.path;
    ...
    TableCatalogEntry &GooseReader::GetTableEntry() {
        auto &attached = GetAttachedDatabase();
        if (!db_wrapper->table_entry) {
            auto &catalog = attached.GetCatalog();
            db_wrapper->table_entry =
                    catalog.GetEntry<TableCatalogEntry>(context, schema_name, table_name,
                                                        OnEntryNotFound::THROW_EXCEPTION);
        }
        return *db_wrapper->table_entry;
    }

    bool GooseReader::TryInitializeScan(ClientContext &context, GlobalTableFunctionState &gstate,
                                        LocalTableFunctionState &lstate_p) {
        auto &lstate = lstate_p.Cast<GooseReadLocalState>();
        if (finished) {
        ...
        if (!global_state) {
            ...
            for (auto &col: column_indexes) {
                if (col.GetPrimaryIndex() >= column_count) {
                    ...
                    if (column.Generated()) {
                        throw NotImplementedException("Unsupported: read_goose cannot read generated column %s",
                                                      column.Name());
                    }
                }
            }
    ...
    AsyncResult GooseReader::Scan(ClientContext &context, GlobalTableFunctionState &gstate_p,
        ...
        if (!scan_function.function) {
            throw InternalException("GooseReader works only with simple table functions");
        } else {
            ...
            switch (input.async_result.GetResultType()) {
                ...
                default:
                    throw InternalException(
                        "GooseReader call of scan_function.function returned unexpected return '%'",
                        EnumUtil::ToChars(input.async_result.GetResultType()));
            }
            throw InternalException("GooseReader hasn't handled a scan_function.function return");
        }
    }

    void GooseReader::FinishFile(ClientContext &context, GlobalTableFunctionState &gstate) {
        db_wrapper.reset();
    }

    optional_idx GooseReader::NumRows() {
        auto &table_entry = GetTableEntry();
        return table_entry.GetStorage().GetTotalRows();
    }

    unique_ptr<BaseStatistics> GooseReader::GetStatistics(ClientContext &context, const string &name) {
    ...
    double GooseReader::GetProgressInFile(ClientContext &context) {
    ...
    void GooseReader::AddVirtualColumn(column_t virtual_column_id) {
        if (virtual_column_id != COLUMN_IDENTIFIER_ROW_ID) {
            throw InternalException("Unsupported virtual column id %d for goose reader", virtual_column_id);
        }
    }

    unique_ptr<MultiFileReaderInterface> GooseMultiFileInfo::CreateInterface(ClientContext &context) {
        return make_uniq<GooseMultiFileInfo>();
    }

    unique_ptr<BaseFileReaderOptions> GooseMultiFileInfo::InitializeOptions(ClientContext &context,
                                                                            optional_ptr<TableFunctionInfo> info) {
        return make_uniq<GooseFileReaderOptions>();
    }

    bool GooseMultiFileInfo::ParseCopyOption(ClientContext &context, const string &key, const vector<Value> &values,
                                             BaseFileReaderOptions &options, vector<string> &expected_names,
                                             vector<LogicalType> &expected_types) {
        return false;
    }

    bool GooseMultiFileInfo::ParseOption(ClientContext &context, const string &key, const Value &val,
                                         MultiFileOptions &file_options, BaseFileReaderOptions &options_p) {
        auto &options = options_p.Cast<GooseFileReaderOptions>();
        if (key == "schema_name") {
    ...
    void GooseMultiFileInfo::FinalizeCopyBind(ClientContext &context, BaseFileReaderOptions &options,
                                              const vector<string> &expected_names,
                                              const vector<LogicalType> &expected_types) {
        throw InternalException("Unimplemented method in GooseMultiFileInfo");
    }

    void GooseMultiFileInfo::FinalizeBindData(MultiFileBindData &multi_file_data) {
        auto &bind_data = multi_file_data.bind_data->Cast<GooseReadBindData>();
        if (multi_file_data.initial_reader) {
            auto &initial_reader = multi_file_data.initial_reader->Cast<GooseReader>();
            bind_data.initial_file_cardinality = initial_reader.NumRows();
        }
    }

    unique_ptr<TableFunctionData> GooseMultiFileInfo::InitializeBindData(MultiFileBindData &multi_file_data,
    ...
    void GooseMultiFileInfo::BindReader(ClientContext &context, vector<LogicalType> &return_types,
        ...
        auto &goose_bind_data = bind_data.bind_data->Cast<GooseReadBindData>();
        bind_data.reader_bind =
                bind_data.multi_file_reader->BindReader(context, return_types, names, *bind_data.file_list, bind_data,
                                                        *goose_bind_data.options, bind_data.file_options);
    }

    unique_ptr<GlobalTableFunctionState> GooseMultiFileInfo::InitializeGlobalState(ClientContext &context,
        MultiFileBindData &bind_data,
        MultiFileGlobalState &global_state) {
        return make_uniq<GooseReadGlobalState>();
    }

    unique_ptr<LocalTableFunctionState> GooseMultiFileInfo::InitializeLocalState(ExecutionContext &,
        GlobalTableFunctionState &) {
        return make_uniq<GooseReadLocalState>();
    }

    struct GooseReaderUnionData : BaseUnionData {
        explicit GooseReaderUnionData(OpenFileInfo file_p) : BaseUnionData(std::move(file_p)) {
        }
    };

    shared_ptr<BaseFileReader> GooseMultiFileInfo::CreateReader(ClientContext &context,
                                                                GlobalTableFunctionState &gstate,
                                                                BaseUnionData &union_data_p,
                                                                const MultiFileBindData &multi_bind_data) {
        auto &union_data = union_data_p.Cast<GooseReaderUnionData>();
        auto &bind_data = multi_bind_data.bind_data->Cast<GooseReadBindData>();
        return make_shared_ptr<GooseReader>(context, union_data.file, *bind_data.options);
    }
    ...
    shared_ptr<BaseFileReader> GooseMultiFileInfo::CreateReader(ClientContext &context,
                                                                ...
                                                                const MultiFileBindData &multi_bind_data) {
        auto &bind_data = multi_bind_data.bind_data->Cast<GooseReadBindData>();
        return make_shared_ptr<GooseReader>(context, file, *bind_data.options);
    }

    shared_ptr<BaseFileReader> GooseMultiFileInfo::CreateReader(ClientContext &context, const OpenFileInfo &file,
                                                                BaseFileReaderOptions &options,
                                                                const MultiFileOptions &file_options) {
        return make_shared_ptr<GooseReader>(context, file, options.Cast<GooseFileReaderOptions>());
    }

    shared_ptr<BaseUnionData> GooseReader::GetUnionData(idx_t file_idx) {
        auto result = make_uniq<GooseReaderUnionData>(file);
    ...
    void GooseMultiFileInfo::FinishReading(ClientContext &context, GlobalTableFunctionState &global_state,
                                           LocalTableFunctionState &lstate_p) {
        auto &lstate = lstate_p.Cast<GooseReadLocalState>();
        lstate.attached_database.reset();
    }

    unique_ptr<NodeStatistics> GooseMultiFileInfo::GetCardinality(const MultiFileBindData &bind_data_p,
                                                                  idx_t file_count) {
        auto &bind_data = bind_data_p.bind_data->Cast<GooseReadBindData>();
    ...
    unique_ptr<MultiFileReaderInterface> GooseMultiFileInfo::Copy() {
        return make_uniq<GooseMultiFileInfo>();
    }

    FileGlobInput GooseMultiFileInfo::GetGlobInput() {
        return FileGlobInput(FileGlobOptions::FALLBACK_GLOB, "db");
    }

    void GooseMultiFileInfo::GetVirtualColumns(ClientContext &, MultiFileBindData &, virtual_column_map_t &result) {
        result.insert(make_pair(COLUMN_IDENTIFIER_ROW_ID, TableColumn("rowid", LogicalType::BIGINT)));
    }

    void ReadGooseAddNamedParameters(TableFunction &table_function) {
    ...
    static vector<column_t> GooseGetRowIdColumns(ClientContext &, optional_ptr<FunctionData>) {
    ...
    static bool GooseScanPushdownExpression(ClientContext &context, const LogicalGet &get, Expression &expr) {
        return true;
    }

    TableFunction ReadGooseTableFunction::GetFunction() {
        MultiFileFunction<GooseMultiFileInfo> read_goose("read_goose");
        read_goose.statistics = MultiFileFunction<GooseMultiFileInfo>::MultiFileScanStats;
        read_goose.get_row_id_columns = GooseGetRowIdColumns;
        read_goose.pushdown_expression = GooseScanPushdownExpression;
        read_goose.filter_pushdown = true;
        read_goose.filter_prune = true;
        read_goose.late_materialization = true;
        ReadGooseAddNamedParameters(read_goose);
        return static_cast<TableFunction>(read_goose);
    }

    unique_ptr<TableRef> ReadGooseTableFunction::ReplacementScan(ClientContext &context, ReplacementScanInput &input,
        ...
        if (!StringUtil::EndsWith(lower_name, ".db") && !StringUtil::Contains(lower_name, ".db?") &&
            ...
            !StringUtil::EndsWith(lower_name, ".gsst") && !StringUtil::Contains(lower_name, ".gsst?")) {
            return nullptr;
        }
        auto table_function = make_uniq<TableFunctionRef>();
        vector<unique_ptr<ParsedExpression> > children;
        children.push_back(make_uniq<ConstantExpression>(Value(table_name)));
        table_function->function = make_uniq<FunctionExpression>("read_goose", std::move(children));
