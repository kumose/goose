...
#include <goose/planner/bind_context.h>
...
namespace goose {
            ...
            if (binding.HasMatchingBinding(column_name)) {
                if (result || is_using_binding) {
                    throw BinderException(
                        ...
                        "or \"%s.%s\")",
                        column_name, MinimumUniqueAlias(result->GetBindingAlias(), binding.GetBindingAlias()),
                        column_name,
    ...
    unique_ptr<ParsedExpression> BindContext::CreateColumnReference(const BindingAlias &table_alias,
        ...
        return CreateColumnReference(table_alias.GetCatalog(), table_alias.GetSchema(), table_alias.GetAlias(),
                                     column_name,
                                     bind_type);
    }
    ...
    vector<reference<Binding> > BindContext::GetBindings(const BindingAlias &alias, ErrorData &out_error) {
        ...
        if (matching_bindings.empty()) {
            ...
            string candidate_str =
                    StringUtil::CandidatesMessage(StringUtil::TopNJaroWinkler(candidates, main_alias),
                                                  "Candidate tables");
            out_error = ErrorData(ExceptionType::BINDER,
                                  StringUtil::Format("Referenced table \"%s\" not found!%s", main_alias,
                                                     candidate_str));
        }
                ...
                for (auto &column_name: column_names) {
                    ...
                    if (using_binding_ptr) {
                        ...
                        if (!using_binding.primary_binding.IsSet()) {
                            ...
                            auto coalesce =
                                    make_uniq_base<ParsedExpression, OperatorExpression>(
                    ...
                    auto new_expr =
                            CreateColumnReference(binding_alias, column_name,
