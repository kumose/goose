...
#include <goose/utility/xxhash.h>
...
unique_ptr<BaseStatistics> ParquetStatisticsUtils::CreateNumericStats(const LogicalType &type,
                                                                      ...
                                                                      const goose_parquet::Statistics &parquet_stats) {
...
static bool HasFilterConstants(const TableFilter &goose_filter) {
	switch (goose_filter.filter_type) {
	case TableFilterType::CONSTANT_COMPARISON: {
		auto &constant_filter = goose_filter.Cast<ConstantFilter>();
		return (constant_filter.comparison_type == ExpressionType::COMPARE_EQUAL && !constant_filter.constant.IsNull());
	}
	case TableFilterType::CONJUNCTION_AND: {
		auto &conjunction_and_filter = goose_filter.Cast<ConjunctionAndFilter>();
	...
	case TableFilterType::CONJUNCTION_OR: {
		auto &conjunction_or_filter = goose_filter.Cast<ConjunctionOrFilter>();
		bool child_has_constant = false;
...
static uint64_t ValueXXH64(const Value &constant) {
	...
	case PhysicalType::VARCHAR: {
		...
		return goose::XXH64(val.c_str(), val.length(), 0);
	}
	default:
		return 0;
	}
...
static bool ApplyBloomFilter(const TableFilter &goose_filter, ParquetBloomFilter &bloom_filter) {
	switch (goose_filter.filter_type) {
	case TableFilterType::CONSTANT_COMPARISON: {
		auto &constant_filter = goose_filter.Cast<ConstantFilter>();
...
bool ParquetStatisticsUtils::BloomFilterExcludes(const TableFilter &goose_filter,
                                                 const goose_parquet::ColumnMetaData &column_meta_data,
                                                 TProtocol &file_proto, Allocator &allocator) {
	if (!HasFilterConstants(goose_filter) || !column_meta_data.__isset.bloom_filter_offset ||
	...
	return ApplyBloomFilter(goose_filter, bloom_filter);
}
