...
#include <goose/common/http_util.h>
...
namespace goose {
...
#ifdef GOOSE_WASM_VERSION
        return GOOSE_QUOTE_DEFINE(GOOSE_WASM_VERSION);
#endif
        if (IsRelease(Goose::LibraryVersion())) {
            return NormalizeVersionTag(Goose::LibraryVersion());
        } else {
    ...
    const vector<string> ExtensionHelper::PathComponents() {
        return vector<string>{GetVersionDirectoryName(), Goose::Platform()};
    }
    ...
    vector<goose::string> ExtensionHelper::DefaultExtensionFolders(FileSystem &fs) {
        vector<goose::string> default_folders;
        // These fallbacks are necessary if the user doesn't use the CMake build.
#ifndef GOOSE_EXTENSION_DIRECTORIES
#ifdef _WIN32
#define GOOSE_EXTENSION_DIRECTORIES "~\\.gsst\\extensions"
#else
#define GOOSE_EXTENSION_DIRECTORIES "~/.gsst/extensions"
#endif
#endif
        string dirs_string(GOOSE_EXTENSION_DIRECTORIES);
    ...
    string ExtensionHelper::ExtensionDirectory(DatabaseInstance &db, FileSystem &fs) {
        ...
        throw PermissionException("ExtensionDirectory functionality is not supported in goose-wasm");
#endif
    ...
    string ExtensionHelper::ExtensionUrlTemplate(optional_ptr<const DatabaseInstance> db,
        ...
        if (!version.empty()) {
            versioned_path = "/${NAME}/" + version + "/${REVISION}/${PLATFORM}/${NAME}.goose_extension";
        } else {
            versioned_path = "/${REVISION}/${PLATFORM}/${NAME}.goose_extension";
        }
    ...
    string ExtensionHelper::ExtensionFinalizeUrlTemplate(const string &url_template, const string &extension_name) {
        ...
        url = StringUtil::Replace(url, "${PLATFORM}", Goose::Platform());
    ...
    static void CheckExtensionMetadataOnInstall(DatabaseInstance &db, void *in_buffer, idx_t file_size,
        ...
        if (file_size < ParsedExtensionMetaData::FOOTER_SIZE) {
            throw IOException("Failed to install '%s', file too small to be a valid Goose extension!", extension_name);
        }
    ...
    static unique_ptr<ExtensionInstallInfo> DirectInstallExtension(DatabaseInstance &db, FileSystem &fs,
                                                                   const string &path,
        ...
        WriteExtensionFiles(fs, temp_path, local_extension_path, extension_decompressed, extension_decompressed_size,
                            info);
    ...
    static unique_ptr<ExtensionInstallInfo> InstallFromRepository(DatabaseInstance &db, FileSystem &fs,
                                                                  const string &url,
                                                                  const string &extension_name, const string &temp_path,
                                                                  const string &local_extension_path,
                                                                  ExtensionInstallOptions &options,
    ...
    static void ThrowErrorOnMismatchingExtensionOrigin(FileSystem &fs, const string &local_extension_path,
        ...
        if (install_info) {
            ...
            if (install_info->mode == ExtensionInstallMode::REPOSITORY && ExtensionHelper::IsFullPath(extension)) {
                throw InvalidInputException(format_string, extension_name, repo, install_info->repository_url,
                                            custom_path,
                                            extension);
            }
        }
    }
#endif // GOOSE_DISABLE_EXTENSION_LOAD
    ...
    unique_ptr<ExtensionInstallInfo> ExtensionHelper::InstallExtensionInternal(DatabaseInstance &db, FileSystem &fs,
        ...
        string local_extension_path = fs.JoinPath(local_path, extension_name + ".goose_extension");
...
#ifdef DISABLE_GOOSE_REMOTE_INSTALL
        ...
        if (IsFullPath(extension)) {
            ...
            return DirectInstallExtension(db, fs, extension, temp_path, extension, local_extension_path, options,
                                          context);
        }
