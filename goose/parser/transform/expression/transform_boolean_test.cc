    ...
    unique_ptr<ParsedExpression> Transformer::TransformBooleanTest(cantor::PGBooleanTest &node) {
        ...
        switch (node.booltesttype) {
            case cantor::PGBoolTestType::PG_IS_TRUE:
                return TransformBooleanTestInternal(std::move(argument), ExpressionType::COMPARE_NOT_DISTINCT_FROM,
                                                    true,
                                                    node.location);
            ...
            case cantor::PGBoolTestType::IS_NOT_UNKNOWN: // IS NOT NULL
                return TransformBooleanTestIsNull(std::move(argument), ExpressionType::OPERATOR_IS_NOT_NULL,
                                                  node.location);
            default:
