...
//                         Goose
//
// core_functions/scalar/list_functions.h
//
//
//===----------------------------------------------------------------------===//
...
struct ListSliceFun {
	...
	static constexpr const char *Example = "list_slice([4, 5, 6], 2, 3)\002array_slice('Goose', 3, 4)\002array_slice('Goose', 3, NULL)\002array_slice('Goose', 0, -3)\001list_slice([4, 5, 6], 1, 3, 2)";
...
struct ListFilterFun {
	...
	static constexpr const char *Parameters = "list,lambda(x)";
	static constexpr const char *Description = "Constructs a list from those elements of the input `list` for which the `lambda` function returns `true`. Goose must be able to cast the `lambda` function's return type to `BOOL`. The return type of `list_filter` is the same as the input list's.";
