...
static void visit_callback(ffi::NullableCvoid engine_context, struct ffi::KernelStringSlice path, int64_t size,
	...
	context->resolved_files.push_back(DeltaSnapshot::ToGoosePath(path_string));
...
static ffi::EngineBuilder *CreateBuilder(ClientContext &context, const string &path) {
	...
	if (!StringUtil::StartsWith(path, "s3://")) {
		auto interface_builder_res =
		    ffi::get_engine_builder(KernelUtils::ToDeltaString(path), GooseEngineError::AllocateError);
...
string DeltaSnapshot::ToGoosePath(const string &raw_path) {
...
static SelectionVector GooseSVFromDeltaSV(const ffi::KernelBoolSlice &dv, Vector row_id_column, idx_t count,
                                         idx_t &select_count) {
...
unique_ptr<MultiFileReaderGlobalState> DeltaMultiFileReader::InitializeGlobalState(
    goose::ClientContext &context, const goose::MultiFileOptions &file_options,
    const goose::MultiFileReaderBindData &bind_data, const goose::MultiFileList &file_list,
    const vector<goose::LogicalType> &global_types, const vector<std::string> &global_names,
    const vector<goose::column_t> &global_column_ids) {
...
void DeltaMultiFileReader::FinalizeChunk(ClientContext &context, const MultiFileReaderBindData &bind_data,
	...
	if (metadata->selection_vector.ptr && chunk.size() != 0) {
		...
		idx_t select_count;
		auto sv = GooseSVFromDeltaSV(metadata->selection_vector, file_row_number_column, chunk.size(), select_count);
		chunk.Slice(sv, select_count);
	}

	// Note: this demo function shows how we can use Goose's Binder create expression-based generated columns
