...
struct GooseColumnsData : public GlobalTableFunctionState {
	GooseColumnsData() : offset(0), column_offset(0) {
	}
...
static unique_ptr<FunctionData> GooseColumnsBind(ClientContext &context, TableFunctionBindInput &input,
...
static unique_ptr<GlobalTableFunctionState> GooseColumnsInit(ClientContext &context, TableFunctionInitInput &input) {
	auto result = make_uniq<GooseColumnsData>();
...
unique_ptr<ColumnHelper> ColumnHelper::Create(CatalogEntry &entry) {
	...
	default:
		throw NotImplementedException({{"catalog_type", CatalogTypeToString(entry.type)}},
		                              "Unsupported catalog type for goose_columns");
	}
}
...
static void GooseColumnsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	auto &data = data_p.global_state->Cast<GooseColumnsData>();
...
void GooseColumnsFun::RegisterFunction(BuiltinFunctions &set) {
	set.AddFunction(TableFunction("goose_columns", {}, GooseColumnsFunction, GooseColumnsBind, GooseColumnsInit));
}
