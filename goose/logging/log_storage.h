...
class LogStorage {
public:
	GOOSE_API explicit LogStorage() {
	}
	GOOSE_API virtual ~LogStorage() = default;
	...
	//! WRITING
	GOOSE_API virtual void WriteLogEntry(timestamp_t timestamp, LogLevel level, const string &log_type,
	                                      const string &log_message, const RegisteredLoggingContext &context) = 0;
	GOOSE_API virtual void WriteLogEntries(DataChunk &chunk, const RegisteredLoggingContext &context) = 0;
	GOOSE_API virtual void FlushAll() = 0;
	GOOSE_API virtual void Flush(LoggingTargetTable table) = 0;
	GOOSE_API virtual void Truncate();
	GOOSE_API virtual bool IsEnabled(LoggingTargetTable table) = 0;

	//! READING (OPTIONAL)
	GOOSE_API virtual bool CanScan(LoggingTargetTable table) {
		return false;
	}
	// Reading interface 1: basic single-threaded scan
	GOOSE_API virtual unique_ptr<LogStorageScanState> CreateScanState(LoggingTargetTable table) const;
	GOOSE_API virtual bool Scan(LogStorageScanState &state, DataChunk &result) const;
	GOOSE_API virtual void InitializeScan(LogStorageScanState &state) const;

	// Reading interface 2: using bind_replace
	GOOSE_API virtual unique_ptr<TableRef> BindReplace(ClientContext &context, TableFunctionBindInput &input,
	                                                    LoggingTargetTable table);

	//! CONFIGURATION
	GOOSE_API virtual void UpdateConfig(DatabaseInstance &db, case_insensitive_map_t<Value> &config);
};
