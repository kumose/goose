...
#include <goose/planner/expression_binder/index_binder.h>
...
#include <goose/storage/database_size.h>
...
namespace goose {
    ...
    optional_ptr<Catalog> Catalog::GetCatalogEntry(CatalogEntryRetriever &retriever, const string &catalog_name) {
        ...
        auto entry =
                db_manager.GetDatabase(context, IsInvalidCatalog(catalog_name)
                                                    ? GetDefaultCatalog(retriever)
                                                    : catalog_name);
        if (!entry) {
            return nullptr;
        }
    ...
    // Table Function
    //===--------------------------------------------------------------------===//
    optional_ptr<CatalogEntry> Catalog::CreateTableFunction(CatalogTransaction transaction,
    ...
    Catalog::CreateCopyFunction(CatalogTransaction transaction, CreateCopyFunctionInfo &info) {
        auto &schema = GetSchema(transaction, info.schema);
    ...
    unique_ptr<LogicalOperator> Catalog::BindCreateIndex(Binder &binder, CreateStatement &stmt,
        ...
        return index_binder.BindCreateIndex(binder.context, std::move(create_index_info), table, std::move(plan),
                                            nullptr);
    }
    ...
    struct CatalogLookup {
        ...
        CatalogLookup(Catalog &catalog, string schema_p, const EntryLookupInfo &lookup_p)
            : catalog(catalog), schema(std::move(schema_p)), name(lookup_p.GetEntryName()),
              lookup_info(lookup_p, name) {
        }
    ...
    // Lookup
    //===--------------------------------------------------------------------===//
    vector<SimilarCatalogEntry> Catalog::SimilarEntriesInSchemas(ClientContext &context,
    ...
    void FindMinimalQualification(CatalogEntryRetriever &retriever, const string &catalog_name,
        ...
        if (type == CatalogType::TABLE_FUNCTION_ENTRY || type == CatalogType::SCALAR_FUNCTION_ENTRY ||
            ...
            do {
                ...
                if (other_types.size() == 1) {
                    ...
                    auto error =
                            CatalogException(
                                "%s with name \"%s\" is not in the catalog, a function by this name exists "
                    ...
                    auto error =
                            CatalogException(
                                "%s with name \"%s\" is not in the catalog, functions with this name exist "
    ...
    CatalogEntryLookup Catalog::TryLookupEntry(CatalogEntryRetriever &retriever, const string &schema,
        ...
        if (!GetCatalogEntry(context, GetDefaultCatalog(retriever))) {
            auto except = CatalogException("%s with name %s does not exist!",
                                           CatalogTypeToString(lookup_info.GetCatalogType()),
    ...
    static void ThrowDefaultTableAmbiguityException(CatalogEntryLookup &base_lookup, CatalogEntryLookup &default_table,
        ...
        string fully_qualified_catalog_name_hint = StringUtil::Format(
            ": '%s.%s.%s'", default_table.schema->catalog.GetName(), default_table.schema->name,
            default_table.entry->name);
    ...
    bool Catalog::HasConflictingAttachOptions(const string &path, const AttachOptions &options) {
        auto const db_type = options.db_type.empty() ? "goose" : options.db_type;
