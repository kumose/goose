#include <goose/extension/parquet/parquet_writer.h>

#include <goose/goose.h>
#include <goose/utility/mbedtls_wrapper.h>
#include <goose/extension/parquet/parquet_crypto.h>
#include <goose/extension/parquet/parquet_decimal_utils.h>
#include <goose/extension/parquet/parquet_shredding.h>
...
#include <goose/common/types/geometry_crs.h>

namespace goose {

using namespace goose_apache::thrift;            // NOLINT
using namespace goose_apache::thrift::protocol;  // NOLINT
using namespace goose_apache::thrift::transport; // NOLINT
...
using goose_parquet::PageHeader;
using goose_parquet::PageType;
using ParquetRowGroup = goose_parquet::RowGroup;
...
bool ParquetWriter::TryGetParquetType(const LogicalType &goose_type, optional_ptr<Type::type> parquet_type_ptr) {
	Type::type parquet_type;
	switch (goose_type.id()) {
	...
	case LogicalTypeId::DECIMAL:
		switch (goose_type.InternalType()) {
...
Type::type ParquetWriter::GooseTypeToParquetType(const LogicalType &goose_type) {
	Type::type result;
	if (TryGetParquetType(goose_type, &result)) {
		return result;
	}
	throw NotImplementedException("Unimplemented type for Parquet \"%s\"", goose_type.ToString());
}

void ParquetWriter::SetSchemaProperties(const LogicalType &goose_type, goose_parquet::SchemaElement &schema_ele,
                                        bool allow_geometry) {
	if (goose_type.IsJSONType()) {
		schema_ele.converted_type = ConvertedType::JSON;
		schema_ele.__isset.converted_type = true;
		schema_ele.__isset.logicalType = true;
		schema_ele.logicalType.__set_JSON(goose_parquet::JsonType());
		return;
	}
	switch (goose_type.id()) {
	case LogicalTypeId::TINYINT:
	...
	case LogicalTypeId::TIME:
		...
		schema_ele.logicalType.TIME.isAdjustedToUTC = (goose_type.id() == LogicalTypeId::TIME_TZ);
	...
	case LogicalTypeId::TIMESTAMP_SEC:
		...
		schema_ele.logicalType.TIMESTAMP.isAdjustedToUTC = (goose_type.id() == LogicalTypeId::TIMESTAMP_TZ);
	...
	case LogicalTypeId::DECIMAL:
		...
		schema_ele.precision = DecimalType::GetWidth(goose_type);
		schema_ele.scale = DecimalType::GetScale(goose_type);
		schema_ele.__isset.converted_type = true;
		schema_ele.__isset.precision = true;
		schema_ele.__isset.scale = true;
		if (goose_type.InternalType() == PhysicalType::INT128) {
			schema_ele.type_length = 16;
			schema_ele.__isset.type_length = true;
		}
		schema_ele.__isset.logicalType = true;
	...
	case LogicalTypeId::GEOMETRY:
		if (allow_geometry) { // Don't set this if we write GeoParquet V1
			...
			if (GeoType::HasCRS(goose_type)) {
				const auto &crs = GeoType::GetCRS(goose_type);
...
uint32_t ParquetWriter::Write(const goose_apache::thrift::TBase &object) {
...
ParquetWriter::ParquetWriter(ClientContext &context, FileSystem &fs, string file_name_p, vector<LogicalType> types_p,
	...
	if (encryption_config) {
		...
		if (config.encryption_util && debug_use_openssl) {
			...
			encryption_util = make_shared_ptr<goose_mbedtls::MbedTlsWrapper::AESStateMBEDTLSFactory>();
		}
	...
	protocol = tproto_factory.getProtocol(goose_base_std::make_shared<MyTransport>(*writer));
	...
	file_meta_data.created_by =
	    StringUtil::Format("Goose version %s (build %s)", Goose::LibraryVersion(), Goose::SourceID());

	for (auto &kv_pair : kv_metadata) {
		goose_parquet::KeyValue kv;
...
void ParquetWriter::InitializeSchemaElements() {
	...
	file_meta_data.schema[0].name = "goose_schema";
	file_meta_data.schema[0].num_children = NumericCast<int32_t>(sql_types.size());
	file_meta_data.schema[0].__isset.num_children = true;
	file_meta_data.schema[0].repetition_type = goose_parquet::FieldRepetitionType::REQUIRED;
...
static void GetStatsUnifier(const ColumnWriter &column_writer, vector<unique_ptr<ColumnStatsUnifier>> &unifiers,
	...
	if (schema.repetition_type != goose_parquet::FieldRepetitionType::REPEATED) {
		if (!base_name.empty()) {
			base_name += ".";
		}
...
void ParquetWriter::FlushColumnStats(idx_t col_idx, goose_parquet::ColumnChunk &column,
...
void ParquetWriter::Finalize() {
	...
	for (auto &bloom_filter_entry : bloom_filters) {
		...
		goose_parquet::BloomFilterHeader filter_header;
		...
		filter_header.algorithm.__set_BLOCK(goose_parquet::SplitBlockAlgorithm());
		filter_header.compression.__set_UNCOMPRESSED(goose_parquet::Uncompressed());
		filter_header.hash.__set_XXHASH(goose_parquet::XxHash());
	...
	if (encryption_config) {
		...
		goose_parquet::AesGcmV1 aes_gcm_v1;
		goose_parquet::EncryptionAlgorithm alg;
