...
namespace goose {
    ...
    idx_t RadixPartitioning::Select(Vector &hashes, const SelectionVector *sel, const idx_t count,
                                    const idx_t radix_bits,
                                    const ValidityMask &partition_mask, SelectionVector *true_sel,
                                    SelectionVector *false_sel) {
        return RadixBitsSwitch<SelectFunctor, idx_t>(radix_bits, hashes, sel, count, partition_mask, true_sel,
                                                     false_sel);
    }
    ...
    RadixPartitionedColumnData::RadixPartitionedColumnData(ClientContext &context_p, vector<LogicalType> types_p,
        ...
        : PartitionedColumnData(PartitionedColumnDataType::RADIX, context_p, std::move(types_p)),
    ...
    void RadixPartitionedColumnData::ComputePartitionIndices(PartitionedColumnDataAppendState &state,
                                                             DataChunk &input) {
        ...
        RadixBitsSwitch<ComputePartitionIndicesFunctor, void>(radix_bits, input.data[hash_col_idx],
    ...
    RadixPartitionedTupleData::RadixPartitionedTupleData(BufferManager &buffer_manager,
        ...
        : PartitionedTupleData(PartitionedTupleDataType::RADIX, buffer_manager, layout_ptr, tag),
          radix_bits(radix_bits_p),
          hash_col_idx(hash_col_idx_p) {
        D_ASSERT(radix_bits <= RadixPartitioning::MAX_RADIX_BITS);
    ...
    void RadixPartitionedTupleData::ComputePartitionIndices(Vector &row_locations, idx_t count,
        ...
        partitions[0]->Gather(row_locations, *FlatVector::IncrementalSelectionVector(), count, hash_col_idx,
                              intermediate,
