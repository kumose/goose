...
namespace goose {
    ...
    public:
        StructColumnWriterState(goose_parquet::RowGroup &row_group, idx_t col_idx)
    ...
    unique_ptr<ColumnWriterState> StructColumnWriter::InitializeWriteState(goose_parquet::RowGroup &row_group) {
    ...
    void StructColumnWriter::Analyze(ColumnWriterState &state_p, ColumnWriterState *parent, Vector &vector,
                                     idx_t count) {
    ...
    void StructColumnWriter::Prepare(ColumnWriterState &state_p, ColumnWriterState *parent, Vector &vector, idx_t count,
        ...
        auto &child_vectors = StructVector::GetEntries(vector);
        for (idx_t child_idx = 0; child_idx < child_writers.size(); child_idx++) {
            child_writers[child_idx]->Prepare(*state.child_states[child_idx], &state_p, *child_vectors[child_idx],
                                              count,
    ...
    void StructColumnWriter::FinalizeSchema(vector<goose_parquet::SchemaElement> &schemas) {
