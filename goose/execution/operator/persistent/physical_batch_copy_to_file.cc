...
#include <goose/execution/operator/persistent/physical_batch_copy_to_file.h>
...
#include <goose/execution/operator/persistent/physical_copy_to_file.h>
...
namespace goose {
    ...
    public:
        ...
        void Initialize(ClientContext &context, const PhysicalBatchCopyToFile &op) {
            if (initialized) {
                return;
            }
        ...
        void AddBatchData(idx_t batch_index, unique_ptr<PreparedBatchData> new_batch, idx_t memory_usage) {
            ...
            if (!entry.second) {
                throw InternalException("Duplicate batch index %llu encountered in PhysicalFixedBatchCopy",
                                        batch_index);
            }
        }
    ...
    public:
        void Schedule() override {
            ...
            for (idx_t i = 0; i < idx_t(TaskScheduler::GetScheduler(context).NumberOfThreads()); i++) {
                auto process_task =
                        make_uniq<ProcessRemainingBatchesTask>(pipeline->executor, shared_from_this(), gstate, context,
                                                               op);
    ...
    public:
        ...
        void Execute(const PhysicalBatchCopyToFile &op, ClientContext &context, GlobalSinkState &gstate_p) override {
            ...
            auto prepared_batch =
                    op.function.prepare_batch(context, *op.bind_data, *gstate.global_state,
