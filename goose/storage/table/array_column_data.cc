#include <goose/storage/table/array_column_data.h>
#include <goose/storage/statistics/array_stats.h>
...
namespace goose {
    ...
    void ArrayColumnData::Select(TransactionData transaction, idx_t vector_index, ColumnScanState &state,
                                 Vector &result,
                                 SelectionVector &sel, idx_t sel_count) {
        bool is_supported = !child_column->type.IsNested() && child_column->type.InternalType() !=
        ...
        for (idx_t i = 0; i < sel_count; i++) {
            ...
            child_column->ScanCount(state.child_states[1], child_vec, scan_count * array_size,
    ...
    void ArrayColumnData::FetchRow(TransactionData transaction, ColumnFetchState &state,
    ...
    void ArrayColumnData::GetColumnSegmentInfo(const QueryContext &context, idx_t row_group_index,
