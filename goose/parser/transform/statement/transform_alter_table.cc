...
vector<string> Transformer::TransformNameList(cantor::PGList &list) {
...
unique_ptr<MultiStatement> TransformAndMaterializeAlter(const cantor::PGAlterTableStmt &stmt,
                                                        AlterEntryData &data,
...
unique_ptr<SQLStatement> Transformer::TransformAlter(cantor::PGAlterTableStmt &stmt) {
	...
	for (auto c = stmt.cmds->head; c != nullptr; c = c->next) {
		auto command = PGPointerCast<cantor::PGAlterTableCmd>(c->data.ptr_value);
		...
		case cantor::PG_AT_AddColumn: {
			auto column_name_list = PGPointerCast<cantor::PGList>(command->def_list->head->data.ptr_value);
			auto column_def = PGPointerCast<cantor::PGColumnDef>(command->def_list->tail->data.ptr_value);
			if (stmt.relkind != cantor::PG_OBJECT_TABLE) {
				throw ParserException("Adding columns is only supported for tables");
			}
			if (column_def->category == cantor::COL_GENERATED) {
		...
		case cantor::PG_AT_DropColumn: {
			auto cascade = command->behavior == cantor::PG_DROP_CASCADE;
			if (stmt.relkind != cantor::PG_OBJECT_TABLE) {
		...
		case cantor::PG_AT_ColumnDefault: {
		...
		case cantor::PG_AT_AlterColumnType: {
			auto column_def = PGPointerCast<cantor::PGColumnDef>(command->def);
			auto column_entry = TransformColumnDefinition(*column_def);
		...
		case cantor::PG_AT_SetNotNull: {
			result->info = make_uniq<SetNotNullInfo>(std::move(data), command->name);
			break;
		}
		case cantor::PG_AT_DropNotNull: {
			result->info = make_uniq<DropNotNullInfo>(std::move(data), command->name);
			break;
		}
		case cantor::PG_AT_AddConstraint: {
			auto pg_constraint = PGCast<cantor::PGConstraint>(*command->def);
			if (pg_constraint.contype != cantor::PGConstrType::PG_CONSTR_PRIMARY) {
		...
		case cantor::PG_AT_SetPartitionedBy: {
