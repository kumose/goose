...
#include <goose/shell/linenoise/terminal.h>
...
namespace goose {
    ...
    size_t Linenoise::ComputeRenderWidth(const char *buf, size_t len) {
        ...
        while (cpos < len) {
            ...
            int codepoint = goose::Utf8Proc::UTF8ToCodepoint(buf + cpos, sz);
            if (codepoint < 0) {
    ...
    void Linenoise::NextPosition(const char *buf, size_t len, size_t &cpos, int &rows, int &cols, int plen) const {
        ...
        if (goose::Utf8Proc::UTF8ToCodepoint(buf + cpos, sz) < 0) {
            char_render_width = 1;
            cpos++;
        } else {
            char_render_width = (int) goose::Utf8Proc::RenderWidth(buf, len, cpos);
            cpos = goose::Utf8Proc::NextGraphemeCluster(buf, len, cpos);
        }
    ...
    size_t Linenoise::PrevChar() const {
        return goose::Utf8Proc::PreviousGraphemeCluster(buf, len, pos);
    }

    size_t Linenoise::NextChar() const {
        return goose::Utf8Proc::NextGraphemeCluster(buf, len, pos);
    }
    ...
    void Linenoise::HandleTerminalResize() {
        ...
        if (rendered_completion_lines > 0) {
            ...
            PositionToColAndRow(0, completion_text.c_str(), completion_text.size(), 0, cursor_row, cursor_col, rows,
                                cols);
    ...
    bool Linenoise::Write(int fd, const char *data, idx_t size) {
        ...
        auto unicode_text = goose_shell::ShellState::Win32Utf8ToUnicode(data);
            ...
            switch (key_press.action) {
                ...
                case ENTER: {
                    ...
                    if (Terminal::IsMultiline() && len > 0) {
                        ...
                        buf[len] = '\0';
                        if (buf[0] != '.' && !AllWhitespace(buf) && !goose_shell::ShellState::SQLIsComplete(buf)) {
