...
#include <goose/common/bignum.h>
#include <goose/common/operator/add.h>
#include <goose/common/operator/interpolate.h>
#include <goose/common/operator/multiply.h>
#include <goose/common/operator/numeric_binary_operators.h>
...
#include <goose/function/scalar/operators.h>
...
namespace goose {
    ...
    static scalar_function_t GetScalarIntegerFunction(PhysicalType type) {
        ...
        switch (type) {
            ...
            case PhysicalType::UINT128:
                ...
                break;
            default:
                throw NotImplementedException("Unimplemented type for GetScalarBinaryFunction: %s",
                                              TypeIdToString(type));
        }
        return function;
    }
    ...
    namespace {
        struct AddPropagateStatistics {
            ...
            static bool Operation(const LogicalType &type, BaseStatistics &lstats, BaseStatistics &rstats,
                                  Value &new_min,
                                  Value &new_max) {
                T min, max;
                // new min is min+min
        ...
        unique_ptr<BaseStatistics> PropagateNumericStats(ClientContext &context, FunctionStatisticsInput &input) {
            ...
            if (NumericStats::HasMinMax(lstats) && NumericStats::HasMinMax(rstats)) {
                switch (expr.return_type.InternalType()) {
                    case PhysicalType::INT8:
                        potential_overflow =
                                PROPAGATE::template Operation<int8_t, OP>(
                                    expr.return_type, lstats, rstats, new_min, new_max);
                        break;
                    case PhysicalType::INT16:
                        potential_overflow =
                                PROPAGATE::template Operation<int16_t, OP>(
                                    expr.return_type, lstats, rstats, new_min, new_max);
                        break;
                    case PhysicalType::INT32:
                        potential_overflow =
                                PROPAGATE::template Operation<int32_t, OP>(
                                    expr.return_type, lstats, rstats, new_min, new_max);
                        break;
                    case PhysicalType::INT64:
                        potential_overflow =
                                PROPAGATE::template Operation<int64_t, OP>(
                                    expr.return_type, lstats, rstats, new_min, new_max);
                        break;
                    default:
                        return nullptr;
        ...
        unique_ptr<DecimalArithmeticBindData> BindDecimalArithmetic(ClientContext &context,
            ...
            for (idx_t i = 0; i < arguments.size(); i++) {
                ...
                if (!can_convert) {
                    throw InternalException("Could not convert type %s to a decimal.",
                                            arguments[i]->return_type.ToString());
                }
            ...
            for (idx_t i = 0; i < arguments.size(); i++) {
                ...
                if (scale == DecimalType::GetScale(result_type) && argument_type.InternalType() == result_type.
                    InternalType()) {
        ...
        unique_ptr<FunctionData> BindDecimalAddSubtract(ClientContext &context, ScalarFunction &bound_function,
            ...
            if (bind_data->check_overflow) {
                bound_function.
                        SetFunctionCallback(GetScalarBinaryFunction<OPOVERFLOWCHECK>(result_type.InternalType()));
            } else {
                bound_function.SetFunctionCallback(GetScalarBinaryFunction<OP>(result_type.InternalType()));
            }
            if (result_type.InternalType() != PhysicalType::INT128 && result_type.InternalType() !=
        ...
        template<class OP, class OPOVERFLOWCHECK, bool IS_SUBTRACT = false>
        unique_ptr<FunctionData> DeserializeDecimalArithmetic(Deserializer &deserializer,
                                                              ScalarFunction &bound_function) {
            ...
            if (check_overflow) {
                bound_function.
                        SetFunctionCallback(GetScalarBinaryFunction<OPOVERFLOWCHECK>(return_type.InternalType()));
            } else {
    ...
    ScalarFunction AddFunction::GetFunction(const LogicalType &left_type, const LogicalType &right_type) {
        ...
        switch (left_type.id()) {
            ...
            case LogicalTypeId::DATE:
                if (right_type.id() == LogicalTypeId::INTEGER) {
                    ...
                    ScalarFunction function("+", {left_type, right_type}, LogicalType::TIMESTAMP,
                                            ScalarFunction::BinaryFunction<date_t, interval_t, timestamp_t,
                                                AddOperator>);
                    function.SetFallible();
                    return function;
                    ...
                    ScalarFunction function("+", {left_type, right_type}, LogicalType::TIMESTAMP_TZ,
                                            ScalarFunction::BinaryFunction<date_t, dtime_tz_t, timestamp_t,
                                                AddOperator>);
                    function.SetFallible();
                    return function;
                }
            ...
            case LogicalTypeId::INTERVAL:
                if (right_type.id() == LogicalTypeId::INTERVAL) {
                    ScalarFunction function("+", {left_type, right_type}, LogicalType::INTERVAL,
                                            ScalarFunction::BinaryFunction<interval_t, interval_t, interval_t,
                                                AddOperator>);
                    function.SetFallible();
                    return function;
                } else if (right_type.id() == LogicalTypeId::DATE) {
                    ScalarFunction function("+", {left_type, right_type}, LogicalType::TIMESTAMP,
                                            ScalarFunction::BinaryFunction<interval_t, date_t, timestamp_t,
                                                AddOperator>);
                    function.SetFallible();
                    return function;
                } else if (right_type.id() == LogicalTypeId::TIME) {
                    ScalarFunction function("+", {left_type, right_type}, LogicalType::TIME,
                                            ScalarFunction::BinaryFunction<interval_t, dtime_t, dtime_t,
                                                AddTimeOperator>);
                    function.SetFallible();
                    return function;
                } else if (right_type.id() == LogicalTypeId::TIME_TZ) {
                    ...
                    ScalarFunction function("+", {left_type, right_type}, LogicalType::TIMESTAMP,
                                            ScalarFunction::BinaryFunction<interval_t, timestamp_t, timestamp_t,
                                                AddOperator>);
                    function.SetFallible();
                    return function;
                }
            ...
            case LogicalTypeId::TIME:
                if (right_type.id() == LogicalTypeId::INTERVAL) {
                    ScalarFunction function("+", {left_type, right_type}, LogicalType::TIME,
                                            ScalarFunction::BinaryFunction<dtime_t, interval_t, dtime_t,
                                                AddTimeOperator>);
                    function.SetFallible();
                    return function;
                } else if (right_type.id() == LogicalTypeId::DATE) {
            ...
            case LogicalTypeId::TIME_TZ:
                if (right_type.id() == LogicalTypeId::DATE) {
                    ScalarFunction function("+", {left_type, right_type}, LogicalType::TIMESTAMP_TZ,
                                            ScalarFunction::BinaryFunction<dtime_tz_t, date_t, timestamp_t,
                                                AddOperator>);
                    function.SetFallible();
                    return function;
                } else if (right_type.id() == LogicalTypeId::INTERVAL) {
            ...
            case LogicalTypeId::TIMESTAMP:
                if (right_type.id() == LogicalTypeId::INTERVAL) {
                    ScalarFunction function("+", {left_type, right_type}, LogicalType::TIMESTAMP,
                                            ScalarFunction::BinaryFunction<timestamp_t, interval_t, timestamp_t,
                                                AddOperator>);
                    function.SetFallible();
                    return function;
                }
        ...
        unique_ptr<BaseStatistics> NegateBindStatistics(ClientContext &context, FunctionStatisticsInput &input) {
            ...
            if (NumericStats::HasMinMax(istats)) {
                switch (expr.return_type.InternalType()) {
                    case PhysicalType::INT8:
                        potential_overflow =
                                NegatePropagateStatistics::Operation<
                                    int8_t>(expr.return_type, istats, new_min, new_max);
                        break;
                    case PhysicalType::INT16:
                        potential_overflow =
                                NegatePropagateStatistics::Operation<int16_t>(
                                    expr.return_type, istats, new_min, new_max);
                        break;
                    case PhysicalType::INT32:
                        potential_overflow =
                                NegatePropagateStatistics::Operation<int32_t>(
                                    expr.return_type, istats, new_min, new_max);
                        break;
                    case PhysicalType::INT64:
                        potential_overflow =
                                NegatePropagateStatistics::Operation<int64_t>(
                                    expr.return_type, istats, new_min, new_max);
                        break;
                    default:
                        return nullptr;
    ...
    namespace {
        ...
        unique_ptr<FunctionData> BindDecimalMultiply(ClientContext &context, ScalarFunction &bound_function,
            ...
            if (bind_data->check_overflow) {
                ...
                bound_function.SetFunctionCallback(
                    GetScalarBinaryFunction<MultiplyOperator>(result_type.InternalType()));
            }
    ...
    namespace {
            ...
            switch (type) {
                ...
                case PhysicalType::INT128:
                    return BinaryScalarFunctionIgnoreZero<hugeint_t, hugeint_t, hugeint_t, OP,
                        BinaryNumericDivideHugeintWrapper>;
                case PhysicalType::UINT128:
        ...
        unique_ptr<FunctionData> BindBinaryFloatingPoint(ClientContext &context, ScalarFunction &bound_function,
            ...
            if (DBConfig::GetSetting<IeeeFloatingPointOpsSetting>(context)) {
                bound_function.SetFunctionCallback(
                    GetScalarBinaryFunction<OP>(bound_function.GetReturnType().InternalType()));
            } else {
                bound_function.SetFunctionCallback(
    ...
    ScalarFunctionSet OperatorModuloFun::GetFunctions() {
        ...
        for (auto &type: LogicalType::Numeric()) {
            if (type.id() == LogicalTypeId::FLOAT || type.id() == LogicalTypeId::DOUBLE) {
                modulo.AddFunction(ScalarFunction({type, type}, type, nullptr,
                                                  BindBinaryFloatingPoint<ModuloOperator>));
            } else if (type.id() == LogicalTypeId::DECIMAL) {
                modulo.AddFunction(ScalarFunction({type, type}, type, nullptr, BindDecimalModulo<ModuloOperator>));
            } else {
                modulo.AddFunction(
                    ScalarFunction({type, type}, type,
                                   GetBinaryFunctionIgnoreZero<ModuloOperator>(type.InternalType())));
            }
        }
