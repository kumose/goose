...
#include <goose/common/arrow/arrow_buffer.h>
...
#include <goose/common/arrow/appender/list.h>
...
void ArrowAppender::Append(DataChunk &input, const idx_t from, const idx_t to, const idx_t input_size) {
	...
	for (idx_t i = 0; i < input.ColumnCount(); i++) {
		if (root_data[i]->extension_data && root_data[i]->extension_data->goose_to_arrow) {
			Vector input_data(root_data[i]->extension_data->GetInternalType());
			root_data[i]->extension_data->goose_to_arrow(*options.client_context, input.data[i], input_data,
			                                              input_size);
...
static void InitializeFunctionPointers(ArrowAppendData &append_data, const LogicalType &type) {
	...
	case LogicalTypeId::MAP:
		...
		break;
	default:
		throw NotImplementedException("Unsupported type in Goose -> Arrow Conversion: %s\n", type.ToString());
	}
}
