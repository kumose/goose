...
TEST_CASE("Test using connection after database is gone", "[api]") {
	auto db = make_uniq<Goose>(nullptr);
...
static void parallel_query_with_new_connection(Goose *db, bool *correct, size_t threadnr) {
	correct[threadnr] = true;
	for (size_t i = 0; i < 100; i++) {
		auto conn = make_uniq<Connection>(*db);
		auto result = conn->Query("SELECT * FROM integers ORDER BY i");
...
TEST_CASE("Test streaming API errors", "[api]") {
	goose::unique_ptr<QueryResult> result, result2;
	Goose db(nullptr);
	Connection con(db);
...
static void VerifyStreamResult(goose::unique_ptr<QueryResult> result) {
...
TEST_CASE("Test opening an invalid database file", "[api]") {
	...
	bool success = false;
	try {
		db = make_uniq<Goose>("goose:data/parquet-testing/blob.parquet");
		success = true;
	} catch (std::exception &ex) {
		REQUIRE(StringUtil::Contains(ex.what(), "Goose"));
	}
	REQUIRE(!success);
	try {
		db = make_uniq<Goose>("goose:data/parquet-testing/h2oai/h2oai_group_small.parquet");
		success = true;
	} catch (std::exception &ex) {
		REQUIRE(StringUtil::Contains(ex.what(), "Goose"));
	}
...
TEST_CASE("Test large number of connections to a single database", "[api]") {
	...
	goose::vector<goose::unique_ptr<Connection>> connections;
