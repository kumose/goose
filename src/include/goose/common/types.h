...
struct LogicalType {
	GOOSE_API LogicalType();
	GOOSE_API LogicalType(LogicalTypeId id); // NOLINT: Allow implicit conversion from `LogicalTypeId`
	GOOSE_API LogicalType(LogicalTypeId id, shared_ptr<ExtraTypeInfo> type_info);
	GOOSE_API LogicalType(const LogicalType &other);
	GOOSE_API LogicalType(LogicalType &&other) noexcept;
	...
	GOOSE_API bool operator==(const LogicalType &rhs) const;
	inline bool operator!=(const LogicalType &rhs) const {
		return !(*this == rhs);
	}

	GOOSE_API void Serialize(Serializer &serializer) const;
	GOOSE_API static LogicalType Deserialize(Deserializer &deserializer);
	...
	GOOSE_API bool IsIntegral() const;
	GOOSE_API bool IsFloating() const;
	GOOSE_API bool IsNumeric() const;
	GOOSE_API static bool IsNumeric(LogicalTypeId type);
	GOOSE_API bool IsTemporal() const;
	GOOSE_API hash_t Hash() const;
	GOOSE_API void SetAlias(string alias);
	GOOSE_API bool HasAlias() const;
	GOOSE_API string GetAlias() const;

	GOOSE_API bool HasExtensionInfo() const;
	GOOSE_API optional_ptr<const ExtensionTypeInfo> GetExtensionInfo() const;
	GOOSE_API optional_ptr<ExtensionTypeInfo> GetExtensionInfo();
	GOOSE_API void SetExtensionInfo(unique_ptr<ExtensionTypeInfo> info);

	//! Returns the maximum logical type when combining the two types - or throws an exception if combining is not possible
	GOOSE_API static LogicalType MaxLogicalType(ClientContext &context, const LogicalType &left, const LogicalType &right);
	GOOSE_API static bool TryGetMaxLogicalType(ClientContext &context, const LogicalType &left, const LogicalType &right, LogicalType &result);
	...
	GOOSE_API static LogicalType ForceMaxLogicalType(const LogicalType &left, const LogicalType &right);
	//! Normalize a type - removing literals
	GOOSE_API static LogicalType NormalizeType(const LogicalType &type);


	//! Gets the decimal properties of a numeric type. Fails if the type is not numeric.
	GOOSE_API bool GetDecimalProperties(uint8_t &width, uint8_t &scale) const;

	GOOSE_API void Verify() const;

	GOOSE_API bool IsSigned() const;
	GOOSE_API bool IsUnsigned() const;

	GOOSE_API bool IsValid() const;
	GOOSE_API bool IsComplete() const;
	GOOSE_API bool IsTemplated() const;
...
public:
	...
	GOOSE_API static LogicalType DECIMAL(uint8_t width, uint8_t scale);                 // NOLINT
	GOOSE_API static LogicalType VARCHAR_COLLATION(string collation);           // NOLINT
	GOOSE_API static LogicalType LIST(const LogicalType &child);                // NOLINT
	GOOSE_API static LogicalType STRUCT(child_list_t<LogicalType> children);    // NOLINT
	GOOSE_API static LogicalType AGGREGATE_STATE(aggregate_state_t state_type); // NOLINT
	GOOSE_API static LogicalType MAP(const LogicalType &child);                 // NOLINT
	GOOSE_API static LogicalType MAP(LogicalType key, LogicalType value);       // NOLINT
	GOOSE_API static LogicalType UNION(child_list_t<LogicalType> members);      // NOLINT
	GOOSE_API static LogicalType ARRAY(const LogicalType &child, optional_idx index);   // NOLINT
	GOOSE_API static LogicalType ENUM(Vector &ordered_data, idx_t size); // NOLINT
	GOOSE_API static LogicalType GEOMETRY(); // NOLINT
	GOOSE_API static LogicalType GEOMETRY(const string &crs);
	GOOSE_API static LogicalType GEOMETRY(const CoordinateReferenceSystem &crs);
	// ANY but with special rules (default is LogicalType::ANY, 5)
	GOOSE_API static LogicalType ANY_PARAMS(LogicalType target, idx_t cast_score = 5); // NOLINT
	GOOSE_API static LogicalType TEMPLATE(const string &name);							// NOLINT
	GOOSE_API static LogicalType VARIANT(); // NOLINT
	//! Integer literal of the specified value
	GOOSE_API static LogicalType INTEGER_LITERAL(const Value &constant);               // NOLINT
	// DEPRECATED - provided for backwards compatibility
	GOOSE_API static LogicalType ENUM(const string &enum_name, Vector &ordered_data, idx_t size); // NOLINT
	GOOSE_API static LogicalType USER(const string &user_type_name);                              // NOLINT
	GOOSE_API static LogicalType USER(const string &user_type_name, const vector<Value> &user_type_mods); // NOLINT
	GOOSE_API static LogicalType USER(string catalog, string schema, string name, vector<Value> user_type_mods); // NOLINT
	//! A list of all NUMERIC types (integral and floating point types)
	GOOSE_API static const vector<LogicalType> Numeric();
	//! A list of all INTEGRAL types
	GOOSE_API static const vector<LogicalType> Integral();
	//! A list of all REAL types
	GOOSE_API static const vector<LogicalType> Real();
	//! A list of ALL SQL types
	GOOSE_API static const vector<LogicalType> AllTypes();
...
public:
	...
	GOOSE_API static LogicalType JSON(); // NOLINT
	GOOSE_API bool IsJSONType() const;
};

struct DecimalType {
	GOOSE_API static uint8_t GetWidth(const LogicalType &type);
	GOOSE_API static uint8_t GetScale(const LogicalType &type);
	GOOSE_API static uint8_t MaxWidth();
};

struct StringType {
	GOOSE_API static string GetCollation(const LogicalType &type);
};

struct ListType {
	GOOSE_API static const LogicalType &GetChildType(const LogicalType &type);
};

struct UserType {
	GOOSE_API static const string &GetCatalog(const LogicalType &type);
	GOOSE_API static const string &GetSchema(const LogicalType &type);
	GOOSE_API static const string &GetTypeName(const LogicalType &type);
	GOOSE_API static const vector<Value> &GetTypeModifiers(const LogicalType &type);
	GOOSE_API static vector<Value> &GetTypeModifiers(LogicalType &type);
};

struct EnumType {
	GOOSE_API static int64_t GetPos(const LogicalType &type, const string_t &key);
	GOOSE_API static const Vector &GetValuesInsertOrder(const LogicalType &type);
	GOOSE_API static idx_t GetSize(const LogicalType &type);
	GOOSE_API static const string GetValue(const Value &val);
	GOOSE_API static PhysicalType GetPhysicalType(const LogicalType &type);
	GOOSE_API static string_t GetString(const LogicalType &type, idx_t pos);
};

struct StructType {
	GOOSE_API static const child_list_t<LogicalType> &GetChildTypes(const LogicalType &type);
	GOOSE_API static const LogicalType &GetChildType(const LogicalType &type, idx_t index);
	GOOSE_API static const string &GetChildName(const LogicalType &type, idx_t index);
	GOOSE_API static idx_t GetChildIndexUnsafe(const LogicalType &type, const string &name);
	GOOSE_API static idx_t GetChildCount(const LogicalType &type);
	GOOSE_API static bool IsUnnamed(const LogicalType &type);
};

struct MapType {
	GOOSE_API static const LogicalType &KeyType(const LogicalType &type);
	GOOSE_API static const LogicalType &ValueType(const LogicalType &type);
};

struct UnionType {
	GOOSE_API static const idx_t MAX_UNION_MEMBERS = 256;
	GOOSE_API static idx_t GetMemberCount(const LogicalType &type);
	GOOSE_API static const LogicalType &GetMemberType(const LogicalType &type, idx_t index);
	GOOSE_API static const string &GetMemberName(const LogicalType &type, idx_t index);
	GOOSE_API static const child_list_t<LogicalType> CopyMemberTypes(const LogicalType &type);
};

struct ArrayType {
	GOOSE_API static const LogicalType &GetChildType(const LogicalType &type);
	GOOSE_API static idx_t GetSize(const LogicalType &type);
	GOOSE_API static bool IsAnySize(const LogicalType &type);
	GOOSE_API static constexpr idx_t MAX_ARRAY_SIZE = 100000; // 100k for now
	//! Recursively replace all ARRAY types to LIST types within the given type
	GOOSE_API static LogicalType ConvertToList(const LogicalType &type);
};

struct AggregateStateType {
	GOOSE_API static const string GetTypeName(const LogicalType &type);
	GOOSE_API static const aggregate_state_t &GetStateType(const LogicalType &type);
};

struct AnyType {
	GOOSE_API static LogicalType GetTargetType(const LogicalType &type);
	GOOSE_API static idx_t GetCastScore(const LogicalType &type);
};

struct IntegerLiteral {
	//! Returns the type that this integer literal "prefers"
	GOOSE_API static LogicalType GetType(const LogicalType &type);
	//! Whether or not the integer literal fits into the target numeric type
	GOOSE_API static bool FitsInType(const LogicalType &type, const LogicalType &target);
};

struct TemplateType {
	// Get the name of the template type
	GOOSE_API static const string &GetName(const LogicalType &type);
};

struct GeoType {
	GOOSE_API static bool HasCRS(const LogicalType &type);
	GOOSE_API static const CoordinateReferenceSystem &GetCRS(const LogicalType &type);
};

// **DEPRECATED**: Use EnumUtil directly instead.
GOOSE_API string LogicalTypeIdToString(LogicalTypeId type);

GOOSE_API LogicalTypeId TransformStringToLogicalTypeId(const string &str);

GOOSE_API LogicalType TransformStringToLogicalType(const string &str);

GOOSE_API LogicalType TransformStringToLogicalType(const string &str, ClientContext &context);

//! The PhysicalType used by the row identifiers column
extern const PhysicalType ROW_TYPE;

GOOSE_API string TypeIdToString(PhysicalType type);
GOOSE_API idx_t GetTypeIdSize(PhysicalType type);
GOOSE_API bool TypeIsConstantSize(PhysicalType type);
GOOSE_API bool TypeIsIntegral(PhysicalType type);
GOOSE_API bool TypeIsNumeric(PhysicalType type);
GOOSE_API bool TypeIsInteger(PhysicalType type);
