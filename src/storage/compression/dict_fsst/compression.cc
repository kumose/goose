#include <goose/storage/compression/dict_fsst/compression.h>
...
DictFSSTCompressionState::~DictFSSTCompressionState() {
	if (encoder) {
		auto fsst_encoder = reinterpret_cast<xfsst_encoder_t *>(encoder);
		xfsst_destroy(fsst_encoder);
	}
}

static constexpr uint16_t FSST_SYMBOL_TABLE_SIZE = sizeof(xfsst_decoder_t);
...
void DictFSSTCompressionState::FlushEncodingBuffer() {
	...
	auto fsst_encoder = reinterpret_cast<xfsst_encoder_t *>(encoder);
	auto res =
	    xfsst_compress(fsst_encoder, string_count, fsst_string_sizes.data(), fsst_string_ptrs.data(),
...
bool DictFSSTCompressionState::CompressInternal(UnifiedVectorFormat &vector_format, const string_t &str, bool is_null,
	...
	case DictionaryAppendState::ENCODED_ALL_UNIQUE: {
...
#ifdef DEBUG
		auto temp_decoder = alloca(sizeof(xfsst_decoder_t));
		xfsst_import(reinterpret_cast<xfsst_decoder_t *>(temp_decoder), fsst_serialized_symbol_table.get());
		...
		if (encoded_input.data.empty()) {
			...
			// able to encode I believe 'xfsst_compress' tells us how many of the input strings it was able to
			// compress We can work backwards from there to see how many strings actually fit (probably worst case ret-1
			// ??)
			auto fsst_encoder = reinterpret_cast<xfsst_encoder_t *>(encoder);
			auto res = xfsst_compress(fsst_encoder, input_string_lengths.size(), input_string_lengths.data(),
...
DictionaryAppendState DictFSSTCompressionState::TryEncode() {
	...
	encoder = reinterpret_cast<void *>(
	    xfsst_create(string_count, fsst_string_sizes.data(), fsst_string_ptrs.data(), 0));
	auto fsst_encoder = reinterpret_cast<xfsst_encoder_t *>(encoder);
	...
	// Compress the dictionary, straight to the segment
	auto res = xfsst_compress(fsst_encoder, string_count, fsst_string_sizes.data(), fsst_string_ptrs.data(),
	...
	do {
		...
		if (!fsst_serialized_symbol_table) {
			fsst_serialized_symbol_table =
			    make_unsafe_uniq_array_uninitialized<unsigned char>(sizeof(xfsst_decoder_t));
		}
		symbol_table_size = xfsst_export(fsst_encoder, fsst_serialized_symbol_table.get());
...
#ifdef DEBUG
	auto temp_decoder = alloca(sizeof(xfsst_decoder_t));
	xfsst_import((xfsst_decoder_t *)temp_decoder, fsst_serialized_symbol_table.get());
