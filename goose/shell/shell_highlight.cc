...
namespace goose_shell {
    ...
    void ShellHighlight::PrintError(string error_msg) {
        ...
        vector<goose::SimplifiedToken> tokens;
        string error_type;
        auto error_location = goose::StringUtil::Find(error_msg, "Error: ");
        ...
        try {
            tokens = goose::Parser::TokenizeError(error_msg);
        } catch (...) {
            // fallback
        ...
        if (!tokens.empty() && tokens[0].start > 0) {
            goose::SimplifiedToken new_token;
            new_token.type = goose::SimplifiedTokenType::SIMPLIFIED_TOKEN_IDENTIFIER;
            new_token.start = 0;
        ...
        for (idx_t i = 0; i < tokens.size(); i++) {
            ...
            switch (tokens[i].type) {
                case goose::SimplifiedTokenType::SIMPLIFIED_TOKEN_IDENTIFIER:
                    break;
                case goose::SimplifiedTokenType::SIMPLIFIED_TOKEN_ERROR:
                    element_type = HighlightElementType::ERROR_TOKEN;
                    break;
                case goose::SimplifiedTokenType::SIMPLIFIED_TOKEN_ERROR_EMPHASIS:
                    element_type = HighlightElementType::ERROR_EMPHASIS;
                    break;
                case goose::SimplifiedTokenType::SIMPLIFIED_TOKEN_ERROR_SUGGESTION:
    ...
    void ShellHighlight::ToggleMode(HighlightMode mode) {
        ...
        if (mode == HighlightMode::MIXED_MODE) {
            ...
            SetColor(HighlightElementType::STRING_CONSTANT, PrintColor::YELLOW, PrintIntensity::STANDARD,
                     user_configured);
            SetColor(HighlightElementType::NUMERIC_CONSTANT, PrintColor::YELLOW, PrintIntensity::STANDARD,
                     user_configured);
            ...
            SetColor(HighlightElementType::DATABASE_NAME, PrintColor::ORANGE3, PrintIntensity::STANDARD,
                     user_configured);
        ...
        if (mode == HighlightMode::DARK_MODE) {
            ...
            SetColor(HighlightElementType::STRING_CONSTANT, PrintColor::GOLD1, PrintIntensity::STANDARD,
                     user_configured);
            ...
            SetColor(HighlightElementType::CONTINUATION_SELECTED, PrintColor::DODGERBLUE1, PrintIntensity::STANDARD,
                     user_configured);
            SetColor(HighlightElementType::ERROR_TOKEN, PrintColor::INDIANRED3, PrintIntensity::STANDARD,
                     user_configured);
            SetColor(HighlightElementType::ERROR_EMPHASIS, PrintColor::INDIANRED3, PrintIntensity::BOLD,
                     user_configured);
            SetColor(HighlightElementType::ERROR_SUGGESTION, PrintColor::INDIANRED3, PrintIntensity::BOLD,
                     user_configured);
        }
        ...
        if (mode == HighlightMode::LIGHT_MODE) {
            ...
            SetColor(HighlightElementType::STRING_CONSTANT, PrintColor::ORANGE4, PrintIntensity::STANDARD,
                     user_configured);
    ...
    bool ShellHighlight::SetColor(const char *element_type, const char *color, const char *intensity) {
        ...
        if (intensity) {
            if (goose::StringUtil::CIEquals(intensity, "standard")) {
                print_intensity = PrintIntensity::STANDARD;
            } else if (goose::StringUtil::CIEquals(intensity, "bold")) {
                print_intensity = PrintIntensity::BOLD;
            } else if (goose::StringUtil::CIEquals(intensity, "underline")) {
                print_intensity = PrintIntensity::UNDERLINE;
            } else if (goose::StringUtil::CIEquals(intensity, "bold_underline")) {
                print_intensity = PrintIntensity::BOLD_UNDERLINE;
            } else {
                state.Print(PrintOutput::STDERR,
                            goose::StringUtil::Format(
    ...
    static const HighlightColorInfo highlight_color_info[] = {
