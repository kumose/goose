...
namespace goose {
    ...
    unique_ptr<AlterInfo> PEGTransformerFactory::TransformSetSequenceOption(PEGTransformer &transformer,
        ...
        for (auto option: option_list) {
            ...
            if (seq_option.first == "owned") {
                ...
                auto schema = owned_by->qualified_name.schema.empty()
                                  ? DEFAULT_SCHEMA
    ...
    unique_ptr<AlterTableInfo> PEGTransformerFactory::TransformAddColumn(PEGTransformer &transformer,
        ...
        if (new_column.column_path.size() == 1) {
            ...
            result = make_uniq<AddFieldInfo>(AlterEntryData(), parent_path, std::move(column_definition),
                                             if_not_exists);
        }
        return result;
    }
    ...
    unique_ptr<AlterTableInfo> PEGTransformerFactory::TransformAlterColumn(PEGTransformer &transformer,
                                                                           optional_ptr<ParseResult> parse_result) {
        auto &list_pr = parse_result->Cast<ListParseResult>();
        auto nested_column_name = transformer.Transform<unique_ptr<ColumnRefExpression> >(
            list_pr.Child<ListParseResult>(2));
    ...
    string PEGTransformerFactory::TransformDropOrSet(PEGTransformer &transformer,
                                                     optional_ptr<ParseResult> parse_result) {
        auto &list_pr = parse_result->Cast<ListParseResult>();
