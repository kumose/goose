...
#include <goose/shell/shell_highlight.h>
...
namespace goose_shell {
    ...
    void PrintStream::RenderAlignedValue(const char *str, idx_t str_len, idx_t width, TextAlignment alignment) {
        ...
        Print(goose::string_t(str, str_len));
        if (rspace > 0) {
            Print(string(rspace, ' '));
        }
    }
    ...
    void PrintStream::RenderAlignedValue(goose::string_t str, idx_t width, TextAlignment alignment) {
    ...
    public:
        ...
        void AssignData() {
            ...
            for (idx_t c = 0; c < column_count; c++) {
                ...
                if (goose::FlatVector::IsNull(vector, row_in_chunk)) {
                    row_data.data[c] = goose::string_t(result->renderer.NullValue());
                    row_data.is_null[c] = true;
                } else {
                    row_data.data[c] = goose::FlatVector::GetData<goose::string_t>(chunk.data[c])[row_in_chunk];
    ...
    RenderingQueryResult::RenderingQueryResult(goose::QueryResult &result, ShellRenderer &renderer)
    ...
    SuccessState ShellState::RenderQueryResult(ShellRenderer &renderer, goose::QueryResult &query_result,
    ...
    // Result Metadata
    //===--------------------------------------------------------------------===//
    string GetTypeName(goose::LogicalType &type) {
        switch (type.id()) {
            case goose::LogicalTypeId::BOOLEAN:
                return "BOOLEAN";
            case goose::LogicalTypeId::TINYINT:
                return "TINYINT";
            case goose::LogicalTypeId::SMALLINT:
                return "SMALLINT";
            case goose::LogicalTypeId::INTEGER:
                return "INTEGER";
            case goose::LogicalTypeId::BIGINT:
                return "BIGINT";
            case goose::LogicalTypeId::FLOAT:
                return "FLOAT";
            case goose::LogicalTypeId::DOUBLE:
                return "DOUBLE";
            case goose::LogicalTypeId::DECIMAL:
                return "DECIMAL";
            case goose::LogicalTypeId::DATE:
                return "DATE";
            case goose::LogicalTypeId::TIME:
                return "TIME";
            case goose::LogicalTypeId::TIMESTAMP:
            case goose::LogicalTypeId::TIMESTAMP_NS:
            case goose::LogicalTypeId::TIMESTAMP_MS:
            case goose::LogicalTypeId::TIMESTAMP_SEC:
                return "TIMESTAMP";
            case goose::LogicalTypeId::VARCHAR:
                return "VARCHAR";
            case goose::LogicalTypeId::LIST:
                return "LIST";
            case goose::LogicalTypeId::MAP:
                return "MAP";
            case goose::LogicalTypeId::STRUCT:
                return "STRUCT";
            case goose::LogicalTypeId::BLOB:
                return "BLOB";
            default:
                return "NULL";
        }
    ...
    ResultMetadata::ResultMetadata(goose::QueryResult &result) {
    ...
    unique_ptr<goose::DataChunk> ShellRenderer::ConvertChunk(goose::DataChunk &chunk) {
        // cast to the varchar chunk
        auto varchar_chunk = make_uniq<goose::DataChunk>();
        vector<goose::LogicalType> all_varchar;
        for (idx_t c = 0; c < chunk.ColumnCount(); c++) {
            all_varchar.emplace_back(goose::LogicalType(goose::LogicalTypeId::VARCHAR));
        }
        varchar_chunk->Initialize(goose::Allocator::DefaultAllocator(), all_varchar);

        for (idx_t c = 0; c < chunk.ColumnCount(); c++) {
            goose::VectorOperations::Cast(*state.conn->context, chunk.data[c], varchar_chunk->data[c], chunk.size());
        }
    ...
    bool RenderingQueryResult::TryConvertChunk() {
        ...
        if (renderer.HasConvertValue()) {
            for (idx_t c = 0; c < result.ColumnCount(); c++) {
                ...
                auto strings = goose::FlatVector::GetData<goose::string_t>(str_vec);
                for (idx_t r = 0; r < varchar_chunk->size(); r++) {
                    if (goose::FlatVector::IsNull(str_vec, r)) {
                        continue;
                    }
                    ...
                    strings[r] = goose::StringVector::AddStringOrBlob(str_vec, renderer.ConvertValue(val, str_len));
                }
            }
        }
    ...
    void ColumnRenderer::Analyze(RenderingQueryResult &result) {
        ...
        for (idx_t column_idx = 0; column_idx < result.ColumnCount(); column_idx++) {
            for (auto &chunk: result.chunks) {
                ...
                auto string_data = goose::FlatVector::GetData<goose::string_t>(vector);
                for (idx_t r = 0; r < chunk->size(); r++) {
                    idx_t width;
                    if (goose::FlatVector::IsNull(vector, r)) {
    ...
    public:
        ...
        void PrintMarkdownSeparator(PrintStream &out, idx_t nArg, const char *zSep,
                                    const vector<goose::LogicalType> &colTypes, const vector<idx_t> &actualWidth) {
            if (nArg > 0) {
...
#define BOX_1234 "\342\224\274" /* U+253c -|- */
    ...
    public:
        ...
        void Print(goose::string_t str) override {
    ...
    public:
        ...
        void RenderRow(PrintStream &out, ResultMetadata &result, RowData &row) override {
            ...
            if (goose::StringUtil::Equals(data[0], "logical_plan") || goose::StringUtil::Equals(data[0], "logical_opt")
                ||
                goose::StringUtil::Equals(data[0], "physical_plan")) {
                out.Print("\n┌─────────────────────────────┐\n");
                out.Print("│┌───────────────────────────┐│\n");
                if (goose::StringUtil::Equals(data[0], "logical_plan")) {
                    out.Print("││ Unoptimized Logical Plan  ││\n");
                } else if (goose::StringUtil::Equals(data[0], "logical_opt")) {
                    out.Print("││  Optimized Logical Plan   ││\n");
                } else if (goose::StringUtil::Equals(data[0], "physical_plan")) {
        ...
        bool ShouldUsePager(RenderingQueryResult &result, PagerMode global_mode) override {
            ...
            for (auto &chunk: result.chunks) {
                ...
                auto string_data = goose::FlatVector::GetData<goose::string_t>(plan_vector);
                for (idx_t r = 0; r < chunk->size(); r++) {
                    if (goose::FlatVector::IsNull(plan_vector, r)) {
                        continue;
                    }
        ...
        void PrintEscapedCSV(PrintStream &out, const char *str, idx_t str_len) {
            ...
            for (idx_t idx = 0; idx < str_len; idx++) {
                ...
                if (idx + col_sep_size <= str_len) {
                    if (memcmp((const void *) (str + idx), (const void *) state.colSeparator.c_str(),
                               col_sep_size) == 0) {
                        needs_quote = true;
                        break;
                    }
            ...
            if (!needs_quote) {
                out.Print(goose::string_t(str, str_len));
                return;
            }
    ...
    public:
        ...
        void RenderRow(PrintStream &out, ResultMetadata &result, RowData &row) override {
            ...
            for (idx_t i = 0; i < data.size(); i++) {
                ...
                if (types[i].IsNumeric() || is_null[i] || types[i].id() == goose::LogicalTypeId::BOOLEAN) {
                    out.Print(data[i]);
                } else {
    ...
    public:
        ...
        bool RequiresQuotes(const goose::LogicalType &type) {
            if (!type.IsNumeric()) {
                return true;
            }
            switch (type.id()) {
                case goose::LogicalTypeId::UBIGINT:
                case goose::LogicalTypeId::HUGEINT:
                case goose::LogicalTypeId::UHUGEINT:
                case goose::LogicalTypeId::DECIMAL:
                    return true;
                default:
                    return false;
        ...
        static bool RequiresJSONCast(const goose::LogicalType &type) {
            if (type.IsNested()) {
        ...
        unique_ptr<goose::DataChunk> ConvertChunk(goose::DataChunk &chunk) override {
            // convert all nested types to JSON directly
            goose::DataChunk json_chunk;
            vector<goose::LogicalType> all_json;
            for (idx_t c = 0; c < chunk.ColumnCount(); c++) {
                if (!RequiresJSONCast(chunk.data[c].GetType())) {
                    all_json.emplace_back(goose::LogicalType(goose::LogicalTypeId::VARCHAR));
                } else {
                    all_json.emplace_back(goose::LogicalType::JSON());
                }
            }
            json_chunk.Initialize(goose::Allocator::DefaultAllocator(), all_json);

            for (idx_t c = 0; c < chunk.ColumnCount(); c++) {
                goose::VectorOperations::Cast(*state.conn->context, chunk.data[c], json_chunk.data[c], chunk.size());
            }
    ...
    public:
        ...
        static bool IsSpace(char c) {
            return goose::StringUtil::CharacterIsSpace(c);
        }
        ...
        void RenderRow(PrintStream &out, ResultMetadata &result, RowData &row) override {
            ...
            char c;
            int nLine = 0;
            if (goose::StringUtil::StartsWith(data[0].GetString(), "CREATE VIEW") ||
                goose::StringUtil::StartsWith(data[0].GetString(), "CREATE TRIG")) {
                out.Print(data[0]);
                out.Print(";\n");
                return;
            }
    ...
    // DuckBox Renderer
    //===--------------------------------------------------------------------===//
    class DuckBoxRenderer : public goose::BaseResultRenderer {
    public:
        ...
        void RenderValue(const string &text, const goose::LogicalType &type) override {
            if (type.IsNumeric()) {
        ...
        void RenderStringLiteral(const string &text, const goose::LogicalType &type) override {
            PrintText(text, HighlightElementType::STRING_CONSTANT);
        }

        void RenderNull(const string &text, const goose::LogicalType &type) override {
    ...
    private:
        goose::BoxRendererConfig config;
        unique_ptr<goose::BoxRendererState> render_state;
        string error_str;
    };
    ...
    ModeDuckBoxRenderer::ModeDuckBoxRenderer(ShellState &state) : ShellRenderer(state) {
        ...
        if (state.columns) {
            config.render_mode = goose::RenderMode::COLUMNS;
        }
        config.decimal_separator = state.decimal_separator;
        config.thousand_separator = state.thousand_separator;
        config.large_number_rendering = static_cast<goose::LargeNumberRendering>(static_cast<int>(large_rendering));
    }
    ...
    void ModeDuckBoxRenderer::Analyze(RenderingQueryResult &result) {
        goose::BoxRenderer renderer(config);
        auto &query_result = result.result;
        auto &materialized = query_result.Cast<goose::MaterializedQueryResult>();
        auto &con = *state.conn;
        try {
    ...
    SuccessState ModeDuckBoxRenderer::RenderQueryResult(PrintStream &out, ShellState &state,
                                                        RenderingQueryResult &result) {
        ...
        try {
            ...
            string error_str = goose::ErrorData(ex).Message() + "\n";
    ...
    SuccessState
    ModeDescribeRenderer::RenderQueryResult(PrintStream &out, ShellState &state, RenderingQueryResult &res) {
    ...
    void ShellLogStorage::WriteLogEntry(goose::timestamp_t timestamp, goose::LogLevel level, const string &log_type,
                                        const string &log_message, const goose::RegisteredLoggingContext &context) {
        HighlightElementType element_type;
        switch (level) {
            case (goose::LogLevel::LOG_TRACE):
                element_type = HighlightElementType::LOG_TRACE;
                break;
            case (goose::LogLevel::LOG_DEBUG):
                element_type = HighlightElementType::LOG_DEBUG;
                break;
            case (goose::LogLevel::LOG_INFO):
                element_type = HighlightElementType::LOG_INFO;
                break;
            case (goose::LogLevel::LOG_WARNING):
                element_type = HighlightElementType::LOG_WARNING;
                break;
            case (goose::LogLevel::LOG_ERROR):
            case (goose::LogLevel::LOG_FATAL):
        ...
        const auto log_level = goose::EnumUtil::ToString(level);
