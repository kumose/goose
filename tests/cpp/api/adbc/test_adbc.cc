...
using goose_adbc::ConnectionRelease;
using goose_adbc::DatabaseInit;
using goose_adbc::DatabaseRelease;
using goose_adbc::InitializeADBCError;
using goose_adbc::StatementBindStream;
using goose_adbc::StatementExecuteQuery;
using goose_adbc::StatementSetOption;

bool SUCCESS(AdbcStatusCode status) {
	return status == ADBC_STATUS_OK;
}

const char *goose_lib = std::getenv("GOOSE_INSTALL_LIB");
...
public:
	explicit ADBCTestDatabase(const string &path_parameter = ":memory:") {
		...
		REQUIRE(SUCCESS(AdbcDatabaseSetOption(&adbc_database, "driver", goose_lib, &adbc_error)));
		REQUIRE(SUCCESS(AdbcDatabaseSetOption(&adbc_database, "entrypoint", "goose_adbc_init", &adbc_error)));
	...
	bool QueryAndCheck(const string &query) {
		QueryArrow(query);
		auto conn_wrapper = static_cast<GooseAdbcConnectionWrapper *>(adbc_connection.private_data);
...
TEST_CASE("ADBC - Test Ingestion - Funky identifiers", "[adbc]") {
	...
	ArrowSchema arrow_schema;
	goose_nanoarrow::ArrowSchemaInit(&arrow_schema, goose_nanoarrow::ArrowType::NANOARROW_TYPE_STRUCT);
	goose_nanoarrow::ArrowSchemaAllocateChildren(&arrow_schema, static_cast<int64_t>(column_names.size()));

	// Create the child schemas and build the select query to get test data
	std::string query = "SELECT ";
	for (size_t i = 0; i < column_names.size(); i++) {
		goose_nanoarrow::ArrowSchemaInit(arrow_schema.children[i], goose_nanoarrow::ArrowType::NANOARROW_TYPE_INT32);
		goose_nanoarrow::ArrowSchemaSetName(arrow_schema.children[i], column_names.at(i).c_str());
		if (i > 0) {
			query += ",";
		}
...
TEST_CASE("Test Not-Implemented Partition Functions", "[adbc]") {
	...
	REQUIRE((std::strcmp(adbc_error.message, "Read Partitions are not supported in Goose") == 0));
	adbc_error.release(&adbc_error);
	...
	REQUIRE((status == ADBC_STATUS_NOT_IMPLEMENTED));
	REQUIRE((std::strcmp(adbc_error.message, "Execute Partitions are not supported in Goose") == 0));
	adbc_error.release(&adbc_error);
	REQUIRE(SUCCESS(AdbcStatementRelease(&adbc_statement, &adbc_error)));
	REQUIRE(SUCCESS(AdbcConnectionRelease(&adbc_connection, &adbc_error)));
	REQUIRE(SUCCESS(AdbcDatabaseRelease(&adbc_database, &adbc_error)));
...
TEST_CASE("Test ADBC ConnectionGetTableSchema", "[adbc]") {
	...
	REQUIRE(SUCCESS(
	    AdbcConnectionGetTableSchema(&adbc_connection, nullptr, "main", "goose_indexes", &arrow_schema, &adbc_error)));
	REQUIRE((arrow_schema.n_children == 14));
	arrow_schema.release(&arrow_schema);

	// Test Catalog Name
	REQUIRE(!SUCCESS(
	    AdbcConnectionGetTableSchema(&adbc_connection, "bla", "main", "goose_indexes", &arrow_schema, &adbc_error)));
	REQUIRE(StringUtil::Contains(adbc_error.message, "Catalog \"bla\" does not exist"));
	adbc_error.release(&adbc_error);

	REQUIRE(SUCCESS(AdbcConnectionGetTableSchema(&adbc_connection, "system", "main", "goose_indexes", &arrow_schema,
	                                             &adbc_error)));
	REQUIRE((arrow_schema.n_children == 14));
	arrow_schema.release(&arrow_schema);
	...
	REQUIRE(SUCCESS(
	    AdbcConnectionGetTableSchema(&adbc_connection, nullptr, "", "goose_indexes", &arrow_schema, &adbc_error)));
	REQUIRE((arrow_schema.n_children == 14));
	arrow_schema.release(&arrow_schema);
	...
	REQUIRE(!SUCCESS(
	    AdbcConnectionGetTableSchema(&adbc_connection, nullptr, "b", "goose_indexes", &arrow_schema, &adbc_error)));
	REQUIRE((std::strstr(adbc_error.message, "Catalog Error") != nullptr));
	adbc_error.release(&adbc_error);

	// Test invalid table
	REQUIRE(!SUCCESS(
	    AdbcConnectionGetTableSchema(&adbc_connection, nullptr, "", "goose_indexeeees", &arrow_schema, &adbc_error)));
	REQUIRE((std::strstr(adbc_error.message, "Catalog Error") != nullptr));
	REQUIRE(SUCCESS(AdbcConnectionRelease(&adbc_connection, &adbc_error)));
	REQUIRE(SUCCESS(AdbcDatabaseRelease(&adbc_database, &adbc_error)));
	adbc_error.release(&adbc_error);
...
TEST_CASE("Test AdbcConnectionGetObjects", "[adbc]") {
	...
	// 6. Test constraints
	//
	// Available constraints: https://pub.kumose.cc/goose/docs/sql/constraints.html
	{
...
TEST_CASE("Test ADBC URI option", "[adbc]") {
	...
	// Test URI overrides path (uri takes precedence)
	{
		...
		REQUIRE(SUCCESS(AdbcDatabaseSetOption(&adbc_database, "driver", goose_lib, &adbc_error)));
		REQUIRE(SUCCESS(AdbcDatabaseSetOption(&adbc_database, "entrypoint", "goose_adbc_init", &adbc_error)));
		REQUIRE(SUCCESS(AdbcDatabaseSetOption(&adbc_database, "path", "/invalid/path/db.gsst", &adbc_error)));
		REQUIRE(SUCCESS(AdbcDatabaseSetOption(&adbc_database, "uri", ":memory:", &adbc_error)));
		REQUIRE(SUCCESS(AdbcDatabaseInit(&adbc_database, &adbc_error)));
		REQUIRE(SUCCESS(AdbcConnectionNew(&adbc_connection, &adbc_error)));
		REQUIRE(SUCCESS(AdbcConnectionInit(&adbc_connection, &adbc_database, &adbc_error)));
	...
	// Test file://localhost/<abs path> is accepted
	{
		...
		if (!abs_path.empty() && abs_path[0] != '/') {
			abs_path = goose::FileSystem::GetWorkingDirectory() + "/" + abs_path;
		}
