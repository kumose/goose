...
#include <goose/extension/httpfs/hash_functions.h>

namespace goose {
    void sha256(const char *in, size_t in_len, hash_bytes &out) {
        goose_mbedtls::MbedTlsWrapper::ComputeSha256Hash(in, in_len, (char *) out);
    }

    void hmac256(const std::string &message, const char *secret, size_t secret_len, hash_bytes &out) {
        goose_mbedtls::MbedTlsWrapper::Hmac256(secret, secret_len, message.data(), message.size(), (char *) out);
    }
