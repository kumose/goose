...
#include <goose/common/operator/double_cast_operator.h>
...
string_t CastFromPointer::Operation(uintptr_t input, Vector &vector) {
	std::string s = goosedb_fmt::format("0x{:x}", input);
...
struct HugeIntegerCastOperation {
	...
	static bool HandleDigit(T &state, uint8_t digit) {
		if (NEGATIVE) {
			if (TURBO_UNLIKELY(state.intermediate <
	...
	static bool HandleDecimal(T &state, uint8_t digit) {
		...
		if (TURBO_UNLIKELY(state.decimal_intermediate > (NumericLimits<int64_t>::Maximum() - digit) / 10)) {
			if (!state.FlushDecimal()) {
				return false;
			}
		}
