...
#include <goose/extension/vss/hnsw/hnsw_index_physical_create.h>
...
namespace goose {
    ...
    unique_ptr<GlobalSinkState> PhysicalCreateHNSWIndex::GetGlobalSinkState(ClientContext &context) const {
        ...
        auto &db = storage.db;
        gstate->global_index =
                make_uniq<HNSWIndex>(info->index_name, constraint_type, storage_ids, table_manager, unbound_expressions,
                                     db,
            ...
            while (collection->Scan(scan_state, local_scan_state, scan_chunk)) {
                ...
                for (idx_t i = 0; i < count; i++) {
                    ...
                    if (!vec_valid || !rowid_valid) {
                        executor.PushError(
                            ErrorData(
                                "Invalid data in HNSW index construction: Cannot construct index with NULL values."));
    ...
    public:
        void Schedule() override {
            ...
            for (size_t tnum = 0; tnum < num_threads; tnum++) {
                construct_tasks.push_back(
                    make_uniq<HNSWIndexConstructTask>(shared_from_this(), context, gstate, tnum, op));
            }
        ...
        void FinishEvent() override {
            ...
            const auto index_entry = schema.CreateIndex(schema.GetCatalogTransaction(*gstate.context), info, table).
                    get();
            D_ASSERT(index_entry);
            auto &goose_index = index_entry->Cast<GooseIndexEntry>();
            goose_index.initial_index_size = gstate.global_index->Cast<BoundIndex>().GetInMemorySize();
    ...
    SinkFinalizeType PhysicalCreateHNSWIndex::Finalize(Pipeline &pipeline, Event &event, ClientContext &context,
        ...
        auto new_event = make_shared_ptr<
            HNSWIndexConstructionEvent>(*this, gstate, pipeline, *info, storage_ids, table);
