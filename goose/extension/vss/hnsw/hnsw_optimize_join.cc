...
namespace goose {
    ...
    public:
        ...
        PhysicalHNSWIndexJoin(PhysicalPlan &physical_plan, const vector<LogicalType> &types_p,
                              const idx_t estimated_cardinality, GooseTableEntry &table_p, HNSWIndex &hnsw_index_p,
                              const idx_t limit_p)
    ...
    OperatorResultType PhysicalHNSWIndexJoin::Execute(ExecutionContext &context, DataChunk &input, DataChunk &chunk,
        ...
        auto &transcation = GooseTransaction::Get(context.client, table.catalog);
    ...
    public:
        explicit LogicalHNSWIndexJoin(const idx_t table_index_p, GooseTableEntry &table_p, HNSWIndex &hnsw_index_p,
                                      const idx_t limit_p)
...
#undef MATCH_OPERATOR
        ...
        if (cp_lhs->type == LogicalOperatorType::LOGICAL_DELIM_GET && cp_rhs->type ==
        ...
        auto index_join = make_uniq<
            LogicalHNSWIndexJoin>(binder.GenerateTableIndex(), goose_table, *index_ptr, k_value);
