...
namespace goose {
    ...
    unique_ptr<ParsedExpression> Transformer::TransformSubquery(cantor::PGSubLink &root) {
        ...
        switch (root.subLinkType) {
            case cantor::PG_EXISTS_SUBLINK: {
                subquery_expr->subquery_type = SubqueryType::EXISTS;
                break;
            }
            case cantor::PG_ANY_SUBLINK:
            case cantor::PG_ALL_SUBLINK: {
                ...
                if (!root.operName) {
                    ...
                    auto operator_name =
                            string((PGPointerCast<cantor::PGValue>(root.operName->head->data.ptr_value))->val.str);
                ...
                if (root.subLinkType == cantor::PG_ALL_SUBLINK) {
                    ...
                    for (auto &order: aggr->order_bys->orders) {
                        if (order.expression->GetExpressionType() == ExpressionType::VALUE_CONSTANT) {
                            ...
                            if (constant_expr.value.DefaultTryCastAs(LogicalType::BIGINT, bigint_value, &error)) {
                                ...
                                idx_t positional_index = order_index < 0
