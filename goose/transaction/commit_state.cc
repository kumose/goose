...
namespace goose {
    ...
    // IndexDataRemover
    //===--------------------------------------------------------------------===//
    IndexDataRemover::IndexDataRemover(GooseTransaction &transaction_p, QueryContext context,
    ...
    void IndexDataRemover::Flush(DataTable &table, row_t *row_numbers, idx_t count) {
        ...
        auto active_checkpoint = transaction.GetTransactionManager().Cast<GooseTransactionManager>().
                GetActiveCheckpoint();
    ...
    // CommitState
    //===--------------------------------------------------------------------===//
    CommitState::CommitState(GooseTransaction &transaction_p, transaction_t commit_id,
    ...
    IndexRemovalType
    CommitState::GetIndexRemovalType(ActiveTransactionState transaction_state, CommitMode commit_mode) {
        ...
        switch (parent.type) {
                    ...
                    switch (parent.type) {
                        case CatalogType::TABLE_ENTRY:
                            if (!column_name.empty()) {
                                ...
                                auto &table_entry = entry.Cast<GooseTableEntry>();
                                D_ASSERT(table_entry.is_goose_table());
            ...
            case CatalogType::DELETED_ENTRY:
                switch (entry.type) {
                    ...
                    case CatalogType::INDEX_ENTRY: {
                        auto &index_entry = entry.Cast<GooseIndexEntry>();
    ...
    void CommitState::CommitEntry(UndoFlags type, data_ptr_t data) {
        switch (type) {
            case UndoFlags::CATALOG_ENTRY: {
                ...
                D_ASSERT(catalog.IsGooseCatalog());
                ...
                auto &goose_catalog = catalog.Cast<GooseCatalog>();
                lock_guard<mutex> write_lock(goose_catalog.GetWriteLock());
