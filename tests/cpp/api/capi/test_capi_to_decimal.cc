...
void CompareGooseDecimal(const goose_decimal &left, const goose_decimal &right) {
...
void TestFetchAsDecimal(CAPITester &tester, string query, string type_cast) {
	...
	goose_decimal expected_res;
	expected_res.scale = 0;
	expected_res.width = 0;
	expected_res.value.lower = 0;
	expected_res.value.upper = 0;

	auto converted_res = result->Fetch<goose_decimal>(0, 0);
	CompareGooseDecimal(expected_res, converted_res);
}

TEST_CASE("Test CAPI goose_decimal_as_properties", "[capi]") {
	CAPITester tester;
