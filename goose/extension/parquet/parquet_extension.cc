#include <goose/extension/parquet/parquet_extension.h>
...
#include <goose/extension/parquet/parquet_geometry.h>
#include <goose/extension/parquet/parquet_crypto.h>
#include <goose/extension/parquet/parquet_metadata.h>
...
#include <goose/extension/parquet/zstd_file_system.h>
#include <goose/extension/parquet/writer/primitive_column_writer.h>
#include <goose/extension/parquet/writer/variant_column_writer.h>
...
#include <goose/common/primitive_dictionary.h>
#include <goose/logging/log_manager.h>
#include <goose/main/settings.h>
#include <goose/extension/parquet/parquet_multi_file_info.h>

namespace goose {
    struct ParquetWriteBindData : public TableFunctionData {
        vector<LogicalType> sql_types;
        vector<string> column_names;
        goose_parquet::CompressionCodec::type codec = goose_parquet::CompressionCodec::SNAPPY;
        ...
        //! Defaults to 1/5th of the row group size if unset (in templated_column_writer.h)
    ...
    void ParquetWriteGlobalState::LogFlushingRowGroup(const ColumnDataCollection &buffer, const string &reason) {
        ...
        GOOSE_LOG(writer->GetContext(), PhysicalOperatorLogType, *op, "ParquetWriter", "FlushRowGroup",
    ...
    static void ParquetListCopyOptions(ClientContext &context, CopyOptionsInput &input) {
        ...
        copy_options["dictionary_compression_ratio_threshold"] = CopyOption(
            LogicalType::ANY, CopyOptionMode::WRITE_ONLY);
        copy_options["dictionary_size_limit"] = CopyOption(LogicalType::BIGINT, CopyOptionMode::WRITE_ONLY);
        copy_options["string_dictionary_page_size_limit"] =
        ...
        for (auto &option: input.info.options) {
            ...
            if (loption == "row_group_size" || loption == "chunk_size") {
                ...
                if (roption == "uncompressed") {
                    bind_data->codec = goose_parquet::CompressionCodec::UNCOMPRESSED;
                } else if (roption == "snappy") {
                    bind_data->codec = goose_parquet::CompressionCodec::SNAPPY;
                } else if (roption == "gzip") {
                    bind_data->codec = goose_parquet::CompressionCodec::GZIP;
                } else if (roption == "zstd") {
                    bind_data->codec = goose_parquet::CompressionCodec::ZSTD;
                } else if (roption == "brotli") {
                    bind_data->codec = goose_parquet::CompressionCodec::BROTLI;
                } else if (roption == "lz4" || roption == "lz4_raw") {
                    /* LZ4 is technically another compression scheme, but deprecated and arrow also uses them
                     * interchangeably */
                    bind_data->codec = goose_parquet::CompressionCodec::LZ4_RAW;
                } else {
                ...
                if (option.second[0].type().id() == LogicalTypeId::VARCHAR &&
                    ...
                    for (idx_t i = 0; i < struct_children.size(); i++) {
                        ...
                        if (it == variant_names.end()) {
                            ...
                            if (names.empty()) {
                                throw BinderException(
                                    "VARIANT by name \"%s\" specified in SHREDDING not found. There are "
                ...
                bind_data->enable_bloom_filters = BooleanValue::Get(
                ...
                if (val < ZStdFileSystem::MinimumCompressionLevel() || val >
    ...
    static unique_ptr<LocalFunctionData> ParquetWriteInitializeLocal(ExecutionContext &context,
    ...
    template<>
    const char *EnumUtil::ToChars<goose_parquet::CompressionCodec::type>(goose_parquet::CompressionCodec::type value) {
        switch (value) {
    ...
    template<>
    goose_parquet::CompressionCodec::type EnumUtil::FromString<
        goose_parquet::CompressionCodec::type>(const char *value) {
    ...
    static optional_idx SerializeCompressionLevel(const int64_t compression_level) {
        return compression_level < 0
                   ? NumericLimits<idx_t>::Maximum() - NumericCast<idx_t>(AbsValue(compression_level))
    ...
    static unique_ptr<FunctionData> ParquetCopyDeserialize(Deserializer &deserializer, CopyFunction &function) {
        ...
        data->codec = deserializer.ReadProperty<goose_parquet::CompressionCodec::type>(102, "codec");
        ...
        data->debug_use_openssl =
                deserializer.ReadPropertyWithExplicitDefault<bool>(111, "debug_use_openssl",
                                                                   default_value.debug_use_openssl);
        data->dictionary_size_limit =
                deserializer.ReadPropertyWithExplicitDefault<
                    optional_idx>(112, "dictionary_size_limit", optional_idx());
        ...
        data->geoparquet_version =
                deserializer.ReadPropertyWithExplicitDefault(116, "geoparquet_version",
                                                             default_value.geoparquet_version);
        ...
        for (auto &expr: input.select_list) {
            ...
            // If this is an EXPORT DATABASE statement, we dont want to write "lossy" types, instead cast them to VARCHAR
                ...
                auto new_type = TypeVisitor::VisitReplace(
                    type, [](const LogicalType &ty) -> LogicalType {
...
#ifdef GOOSE_BUILD_LOADABLE_EXTENSION
extern "C" {
GOOSE_CPP_EXTENSION_ENTRY(parquet, loader) { // NOLINT
