...
namespace goose {
    ...
    struct ExtensionInitResult {
        ...
        // The deserialized install from the `<ext>.goose_extension.info` file
    ...
    enum class ExtensionUpdateResultTag : uint8_t {
        ...
        NOT_INSTALLED = 3,
        // Statically loaded extensions can not be updated; they are baked into the Goose executable
        STATICALLY_LOADED = 4,
    ...
    public:
        static void LoadAllExtensions(Goose &db);
        ...
        GOOSE_API static bool TryAutoLoadExtension(DatabaseInstance &db, const string &extension_name) noexcept;

        GOOSE_API static bool TryAutoLoadExtension(ClientContext &context, const string &extension_name) noexcept;
        ...
        static string ExtensionUrlTemplate(optional_ptr<const DatabaseInstance> db,
        ...
        //! Default extensions are all extensions that Goose knows and expect to be available (both in-tree and
        //! out-of-tree)
        ...
        static string FindExtensionInEntries(const string &name, const ExtensionEntry (&entries)[N]) {
            ...
            auto it =
                    std::find_if(entries, entries + N, [&](const ExtensionEntry &element) {
    ...
    private:
        static ExtensionLoadResult LoadExtensionInternal(Goose &db, const std::string &extension, bool initial_load);
    };
} // namespace goose
