...
namespace goose {
    ...
    struct GooseFunctionsData : public GlobalTableFunctionState {
        GooseFunctionsData() : window_iterator(WindowExpression::WindowFunctions()), offset(0), offset_in_entry(0) {
        }
    ...
    static unique_ptr<FunctionData> GooseFunctionsBind(ClientContext &context, TableFunctionBindInput &input,
    ...
    unique_ptr<GlobalTableFunctionState> GooseFunctionsInit(ClientContext &context, TableFunctionInitInput &input) {
        auto result = make_uniq<GooseFunctionsData>();
    ...
    namespace {
        ...
        public:
            WindowFunctionCatalogEntry(const SchemaCatalogEntry &schema, const string &name,
                                       vector<LogicalType> arguments,
                                       LogicalType return_type)
                : CatalogEntry(CatalogType::AGGREGATE_FUNCTION_ENTRY, name, 0), schema(schema),
    ...
    static optional_idx GetFunctionDescriptionIndex(vector<FunctionDescription> &function_descriptions,
        ...
        for (idx_t descr_idx = 0; descr_idx < function_descriptions.size(); descr_idx++) {
            ...
            if (specificity_score.IsValid() &&
                (!best_specificity_score.IsValid() || specificity_score.GetIndex() < best_specificity_score.
                 GetIndex())) {
    ...
    bool ExtractFunctionData(CatalogEntry &entry, idx_t function_idx, DataChunk &output, idx_t output_offset) {
        ...
        output.SetValue(col++, output_offset,
                        function.alias_of.empty() || function.alias_of == function.name
                            ? Value()
    ...
    void ExtractWindowFunctionData(ClientContext &context, const WindowFunctionDefinition *it, DataChunk &output,
        ...
        switch (it->expression_type) {
            ...
            case ExpressionType::WINDOW_FIRST_VALUE: {
                ...
                ExtractFunctionData<WindowFunctionCatalogEntry, WindowFunctionExtractor>(
                    function, 0, output, output_offset);
                break;
            }
            case ExpressionType::WINDOW_NTH_VALUE: {
                WindowFunctionCatalogEntry function(default_schema, name,
                                                    {LogicalType::TEMPLATE("T"), LogicalType::BIGINT},
                                                    LogicalType::TEMPLATE("T"));
                ExtractFunctionData<WindowFunctionCatalogEntry, WindowFunctionExtractor>(
                    function, 0, output, output_offset);
                break;
    ...
    static bool Finished(const GooseFunctionsData &data) {
    ...
    void GooseFunctionsFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
        auto &data = data_p.global_state->Cast<GooseFunctionsData>();
        if (Finished(data)) {
        ...
        while (data.offset < data.entries.size() && count < STANDARD_VECTOR_SIZE) {
            ...
            switch (entry.type) {
                ...
                case CatalogType::PRAGMA_FUNCTION_ENTRY:
                    ...
                    break;
                default:
                    throw InternalException("FIXME: unrecognized function type in goose_functions");
            }
            if (finished) {
    ...
    void GooseFunctionsFun::RegisterFunction(BuiltinFunctions &set) {
        set.AddFunction(
            TableFunction("goose_functions", {}, GooseFunctionsFunction, GooseFunctionsBind, GooseFunctionsInit));
    }
} // namespace goose
