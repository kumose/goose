...
namespace goose {
    ...
    void BaseCSVData::Finalize() {
        ...
        if (options.dialect_options.state_machine_options.quote != options.dialect_options.state_machine_options.
            escape) {
        ...
        SubstringDetection(options.dialect_options.state_machine_options.comment.GetValue(), delimiter_string,
                           "COMMENT",
                           "DELIMITER");
        ...
        for (auto &null_str: options.null_str) {
            if (!null_str.empty()) {
                StringDetection(options.dialect_options.state_machine_options.delimiter.GetValue(), null_str,
                                "DELIMITER",
                                "NULL");
    ...
    static unique_ptr<GlobalFunctionData> WriteCSVInitializeGlobal(ClientContext &context, FunctionData &bind_data,
        ...
        auto global_data =
                make_uniq<GlobalWriteCSVData>(options, FileSystem::GetFileSystem(context), file_path,
                                              options.compression);
    ...
    struct WriteCSVBatchData : public PreparedBatchData {
        explicit WriteCSVBatchData(unique_ptr<CSVWriterState> writer_state) : writer_local_state(
