...
// Expecte use is:
//	#ifdef SOME_DEFINE
//	string str = GOOSE_QUOTE_DEFINE(SOME_DEFINE)
...
public:
	...
	GOOSE_API static bool Contains(const string &haystack, const string &needle);
	GOOSE_API static bool Contains(const string &haystack, const char &needle_char);

	//! Returns the position of needle string within the haystack
	GOOSE_API static optional_idx Find(const string &haystack, const string &needle);

	//! Returns true if the target string starts with the given prefix
	GOOSE_API static bool StartsWith(string str, string prefix);

	//! Returns true if the target string <b>ends</b> with the given suffix.
	GOOSE_API static bool EndsWith(const string &str, const string &suffix);

	//! Repeat a string multiple times
	GOOSE_API static string Repeat(const string &str, const idx_t n);

	//! Split the input string based on newline char
	GOOSE_API static vector<string> Split(const string &str, char delimiter);

	//! Split the input string, ignore delimiters within parentheses. Note: leading/trailing spaces are NOT stripped
	GOOSE_API static vector<string> SplitWithParentheses(const string &str, char delimiter = ',', char par_open = '(',
	                                                      char par_close = ')');

	//! Split the input string allong a quote. Note that any escaping is NOT supported.
	GOOSE_API static vector<string> SplitWithQuote(const string &str, char delimiter = ',', char quote = '"');

	//! Join multiple strings into one string. Components are concatenated by the given separator
	GOOSE_API static string Join(const vector<string> &input, const string &separator);
	GOOSE_API static string Join(const set<string> &input, const string &separator);

	//! Encode special URL characters in a string
	GOOSE_API static string URLEncode(const string &str, bool encode_slash = true);
	GOOSE_API static idx_t URLEncodeSize(const char *input, idx_t input_size, bool encode_slash = true);
	GOOSE_API static void URLEncodeBuffer(const char *input, idx_t input_size, char *output, bool encode_slash = true);
	//! Decode URL escape sequences (e.g. %20) in a string
	GOOSE_API static string URLDecode(const string &str, bool plus_to_space = false);
	GOOSE_API static idx_t URLDecodeSize(const char *input, idx_t input_size, bool plus_to_space = false);
	GOOSE_API static void URLDecodeBuffer(const char *input, idx_t input_size, char *output,
	                                       bool plus_to_space = false);

	//! BOM skipping (https://en.wikipedia.org/wiki/Byte_order_mark)
	GOOSE_API static void SkipBOM(const char *buffer_ptr, const idx_t &buffer_size, idx_t &buffer_pos);

	GOOSE_API static idx_t ToUnsigned(const string &str);
	...
	GOOSE_API static string BytesToHumanReadableString(idx_t bytes, idx_t multiplier = 1024);

	//! Convert a string to UPPERCASE
	GOOSE_API static string Upper(const string &str);

	//! Convert a string to lowercase
	GOOSE_API static string Lower(const string &str);

	//! Convert a string to Title Case
	GOOSE_API static string Title(const string &str);

	GOOSE_API static bool IsLower(const string &str);
	GOOSE_API static bool IsUpper(const string &str);

	//! Case insensitive hash
	GOOSE_API static uint64_t CIHash(const string &str);
	GOOSE_API static uint64_t CIHash(const char *str, idx_t size);

	//! Case insensitive equals
	GOOSE_API static bool CIEquals(const string &l1, const string &l2);

	//! Case insensitive equals (null-terminated strings)
	GOOSE_API static bool CIEquals(const char *l1, idx_t l1_size, const char *l2, idx_t l2_size);

	//! Case insensitive compare
	GOOSE_API static bool CILessThan(const string &l1, const string &l2);

	//! Case insensitive find, returns DConstants::INVALID_INDEX if not found
	GOOSE_API static idx_t CIFind(vector<string> &vec, const string &str);
	...
	GOOSE_API static vector<string> Split(const string &input, const string &split);

	//! Remove the whitespace char in the left end of the string
	GOOSE_API static void LTrim(string &str);
	//! Remove the whitespace char in the right end of the string
	GOOSE_API static void RTrim(string &str);
	//! Remove the all chars from chars_to_trim char in the right end of the string
	GOOSE_API static void RTrim(string &str, const string &chars_to_trim);
	//! Remove the whitespace char in the left and right end of the string
	GOOSE_API static void Trim(string &str);

	GOOSE_API static string Replace(string source, const string &from, const string &to);
	...
	GOOSE_API static idx_t LevenshteinDistance(const string &s1, const string &s2, idx_t not_equal_penalty = 1);

	//! Returns the similarity score between two strings (edit distance metric - lower is more similar)
	GOOSE_API static idx_t SimilarityScore(const string &s1, const string &s2);
	//! Returns a normalized similarity rating between 0.0 - 1.0 (higher is more similar)
	GOOSE_API static double SimilarityRating(const string &s1, const string &s2);
	...
	GOOSE_API static vector<string> TopNStrings(vector<pair<string, double>> scores, idx_t n = 5,
	                                             double threshold = 0.5);
	//! DEPRECATED: old TopNStrings method that uses the levenshtein distance metric instead of the normalized 0.0 - 1.0
	//! rating
	GOOSE_API static vector<string> TopNStrings(const vector<pair<string, idx_t>> &scores, idx_t n = 5,
	                                             idx_t threshold = 5);
	//! Computes the levenshtein distance of each string in strings, and compares it to target, then returns TopNStrings
	//! with the given params.
	GOOSE_API static vector<string> TopNLevenshtein(const vector<string> &strings, const string &target, idx_t n = 5,
	                                                 idx_t threshold = 5);
	...
	GOOSE_API static vector<string> TopNJaroWinkler(const vector<string> &strings, const string &target, idx_t n = 5,
	                                                 double threshold = 0.5);
	GOOSE_API static string CandidatesMessage(const vector<string> &candidates,
	...
	GOOSE_API static string CandidatesErrorMessage(const vector<string> &strings, const string &target,
	...
	GOOSE_API static unique_ptr<ComplexJSON> ParseJSONMap(const string &json, bool ignore_errors = false);

	//! JSON method that constructs a { string: value } JSON map
	//! This is the inverse of ParseJSONMap
	//! NOTE: this method is not efficient
	GOOSE_API static string ExceptionToJSONMap(ExceptionType type, const string &message,
	                                            const unordered_map<string, string> &map);

	//! Transforms an unordered map to a JSON string
	GOOSE_API static string ToJSONMap(const unordered_map<string, string> &map);
	//! Transforms an complex JSON to a JSON string
	GOOSE_API static string ToComplexJSONMap(const ComplexJSON &complex_json);

	GOOSE_API static string ValidateJSON(const char *data, const idx_t &len);

	GOOSE_API static string GetFileName(const string &file_path);
	GOOSE_API static string GetFileExtension(const string &file_name);
	GOOSE_API static string GetFileStem(const string &file_name);
	GOOSE_API static string GetFilePath(const string &file_path);
	...
	GOOSE_API static uint32_t StringToEnum(const EnumStringLiteral enum_list[], idx_t enum_count,
	                                        const char *enum_name, const char *str_value);
	GOOSE_API static const char *EnumToString(const EnumStringLiteral enum_list[], idx_t enum_count,
	                                           const char *enum_name, uint32_t enum_value);
	GOOSE_API static const uint8_t ASCII_TO_LOWER_MAP[];
	GOOSE_API static const uint8_t ASCII_TO_UPPER_MAP[];
};
