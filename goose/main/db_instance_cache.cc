...
namespace goose {
    ...
    DatabaseCacheEntry::DatabaseCacheEntry(const shared_ptr<Goose> &database_p) : database(database_p) {
    }
    ...
    shared_ptr<Goose> DBInstanceCache::GetInstanceInternal(const string &database, const DBConfig &config,
    ...
    shared_ptr<Goose> DBInstanceCache::GetInstance(const string &database, const DBConfig &config) {
        unique_lock<mutex> lock{cache_lock};
        return GetInstanceInternal(database, config, lock);
    }

    shared_ptr<Goose> DBInstanceCache::CreateInstanceInternal(const string &database, DBConfig &config,
        ...
        if (cache_instance) {
            ...
            db_instances_lock.unlock();
            db_instance = make_shared_ptr<Goose>(instance_path, &config);
    ...
    shared_ptr<Goose> DBInstanceCache::CreateInstance(const string &database, DBConfig &config, bool cache_instance,
                                                      const std::function<void(Goose &)> &on_create) {
        return CreateInstanceInternal(database, config, cache_instance, unique_lock<mutex>(cache_lock), on_create);
    }

    shared_ptr<Goose> DBInstanceCache::GetOrCreateInstance(const string &database, DBConfig &config_dict,
                                                           bool cache_instance,
                                                           const std::function<void(Goose &)> &on_create) {
