...
namespace goose {
    ...
    public:
        ...
        PartitionWriteInfo &GetPartitionWriteInfo(ExecutionContext &context, const PhysicalCopyToFile &op,
            ...
            string hive_path =
                    GetOrCreateDirectory(op.partition_columns, op.hive_file_pattern, op.names, values, trimmed_path,
                                         fs);
    ...
    PhysicalCopyToFile::PhysicalCopyToFile(PhysicalPlan &physical_plan, vector<LogicalType> types,
                                           CopyFunction function_p,
    ...
    SinkResultType PhysicalCopyToFile::Sink(ExecutionContext &context, DataChunk &chunk,
    ...
    PhysicalCopyToFile::Combine(ExecutionContext &context, OperatorSinkCombineInput &input) const {
        auto &g = input.global_state.Cast<CopyToFunctionGlobalState>();
        auto &l = input.local_state.Cast<CopyToFunctionLocalState>();
    ...
    void PhysicalCopyToFile::ReturnStatistics(DataChunk &chunk, idx_t row_idx, CopyToFileInfo &info) {
        ...
        for (auto &entry: file_stats.column_statistics) {
            ...
            auto map_value =
                    Value::MAP(LogicalType::VARCHAR, LogicalType::VARCHAR, std::move(stats_keys),
    ...
    SourceResultType PhysicalCopyToFile::GetDataInternal(ExecutionContext &context, DataChunk &chunk,
        ...
        if (return_type == CopyFunctionReturnType::WRITTEN_FILE_STATISTICS) {
            ...
            return source_state.offset < g.written_files.size()
