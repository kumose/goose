...
namespace goose_shell {
    ...
    goose::Connection &Prompt::GetConnection(ShellState &state) {
        return *state.conn;
    }

    vector<string> Prompt::GetSupportedSettings() {
        return vector<string>{
            "current_database", "current_schema", "current_database_and_schema",
    ...
    string Prompt::HandleSetting(ShellState &state, const PromptComponent &component) {
        ...
        if (component.literal == "bytes_read") {
            auto &client_data = goose::ClientData::Get(context);
        ...
        auto &current_db = goose::DatabaseManager::GetDefaultDatabase(context);
        auto &current_schema = goose::ClientData::Get(*con.context).catalog_search_path->GetDefault().schema;
