...
void BuiltinFunctions::RegisterReadFunctions() {
	...
	AddFunction(MultiFileReader::CreateFunctionSet(ReadGooseTableFunction::GetFunction()));
	auto &config = DBConfig::GetConfig(*transaction.db);
	config.replacement_scans.emplace_back(ReadCSVReplacement);
	config.replacement_scans.emplace_back(ReadGooseTableFunction::ReplacementScan);
}
