...
namespace goose {
    ...
    JSONTableInOutInitGlobal(ClientContext &, TableFunctionInitInput &input) {
        ...
        for (idx_t i = 0; i < input.column_indexes.size(); i++) {
            ...
            if (!col_idx.IsVirtualColumn()) {
                ...
                if (col_idx.GetPrimaryIndex() == VIRTUAL_COLUMN_START + JSONTableInOutGlobalState::JSON_COLUMN_OFFSET) {
                    ...
                    throw NotImplementedException("Virtual column %llu for json_each/json_tree",
    ...
    struct JSONTableInOutLocalState : LocalTableFunctionState {
        explicit JSONTableInOutLocalState(ClientContext &context)
            : json_allocator(BufferAllocator::Get(context)), alc(json_allocator.GetYYAlc()),
              len(DConstants::INVALID_INDEX),
    ...
    static unique_ptr<LocalTableFunctionState> JSONTableInOutInitLocal(ExecutionContext &context,
        ...
        void AddRow(JSONTableInOutLocalState &lstate, optional_ptr<yyjson_val> vkey, yyjson_val *val) {
            ...
            if (key.enabled) {
                if (vkey) {
                    ...
                    // Array element
                    key.data[count] =
                            StringVector::AddString(key.vector, to_string(recursion_nodes.back().child_index));
                } else {
                    // Other
    ...
    static void InitializeLocalState(JSONTableInOutLocalState &lstate, DataChunk &input, JSONTableInOutResult &result) {
        ...
        if (JSONReadFunctionData::CheckPath(path_value, lstate.path, lstate.len) ==
            JSONCommon::JSONPathType::WILDCARD) {
    ...
    static bool JSONTableInOutHandleValue(JSONTableInOutLocalState &lstate, JSONTableInOutResult &result,
                                          idx_t &child_index, size_t &idx, yyjson_val *child_key,
    ...
    TableFunction GetJSONTableInOutFunction(const LogicalType &input_type, const bool &has_path_param) {
        ...
        TableFunction function(arguments, nullptr, JSONTableInOutBind, JSONTableInOutInitGlobal,
