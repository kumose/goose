...
namespace goose {
    ...
    void CatalogEntryMap::DropEntry(CatalogEntry &entry) {
        ...
        if (!chain) {
            throw InternalException("Attempting to drop entry with name \"%s\" but no chain with that name exists",
                                    name);
        }
    ...
    CatalogSet::CatalogSet(Catalog &catalog_p, unique_ptr<DefaultGenerator> defaults)
        : catalog(catalog_p.Cast<GooseCatalog>()), defaults(std::move(defaults)) {
        D_ASSERT(catalog_p.IsGooseCatalog());
    }
    ...
    void CatalogSet::CheckCatalogEntryInvariants(CatalogEntry &value, const string &name) {
        if (value.internal && !catalog.IsSystemCatalog() && name != DEFAULT_SCHEMA) {
            throw InternalException(
                "Attempting to create internal entry \"%s\" in non-system catalog - internal entries "
    ...
    bool CatalogSet::CreateEntryInternal(CatalogTransaction transaction, const string &name,
        ...
        if (transaction.transaction) {
            GooseTransactionManager::Get(GetCatalog().GetAttached())
    ...
    bool CatalogSet::RenameEntryInternal(CatalogTransaction transaction, CatalogEntry &old, const string &new_name,
        ...
        auto renamed_tombstone = make_uniq<InCatalogEntry>(CatalogType::RENAMED_ENTRY, old.ParentCatalog(),
                                                           original_name);
    ...
    bool CatalogSet::DropDependencies(CatalogTransaction transaction, const string &name, bool cascade,
        ...
        D_ASSERT(entry->ParentCatalog().IsGooseCatalog());
        auto &goose_catalog = entry->ParentCatalog().Cast<GooseCatalog>();
        goose_catalog.GetDependencyManager()->DropObject(transaction, *entry, cascade);
        return true;
    }
    ...
    bool CatalogSet::DropEntry(CatalogTransaction transaction, const string &name, bool cascade,
                               bool allow_drop_internal) {
    ...
    void CatalogSet::VerifyExistenceOfDependency(transaction_t commit_id, CatalogEntry &entry) {
        ...
        CatalogTransaction commit_transaction(goose_catalog.GetDatabase(), transaction_id, tx_start_time);

        D_ASSERT(entry.type == CatalogType::DEPENDENCY_ENTRY);
        auto &dep = entry.Cast<DependencyEntry>();
        goose_catalog.GetDependencyManager()->VerifyExistence(commit_transaction, dep);
    }
    ...
    void CatalogSet::CommitDrop(transaction_t commit_id, transaction_t start_time, CatalogEntry &entry) {
        ...
        goose_catalog.GetDependencyManager()->VerifyCommitDrop(commit_transaction, start_time, entry);
    }

    GooseCatalog &CatalogSet::GetCatalog() {
        return catalog;
    }
    ...
    void CatalogSet::ScanWithReturn(CatalogTransaction transaction,
