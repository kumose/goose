...
public:
	GOOSE_API explicit ValidityData(idx_t count);
	GOOSE_API ValidityData(const ValidityMask &original, idx_t count);
};
...
public:
	GOOSE_API void Resize(idx_t new_size);
	GOOSE_API idx_t Capacity() const;
	GOOSE_API void SliceInPlace(const ValidityMask &other, idx_t target_offset, idx_t source_offset, idx_t count);
	GOOSE_API void Slice(const ValidityMask &other, idx_t source_offset, idx_t count);
	GOOSE_API void CopySel(const ValidityMask &other, const SelectionVector &sel, idx_t source_offset,
	                        idx_t target_offset, idx_t count);
	GOOSE_API void Combine(const ValidityMask &other, idx_t count);
	GOOSE_API string ToString(idx_t count) const;
	GOOSE_API string ToString() const;

	GOOSE_API static bool IsAligned(idx_t count);
