...
#include <goose/extension/parquet/parquet_statistics.h>
...
#include <goose/common/multi_file/multi_file_list.h>
...
namespace goose {
    ...
    public:
        ...
        virtual void ReadRow(vector<reference<Vector> > &output, idx_t output_idx, idx_t row_idx,
                             ParquetReader &reader) = 0;
    ...
    struct ParquetMetadataGlobalState : public GlobalTableFunctionState {
        ...
        double GetProgress() const {
            ...
            return static_cast<double>(file_paths->scan_data.current_file_idx) / file_paths->file_list->
                   GetTotalFileCount();
        }
    ...
    public:
        ...
        void ReadRow(vector<reference<Vector> > &output, idx_t output_idx, idx_t row_idx,
                     ParquetReader &reader) override;
    ...
    static Value ConvertParquetGeoStatsBBOX(const goose_parquet::GeospatialStatistics &stats) {
    ...
    static Value ConvertParquetGeoStatsTypes(const goose_parquet::GeospatialStatistics &stats) {
    ...
    void ParquetRowGroupMetadataProcessor::ReadRow(vector<reference<Vector> > &output, idx_t output_idx, idx_t row_idx,
        ...
        output[24].get().SetValue(output_idx,
                                  ParquetElementBigint(col_meta.bloom_filter_offset,
                                                       col_meta.__isset.bloom_filter_offset));
        // bloom_filter_length
        output[25].get().SetValue(output_idx,
                                  ParquetElementBigint(col_meta.bloom_filter_length,
                                                       col_meta.__isset.bloom_filter_length));
        // min_is_exact
    ...
    static Value ParquetLogicalTypeToString(const goose_parquet::LogicalType &type, bool is_set) {
        if (!is_set) {
            return Value();
        }
    ...
    void ParquetSchemaProcessor::ReadRow(vector<reference<Vector> > &output, idx_t output_idx, idx_t row_idx,
        ...
        // repetition_type
        output[4].get().SetValue(
            output_idx, ParquetElementString(column.repetition_type, column.__isset.repetition_type));
        // num_children
        output[5].get().SetValue(output_idx, ParquetElementBigint(column.num_children, column.__isset.num_children));
        // converted_type
        output[6].get().
                SetValue(output_idx, ParquetElementString(column.converted_type, column.__isset.converted_type));
        // scale
        ...
        // logical_type
        output[10].get().SetValue(
            output_idx, ParquetLogicalTypeToString(column.logicalType, column.__isset.logicalType));
        // goose_type
        ParquetColumnSchema column_schema;
        Value goose_type;
        if (column.__isset.type) {
            goose_type = reader.DeriveLogicalType(column, column_schema).ToString();
        }
        output[11].get().SetValue(output_idx, goose_type);
        // column_id
    ...
    template<>
    void ParquetMetaDataOperator::BindSchema<ParquetMetadataOperatorType::FILE_META_DATA>(
    ...
    void ParquetFileMetadataProcessor::ReadRow(vector<reference<Vector> > &output, idx_t output_idx, idx_t row_idx,
        ...
        output[0].get().SetValue(output_idx, Value(reader.file.path));
        // created_by
        output[1].get().SetValue(
            output_idx, ParquetElementStringVal(meta_data->created_by, meta_data->__isset.created_by));
        // num_rows
    ...
    private:
        ...
        unique_ptr<goose_apache::thrift::protocol::TCompactProtocolT<ThriftFileTransport> > protocol;
        optional_ptr<Allocator> allocator;
        unique_ptr<ConstantFilter> filter;
    };

    template<>
    void ParquetMetaDataOperator::BindSchema<ParquetMetadataOperatorType::BLOOM_PROBE>(
        vector<LogicalType> &return_types,
        vector<string> &names) {
        names.emplace_back("file_name");
        return_types.emplace_back(LogicalType::VARCHAR);
    ...
    void ParquetBloomProbeProcessor::InitializeInternal(ClientContext &context, ParquetReader &reader) {
        ...
        auto transport = goose_base_std::make_shared<ThriftFileTransport>(reader.GetHandle(), false);
        protocol = make_uniq<goose_apache::thrift::protocol::TCompactProtocolT<ThriftFileTransport> >(
            std::move(transport));
    ...
    void ParquetBloomProbeProcessor::ReadRow(vector<reference<Vector> > &output, idx_t output_idx, idx_t row_idx,
        ...
        auto bloom_excludes = ParquetStatisticsUtils::BloomFilterExcludes(
            *filter, column.meta_data, *protocol, *allocator);
    ...
    template<>
    void ParquetMetaDataOperator::BindSchema<ParquetMetadataOperatorType::FULL_METADATA>(
        vector<LogicalType> &return_types,
        vector<string> &names) {
        names.emplace_back("parquet_file_metadata");
        vector<LogicalType> file_meta_types;
        vector<string> file_meta_names;
        ParquetMetaDataOperator::BindSchema<ParquetMetadataOperatorType::FILE_META_DATA>(
            file_meta_types, file_meta_names);
    ...
    unique_ptr<LocalTableFunctionState> ParquetMetaDataOperator::InitLocal(ExecutionContext &context,
        ...
        switch (OP_TYPE) {
            ...
            case ParquetMetadataOperatorType::BLOOM_PROBE: {
                ...
                res->processor =
                        make_uniq<ParquetBloomProbeProcessor>(probe_bind_data.probe_column_name,
