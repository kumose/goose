...
//! The DeltaSnapshot implements the MultiFileList API to allow injecting it into the regular Goose parquet scan
struct DeltaSnapshot : public MultiFileList {
	DeltaSnapshot(ClientContext &context, const string &path);
	string GetPath();
	static string ToGoosePath(const string &raw_path);
...
struct DeltaMultiFileReader : public MultiFileReader {
	...
	//! Override the regular parquet bind using the MultiFileReader Bind. The bind from these are what Goose's file
