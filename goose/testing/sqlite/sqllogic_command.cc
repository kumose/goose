...
#include <goose/testing/sqlite/result_helper.h>
...
namespace goose {
    ...
    static Connection *GetConnection(SQLLogicTestRunner &runner, Goose &db,
    ...
    bool CanRestart(Connection &conn) {
        ...
        for (auto &conn_ref: connection_list) {
            ...
            auto &temp_catalog = temp->GetCatalog().Cast<GooseCatalog>();
            vector<reference<GooseSchemaEntry> > schemas;
            temp_catalog.ScanSchemas(
                [&](SchemaCatalogEntry &schema) { schemas.push_back(schema.Cast<GooseSchemaEntry>()); });
            if (schemas.size() != 1) {
                return false;
            }
            auto &temp_schema = schemas[0].get();
            vector<CatalogType> catalog_types{
                CatalogType::TABLE_ENTRY, CatalogType::VIEW_ENTRY,
    ...
    struct ParallelExecuteContext {
        ParallelExecuteContext(SQLLogicTestRunner &runner, const vector<goose::unique_ptr<Command> > &loop_commands,
        ...
        const vector<goose::unique_ptr<Command> > &loop_commands;
    ...
    static void ParallelExecuteLoop(ParallelExecuteContext *execute_context) {
        try {
            ...
            execute_context->error_message = StringUtil::Format("Failure at %s:%d: Unknown error message",
                                                                execute_context->error_file,
                                                                execute_context->error_line);
            execute_context->success = false;
        }
    }
    ...
    void RestartCommand::ExecuteInternal(ExecuteContext &context) const {
        ...
        runner.con->context->client_data->catalog_search_path->Set(catalog_search_paths,
                                                                   CatalogSetPathType::SET_SCHEMAS);
        runner.con->Commit();
    ...
    void UnzipCommand::ExecuteInternal(ExecuteContext &context) const {
        ...
        while (true) {
            goose::unique_ptr<char[]> compressed_buffer(new char[BUFFER_SIZE]);
    ...
    void LoadCommand::ExecuteInternal(ExecuteContext &context) const {
        ...
        if (runner.db) {
            if (version.empty()) {
                ...
                try {
                    runner.config->options.serialization_compatibility =
                            SerializationCompatibility::FromString(version);
                } catch (std::exception &ex) {
                    ErrorData err(ex);
