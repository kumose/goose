#include <goose/extension/autocomplete/ast/column_constraints.h>
...
namespace goose {
    ...
    CreateTableAs PEGTransformerFactory::TransformCreateTableAs(PEGTransformer &transformer,
        ...
        result.select_statement = transformer.Transform<unique_ptr<
            SelectStatement> >(list_pr.Child<ListParseResult>(2));
    ...
    ColumnList PEGTransformerFactory::TransformIdentifierList(PEGTransformer &transformer,
        ...
        for (auto identifier: identifier_list) {
            result.AddColumn(ColumnDefinition(identifier->Cast<IdentifierParseResult>().identifier,
                                              LogicalType::UNKNOWN));
        }
        return result;
    }
    ...
    string PEGTransformerFactory::TransformIdentifier(PEGTransformer &transformer,
                                                      optional_ptr<ParseResult> parse_result) {
        auto &list_pr = parse_result->Cast<ListParseResult>();
