#include <dbgen/dbgen.h>
#include <dbgen/dbgen_gunk.h>
...
namespace tpch {
    struct tpch_append_information {
        goose::unique_ptr<InternalAppender> appender;
    };
    ...
    const LogicalType RegionInfo::Types[] = {
    ...
    const LogicalType NationInfo::Types[] = {
        LogicalType(LogicalTypeId::INTEGER), LogicalType(LogicalTypeId::VARCHAR),
        LogicalType(LogicalTypeId::INTEGER), LogicalType(LogicalTypeId::VARCHAR)
    };
    ...
    const char *SupplierInfo::Columns[] = {
    ...
    const char *CustomerInfo::Columns[] = {
    ...
    const char *LineitemInfo::Columns[] = {
    ...
    public:
        TPCHDataAppender(ClientContext &context, TPCHDBgenParameters &parameters, DBGenContext base_context,
                         idx_t flush_count) : context(context), parameters(parameters) {
            dbgen_ctx = base_context;
            append_info = goose::unique_ptr<tpch_append_information[]>(new tpch_append_information[REGION + 1]);
            ...
            for (size_t i = PART; i <= REGION; i++) {
                if (parameters.tables[i]) {
                    ...
                    if (!tbl_catalog.is_goose_table()) {
                        throw InvalidInputException("dbgen is only supported for Goose database files");
                    }
    ...
    void DBGenWrapper::LoadTPCHData(ClientContext &context, double flt_scale, string catalog_name, string schema,
        ...
        if (explicit_partial_generation || thread_count <= 1) {
            ...
            while (step < child_count) {
                ...
                for (idx_t thr_idx = 0; thr_idx < thread_count && launched_step < child_count;
