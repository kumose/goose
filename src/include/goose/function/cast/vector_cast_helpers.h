...
#include <goose/common/vector_operations/general_cast.h>
...
struct VectorTryCastOperator {
	...
	static RESULT_TYPE Operation(INPUT_TYPE input, ValidityMask &mask, idx_t idx, void *dataptr) {
		RESULT_TYPE output;
		if (TURBO_LIKELY((OP::template Operation<INPUT_TYPE, RESULT_TYPE>(input, output)))) {
			return output;
		}
...
struct VectorTryCastStrictOperator {
	...
	static RESULT_TYPE Operation(INPUT_TYPE input, ValidityMask &mask, idx_t idx, void *dataptr) {
		auto data = reinterpret_cast<VectorTryCastData *>(dataptr);
		RESULT_TYPE output;
		if (TURBO_LIKELY((OP::template Operation<INPUT_TYPE, RESULT_TYPE>(input, output, data->parameters.strict)))) {
			return output;
		}
		return HandleVectorCastError::Operation<RESULT_TYPE>(CastExceptionText<INPUT_TYPE, RESULT_TYPE>(input), mask,
		                                                     idx, *data);
...
struct VectorTryCastErrorOperator {
	...
	static RESULT_TYPE Operation(INPUT_TYPE input, ValidityMask &mask, idx_t idx, void *dataptr) {
		auto data = reinterpret_cast<VectorTryCastData *>(dataptr);
		RESULT_TYPE output;
		if (TURBO_LIKELY((OP::template Operation<INPUT_TYPE, RESULT_TYPE>(input, output, data->parameters)))) {
			return output;
		}
...
struct VectorTryCastStringOperator {
	...
	static RESULT_TYPE Operation(INPUT_TYPE input, ValidityMask &mask, idx_t idx, void *dataptr) {
		auto data = reinterpret_cast<VectorTryCastData *>(dataptr);
		RESULT_TYPE output;
		if (TURBO_LIKELY((OP::template Operation<INPUT_TYPE, RESULT_TYPE>(input, output, data->result, data->parameters)))) {
			return output;
		}
		return HandleVectorCastError::Operation<RESULT_TYPE>(CastExceptionText<INPUT_TYPE, RESULT_TYPE>(input), mask,
		                                                     idx, *data);
