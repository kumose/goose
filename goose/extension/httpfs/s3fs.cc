...
#include <goose/function/scalar/string_common.h>
...
namespace goose {
	HTTPHeaders CreateS3Header(string url, string query, string host, string service, string method,
		...
		auto string_to_sign = "AWS4-HMAC-SHA256\n" + datetime_now + "\n" + date_now + "/" + auth_params.region + "/" +
		                      service + "/aws4_request\n" + string((char *) canonical_request_hash_str,
		                                                           sizeof(hash_str));
		// compute signature
	...
	void AWSEnvironmentCredentialsProvider::SetAll() {
		...
		this->SetExtensionOptionValue("s3_endpoint", GOOSE_ENDPOINT_ENV_VAR);
		this->SetExtensionOptionValue("s3_use_ssl", GOOSE_USE_SSL_ENV_VAR);
		this->SetExtensionOptionValue("s3_kms_key_id", GOOSE_KMS_KEY_ID_ENV_VAR);
		this->SetExtensionOptionValue("s3_requester_pays", GOOSE_REQUESTER_PAYS_ENV_VAR);
	}
	...
	S3AuthParams AWSEnvironmentCredentialsProvider::CreateParams() {
		...
		params.endpoint = GOOSE_ENDPOINT_ENV_VAR;
		params.kms_key_id = GOOSE_KMS_KEY_ID_ENV_VAR;
		params.use_ssl = GOOSE_USE_SSL_ENV_VAR;
		params.requester_pays = GOOSE_REQUESTER_PAYS_ENV_VAR;
	...
	void S3FileSystem::FinalizeMultipartUpload(S3FileHandle &file_handle) {
		...
		auto res =
				s3fs.PostRequest(file_handle, file_handle.path, {}, result, (char *) body.c_str(), body.length(),
				                 query_param);
	...
	void S3FileSystem::ReadQueryParams(const string &url_query_param, S3AuthParams &params) {
		...
		if (found_requester_pays_param != query_params.end()) {
			if (found_requester_pays_param->second == "true") {
				...
				throw IOException(
					"Incorrect setting found for s3_requester_pays, allowed values are: 'true' or 'false'");
			}
	...
	unique_ptr<HTTPFileHandle> S3FileSystem::CreateHandle(const OpenFileInfo &file, FileOpenFlags flags,
		...
		return goose::make_uniq<S3FileHandle>(*this, file, flags, std::move(params), auth_params,
	...
	void S3FileHandle::Initialize(optional_ptr<FileOpener> opener) {
			...
			if (error.Type() == ExceptionType::IO || error.Type() == ExceptionType::HTTP) {
				...
				if (context) {
					...
					for (const string type: {"s3", "r2", "gcs", "aws"}) {
						...
						if (res.HasMatch()) {
							refreshed_secret |=
									CreateS3SecretFunctions::TryRefreshS3Secret(*context, *res.secret_entry);
						}
					}
				}
			}
		...
		if (flags.OpenForWriting()) {
			auto aws_minimum_part_size = 5242880;
			// 5 MiB https://docs.aws.amazon.com/AmazonS3/latest/userguide/qfacts.html
	...
	void S3FileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
		...
		GOOSE_LOG_FILE_SYSTEM_WRITE(handle, bytes_written, s3fh.file_offset - bytes_written);
	}
	...
	string S3FileSystem::GetS3AuthError(S3AuthParams &s3_auth_params) {
		...
		extra_text += "\n* See https://goose.org/docs/stable/extensions/httpfs/s3api.html";
		return extra_text;
	}
	...
	HTTPException S3FileSystem::GetS3Error(S3AuthParams &s3_auth_params, const HTTPResponse &response,
	                                       const string &url) {
		string extra_text;
	...
	optional_idx FindTagContents(const string &response, const string &tag, idx_t cur_pos, string &result) {
		...
		if (close_tag_pos == string::npos) {
			throw InternalException(
				"Failed to parse S3 result: found open tag for %s but did not find matching close tag",
				tag);
		}
	...
	vector<string> AWSListObjectV2::ParseCommonPrefix(string &aws_response) {
		...
		while (true) {
			...
			if (next_open_tag_pos == string::npos) {
				...
				auto parsed_path = aws_response.substr(next_open_tag_pos + 8,
				                                       next_close_tag_pos - next_open_tag_pos - 8);
	...
	S3KeyValueReader::S3KeyValueReader(FileOpener &opener_p, optional_ptr<FileOpenerInfo> info,
	                                   const char **secret_types,
