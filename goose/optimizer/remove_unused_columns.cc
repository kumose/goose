...
#include <goose/function/scalar/struct_utils.h>
...
namespace goose {
            ...
            case LogicalOperatorType::LOGICAL_UNION: {
                ...
                if (setop.setop_all && !everything_referenced) {
                    ...
                    for (idx_t child_idx = 0; child_idx < op.children.size(); child_idx++) {
                        ...
                        auto new_projection = make_uniq<LogicalProjection>(
                            binder.GenerateTableIndex(), std::move(expressions));
            ...
            while (true) {
                ...
                if (!path_iter.get().HasChildren()) {
                    throw InternalException(
                        "This path wasn't found in the registered paths for this expression at all!?");
                }
        ...
        for (auto &col_sel_idx: col_sel) {
            ...
            WritePushdownExtractColumns(entry->first, entry->second, col_sel_idx, column_type,
                                        [&](const ColumnIndex &extract_path,
                                            optional_ptr<LogicalType> cast_type) -> idx_t {
    ...
    void BaseColumnPruner::MergeChildColumns(vector<ColumnIndex> &current_child_columns,
                                             ColumnIndex &new_child_column) {
