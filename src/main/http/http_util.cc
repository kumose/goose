...
#ifndef DISABLE_GOOSE_REMOTE_INSTALL
...
HTTPHeaders::HTTPHeaders(DatabaseInstance &db) {
	headers.insert({"User-Agent", StringUtil::Format("%s %s", db.config.UserAgent(), Goose::SourceID())});
}
...
unique_ptr<HTTPResponse> TransformResponse(goose_httplib::Result &res) {
	auto status_code = HTTPUtil::ToStatusCode(res ? res->status : 0);
	auto result = make_uniq<HTTPResponse>(status_code);
	if (res.error() == goose_httplib::Error::Success) {
...
public:
	HTTPLibClient(HTTPParams &http_params, const string &proto_host_port) {
		client = make_uniq<goose_httplib::Client>(proto_host_port);
	...
	unique_ptr<HTTPResponse> Get(GetRequestInfo &info) override {
		...
		if (!info.response_handler && !info.content_handler) {
			...
			return TransformResult(client->Get(
			    info.path, headers,
			    [&](const goose_httplib::Response &response) {
	...
	unique_ptr<goose_httplib::Client> client;

private:
	goose_httplib::Headers TransformHeaders(const HTTPHeaders &header_map, const HTTPParams &params) {
		goose_httplib::Headers headers;
	...
	unique_ptr<HTTPResponse> TransformResponse(const goose_httplib::Response &response) {
	...
	unique_ptr<HTTPResponse> TransformResult(const goose_httplib::Result &res) {
		if (res.error() == goose_httplib::Error::Success) {
			auto &response = res.value();
