...
#include <goose/function/cast_rules.h>
...
namespace goose {
    ...
    hash_t LogicalType::Hash() const {
        return goose::Hash<uint8_t>((uint8_t) id_);
    }
    ...
    LogicalType TransformStringToLogicalType(const string &str) {
        ...
        try {
            ...
            const vector<string> suggested_types{
                "BIGINT",
                "INT8",
                "LONG",
                "BIT",
            ...
            error << "Value \"" << str << "\" can not be converted to a Goose Type." << '\n';
    ...
    static bool CombineEqualTypes(const LogicalType &left, const LogicalType &right, LogicalType &result) {
        ...
        switch (type_id) {
            ...
            case LogicalTypeId::DECIMAL: {
                ...
                auto extra_width =
                        MaxValue<uint8_t>(NumericCast<uint8_t>(extra_width_left),
                                          NumericCast<uint8_t>(extra_width_right));
    ...
    void LogicalType::Verify() const {
        ...
        switch (id_) {
            case LogicalTypeId::DECIMAL:
                D_ASSERT(
                    DecimalType::GetWidth(*this) >= 1 && DecimalType::GetWidth(*this) <= Decimal::MAX_WIDTH_DECIMAL);
