...
struct FMTPrintf {
	template <class CTX>
	static string OP(const char *format_str, vector<goosedb_fmt::basic_format_arg<CTX>> &format_args) {
		return goosedb_fmt::vsprintf(
		    format_str, goosedb_fmt::basic_format_args<CTX>(format_args.data(), static_cast<int>(format_args.size())));
	}
};
...
static void PrintfFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	for (idx_t idx = 0; idx < count; idx++) {
		...
		vector<goosedb_fmt::basic_format_arg<CTX>> format_args;
		...
		for (idx_t col_idx = 1; col_idx < args.ColumnCount(); col_idx++) {
			...
			case LogicalTypeId::BOOLEAN: {
				...
				format_args.emplace_back(goosedb_fmt::internal::make_arg<CTX>(arg_data[arg_idx]));
				break;
			}
			case LogicalTypeId::TINYINT: {
			...
			case LogicalTypeId::VARCHAR: {
				...
				auto string_view =
				    goosedb_fmt::basic_string_view<char>(arg_data[arg_idx].GetData(), arg_data[arg_idx].GetSize());
				format_args.emplace_back(goosedb_fmt::internal::make_arg<CTX>(string_view));
				break;
			}
			default:
...
ScalarFunction PrintfFun::GetFunction() {
	// goosedb_fmt::printf_context, goosedb_fmt::vsprintf
	ScalarFunction printf_fun({LogicalType::VARCHAR}, LogicalType::VARCHAR,
	                          PrintfFunction<FMTPrintf, goosedb_fmt::printf_context>, BindPrintfFunction);
...
ScalarFunction FormatFun::GetFunction() {
	// goosedb_fmt::format_context, goosedb_fmt::vformat
	ScalarFunction format_fun({LogicalType::VARCHAR}, LogicalType::VARCHAR,
	                          PrintfFunction<FMTFormat, goosedb_fmt::format_context>, BindPrintfFunction);
