...
public:
	...
	GOOSE_API static void ToString(bitstring_t bits, char *output);
	GOOSE_API static string ToString(bitstring_t bits);
	//! Returns the bit size of a string -> bit conversion
	GOOSE_API static bool TryGetBitStringSize(string_t str, idx_t &result_size, string *error_message);
	//! Convert a string to a bit. This function should ONLY be called after calling GetBitSize, since it does NOT
	//! perform data validation.
	GOOSE_API static void ToBit(string_t str, bitstring_t &output);

	GOOSE_API static string ToBit(string_t str);

	//! output needs to have enough space allocated before calling this function (blob size + 1)
	GOOSE_API static void BlobToBit(string_t blob, bitstring_t &output);

	GOOSE_API static string BlobToBit(string_t blob);
	...
	GOOSE_API static void BitString(const string_t &input, idx_t len, bitstring_t &result);
	GOOSE_API static void ExtendBitString(const bitstring_t &input, idx_t bit_length, bitstring_t &result);
	GOOSE_API static void SetEmptyBitString(bitstring_t &target, string_t &input);
	GOOSE_API static void SetEmptyBitString(bitstring_t &target, idx_t len);
	GOOSE_API static idx_t ComputeBitstringLen(idx_t len);

	GOOSE_API static void RightShift(const bitstring_t &bit_string, idx_t shift, bitstring_t &result);
	GOOSE_API static void LeftShift(const bitstring_t &bit_string, idx_t shift, bitstring_t &result);
	GOOSE_API static void BitwiseAnd(const bitstring_t &rhs, const bitstring_t &lhs, bitstring_t &result);
	GOOSE_API static void BitwiseOr(const bitstring_t &rhs, const bitstring_t &lhs, bitstring_t &result);
	GOOSE_API static void BitwiseXor(const bitstring_t &rhs, const bitstring_t &lhs, bitstring_t &result);
	GOOSE_API static void BitwiseNot(const bitstring_t &rhs, bitstring_t &result);

	GOOSE_API static void Verify(const bitstring_t &input);
