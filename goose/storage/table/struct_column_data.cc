...
namespace goose {
        ...
        if (data_type != ColumnDataType::CHECKPOINT_TARGET) {
            ...
            for (auto &child_type: child_types) {
                sub_columns.push_back(
                    ColumnData::CreateColumn(block_manager, info, sub_column_index, child_type.second, data_type,
                                             this));
                sub_column_index++;
            }
        } else {
    ...
    void StructColumnData::InitializeScan(ColumnScanState &state) {
        ...
        IterateFields(
            state, [&](idx_t child_index, optional_idx field_vector_index, ColumnScanState &field_state,
                       bool should_scan) {
                if (!should_scan) {
                    return;
                }
    ...
    static void ScanChild(ColumnScanState &state, Vector &result,
                          const std::function<idx_t(Vector &target)> &callback) {
    ...
    idx_t StructColumnData::Scan(TransactionData transaction, idx_t vector_index, ColumnScanState &state,
                                 Vector &result,
                                 idx_t target_count) {
    ...
    idx_t StructColumnData::ScanCommitted(idx_t vector_index, ColumnScanState &state, Vector &result,
                                          bool allow_updates,
                                          idx_t target_count) {
    ...
    void StructColumnData::UpdateColumn(TransactionData transaction, DataTable &data_table,
        ...
        if (update_column == 0) {
            ...
            validity->UpdateColumn(transaction, data_table, column_path, update_vector, row_ids, update_count,
                                   depth + 1,
                                   row_group_start);
        } else {
    ...
    void StructColumnData::FetchRow(TransactionData transaction, ColumnFetchState &state,
    ...
    void StructColumnData::GetColumnSegmentInfo(const QueryContext &context, idx_t row_group_index,
