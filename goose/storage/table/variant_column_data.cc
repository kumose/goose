...
#include <goose/storage/table/variant_column_data.h>
#include <goose/storage/table/struct_column_data.h>
...
#include <goose/storage/statistics/variant_stats.h>
#include <goose/function/variant/variant_shredding.h>
...
namespace goose {
    ...
    idx_t VariantColumnData::Scan(TransactionData transaction, idx_t vector_index, ColumnScanState &state,
                                  Vector &result,
                                  idx_t target_count) {
        if (IsShredded()) {
            ...
            auto scan_count = validity->Scan(transaction, vector_index, state.child_states[0], intermediate,
                                             target_count);
    ...
    idx_t VariantColumnData::ScanCommitted(idx_t vector_index, ColumnScanState &state, Vector &result,
                                           bool allow_updates,
                                           idx_t target_count) {
        if (IsShredded()) {
            auto intermediate = CreateUnshreddingIntermediate(target_count);
            ...
            sub_columns[1]->ScanCommitted(vector_index, state.child_states[2], *child_vectors[1], allow_updates,
                                          target_count);
            auto scan_count =
                    validity->ScanCommitted(vector_index, state.child_states[0], intermediate, allow_updates,
                                            target_count);
    ...
    static void AppendShredded(Vector &input, Vector &append_vector, idx_t count,
    ...
    void VariantColumnData::Append(BaseStatistics &stats, ColumnAppendState &state, Vector &vector, idx_t count) {
        ...
        if (IsShredded()) {
            ...
            for (idx_t i = 0; i < sub_columns.size(); i++) {
                sub_columns[i]->Append(VariantStats::GetUnshreddedStats(stats), state.child_appends[i + 1], vector,
                                       count);
            }
    ...
    void VariantColumnData::FetchRow(TransactionData transaction, ColumnFetchState &state,
        ...
        if (IsShredded()) {
            ...
            for (idx_t i = 0; i < sub_columns.size(); i++) {
                sub_columns[i]->FetchRow(transaction, *state.child_states[i + 1], storage_index, row_id,
                                         *child_vectors[i],
                                         result_idx);
            }
            if (result_idx) {
    ...
    vector<shared_ptr<ColumnData> > VariantColumnData::WriteShreddedData(const RowGroup &row_group,
        ...
        VariantShreddedAppendInput append_data{
            *unshredded, *shredded, unshredded_append_state,
    ...
    void VariantColumnData::InitializeColumn(PersistentColumnData &column_data, BaseStatistics &target_stats) {
        ...
        if (column_data.child_columns.size() == 3) {
            ...
            if (!IsShredded()) {
                ...
                sub_columns.push_back(
                    ColumnData::CreateColumn(block_manager, info, 2, shredded_type, GetDataType(), this));
            }
    ...
    void VariantColumnData::GetColumnSegmentInfo(const QueryContext &context, idx_t row_group_index,
                                                 vector<idx_t> col_path,
