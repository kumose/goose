...
namespace goose {
    ...
    static void VerifyCompressionType(ClientContext &context, optional_ptr<StorageManager> storage_manager,
        ...
        for (auto &col: base.columns.Logical()) {
            ...
            if (!compression_method) {
                throw BinderException(
                    "Can't compress column \"%s\" with type '%s' (physical: %s) using compression type '%s'",
                    col.Name(),
                    logical_type.ToString(), EnumUtil::ToString(physical_type),
                    CompressionTypeToString(compression_type));
            }
        }
    }
    ...
    static void FindMatchingPrimaryKeyColumns(const ColumnList &columns,
                                              const vector<unique_ptr<Constraint> > &constraints,
        ...
        if (!found_constraint) {
            ...
            throw BinderException("Failed to create foreign key: there is no %s for referenced table \"%s\"",
                                  search_term,
                                  fk.info.table);
        }
        ...
        throw BinderException(
            "Failed to create foreign key: referenced table \"%s\" does not have a primary key or unique "
            "constraint on the columns %s",
            fk.info.table, fk_names);
    }

    static void CheckForeignKeyTypes(const ColumnList &pk_columns, const ColumnList &fk_columns,
                                     ForeignKeyConstraint &fk) {
        ...
        for (idx_t c_idx = 0; c_idx < fk.info.pk_keys.size(); c_idx++) {
            ...
            if (pk_col.Type() != fk_col.Type()) {
                throw BinderException(
                    "Failed to create foreign key: incompatible types between column \"%s\" (\"%s\") and "
                    "column \"%s\" (\"%s\")",
    ...
    static void BindCreateTableConstraints(CreateTableInfo &create_info, CatalogEntryRetriever &entry_retriever,
        ...
        for (idx_t i = 0; i < create_info.constraints.size(); i++) {
            ...
            if (pk_table_entry_ptr.is_goose_table()) {
    ...
    unique_ptr<BoundCreateTableInfo> Binder::BindCreateTableInfo(unique_ptr<CreateInfo> info,
        ...
        if (catalog.IsGooseCatalog() && !catalog.InMemory()) {
