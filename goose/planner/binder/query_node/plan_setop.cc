...
namespace goose {
        ...
        if (node->type == LogicalOperatorType::LOGICAL_PROJECTION) {
            ...
            for (idx_t i = 0; i < target_types.size(); i++) {
                if (source_types[i] != target_types[i]) {
                    ...
                    node->expressions[i] =
                            BoundCastExpression::AddCastToType(context, std::move(node->expressions[i]),
                                                               target_types[i]);
    ...
    unique_ptr<LogicalOperator> Binder::CreatePlan(BoundSetOperationNode &node) {
        ...
        auto root = make_uniq<LogicalSetOperation>(node.setop_index, node.types.size(), std::move(children),
                                                   logical_type,
                                                   node.setop_all);
