...
namespace goose {
    PhysicalInsert::PhysicalInsert(PhysicalPlan &physical_plan, vector<LogicalType> types_p, TableCatalogEntry &table,
        ...
        : PhysicalOperator(physical_plan, PhysicalOperatorType::INSERT, std::move(types_p), estimated_cardinality),
          ...
          set_expressions(std::move(set_expressions)), set_columns(std::move(set_columns)),
    ...
    void CheckOnConflictCondition(ExecutionContext &context, DataChunk &conflicts,
    ...
    static void CreateUpdateChunk(ExecutionContext &context, DataChunk &chunk, TableCatalogEntry &table,
                                  Vector &row_ids,
    ...
    static idx_t HandleInsertConflicts(TableCatalogEntry &table, ExecutionContext &context, InsertLocalState &lstate,
        ...
        if (GLOBAL) {
            auto &transaction = GooseTransaction::Get(context.client, table.catalog);
        ...
        if (!types_to_fetch.empty()) {
            ...
            if (GLOBAL) {
                ...
                local_storage.FetchChunk(data_table, row_ids, conflict_count, columns_to_fetch, scan_chunk,
                                         *fetch_state);
            }
        }
        ...
        auto affected_tuples = PerformOnConflictAction<GLOBAL>(lstate, gstate, context, combined_chunk, table, row_ids,
                                                               op);
