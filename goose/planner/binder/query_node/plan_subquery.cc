...
#include <goose/planner/subquery/recursive_dependent_join_planner.h>
...
namespace goose {
    static unique_ptr<Expression> PlanUncorrelatedSubquery(Binder &binder, BoundSubqueryExpression &expr,
        ...
        switch (expr.subquery_type) {
            case SubqueryType::EXISTS: {
                ...
                auto aggregate =
                        make_uniq<LogicalAggregate>(binder.GenerateTableIndex(), aggregate_index,
            ...
            case SubqueryType::SCALAR: {
                ...
                bool error_on_multiple_rows = DBConfig::GetSetting<ScalarSubqueryErrorOnMultipleRowsSetting>(
                    binder.context);
                ...
                auto first_agg =
                        function_binder.BindAggregateFunction(FirstFunctionGetter::GetFunction(expr.return_type),
                                                              std::move(first_children), nullptr,
                ...
                auto aggr = make_uniq<
                    LogicalAggregate>(binder.GenerateTableIndex(), aggr_index, std::move(expressions));
                ...
                if (error_on_multiple_rows) {
                    ...
                    auto first_ref =
                            make_uniq<BoundColumnRefExpression>(plan->expressions[0]->return_type,
                                                                ColumnBinding(aggr_index, 0));
                    auto count_ref =
                            make_uniq<BoundColumnRefExpression>(plan->expressions[1]->return_type,
                                                                ColumnBinding(aggr_index, 1));
                    ...
                    auto error_expr = function_binder.BindScalarFunction(
                        ErrorFun::GetFunction(), std::move(error_children));
                    error_expr->return_type = first_ref->return_type;
                    auto case_expr =
                            make_uniq<BoundCaseExpression>(std::move(count_check), std::move(error_expr),
                                                           std::move(first_ref));
    ...
    static unique_ptr<Expression> PlanCorrelatedSubquery(Binder &binder, BoundSubqueryExpression &expr,
        ...
        bool perform_delim =
                expr.subquery_type == SubqueryType::ANY
                    ? true
                    : PerformDuplicateElimination(binder, correlated_columns);
        D_ASSERT(expr.IsCorrelated());
        // correlated subquery
        ...
        switch (expr.subquery_type) {
            case SubqueryType::SCALAR: {
                ...
                auto delim_join =
                        CreateDuplicateEliminatedJoin(correlated_columns, JoinType::SINGLE, std::move(root),
                                                      perform_delim);
            ...
            case SubqueryType::EXISTS: {
                ...
                idx_t mark_index = binder.GenerateTableIndex();
                auto delim_join =
                        CreateDuplicateEliminatedJoin(correlated_columns, JoinType::MARK, std::move(root),
                                                      perform_delim);
