...
#include <goose/catalog/catalog_entry/dependency/dependency_subject_entry.h>
...
namespace goose {
    ...
    DependencyManager::DependencyManager(GooseCatalog &catalog) : catalog(catalog), subjects(catalog),
                                                                  dependents(catalog) {
    }
    ...
    DependencyManager::LookupEntry(CatalogTransaction transaction, CatalogEntry &dependency) {
    ...
    void DependencyManager::VerifyExistence(CatalogTransaction transaction, DependencyEntry &object) {
        ...
        auto &schema_catalog_set = goose_catalog.GetSchemaCatalogSet();
    ...
    void DependencyManager::ReorderEntry(CatalogTransaction transaction, CatalogEntry &entry,
    ...
    void DependencyManager::PrintSubjects(CatalogTransaction transaction, const CatalogEntryInfo &info) {
        ...
        subjects.Scan(transaction, [&](CatalogEntry &dependency) {
            ...
            Printer::Print(StringUtil::Format(
                "Schema: %s | Name: %s | Type: %s | Dependent type: %s | Subject type: %s",
