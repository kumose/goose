...
namespace goose {
    ...
    void ListColumnWriter::Prepare(ColumnWriterState &state_p, ColumnWriterState *parent, Vector &vector, idx_t count,
        ...
        for (idx_t i = start; i < vcount; i++) {
            ...
            auto first_repeat_level =
                    parent && !parent->repetition_levels.empty()
                        ? parent->repetition_levels[parent_index]
                        : MaxRepeat();
    ...
    void ListColumnWriter::FinalizeSchema(vector<goose_parquet::SchemaElement> &schemas) {
        ...
        goose_parquet::SchemaElement optional_element;
        ...
        if (type.id() != LogicalTypeId::MAP) {
            goose_parquet::SchemaElement repeated_element;
