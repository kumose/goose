...
#include <goose/planner/constraints/bound_check_constraint.h>
...
#include <goose/planner/expression_binder/alter_binder.h>
...
namespace goose {
    IndexStorageInfo GetIndexInfo(const IndexConstraintType type, const bool v1_0_0_storage,
    ...
    GooseTableEntry::GooseTableEntry(Catalog &catalog, SchemaCatalogEntry &schema, BoundCreateTableInfo &info,
        ...
        for (idx_t i = 0; i < constraints.size(); i++) {
            ...
            if (constraint->type == ConstraintType::FOREIGN_KEY) {
                ...
                if (bfk.info.type == ForeignKeyType::FK_TYPE_FOREIGN_KEY_TABLE ||
                    ...
                    // Now we can add the index.
                    storage->AddIndex(columns, column_indexes, IndexConstraintType::FOREIGN,
    ...
    unique_ptr<BaseStatistics> GooseTableEntry::GetStatistics(ClientContext &context, const StorageIndex &column_id) {
        return storage->GetStatistics(context, column_id);
    }

    unique_ptr<BaseStatistics> GooseTableEntry::GetStatistics(ClientContext &context, column_t column_id) {
    ...
    unique_ptr<BlockingSample> GooseTableEntry::GetSample() {
        return storage->GetSample();
    }

    unique_ptr<CatalogEntry> GooseTableEntry::AlterEntry(CatalogTransaction transaction, AlterInfo &info) {
    ...
    unique_ptr<CatalogEntry> GooseTableEntry::AlterEntry(ClientContext &context, AlterInfo &info) {
        D_ASSERT(!internal);
        ...
        switch (table_info.alter_table_type) {
            ...
            case AlterTableType::SET_PARTITIONED_BY:
                throw NotImplementedException("SET PARTITIONED BY is not supported for Goose tables");
            case AlterTableType::SET_SORTED_BY:
                throw NotImplementedException("SET SORTED BY is not supported for Goose tables");
            default:
                throw InternalException("Unrecognized alter table type!");
        }
    }

    void GooseTableEntry::UndoAlter(ClientContext &context, AlterInfo &info) {
        D_ASSERT(!internal);
    ...
    static void RenameExpression(ParsedExpression &root_expr, RenameColumnInfo &info) {
        ParsedExpressionIterator::VisitExpressionMutable<ColumnRefExpression>(
            root_expr, [&](ColumnRefExpression &colref) {
    ...
    unique_ptr<CatalogEntry> GooseTableEntry::RenameColumn(ClientContext &context, RenameColumnInfo &info) {
        ...
        return make_uniq<GooseTableEntry>(catalog, schema, *bound_create_info, storage);
    }

    unique_ptr<CatalogEntry> GooseTableEntry::AddColumn(ClientContext &context, AddColumnInfo &info) {
        ...
        return make_uniq<GooseTableEntry>(catalog, schema, *bound_create_info, new_storage);
    }
        ...
        if (last_entry) {
            ...
            for (auto &entry: child_list) {
                if (StringUtil::CIEquals(entry.first, new_field.Name())) {
                    // already exists!
                    result.error = ErrorData(CatalogException(
    ...
    unique_ptr<CatalogEntry> GooseTableEntry::AddField(ClientContext &context, AddFieldInfo &info) {
        // follow the path
    ...
    void GooseTableEntry::UpdateConstraintsOnColumnDrop(const LogicalIndex &removed_index,
    ...
    unique_ptr<CatalogEntry> GooseTableEntry::RemoveColumn(ClientContext &context, RemoveColumnInfo &info) {
        ...
        auto new_storage =
                make_shared_ptr<DataTable>(context, *storage,
    ...
    unique_ptr<CatalogEntry> GooseTableEntry::RemoveField(ClientContext &context, RemoveFieldInfo &info) {
    ...
    unique_ptr<CatalogEntry> GooseTableEntry::RenameField(ClientContext &context, RenameFieldInfo &info) {
        if (!ColumnExists(info.column_path[0])) {
    ...
    unique_ptr<CatalogEntry> GooseTableEntry::SetDefault(ClientContext &context, SetDefaultInfo &info) {
    ...
    unique_ptr<CatalogEntry> GooseTableEntry::SetNotNull(ClientContext &context, SetNotNullInfo &info) {
    ...
    unique_ptr<CatalogEntry> GooseTableEntry::DropNotNull(ClientContext &context, DropNotNullInfo &info) {
        auto not_null_idx = GetColumnIndex(info.column_name);
    ...
    unique_ptr<CatalogEntry> GooseTableEntry::ChangeColumnType(ClientContext &context, ChangeColumnTypeInfo &info) {
        auto binder = Binder::CreateBinder(context);
    ...
    unique_ptr<CatalogEntry> GooseTableEntry::AddForeignKeyConstraint(AlterForeignKeyInfo &info) {
    ...
    unique_ptr<CatalogEntry> GooseTableEntry::DropForeignKeyConstraint(ClientContext &context,
    ...
    void GooseTableEntry::Rollback(CatalogEntry &prev_entry) {
        ...
        auto &table = Cast<GooseTableEntry>();
        auto &prev_table = prev_entry.Cast<GooseTableEntry>();
    ...
    void GooseTableEntry::OnDrop() {
        storage->SetAsDropped();
    }

    unique_ptr<CatalogEntry> GooseTableEntry::AddConstraint(ClientContext &context, AddConstraintInfo &info) {
        auto create_info = GetInfo();
        auto &table_info = create_info->Cast<CreateTableInfo>();
    ...
    void GooseTableEntry::SetAsRoot() {
        storage->SetAsMainTable();
        storage->SetTableName(name);
    }

    void GooseTableEntry::CommitAlter(string &column_name) {
    ...
    void GooseTableEntry::CommitDrop() {
        storage->CommitDropTable();
    }

    DataTable &GooseTableEntry::GetStorage() {
        return *storage;
    }

    TableFunction GooseTableEntry::GetScanFunction(ClientContext &context, unique_ptr<FunctionData> &bind_data) {
        bind_data = make_uniq<TableScanBindData>(*this);
        return TableScanFunction::GetFunction();
    }

    vector<ColumnSegmentInfo> GooseTableEntry::GetColumnSegmentInfo(const QueryContext &context) {
        return storage->GetColumnSegmentInfo(context);
    }

    TableStorageInfo GooseTableEntry::GetStorageInfo(ClientContext &context) {
