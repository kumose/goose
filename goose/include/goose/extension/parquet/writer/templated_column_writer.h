...
#include <goose/extension/parquet/writer/parquet_write_operators.h>
#include <goose/extension/parquet/parquet_dbp_encoder.h>
#include <goose/extension/parquet/parquet_dlba_encoder.h>
#include <goose/extension/parquet/parquet_rle_bp_encoder.h>
...
public:
	StandardColumnWriterState(ParquetWriter &writer, goose_parquet::RowGroup &row_group, idx_t col_idx)
	    : PrimitiveColumnWriterState(writer, row_group, col_idx),
	      ...
	      encoding(goose_parquet::Encoding::PLAIN) {
	}
	...
	goose_parquet::Encoding::type encoding;
};
...
public:
	explicit StandardWriterPageState(const idx_t total_value_count, const idx_t total_string_size,
	                                 goose_parquet::Encoding::type encoding_p,
...
public:
	unique_ptr<ColumnWriterState> InitializeWriteState(goose_parquet::RowGroup &row_group) override {
		...
		result->encoding = writer.GetParquetVersion() == ParquetVersion::V1 ? goose_parquet::Encoding::PLAIN_DICTIONARY
		                                                                    : goose_parquet::Encoding::RLE_DICTIONARY;
	...
	void FlushPageState(WriteStream &temp_writer, ColumnWriterPageState *state_p) override {
		...
		case goose_parquet::Encoding::DELTA_BINARY_PACKED:
			...
			break;
		case goose_parquet::Encoding::PLAIN_DICTIONARY:
			// PLAIN_DICTIONARY can be treated the same as RLE_DICTIONARY
		case goose_parquet::Encoding::RLE_DICTIONARY:
			...
			break;
		case goose_parquet::Encoding::DELTA_LENGTH_BYTE_ARRAY:
			...
			break;
		case goose_parquet::Encoding::BYTE_STREAM_SPLIT:
			...
			break;
		case goose_parquet::Encoding::PLAIN:
			break;
		default:
	...
	goose_parquet::Encoding::type GetEncoding(PrimitiveColumnWriterState &state_p) override {
		auto &state = state_p.Cast<StandardColumnWriterState<SRC, TGT, OP>>();
	...
	void FinalizeAnalyze(ColumnWriterState &state_p) override {
		...
		if (state.dictionary.GetSize() == 0 || state.dictionary.IsFull()) {
			...
			if (writer.GetParquetVersion() == ParquetVersion::V1) {
				...
				state.encoding = goose_parquet::Encoding::PLAIN;
			} else {
				// If we aren't doing dictionary encoding, these encodings are virtually always better than PLAIN
				switch (type) {
				case goose_parquet::Type::type::INT32:
				case goose_parquet::Type::type::INT64:
					state.encoding = goose_parquet::Encoding::DELTA_BINARY_PACKED;
					break;
				case goose_parquet::Type::type::BYTE_ARRAY:
					state.encoding = goose_parquet::Encoding::DELTA_LENGTH_BYTE_ARRAY;
					break;
				case goose_parquet::Type::type::FLOAT:
				case goose_parquet::Type::type::DOUBLE:
					state.encoding = goose_parquet::Encoding::BYTE_STREAM_SPLIT;
					break;
				default:
					state.encoding = goose_parquet::Encoding::PLAIN;
				}
	...
	bool HasDictionary(PrimitiveColumnWriterState &state_p) override {
		auto &state = state_p.Cast<StandardColumnWriterState<SRC, TGT, OP>>();
		return state.encoding == goose_parquet::Encoding::RLE_DICTIONARY ||
		       state.encoding == goose_parquet::Encoding::PLAIN_DICTIONARY;
	}
	...
	void FlushDictionary(PrimitiveColumnWriterState &state_p, ColumnWriterStatistics *stats) override {
		auto &state = state_p.Cast<StandardColumnWriterState<SRC, TGT, OP>>();
		D_ASSERT(state.encoding == goose_parquet::Encoding::RLE_DICTIONARY ||
		         state.encoding == goose_parquet::Encoding::PLAIN_DICTIONARY);
	...
	idx_t GetRowSize(const Vector &vector, const idx_t index,
		...
		auto &state = state_p.Cast<StandardColumnWriterState<SRC, TGT, OP>>();
		if (state.encoding == goose_parquet::Encoding::RLE_DICTIONARY ||
		    state.encoding == goose_parquet::Encoding::PLAIN_DICTIONARY) {
...
private:
	...
	void WriteVectorInternal(WriteStream &temp_writer, ColumnWriterStatistics *stats,
		...
		case goose_parquet::Encoding::PLAIN: {
			D_ASSERT(page_state.encoding == goose_parquet::Encoding::PLAIN);
			if (mask.AllValid()) {
