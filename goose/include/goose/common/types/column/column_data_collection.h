...
#include <goose/common/types/column/column_data_collection_iterators.h>
...
public:
	...
	GOOSE_API ColumnDataCollection(Allocator &allocator, vector<LogicalType> types);
	//! Constructs an empty (but valid) in-memory column data collection from an allocator
	GOOSE_API explicit ColumnDataCollection(Allocator &allocator);
	...
	GOOSE_API ColumnDataCollection(ColumnDataCollection &parent);
	GOOSE_API ColumnDataCollection(shared_ptr<ColumnDataAllocator> allocator, vector<LogicalType> types);
	GOOSE_API ~ColumnDataCollection();
...
public:
	...
	//! Get the allocator
	GOOSE_API Allocator &GetAllocator() const;

	//! Initializes an Append state - useful for optimizing many appends made to the same column data collection
	GOOSE_API void InitializeAppend(ColumnDataAppendState &state);
	//! Append a DataChunk to this ColumnDataCollection using the specified append state
	GOOSE_API void Append(ColumnDataAppendState &state, DataChunk &new_chunk);

	//! Initializes a chunk with the correct types that can be used to call Scan
	GOOSE_API void InitializeScanChunk(DataChunk &chunk) const;
	GOOSE_API void InitializeScanChunk(Allocator &allocator, DataChunk &chunk) const;
	//! Initializes a chunk with the correct types for a given scan state
	GOOSE_API void InitializeScanChunk(ColumnDataScanState &state, DataChunk &chunk) const;
	...
	GOOSE_API bool Scan(ColumnDataScanState &state, DataChunk &result) const;
	//! Scans a DataChunk from the ColumnDataCollection
	GOOSE_API bool Scan(ColumnDataParallelScanState &state, ColumnDataLocalScanState &lstate, DataChunk &result) const;

	//! Append a DataChunk directly to this ColumnDataCollection - calls InitializeAppend and Append internally
	GOOSE_API void Append(DataChunk &new_chunk);

	//! Appends the other ColumnDataCollection to this, destroying the other data collection
	GOOSE_API void Combine(ColumnDataCollection &other);
	...
	GOOSE_API idx_t ChunkCount() const;
	//! Fetch an individual chunk from the ColumnDataCollection
	GOOSE_API void FetchChunk(idx_t chunk_idx, DataChunk &result) const;
	...
	GOOSE_API ColumnDataChunkIterationHelper Chunks() const;
	//! Constructs a class that can be iterated over to fetch individual chunks
	//! Only the column indexes specified in the column_ids list are scanned
	GOOSE_API ColumnDataChunkIterationHelper Chunks(vector<column_t> column_ids) const;
	...
	GOOSE_API ColumnDataRowIterationHelper Rows() const;
	...
	//! about performance
	GOOSE_API ColumnDataRowCollection GetRows() const;
...
public:
	...
	GOOSE_API ColumnDataRow &operator[](idx_t i);
	GOOSE_API const ColumnDataRow &operator[](idx_t i) const;
