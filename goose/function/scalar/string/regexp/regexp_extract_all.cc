...
bool ExtractAll(std::string_view &input, xre2::RE2 &pattern, idx_t *startpos,
                std::string_view *groups, int ngroups) {
	D_ASSERT(pattern.ok());
...
void ExtractSingleTuple(const string_t &string, xre2::RE2 &pattern, int32_t group, RegexStringPieceArgs &args,
...
xre2::RE2 &GetPattern(const RegexpBaseBindData &info, ExpressionState &state,
                            unique_ptr<xre2::RE2> &pattern_p) {
	if (info.constant_pattern) {
	...
	for (idx_t row = 0; row < tuple_count; row++) {
		...
		if (!info.constant_pattern) {
			...
			if (!pattern_data.validity.RowIsValid(pattern_idx)) {
				...
				stored_re = make_uniq<xre2::RE2>(pattern_strpiece, info.options);
...
static inline bool ExtractAllStruct(std::string_view &input, xre2::RE2 &re, idx_t &startpos,
                                    std::string_view *groups, int provided_groups) {
	D_ASSERT(re.ok());
...
static void ExtractStructAllSingleTuple(const string_t &string_val, xre2::RE2 &re,
                                        vector<std::string_view> &group_spans,
...
void RegexpExtractAllStruct::Execute(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	vector<std::string_view> group_spans(group_count + 1);
