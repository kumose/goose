...
#include <goose/main/relation/value_relation.h>
#include "iostream"
...
TEST_CASE("Test simple relation API", "[relation_api]") {
	...
	goose::shared_ptr<Relation> tbl, filter, proj, proj2, v1, v2, v3;
	...
	REQUIRE_NOTHROW(result = tbl->Project(goose::vector<string> {"i + 1", "i + 2"})->Execute());
	REQUIRE(CHECK_COLUMN(result, 0, {2, 3, 4}));
	REQUIRE(CHECK_COLUMN(result, 1, {3, 4, 5}));

	REQUIRE_NOTHROW(result = tbl->Project(goose::vector<string> {"i + 1"}, {"i"})->Execute());
	REQUIRE(CHECK_COLUMN(result, 0, {2, 3, 4}));
	...
	// multi filter
	REQUIRE_NOTHROW(result = tbl->Filter(goose::vector<string> {"i <> 2", "i <> 3"})->Execute());
	...
	REQUIRE_NOTHROW(result = proj->Order(goose::vector<string> {"a DESC", "a ASC"})->Execute());
	REQUIRE(CHECK_COLUMN(result, 0, {4, 4, 2, 2}));
	...
	// incorrect API usage
	REQUIRE_THROWS(tbl->Project(goose::vector<string> {})->Execute());
	REQUIRE_THROWS(tbl->Project(goose::vector<string> {"1, 2, 3"})->Execute());
	REQUIRE_THROWS(tbl->Project(goose::vector<string> {""})->Execute());
	REQUIRE_THROWS(tbl->Filter("i=1, i=2")->Execute());
	REQUIRE_THROWS(tbl->Filter("")->Execute());
	REQUIRE_THROWS(tbl->Filter(goose::vector<string> {})->Execute());
	REQUIRE_THROWS(tbl->Filter(goose::vector<string> {"1, 2, 3"})->Execute());
	REQUIRE_THROWS(tbl->Filter(goose::vector<string> {""})->Execute());
	REQUIRE_THROWS(tbl->Order(goose::vector<string> {})->Execute());
	REQUIRE_THROWS(tbl->Order(goose::vector<string> {"1, 2, 3"})->Execute());
...
TEST_CASE("Test combinations of set operations", "[relation_api]") {
	...
	con.EnableQueryVerification();
	goose::unique_ptr<QueryResult> result;
	goose::shared_ptr<Relation> values, v1, v2, v3;
...
TEST_CASE("Test combinations of joins", "[relation_api]") {
	...
	con.EnableQueryVerification();
	goose::unique_ptr<QueryResult> result;
	goose::shared_ptr<Relation> values, vjoin;
...
TEST_CASE("Test crossproduct relation", "[relation_api]") {
	...
	con.EnableQueryVerification();
	goose::unique_ptr<QueryResult> result;
	goose::shared_ptr<Relation> values, vcross;
...
TEST_CASE("Test view creation of relations", "[relation_api]") {
	...
	con.EnableQueryVerification();
	goose::unique_ptr<QueryResult> result;
	goose::shared_ptr<Relation> tbl, filter, proj, proj2;
	...
	goose::vector<goose::unique_ptr<ParsedExpression>> expressions;
	expressions.push_back(goose::make_uniq<goose::ColumnRefExpression>("i"));
	goose::vector<goose::string> aliases;
	aliases.push_back("j");
...
TEST_CASE("Test table creations using the relation API", "[relation_api]") {
	...
	con.EnableQueryVerification();
	goose::unique_ptr<QueryResult> result;
	goose::shared_ptr<Relation> values;
...
TEST_CASE("Test table creations with on_create_conflict using the relation API", "[relation_api]") {
	...
	con.EnableQueryVerification();
	goose::unique_ptr<QueryResult> result;
	goose::shared_ptr<Relation> values, values1, proj;
...
TEST_CASE("Test aggregates in relation API", "[relation_api]") {
	...
	REQUIRE_NOTHROW(result = tbl->Aggregate(goose::vector<string> {"SUM(i)", "SUM(j)"})->Execute());
	REQUIRE(CHECK_COLUMN(result, 0, {4}));
	REQUIRE(CHECK_COLUMN(result, 1, {18}));
	...
	REQUIRE_NOTHROW(
	    result =
	        tbl->Aggregate(goose::vector<string> {"SUM(j)"}, goose::vector<string> {"i"})->Order("1")->Execute());
...
TEST_CASE("Test table function relations", "[relation_api]") {
	...
	auto i1 = con.TableFunction("goose_tables");
	result = i1->Execute();
	...
	auto summary = values->TableFunction("summary", goose::vector<Value> {});
...
TEST_CASE("Test CSV Relation with union by name", "[relation_api]") {
	...
	goose::vector<string> paths {"data/csv/sample-0.csv", "data/csv/sample-0.csv"};
...
TEST_CASE("Test CSV reading/writing from relations", "[relation_api]") {
	...
	case_insensitive_map_t<goose::vector<Value>> options;
	options["header"] = {goose::Value(0)};
...
TEST_CASE("Construct ValueRelation with RelationContextWrapper and operate on it", "[relation_api][txn][wrapper]") {
	...
	goose::vector<goose::vector<goose::unique_ptr<goose::ParsedExpression>>> expressions;
	{
		goose::vector<goose::unique_ptr<goose::ParsedExpression>> row;

		{
			goose::ConstantExpression ce1(goose::Value::INTEGER(1));
			row.push_back(ce1.Copy());
		}
		{
			goose::ConstantExpression ce2(goose::Value::INTEGER(2));
	...
	auto rcw = goose::make_shared_ptr<goose::RelationContextWrapper>(con.context);

	// Manually construct a ValueRelation using the RelationContextWrapper + expressions ctor
	goose::shared_ptr<goose::Relation> rel;
	REQUIRE_NOTHROW(rel = goose::make_shared_ptr<goose::ValueRelation>(rcw, std::move(expressions),
	                                                                     goose::vector<std::string> {}, "vr"));
...
TEST_CASE("Test create table with empty name", "[relation_api]") {
	...
	goose::unique_ptr<QueryResult> result = con.Query("CREATE TABLE '' AS SELECT 42;");
	REQUIRE_FAIL(result);
